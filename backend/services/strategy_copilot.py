...
class StrategyCopilot:
    ...
    async def _context_block(self, ctx: Dict) -> str:
        ...
        if result:
            ...
            # Drawdown wurde als 1600 % gelesen), dann das Roh-JSON als Detail.
            summary = metrics_summary(result, (ctx or {}).get("settings"))
            if summary:
                parts.append(summary)
            raw = _dumps(result, 2500)
            truncated = len(json.dumps(result, ensure_ascii=False, default=str)) > 2500
            parts.append("LETZTES ERGEBNIS (Roh-JSON"
