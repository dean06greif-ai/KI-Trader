...
DEFAULT_CONFIG: Dict = {
    ...
    # --- Detektor "jump": Statistisches Jump-Modell (services/regime_jump.py) ---
    # Zustand je Kerze = min(Abstand zu auf/seit/ab-Zentrum + Sprungkosten).
    "jump_fast_days": 1.0,            # Halbwertszeit schnelles Trend-Feature (Tage)
...
CONFIG_META = [
    ...
    ("detector", "Erkennungs-Prinzip", "reactive (Standard) = Umkehrpunkte: vom "
     ...
     "bleiben Teil des Trends). jump = Statistisches Jump-Modell (Stand der "
     "Forschung): je Kerze der Zustand mit dem kleinsten Abstand zum auf/seit/ab-"
     "Zentrum PLUS Sprungkosten je Wechsel – optimale statt Ad-hoc-Persistenz; im "
     "Benchmark (BTC/ETH/SOL/XRP/DOGE, 1h+4h, 1080 d) klar bester Referenz-Treffer. "
     "regression = alte statistische Trend-Erkennung."),
    ("jump_fast_days", "Jump: schnelles Feature (Tage)", "Nur Detektor 'jump': "
     "Halbwertszeit der schnellen, vola-normierten Trend-Messung. Kleiner = "
     "reagiert früher, etwas mehr Fehlalarme."),
    ("jump_slow_days", "Jump: langsames Feature (Tage)", "Nur Detektor 'jump': "
     "Halbwertszeit der langsamen Trend-Messung (Richtungs-Anker)."),
    ("jump_center", "Jump: Trend-Zentrum (x Tagesvola)", "Nur Detektor 'jump': "
     "so stark (Rendite je Tag in Tagesvola) muss ein typischer Trend sein. "
     "Größer = mehr Seitwärts, weniger (aber sicherere) Trends."),
...
_grp("Phasen-Glättung", ["reactive", "ema", "kombi", "jump"], "min_phase_days")
...
def resolve_config(config: Optional[Dict], timeframe: str, n_bars: int = 10 ** 9) -> Dict:
    ...
    cfg["detector"] = (det_in if det_in in ("regression", "ema", "kombi", "jump")
                       else "reactive")
    _f("jump_fast_days", 0.1, 30.0)
...
def detector_warmup_bars(cfg: Dict) -> int:
    """Kerzen, bis alle Features des GEWÄHLTEN Detektors eingeschwungen sind.
    `warmup_bars` deckt nur den Regressions-Detektor ab (längster Horizont);
    reactive/ema/kombi brauchen EMA-Trendlinien, Vola-Referenz und Pivot-
    Gedächtnis – das war bisher nirgends beziffert."""
    if not cfg or not cfg.get("bars_per_day"):
        return 0
    bpd = float(cfg.get("bars_per_day") or 1.0)
    det = str(cfg.get("detector") or "reactive")
    mode = norm_mode(cfg.get("regime_mode", 9))
    need = [int(cfg.get("vol_window_bars") or 5) + 5]
    if det == "regression":
        need.append(int(cfg.get("warmup_bars") or 0))
    elif det == "ema":
        need.append(4 * max(int(round(float(cfg.get("ema_regime_days") or 9.0) * bpd)), 2))
    elif det == "kombi":
        need.append(4 * max(int(round(float(cfg.get("kombi_ema_days") or 14.0) * bpd)), 2))
    elif det == "jump":
        need.append(3 * max(int(round(float(cfg.get("jump_slow_days") or 14.0) * bpd)), 2))
    else:  # reactive: Pivot-Gedächtnis (Legs, Stall, Persistenz)
        need.append(int(cfg.get("stall_bars") or 2) * 4)
        need.append(int(cfg.get("persist_bars") or 3) * 20)
    if det in ("reactive", "regression") and bool(cfg.get("use_ema_confirm", True)):
...
def required_history_bars(cfg: Dict, extra_days: float = 2.0) -> int:
    """Mindest-Historie für eine LIVE-Klassifikation, die den Lab-Labels
    entspricht (Detektor-Warmup + Puffer). Ohne genügend Kerzen werden
    Vola-z, EMA-Anker und Pivot-Zustand auf einem anderen Fenster berechnet
    als im Lab – das Live-Regime weicht dann vom Lab-Regime ab."""
    if not cfg or not cfg.get("bars_per_day"):
        return 0
    bpd = float(cfg.get("bars_per_day") or 1.0)
...
def required_history_days(cfg: Dict, min_days: int = 30, max_days: int = 600) -> int:
    ...
    if not cfg or not cfg.get("bars_per_day"):
        return int(min_days)
    bpd = float(cfg.get("bars_per_day") or 1.0)
    days = math.ceil(required_history_bars(cfg) / max(bpd, 1e-9))
    return int(min(max(days, min_days), max_days))
...
def _bands(f: Dict[str, np.ndarray], cfg: Dict):
    ...
    # "Stark" verlangt zusätzlich echtes Tempo (Netto-%/Tag relativ zur
    # Tagesvola, siehe strong_speed_ratio) – ein Random Walk kann sonst über
    # den t-Score allein als starker Trend durchgehen.
    a_score = np.abs(np.nan_to_num(score))
    ratio = float(cfg.get("strong_speed_ratio") or 0.35)
    tpd = np.nan_to_num(f.get("trend_pct_per_day"), nan=0.0)
    dv = f["daily_vol_pct"]
    dv_ok = np.isfinite(dv) & (dv > 0)
    spd = np.where(dv_ok, np.abs(tpd) / np.where(dv_ok, dv, 1.0), np.inf)
    enter_strong = (a_score >= strong * (1 + h)) & (spd >= ratio)
    stay_strong = (a_score >= strong * (1 - h)) & (spd >= 0.5 * ratio)
...
def validate_labels(candles, labels, model: Dict) -> Dict:
    ...
    for (s, e, rid) in segs:
        ...
        if mode == 5 and t_idx != 1:
            ...
            # Wie bei der Richtungsprüfung zählt auch das Sichtfenster des
            # Klassifikators mit – erst wenn BEIDE zu langsam sind, ist das
            # "stark"-Label wirklich falsch (kein Lookahead-Vorwurf).
            ctx_days = (e - c0) / max(bpd, 1e-9)
            ctx_speed = abs(ctx_net / max(ctx_days, 1e-9)) / max(v, 1e-9)
            if v_idx == 1 and speed < 0.5 * enter and ctx_speed < 0.5 * enter:
                problems.append(
                    f"Label starker Trend, Tempo nur {speed:.2f}x Tagesvola "
                    f"(Sichtfenster {ctx_speed:.2f}x)")
        if problems:
            bad_bars += bars
...
def summarize(model: Dict) -> str:
    ...
    if str(cfg.get("detector") or "reactive") == "jump":
        return (f"Engine v2 · {norm_mode(cfg.get('regime_mode', 9))}-Regime-Modus · "
                f"Jump-Modell (Features {cfg.get('jump_fast_days', 1.0):g}d/"
                f"{cfg.get('jump_slow_days', 14.0):g}d · Zentrum "
                f"{cfg.get('jump_center', 0.6):g}×Vola · Sprungkosten "
                f"{cfg.get('jump_penalty_days', 1.5):g}d)"
                f"{f' (Profil {prof})' if prof else ''} · "
                f"{len(model.get('regimes') or [])} Regime aktiv")
    if str(cfg.get("detector") or "reactive") != "regression":
        return (f"Engine v2 · {norm_mode(cfg.get('regime_mode', 9))}-Regime-Modus · "
