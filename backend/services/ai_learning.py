...
REEVAL_SYSTEM = (
    "Du bist das Lern-Modul des 'KI Trader'. Der Trader hat seine Strategie/Einstellungen "
    "GEÄNDERT – prüfe jede gespeicherte Lektion kritisch: Gilt sie unter den NEUEN "
...
# Kompakte Whitelist der Engine-Einstellungen für den Neu-Bewertungs-Kontext
_REEVAL_CFG_KEYS = (
    "interval_min", "min_confidence", "cooldown_min", "crv_min", "crv_max",
    "use_ai_levels", "lev_mode", "trade_frame", "autonomy", "max_lessons",
)
...
LEARNING_SYSTEM = (
    ...
    "Passen SL/TP zum beobachteten Verhalten (z.B. SL zu eng -> viele knappe Stop-Outs, "
    ...
    "WICHTIG – KONTEXT STATT PAUSCHALREGELN: Laufen z.B. Longs gerade besser als Shorts, "
    "ist das eine Eigenschaft des AKTUELLEN Marktregimes, keine allgemeine Wahrheit. "
    "Formuliere solche Erkenntnisse IMMER bedingt (Feld \"context\": unter welcher "
    ...
    "SL-Abstand. Hebel-Deckel-Lektionen werden automatisch verworfen. "
    ...
    '"context": "Marktbedingung, unter der die Lektion gilt (leer = allgemeingültig)", '
    '"expires_days": 0, '
    ...
    "REGIME-EBENEN TRENNEN: Kurzfrist-Regime der Symbolzeilen heißen trend_up/range_ruhig/…; "
    "das strukturelle Lab-Regime heißt 'strukturell Bär/Bulle/Seitwärts'. Nenne im 'context' "
    "einer Regime-Lektion IMMER eindeutig die Ebene (z.B. 'strukturell Bär' vs. 'Kurzfrist range_ruhig').\n"
...
class AILearning:
    ...
    async def lessons_text(self) -> str:
        return ai_lessons.lessons_text(
            await self.get_lessons(), risk_sizing=self._risk_sizing(),
            with_ids=bool((self.engine.config or {}).get("lesson_attribution_enabled", True)))

    async def _impact_block(self) -> str:
        """Lektions-Bilanz für den Lernlauf (nur mit Flag `lesson_impact_in_prompt`)."""
        try:
            from services.lesson_impact import lesson_impact
            return await lesson_impact.prompt_block(self.engine.config or {})
        except Exception as e:  # noqa: BLE001
            logger.warning(f"Lektions-Bilanz-Block fehlgeschlagen: {e}")
            return ""
    ...
    # ---------------- Lern-Reset: Lektionen neu bewerten ----------------
    def _reeval_apply(self, lessons: List[Dict], verdicts: List[Dict]) -> Dict:
        ...
        now = _now_iso()
        for l in lessons:
            v = by_id.get(str(l.get("id", "")))
            verdict = str((v or {}).get("verdict", "gueltig")).lower()
            reason = str((v or {}).get("reason", ""))[:200]
            if l.get("locked") or l.get("origin") == "trader":
                if verdict != "gueltig":
                    protected.append({"id": l.get("id"), "title": l.get("title"),
            ...
            if verdict == "anpassen":
                ...
                if new_detail:
                    ...
                    l["detail"] = new_detail
                    l["updated_at"] = now
                    l["revalidated_at"] = now
                    adjusted.append({"id": l.get("id"), "title": l.get("title"),
                                     "reason": reason})
            else:
                l = dict(l)
                l["revalidated_at"] = now
        ...
        return {"kept": kept, "removed": removed,
                "adjusted": adjusted, "protected": protected}

    async def reevaluate_lessons(self, reason: str = "Strategie-Änderung") -> Dict:
        """Alle Lektionen gegen die AKTUELLE Strategie/Einstellungen prüfen.

        Vom Trader per Knopf ausgelöst (z.B. nach Strategie-Änderungen), damit
        unter alten Bedingungen gelernte Lektionen die neue Ausrichtung nicht
        ...
        if not lessons:
            return {"status": "ok", "checked": 0, "removed": [], "adjusted": [],
                    "protected": [], "detail": "Keine Lektionen vorhanden"}
        self.learning_now = True
        try:
            cfg = {k: self.engine.config.get(k) for k in _REEVAL_CFG_KEYS
                   if self.engine.config.get(k) is not None}
            directives = await self.engine._user_directives(8)
            impact_rows = []
            from services.lesson_impact import evidence_for
            try:
                from services.lesson_impact import lesson_impact
                impact_rows = await lesson_impact.rows(self.engine.config or {})
            except Exception as e:  # noqa: BLE001
                logger.warning(f"Lektions-Bilanz für Neubewertung fehlgeschlagen: {e}")
            lines = []
            for l in lessons:
                tag = " [LOCKED]" if (l.get("locked") or l.get("origin") == "trader") else ""
                ev = evidence_for(impact_rows, str(l.get("id"))) if impact_rows else ""
                lines.append(f"- id={l.get('id')}{tag} [{l.get('updated_at', '')[:10]}] "
                             f"{l.get('title')}: {l.get('detail')}"
                             + (f" || {ev}" if ev else ""))
            prompt = (
                f"{master_prompt.prompt_block()}\n\n"
                f"=== ANLASS ===\n{str(reason)[:200]}\n\n"
                f"=== AKTUELLE STRATEGIE-EINSTELLUNGEN ===\n{cfg}\n\n"
                f"=== AKTUELLE TRADER-DIREKTIVEN ===\n{directives}\n\n"
                f"=== GESPEICHERTE LEKTIONEN ({len(lessons)}) ===\n"
                + "\n".join(lines) + "\n\n"
                "Bewerte JEDE Lektion (id angeben). 'veraltet' NUR, wenn die Lektion "
                "an Bedingungen hängt, die es so nicht mehr gibt – allgemeingültige "
                "Risiko-Lektionen bleiben 'gueltig'.")
            raw, model_used = await self.engine._generate_json(prompt, REEVAL_SYSTEM,
                                                               role="learner")
            data = self.engine._parse_json(raw) or {}
            verdicts = data.get("verdicts") or []
            if not isinstance(verdicts, list):
                verdicts = []
            res = self._reeval_apply(lessons, verdicts)
            if res["removed"] or res["adjusted"]:
                await lesson_store.save_all(res["kept"])
                self._lessons_cache = None
            assessment = str(data.get("assessment", ""))[:500]
            try:
                msg = (f"🔄 Lektionen neu bewertet ({reason}): {len(lessons)} geprüft, "
                       f"{len(res['removed'])} veraltet entfernt, "
                       f"{len(res['adjusted'])} angepasst"
                ...
                pass
            return {"status": "ok", "checked": len(lessons),
                    "removed": res["removed"], "adjusted": res["adjusted"],
                    "protected": res["protected"], "assessment": assessment,
                    "model": model_used}
        except Exception as e:
            logger.error(f"Lektionen-Neubewertung fehlgeschlagen: {e}")
    ...
    async def _recent_outcomes_text(self, limit: int = 25) -> str:
        ...
        for t in reversed(trades):
            ...
            # Setup + Abschluss-Art gehören in jede Zeile: nur so kann die KI
            # setup-spezifische Lektionen ableiten (statt pauschaler Regeln).
            setup = t.get("setup") or d.get("setup")
            closed_by = str(t.get("closed_by") or "").replace("_", " ")
            lines.append(
                f"- {t.get('symbol')} {t.get('side')} [{t.get('mode')}] Hebel {t.get('leverage')}x, "
                f"PnL {pnl:+.2f} USDT{dur}"
                + (f", Setup {setup}" if setup else "")
                + (f", TF {t.get('timeframe')}" if t.get("timeframe") else "")
                + (f", Exit: {closed_by}" if closed_by else "")
                + (f", Konfidenz {conf}%" if conf is not None else "")
                + (f" | Begründung damals: {reason}" if reason else ""))
        return "\n".join(lines)

    async def _setup_context_text(self, max_chars: int = 1800) -> str:
        """Setup-Reife + Backtest-Edge je Anlageklasse (ai_playbook) als
        Lern-Kontext: Lektionen sollen auf Setup-Ebene ansetzen und Backtest-
        Befunde (Edge/kein Edge) nicht ignorieren."""
        try:
            from services import ai_playbook
            txt = await ai_playbook.context_text(self.db)
        except Exception as e:  # noqa: BLE001
            logger.debug(f"Setup-Kontext für Lernlauf fehlgeschlagen: {e}")
            return ""
    ...
    async def run_learning(self, trigger: str = "manual") -> Dict:
        ...
        try:
            ...
            # Lese-Zugriff auf die anderen Strategien (Trendfolge, Scalping, …):
            # Parameter + echte Ergebnisse als Vergleichskontext NUR fürs Lernen.
            setup_txt = await self._setup_context_text(1400 if lean else 2200)
            insights_txt = ""
            try:
                from services import strategy_insights
                insights_txt = await strategy_insights.context_text(
                    self.db, max_chars=1800 if lean else 2600)
            except Exception as ie_:
                logger.warning(f"Cross-Strategie-Kontext fehlgeschlagen: {ie_}")
            ...
            impact_txt = await self._impact_block()
            ...
            prompt = (
                ...
                + (f"=== SETUP-REIFE & BACKTEST-BEFUNDE (je Anlageklasse) ===\n{setup_txt}\n"
                   "Formuliere Lektionen bevorzugt SETUP-bezogen (welches Setup, welche Klasse, "
                   "welcher Exit) statt pauschal.\n\n" if setup_txt else "")
            ...
            for l in (data.get("lessons") or [])[:max_lessons]:
                ...
                context = str(l.get("context") or "").strip()[:200]
                valid_until = None
                try:
                    exp_days = int(l.get("expires_days") or 0)
                    if 1 <= exp_days <= 90:
                ...
                if key in locked_titles:
                    _skip(title, "vom Trader festgelegt (unveränderlich)", detail,
                          approvable=False)
                    continue
                ok_master, why = master_prompt.check_lesson(
                ...
                if l.get("trader_directive") and prev is None:
                    ...
                    fresh.append({"title": title, "detail": detail,
                                  "context": context, "valid_until": valid_until,
