...
ROLE_PRESETS: Dict[str, Dict] = {
    # Läuft am häufigsten, Prompts 10–20k Tokens (Gruppen-Analyse): Groq-Free-
    # Modelle haben nur ~7k Input-Budget und würden übersprungen -> großer
    # Kontext primär, Gemini als Fallback, Groq nur als letzte Stufe.
    # (Cerebras taucht in keiner Voreinstellung mehr auf: Free-Tier seit 17.08.2026
    ...
    "trade_manager": {"provider": "groq", "model": "openai/gpt-oss-120b",
                      ...
                      "fallback2_provider": "openrouter",
                      "fallback2_model": "nvidia/nemotron-3-super-120b-a12b:free"},
    # 24/7-Betrieb -> billigstes Modell (gratis), günstige Fallbacks
    "news_watcher": {"provider": "groq", "model": "openai/gpt-oss-20b",
                     "fallback_provider": "gemini", "fallback_model": "gemini-3.1-flash-lite",
                     "fallback2_provider": "mistral", "fallback2_model": "ministral-8b-latest"},
...
# Rollen, die dauerhaft laufen oder Geld bewegen: hier muss IMMER eine zweite
# Fallback-Stufe gesetzt sein. Fehlt sie in einer vom Trader konfigurierten
...
def _dedupe(chain: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
    seen, out = set(), []
    for pm in chain:
        if pm not in seen:
            seen.add(pm)
...
class AIRoleManager:
    ...
    async def load(self, db):
        try:
            ...
            if doc is not None and not doc.get("_cost_migration_v1"):
                ...
                # Nutzer-Ein/Aus-Schalter überleben die Migration (nur Modelle
                # werden auf Presets gesetzt, nicht die Aktivierung der Rollen).
                for role, cfg in doc.items():
                    if role in self.config and isinstance(cfg, dict) and "enabled" in cfg:
                        self.config[role]["enabled"] = bool(cfg["enabled"])
            ...
            if doc:
                ...
                if changed:
                    await db.settings.update_one(
                        {"_id": "ai_roles_config"},
                        {"$set": {r: dict(self.config[r]) for r in changed}}, upsert=True)
                    if filled:
    ...
    def _fill_missing_fallback2(self) -> List[str]:
        ...
        ergänzen (rein auf self.config). Primär/Fallback 1 bleiben unverändert;
        gewählt wird das erste Preset-Modell, das noch nicht in der Kette ist."""
        filled: List[str] = []
        for role in FALLBACK2_REQUIRED_ROLES:
            cfg = self.config.get(role)
            if not cfg or cfg.get("fallback2_model"):
                continue
            used = {(cfg.get("provider"), cfg.get("model")),
                    (cfg.get("fallback_provider"), cfg.get("fallback_model"))}
            preset = ROLE_PRESETS.get(role, {})
            for pp, pm in (("fallback2_provider", "fallback2_model"),
                           ("fallback_provider", "fallback_model"),
                           ("provider", "model")):
                cand = (preset.get(pp), preset.get(pm))
                if cand[1] and cand not in used \
                        and cand[1] in ai_providers.allowed_models(cand[0] or ""):
                    cfg["fallback2_provider"], cfg["fallback2_model"] = cand
                    filled.append(role)
                    break
        return filled
    ...
    def _sanitize(self, role: str, updates: Dict) -> Dict:
        ...
        if role == "news_watcher" and "deep_on_news" in updates:
            out["deep_on_news"] = bool(updates["deep_on_news"])
        return out
    ...
    def team_chain(self, role: str, engine_cfg: Dict,
        ...
        die Not-Kette aller verfügbaren Provider. Basis für die Kennzeichnung
        "Not-Fallback außerhalb des Teams" in den KI-Warnungen."""
        cfg = self.role_cfg(role)
        provider = cfg.get("provider") or engine_cfg.get("provider", "gemini")
        model = cfg.get("model") or (engine_cfg.get("model")
                                     if not cfg.get("provider") else None)
    ...
    def chain(self, role: str, engine_cfg: Dict,
        ...
        return _dedupe(self.team_chain(role, engine_cfg, now)
                       + self._available_chain())
