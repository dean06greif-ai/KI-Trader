...
der AKTIVEN Variante bestimmt – der jüngste dieser Zeitpunkte:
  ...
  * Parameter-Profil (lifecycle versions) – NUR Versionen, die die Validierung
    neu starten (strong=True bzw. Alt-Einträge ohne Kennzeichen). Schwache
    Auto-Tunings/Rollbacks (strong=False) setzen die Anzeige NICHT mehr auf 0:
    sonst zeigte die Tabelle nach jedem kleinen Tuning „0 Trades“, obwohl das
    Live-Gate (eval_since) weiterzählt (Bug-Report 23.09, momentum_news).
...
def variant_since_map(scope: Dict, library, lifecycle_key: str = "lifecycle") -> Dict[str, str]:
    ...
    for sid in library:
        ...
        versions = [v for v in (lifecycle.get(sid) or {}).get("versions") or []
                    if v.get("strong") is not False]
        if versions:
...
def policy_report_rows(trades: List[Dict]) -> List[Dict]:
    ...
    from services.policy_fingerprint import PART_KEYS_V2, group_key
    rows: Dict[str, Dict] = {}
    for t in trades or []:
        ...
        row = rows.setdefault(key, {
            ...
            "policy": ({k: fp.get(k) for k in PART_KEYS_V2} if key else None),
...
# Sprechende Namen der Fingerprint-Teile (UI-Wunsch 09/2026: "Policy-Version"
# versteht niemand – jede Version bekommt eine Nummer + was sich geändert hat).
POLICY_PART_LABELS = (
    ("prompt_hash", "Prompt"), ("lessons_hash", "Lektionen"),
    ("playbook_version", "Playbook"), ("model", "Modell"),
    ("gate_version", "ML-Gate"), ("sizing_hash", "Sizing"),
    ("config_hash", "Einstellungen"), ("regime_artifact", "Regime-Modell"),
)


def policy_changes(prev: Optional[Dict], cur: Optional[Dict]) -> List[str]:
    """Welche Policy-Teile unterscheiden sich zwischen zwei Fingerprints?
    Rein (testbar); ohne Vorgänger leer (erste Version)."""
    if not prev or not cur:
        return []
    out = [label for key, label in POLICY_PART_LABELS
           if str(prev.get(key) or "") != str(cur.get(key) or "")]
    return out or ["Sonstiges"]


def annotate_policy_versions(rows: List[Dict]) -> List[Dict]:
    """Ergänzt je Bericht-Zeile `version_no` (chronologisch, 1 = älteste Policy
    mit Fingerprint), `is_current` (jüngste) und `changed` (Teile, die sich
    gegenüber der zeitlich vorherigen Version geändert haben). Alt-Trades ohne
    Fingerprint bekommen keine Nummer. Mutiert die Zeilen in-place (rein sonst)."""
    with_fp = sorted([r for r in rows if r.get("combined")],
                     key=lambda r: (r.get("first_ts") or "", r.get("last_ts") or ""))
    prev = None
    for no, r in enumerate(with_fp, start=1):
        r["version_no"] = no
        r["is_current"] = no == len(with_fp)
        r["changed"] = policy_changes(prev.get("policy") if prev else None, r.get("policy"))
        prev = r
    for r in rows:
