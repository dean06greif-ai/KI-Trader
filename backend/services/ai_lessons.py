...
def lessons_text(lessons: List[Dict], risk_sizing: bool = False,
    ...
    with_ids=True: stabile Kurz-ID je Zeile (`[id:xxxxxx]`) für die
    Lektions-Attribution (services/lesson_attribution.py)."""
    ...
    return "\n".join(_lesson_line(l, risk_sizing, with_ids) for l in ordered)


def _lesson_line(l: Dict, risk_sizing: bool, with_ids: bool = False) -> str:
    ...
    id_tag = ""
    if with_ids:
        from services.lesson_attribution import short_id
        id_tag = f" [id:{short_id(l.get('id'))}]"
    return f"{l['no']}.{id_tag} {mark} {l.get('title')}: {l.get('detail')}{extra}"
