...
def robustness_key(m: Optional[Dict], band: Optional[tuple] = None) -> tuple:
    ...
    dann die Phasendauer, dann weniger Umschaltungen (stabiler). Höher = robuster.
    Mit `band=(min, max)` zählt die NÄHE zum Sweet Spot statt „je länger desto
    besser“ (sonst driftet die Endlos-Suche zu immer trägeren Detektoren)."""
    if not m:
        return (-1e9, -1e9, -1e9)
    ...
    return (float(hold) if hold is not None else -1.0, phase_term,
...
def holdout_regressed(best_m: Optional[Dict], base_m: Optional[Dict]) -> bool:
    ...
    try:
        b = (best_m or {}).get("holdout_direction_pct")
        a = (base_m or {}).get("holdout_direction_pct")
    ...
    try:
        ...
        def phase_txt(extra=""):
            ...
            return (f"Autopilot · {tested} Varianten · {improved} Verbesserungen · "
                    ...
                    f"Referenz {m.get('reference_pct')}% · "
