...
TRADE_SPACES = {
    ...
    "trail": {
        # ATR-Trailing nach TP1: An/Aus + Trail-Abstand. Bewusst bis 4.0 ATR,
        # damit die Suche auch WEITERE Trails testet (Beobachtung: 1.5 oft zu eng).
        "trail_after_tp1": [False, True],
        "trail_atr_mult": [1.0, 1.25, 1.5, 2.0, 2.5, 3.0, 3.5, 4.0],
    },
    "profit_secure": {
...
def create_job(params: Dict) -> str:
    ...
    # RAM-Schutz: Trade-Rohdaten älterer Jobs freigeben (DB-Fallback greift)
...
def _score(m: Dict, objective: str, min_trades: int, dd_max_pct: float = None) -> float:
    ...
    if objective == "win_rate":
        # Verlust-Kandidaten nie über profitable stellen (vorher: 60 % WR mit
        # -500 PnL schlug 55 % WR mit +1000 PnL)
        return wr * 1000 + pnl if pnl > 0 else pnl
    if objective == "pnl":
        return pnl
...
def build_candidates(allowed: List[str] = None, tf_options: List[str] = None) -> List[Dict]:
    ...
    add("days_to_fomc", "FOMC-Filter (kein Einstieg am Meeting-Tag)",
        ...
        {"indicator": "fomc_today", "op": "<=", "value": 0})
    # --- Trendfilter-/Momentum-Erweiterung (Parität zum Rule-Engine-Katalog:
    # strategies/custom_strategy.INDICATOR_META + fast_sim) – vorher konnte der
    # Copilot diese Indikatoren empfehlen, die Suche kannte sie aber nicht.
    for t in (20, 25):
        add("adx", f"ADX > {t} (nur im Trend handeln)",
            {"indicator": "adx", "op": ">", "value": t},
            {"indicator": "adx", "op": ">", "value": t})
    add("plus_di", "DI+ vs. DI- (Richtungsdruck)",
        {"indicator": "plus_di", "op": ">", "value": "minus_di"},
        {"indicator": "minus_di", "op": ">", "value": "plus_di"})
    add("plus_di", "DI Cross (Richtungswechsel)",
        {"indicator": "plus_di", "op": "cross_above", "value": "minus_di"},
        {"indicator": "minus_di", "op": "cross_above", "value": "plus_di"})
    for t in (100, 150):
        add("cci", f"CCI Reversion (< -{t} / > {t})",
            {"indicator": "cci", "op": "<", "value": -t},
            {"indicator": "cci", "op": ">", "value": t})
    add("cci", "CCI Momentum (Nulllinie)",
        {"indicator": "cci", "op": ">", "value": 0},
        {"indicator": "cci", "op": "<", "value": 0})
    add("keltner_lower", "Keltner Reversion",
        {"indicator": "price", "op": "<", "value": "keltner_lower"},
        {"indicator": "price", "op": ">", "value": "keltner_upper"})
    add("keltner_upper", "Keltner Breakout (Cross)",
        {"indicator": "price", "op": "cross_above", "value": "keltner_upper"},
        {"indicator": "price", "op": "cross_below", "value": "keltner_lower"})
    add("donchian_high", "Donchian Breakout (Cross)",
        {"indicator": "price", "op": "cross_above", "value": "donchian_high"},
        {"indicator": "price", "op": "cross_below", "value": "donchian_low"})
    add("supertrend_dir", "Supertrend Richtung",
        {"indicator": "supertrend_dir", "op": ">=", "value": 1},
        {"indicator": "supertrend_dir", "op": "<=", "value": -1})
    for t in (0.1, 0.3):
        add("ema_slope_pct", f"EMA-Steigung > {t}%",
            {"indicator": "ema_slope_pct", "op": ">", "value": t},
            {"indicator": "ema_slope_pct", "op": "<", "value": -t})
    for t in (0.5, 1.0):
        add("roc", f"ROC > {t}%",
            {"indicator": "roc", "op": ">", "value": t},
            {"indicator": "roc", "op": "<", "value": -t})
    for t in (38.2, 50):
        add("chop", f"Choppiness < {t} (Trendmarkt-Filter)",
            {"indicator": "chop", "op": "<", "value": t},
            {"indicator": "chop", "op": "<", "value": t})
    add("chop", "Choppiness > 61.8 (Seitwärts-Filter für Reversion)",
        {"indicator": "chop", "op": ">", "value": 61.8},
        {"indicator": "chop", "op": ">", "value": 61.8})
    add("aroon_up", "Aroon Up vs. Down",
        {"indicator": "aroon_up", "op": ">", "value": "aroon_down"},
        {"indicator": "aroon_down", "op": ">", "value": "aroon_up"})
    for t in (70, 80):
        add("aroon_up", f"Aroon stark (> {t})",
            {"indicator": "aroon_up", "op": ">", "value": t},
            {"indicator": "aroon_down", "op": ">", "value": t})
    for t in (80, 90):
        add("williams_r", f"Williams %R < -{t} / > -{100 - t}",
            {"indicator": "williams_r", "op": "<", "value": -t},
            {"indicator": "williams_r", "op": ">", "value": -(100 - t)})
...
async def _evaluate_batch(job, pool, items, histories, fs_map, should_stop):
    ...
    b = job.setdefault("_bench", {"evaluations": 0, "cpu_seconds": 0.0,
                                  "sim_seconds": 0.0})
    # Pause-Checkpoint: zentraler Engpass aller Such-Modi (params/discovery/
    # combo/deep/explore/dynamic) – hält den Job an, ohne den Event-Loop zu blockieren
    ...
    while it < iterations:
        ...
        for (flat, p, tp), m in zip(metas, ms):
            ...
            progress(it, iterations,
                     f"{algo_tag} · Kombination {it}/{iterations} · Best Score {score_text(best_score)}")
...
async def _finalize_top5(job, mode, candidates, train_hist, test_hist, settings,
    ...
    for i, cand in enumerate(candidates):
        ...
        if len(train_hist) > 1:
            ...
            # Ausreißer-Assets: dieselben Parameter ohne die 1-2 Assets, die
            # komplett aus dem Raster fallen (services/optimizer_outliers.py)
            ol = optimizer_outliers.detect(per)
            if ol["outliers"]:
                job["phase"] = f"Ausreißer-Check: Kandidat {i + 1}/{n} ohne {', '.join(ol['outliers'])}"
                entry["outlier_variant"] = await _outlier_variant(
                    job, ol, st, s_eff, c_eff, train_hist, fs_map, test_hist, fs_test,
                    should_stop, entry["metrics"] or {})
        entry["passed"] = passed
...
async def _outlier_variant(job, ol, st, s_eff, c_eff, train_hist, fs_map, test_hist,
                           fs_test, should_stop, base_metrics) -> Dict:
    """Kandidat ohne Ausreißer-Assets neu bewerten (Training + ggf. Test-Split)."""
    keep = set(ol["kept"])
    tr = {s: c for s, c in train_hist.items() if s in keep}
    tr_fs = {s: f for s, f in (fs_map or {}).items() if s in keep} if fs_map else None
    m = (await _evaluate_batch(job, None, [(st, s_eff, c_eff)], tr, tr_fs, should_stop))[0]
    test_m = None
    if fs_test is not None and test_hist:
        te = {s: c for s, c in test_hist.items() if s in keep}
        te_fs = {s: f for s, f in fs_test.items() if s in keep}
        if te:
            test_m = (await _evaluate_batch(job, None, [(st, s_eff, c_eff)], te, te_fs,
                                            should_stop))[0]
    return {"excluded": ol["outliers"], "reasons": ol["reasons"], "kept": ol["kept"],
            "metrics": m, "test_metrics": test_m,
            "pnl_gain": round(float(m.get("pnl") or 0) - float(base_metrics.get("pnl") or 0), 2)}


def score_text(score) -> str:
    """Lesbarer Score für Fortschritt/UI: Filter-Ausfälle nicht als -5e8 zeigen."""
    try:
        sc = float(score)
    except (TypeError, ValueError):
        return "–"
    if sc <= -9e8:
        return f"zu wenige Trades ({int(round(sc + 1e9))})"
    if sc <= -4e8:
        return f"DD-Filter verletzt (PnL {sc + 5e8:+.2f})"
    return f"{sc:.2f}"
...
async def run_optimizer(job_id: str, body: Dict, registry, settings: Dict,
    ...
    job = JOBS[job_id]
    # should_stop mit Pause-Unterstützung (in Worker-Threads blockierend,
    # auf dem Event-Loop nur Abbruch-Prüfung – siehe services/job_control.py)
    ...
    try:
        ...
        try:
            ...
            result["top5"] = []
        # Ausreißer-Hinweis des Top-Kandidaten für die Ergebnis-Übersicht
        _ov = (result["top5"][0].get("outlier_variant") if result["top5"] else None)
        result["outlier_summary"] = ({"excluded": _ov["excluded"], "kept": _ov["kept"],
                                      "reasons": _ov["reasons"], "pnl_gain": _ov["pnl_gain"]}
                                     if _ov else None)
