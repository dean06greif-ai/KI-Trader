...
async def _backup_head(session, symbol: str, start_ms: int, before_ms: int,
    ...
    """Ältere Historie aus der Backup-Quelle (Dukascopy) vor den vorhandenen
    Anfang hängen – auf das Preisniveau der Primärquelle skaliert (nahtloser
    Übergang, siehe history_sources.scale_to_anchor). Ohne Backup-Quelle oder
    bei Fehlern: leeres Ergebnis (Verhalten wie bisher)."""
    from services import history_sources
    if not history_sources.has_backup(symbol) or before_ms - start_ms < 86400000:
        return CandleArray.empty()
    try:
        blocks = await history_sources.fetch_backup(session, symbol, start_ms,
                                                    before_ms - 60000, job=job)
    except Exception as e:  # noqa: BLE001
        logger.warning(f"candle_cache: Backup-Historie {symbol} fehlgeschlagen: {e}")
        return CandleArray.empty()
    if not blocks:
        return CandleArray.empty()
    m = history_sources.scale_to_anchor(np.concatenate(blocks), anchor_price)
    ca = CandleArray.from_matrix(m).dedup_sorted()
    ca = ca[:int(np.searchsorted(ca.ts, np.int64(before_ms), side="left"))]
    if len(ca):
        logger.info(f"candle_cache BACKUP-HEAD {symbol} +{len(ca)} (Dukascopy)")
    return ca
...
async def get_candles(session, symbol: str, days: int, job: Dict = None) -> CandleArray:
    ...
    if entry is None:
        ...
        if len(candles) and int(candles.ts[0]) > start + 86400000 \
            ...
            if len(extra):
                ...
                _HEAD_EXHAUSTED[symbol] = time.time()
        async with _LOCK:
    ...
    if needs_head:
        ...
        if len(head):
            ...
            # bis morgen nicht erneut ins Leere fragen
            _HEAD_EXHAUSTED[symbol] = now_ts
