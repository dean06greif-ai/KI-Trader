...
# Katalog aller Telegram-Meldungen: (Schlüssel, Gruppe, Beschriftung, Standard).
# Eine Quelle der Wahrheit für Backend-Toggles UND die Einstellungs-UI
# (GET /api/telegram/notify-catalog). Unter-Schalter (z.B. trade_opened_paper)
# wirken ZUSÄTZLICH zum Haupt-Schalter (trade_opened) – beide müssen an sein.
NOTIFY_CATALOG = [
    # Signale
    ("signals", "Signale", "Signal-Meldungen (LONG/SHORT-Signal mit Entry/SL/TP)", True),
    ("signals_scanner", "Signale", "↳ Signale der Scanner-Strategien", True),
    ("signals_ai", "Signale", "↳ Signale des KI-Traders", True),
    ("signals_only_traded", "Signale", "↳ nur melden, wenn daraus wirklich ein Trade eröffnet wurde (mit Modus + Setup)", True),
    ("signals_collection", "Signale", "↳ auch Datensammel-Trades des KI-Traders melden", True),
    ("confluence", "Signale", "Confluence (mehrere Strategien, gleiche Richtung)", True),
    # Trades
    ("trade_opened", "Trades", "Trade eröffnet", True),
    ("trade_opened_live", "Trades", "↳ eröffnet: Echtgeld-Live", True),
    ("trade_opened_paper", "Trades", "↳ eröffnet: Paper", True),
    ("trade_closed", "Trades", "Trade geschlossen (SL/TP/manuell)", True),
    ("trade_closed_live", "Trades", "↳ geschlossen: Echtgeld-Live", True),
    ("trade_closed_paper", "Trades", "↳ geschlossen: Paper", True),
    ("min_trade", "Trades", "Mindest-Trade statt Ablehnung (Kapital-/Risikogrenze, nur Live)", True),
    ("limit_orders", "Trades", "KI-Limit-Order gefüllt / verfallen (Key-Level-Entries)", True),
    ("order_rejected", "Trades", "Order abgebrochen (Börse hat abgelehnt)", True),
    ("order_rejected_internal", "Trades", "↳ auch interne Stopps (Kapital-Grenze, Risikobudget, Börsen-Minimum)", True),
    ("live_gate_bypass", "Trades", "Live-Bypass (Setup noch nicht live-reif, hohe Konfidenz)", True),
    # Risiko & Sicherheit
    ("kill_switch", "Risiko & Sicherheit", "Kill-Switch / Risiko-Notbremse ausgelöst", True),
    ("watchdog", "Risiko & Sicherheit", "Positions-Watchdog (fehlender SL, übernommene Positionen)", True),
    ("funding_guard", "Risiko & Sicherheit", "Funding-Wächter (hohe echte Funding-Kosten)", True),
    ("maker_mode", "Risiko & Sicherheit", "Maker-Modus ausgesetzt (zu wenige Fills)", True),
    ("ibkr_gateway", "Risiko & Sicherheit", "IBKR-Gateway: ausgeloggt / readyz > 5 Min rot (Forex-Live)", True),
    # KI & System
    ("ai_failure", "KI & System", "KI-Ausfall (Primär + Backup gescheitert, Fallback übernimmt)", True),
    ("website_ai_failure", "KI & System", "KI-Ausfall zusätzlich als Website-Meldung", True),
    ("model_watch", "KI & System", "Modell-Wächter (tote Modell-Slugs erkannt)", True),
    ("token_alert", "KI & System", "Token-Kosten-Wächter (ungewöhnlich hoher Verbrauch)", True),
    ("key_credits", "KI & System", "API-Key-Guthaben niedrig", True),
    ("setup_review", "KI & System", "Setup-Review (Setup rück-/hochgestuft)", True),
    ("policy_lab", "KI & System", "Policy-Labor (Kandidat bewertet/befördert)", True),
    ("copilot_weekly", "KI & System", "Wochenbericht des Strategie-Copiloten", True),
    # Labor & Berichte
    ("backtest_done", "Labor & Berichte", "Backtest fertig", True),
    ("optimizer_done", "Labor & Berichte", "Optimizer fertig", True),
    ("job_series", "Labor & Berichte", "Nacht-Serie: Job fertig / Serie komplett", True),
    ("regime_bridge", "Labor & Berichte", "Regime-Brücke läuft ins Leere – max. 1×/Tag", True),
    ("regime_release_ready", "Labor & Berichte", "Regime bereit für „Wirksam“ – 1× je Freigabe", True),
    ("daily_summary", "Labor & Berichte", "Tägliche Zusammenfassung (Mitternacht)", True),
]
DEFAULT_CONFIG = {k: d for k, _g, _l, d in NOTIFY_CATALOG}
# Interne Stopps (keine Börsen-Ablehnung) – für order_rejected_internal
_INTERNAL_REJECT_PREFIXES = ("risikobudget", "kapital-limit", "kapital-grenze", "rest-kapital",
                             "menge ", "börsen-minimum", "mindest-trade")


def catalog() -> list:
    """Katalog für die UI: [{key, group, label, default}]."""
    return [{"key": k, "group": g, "label": lbl, "default": d} for k, g, lbl, d in NOTIFY_CATALOG]


def is_internal_reject(reason) -> bool:
...
async def enabled(db, ntype: str, sub: Optional[str] = None) -> bool:
    """Haupt-Toggle `ntype` (und optionaler Unter-Toggle `sub`) an?"""
    cfg = await get_config(db)
    return bool(cfg.get(ntype, True)) and (not sub or bool(cfg.get(sub, True)))
...
async def telegram_notify(db, telegram, ntype: str, text: str,
    ...
    """Telegram-Nachricht, wenn der Toggle für `ntype` (+ Unter-Toggle `sub`) an ist."""
    if not await enabled(db, ntype, sub):
        return False
    if cooldown_min > 0:
