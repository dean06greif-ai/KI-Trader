...
from services import trade_review_gate
...
DEFAULT_SETTINGS = {
    ...
    # Token-Sparen (services/trade_review_gate.py): LLM-Review nur bei
    # materieller Änderung, spätestens alle max_llm_gap_min Minuten.
    **trade_review_gate.DEFAULTS,
}
...
TRADE_MANAGER_SYSTEM = (
    ...
    "kurz in 'note'.\n"
    "GESTAFFELTE EXITS (optionales Werkzeug für bessere Trades): Du bist NICHT an das feste "
    "TP1/TP-Full-Schema gebunden. Mit mehreren partial_close-Aktionen über die Laufzeit kannst "
    "du eigene TP-Leitern bauen (z.B. TP2/TP3: 30% am ersten Widerstand, 30% am nächsten Level, "
    "Rest laufen lassen) – kombiniere das mit adjust_tp ('tpf' weiter weg schieben) und "
    "nachgezogenem adjust_sl. Genauso als TEIL-ABSICHERUNG nutzbar (partieller SL): Wenn eine "
    "Position schwächelt, die These aber nicht komplett tot ist, schließe erst 30-50% statt "
    "hart alles – das senkt das Risiko und lässt den Rest arbeiten. Jede Stufe wird im "
    "Trade-Verlauf als TEIL-EXIT protokolliert. Nutze Staffelung gezielt an echten Levels "
    "(S/R, Session-Levels, Liquiditätszonen), nicht schematisch in jedem Durchlauf.\n"
...
def trades_text(trades: List[Dict], prices: Dict[str, float]) -> str:
    ...
    for t in trades:
        ...
        fr = t.get("funding_real_usdt")
        fund_txt = (f"Funding bisher {float(fr):+.2f} USDT (+ = erhalten) | "
                    if isinstance(fr, (int, float)) else "")
        lines.append(
            ...
            f"Liq {t.get('liq_price')} | Menge {qty} | {fund_txt}"
...
class AITradeManager:
    ...
    def __init__(self):
        ...
        # Smart-Skip-Zustand (trade_review_gate): Fingerprint + Zeit des letzten LLM-Reviews
        self._last_fp: Optional[Dict[str, Dict]] = None
    ...
    async def update_settings(self, updates: Dict) -> Dict:
        ...
        if "smart_skip" in updates:
            self.settings["smart_skip"] = bool(updates["smart_skip"])
        for key, lo, hi in (("min_llm_gap_min", 1, 120), ("max_llm_gap_min", 5, 240)):
            if key in updates:
                try:
                    self.settings[key] = max(lo, min(hi, int(updates[key])))
                except (TypeError, ValueError):
                    pass
        for key, lo, hi in (("move_r", 0.1, 2.0), ("near_r", 0.05, 1.0)):
            if key in updates:
                try:
                    self.settings[key] = max(lo, min(hi, float(updates[key])))
                except (TypeError, ValueError):
                    pass
    ...
    async def apply_action(self, trade_id: str, action: str, value: Optional[float] = None,
        ...
        # Extern bei Bitunix eröffnete Positionen (Watchdog-Übernahme,
        # external_adopted=True) bleiben komplett unangetastet: Close/Adjust
        # schlägt an der Börse fehl (z.B. QQQ nicht per OpenAPI handelbar) und
        # erzeugte Telegram-Spam. Nur direkt bei Bitunix verwalten.
        if trade.get("external_adopted"):
            return {"status": "blocked",
                    "detail": "Extern bei Bitunix eröffnete Position – wird hier "
                              "nicht gemanagt (nur direkt bei Bitunix verwalten)"}
        # Website-manuelle Trades ('Manuell (Website)') verwaltet der Nutzer
        # selbst: KI-Aktionen bleiben blockiert, manuelle Aktionen über die
        # Website (source='manual') sind erlaubt (Fix 06/26, Testreport-Bug).
        if source == "ki" and (trade.get("manual_trade")
        ...
        # 'hold' ändert nichts – für normale Strategie-Trades sofort ok
        # (Guards für manuelle/externe Trades bleiben oben davor).
        if action == "hold" and not (source == "ki" and trade.get("data_collection")):
        ...
        if source == "ki":
            # Bugfix: 'from services.bitunix_trade import autotrader' schlug
            # mit ImportError fehl (Singleton lebt in core.state) und brach
            # damit JEDE KI-Trade-Verwaltung ab -> self.autotrader nutzen.
            if not self.autotrader.ai_manage_allowed(trade.get("strategy_id"),
                                                     trade.get("symbol")):
                return {"status": "blocked",
        ...
        # BUGFIX: in_profit wurde nie an check_limits übergeben -> secure_profit
        # war IMMER blockiert ("nur bei Trades im Gewinn") und der höhere
        # Profit-Lock-Hebel-Deckel griff nie. Jetzt: unrealisierter PnL zum
        # aktuellen Mark-Preis entscheidet.
        qty_rem = float(trade.get("qty_remaining", trade.get("qty", 0)) or 0)
        entry_p = float(trade.get("entry") or 0)
        upnl = ((mark - entry_p) if str(trade.get("side", "")).upper() == "LONG"
                else (entry_p - mark)) * qty_rem
        in_profit = upnl > 0
        if enforce_limits:
        ...
        try:
            ...
            elif action == "secure_profit":
                # BUGFIX: secure_profit hatte keinen Ausführungszweig -> lief
                # immer auf "Aktion nicht ausgeführt". Jetzt: X% der gebundenen
                # Margin entnehmen (Hebel steigt automatisch, Position bleibt).
                lev = float(trade.get("leverage", 1) or 1)
                margin = float(trade.get("margin_used")
                               or (qty_rem * entry_p / lev if lev else 0))
                amount = margin * float(value) / 100
                if amount <= 0:
                    return {"status": "error", "detail": "Keine gebundene Margin ermittelbar"}
                result = await self.autotrader.adjust_margin(trade_id, -amount)
    ...
    async def open_trade(self, spec: Dict, source: str = "ki") -> Dict:
        ...
        signal = {
            ...
            "ai_secure_runner": bool(runner and (self.engine.config or {})
                                     .get("runner_secure_enabled", True)),
            "ai_secure_runner_trigger": float((self.engine.config or {})
                                              .get("runner_secure_trigger_pct", 30.0) or 30.0),
            "ai_secure_runner_max_lev": float((self.engine.config or {})
                                              .get("runner_secure_max_leverage", 100) or 100),
    ...
    async def review(self, manual: bool = False) -> Dict:
        ...
        try:
            ...
            # (Bugfix: kaputter Import 'autotrader' brach hier den kompletten
            # Review-Lauf mit ImportError ab -> self.autotrader nutzen.)
            trades = [t for t in trades
                      if self.autotrader.ai_manage_allowed(t.get("strategy_id"),
                                                           t.get("symbol"))]
            for t in trades:
                t.pop("_id", None)
            if not trades:
            ...
            for t in trades:
                ...
                if sym not in prices:
                    prices[sym] = float(await self.autotrader._current_mark(sym)
                                        or t.get("entry") or 0)
            # Smart-Skip (services/trade_review_gate.py): kein LLM-Call, wenn sich
            # seit dem letzten Review nichts Materielles bewegt hat.
            cur_fp = trade_review_gate.fingerprint(trades, prices)
            if not manual:
                since = ((time.time() - self._last_llm_ts) / 60.0
                         if self._last_llm_ts else None)
                do_review, why = trade_review_gate.should_review(
                    self._last_fp, cur_fp, since, self.settings)
                if not do_review:
                    self.skipped_reviews += 1
                    self.last_skip_reason = why
                    self.last_run = _now_iso()
                    logger.info(f"Trade-Manager Smart-Skip: {why}")
                    return {"status": "skipped", "detail": f"Smart-Skip: {why}",
                            "trades": len(trades)}
                logger.info(f"Trade-Manager Review-Grund: {why}")
            context = ""
            try:
                context = await self.engine._analysis_extra_blocks(
                    purpose="trade_review", symbols=sorted(prices))
            except Exception:
                pass
            prompt = (
            ...
            if applied or opened:
                # Eigene Anpassungen nicht als 'Änderung' fürs nächste Smart-Skip zählen
                try:
                    fresh = await self.db.auto_trades.find(
                        {"id": {"$in": [t.get("id") for t in trades]}, "status": "open"}) \
                        .to_list(50)
                    self._last_fp = trade_review_gate.fingerprint(fresh, prices)
                except Exception as e:  # noqa: BLE001
                    logger.debug(f"Trade-Manager Fingerprint-Refresh: {e}")
    ...
    def status(self) -> Dict:
        return {"settings": dict(self.settings), "role_enabled": self._role_cfg().get("enabled", True),
                ...
                "smart_skip": {"skipped_reviews": self.skipped_reviews,
                               "last_skip_reason": self.last_skip_reason,
                               "last_llm_review": (datetime.fromtimestamp(
                                   self._last_llm_ts, timezone.utc).isoformat()
                                   if self._last_llm_ts else None)}}
