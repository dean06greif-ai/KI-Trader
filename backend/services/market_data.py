...
async def fetch_yahoo(session, yahoo_symbol: str, yrange: str = "1d") -> List[Dict]:
    """
    Free Yahoo Finance 1-minute candles for commodities/metals/forex
    (e.g. GC=F gold, SI=F silver, CL=F oil, EURUSD=X). Requires a browser User-Agent.
    Markets are not 24/7, so use range=5d for bootstrap to guarantee history.
    Fällt bei Fehler/Rate-Limit (HTTP 429 auf query1) auf den zweiten Yahoo-
    Host zurück – ein hängender Host darf den Forex-Feed nicht stundenlang
    einfrieren (Stale-Price-Guard blockte sonst alle FX-Einstiege).
    """
    data = None
    ...
    for host in YAHOO_HOSTS:
        ...
        try:
            ...
            break
        except Exception as e:  # noqa: BLE001 – nächster Host
            last_err = e
            logger.debug(f"Yahoo {host} {yahoo_symbol}: {e} – nächster Host")
    if data is None:
        raise last_err or RuntimeError("Yahoo: keine Antwort")
    try:
