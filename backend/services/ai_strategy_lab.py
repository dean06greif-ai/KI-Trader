...
DEVELOP_SYSTEM = (
    "Du bist der 'KI Trader' einer Krypto-Daytrading-Plattform im TRAININGSLAGER. "
    "Deine Aufgabe: aus den Vergleichsdaten der anderen Strategien (Parameter + echte "
    "Ergebnisse), deinen eigenen Lektionen und den bisherigen Labor-Kandidaten EIN neues, "
    "vielversprechendes Trading-Setup entwickeln – oder ehrlich sagen, dass die Datenlage "
    "aktuell kein neues Setup rechtfertigt. Regeln:\n"
    "1. Baue auf dem auf, was NACHWEISLICH funktioniert (Winrate/PnL der anderen Strategien, "
    "Optimizer-Erkenntnisse) und vermeide, was nachweislich scheitert.\n"
    "2. KEIN Duplikat: Das Setup muss sich klar von bestehenden Strategien und "
    "Labor-Kandidaten unterscheiden (andere Kombination von Filtern/Triggern/Timeframes).\n"
    "3. Nenne in learned_from konkret, welche Strategien/Parameter/Erkenntnisse dich "
    "inspiriert haben.\n"
    "4. Liefere WENN MÖGLICH eine maschinenlesbare rule_definition (Syntax unten), damit das "
    "Setup sofort im Backtester/Optimizer und als Ghost-Kandidat getestet werden kann.\n"
    "5. Das Setup startet IMMER als Ghost (reine Simulation) – echtes Geld erst nach "
    "bestandenen Schwellen UND Freigabe des Traders.\n"
    "Antworte AUSSCHLIESSLICH mit validem JSON ohne Markdown:\n"
    '{"should_create": true|false, '
    '"reason": "2-4 Sätze: warum dieses Setup jetzt (oder warum keins)", '
    '"name": "prägnanter Name", "thesis": "Idee/Marktlogik auf Deutsch", '
    '"rules_text": "präzise Regeln in Prosa (Einstieg, Ausstieg, Risiko)", '
    '"symbols": ["BTCUSDT"], "timeframe": "1m|5m|15m|1h", '
    '"learned_from": "welche Strategien/Parameter/Erkenntnisse Pate standen", '
    '"rule_definition": {...} oder null}'
)
...
class StrategyLab:
    ...
    async def assist(self, spec: Dict, cid: Optional[str] = None,
        ...
        # Lese-Zugriff auf die anderen Plattform-Strategien als Lernkontext
        insights_block = ""
        try:
            from services import strategy_insights
            insights_block = await strategy_insights.context_text(self.db, max_chars=2000)
        except Exception as e:
            logger.warning(f"Cross-Strategie-Kontext (Assist) fehlgeschlagen: {e}")
        prompt = (
            f"{TESTING_NOTE}\n\n"
    ...
    async def develop(self, focus: str = "") -> Dict:
        """Trainingslager: Der KI-Trader entwickelt SELBST ein neues Setup.

        Kontext: Cross-Strategie-Vergleich (Parameter + echte Ergebnisse der
        anderen Strategien), eigene Lektionen und bisherige Labor-Kandidaten.
        Das Ergebnis durchläuft die normale sichere Pipeline (Ghost -> Freigabe)."""
        if not self.settings.get("allow_ai_create", True):
            return {"status": "blocked", "detail": "Neue KI-Strategien sind deaktiviert"}
        if not self.engine or not self.engine.key:
            return {"status": "error", "detail": "Kein API-Key für den aktiven KI-Provider"}
        active = await self.db[COLL].count_documents({"stage": {"$in": ["ghost", "live_pending"]}})
        if active >= int(self.settings.get("max_active_candidates", 5)):
            return {"status": "blocked",
                    "detail": f"Zu viele Kandidaten in der Testphase ({active}) – "
                              "erst bestehende bewerten/abschließen"}
        insights_block = ""
        try:
            from services import strategy_insights
            insights_block = await strategy_insights.context_text(self.db, max_chars=2600)
        except Exception as e:
            logger.warning(f"Cross-Strategie-Kontext (Develop) fehlgeschlagen: {e}")
        lessons_block = ""
        try:
            if self.engine.learning:
                lessons_block = await self.engine.learning.lessons_text()
        except Exception:
            pass
        lab_block = await self.context_text()
        prompt = (
            (f"=== FOKUS DES TRADERS ===\n{str(focus)[:400]}\n\n" if str(focus).strip() else "")
            + (f"{insights_block}\n\n" if insights_block else "")
            + (f"=== DEINE EIGENEN LEKTIONEN ===\n{lessons_block}\n\n" if lessons_block else "")
            + f"{lab_block}\n\n"
            f"{TESTING_NOTE}\n\n"
            "Entwickle jetzt datenbasiert EIN neues Setup – oder begründe, warum aktuell "
            "keins sinnvoll ist (should_create=false)."
        )
        try:
            text, provider, model = await self.engine.generate_for_role(
                "research_analyst", prompt, DEVELOP_SYSTEM, temperature=0.5)
            data = self.engine._parse_json(text)
        except Exception as e:
            logger.error(f"Setup-Entwicklung fehlgeschlagen: {e}")
            return {"status": "error", "detail": f"KI-Antwort fehlgeschlagen: {str(e)[:150]}"}
        reason = str(data.get("reason", ""))[:600]
        if not data.get("should_create"):
            await self.db.ai_chat.insert_one({
                "id": str(uuid.uuid4()), "role": "strategy",
                "text": f"Trainingslager: aktuell KEIN neues Setup entwickelt – {reason}",
                "ts": _now_iso()})
            return {"status": "skipped", "reason": reason or "Datenlage reicht nicht",
                    "model": f"{provider}/{model}"}
        rd = data.get("rule_definition")
        if not valid_rule_definition(rd):
            rd = None
        spec = {"name": data.get("name"), "thesis": data.get("thesis"),
                "rules_text": data.get("rules_text"),
                "symbols": data.get("symbols") or [],
                "timeframe": data.get("timeframe") or "1m",
                "learned_from": data.get("learned_from"),
                "rule_definition": rd}
        res = await self.create_candidate(spec, source="ki")
        res["reason"] = reason
        res["model"] = f"{provider}/{model}"
        res["backtestable"] = bool(rd)
        if res.get("status") == "ok":
            await memory.remember(
                "insight", f"Trainingslager-Setup {spec.get('name')}",
                f"{reason} | Gelernt von: {spec.get('learned_from') or '-'}",
                meta={"candidate_id": (res.get("candidate") or {}).get("id")},
                tags=["strategy", "training"], weight=3, source="strategy_lab")
        return res
