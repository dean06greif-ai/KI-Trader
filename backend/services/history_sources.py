...
Zusätzlich gibt es eine BACKUP-Quelle (Dukascopy, kostenlos & ohne Key) für
ältere Historie, die die Primärquelle nicht mehr hat – siehe Abschnitt
"Backup-Quelle" unten und services/candle_cache.py (_backup_head).
...
def days_cap(symbol: str, days: int) -> int:
    ...
    mehr als das Yahoo-Limit aus core.instruments). Mit Backup-Quelle
    (Dukascopy) sind auch über das Quellen-Limit hinaus Kerzen ladbar."""
    if _resolve(symbol)[0] == "ibkr":
        return max(1, min(int(days), IBKR_MAX_DAYS))
    cap = instruments.history_days_cap(symbol, days)
    if cap < int(days) and has_backup(symbol):
        cap = min(int(days), BACKUP_MAX_DAYS)
    return max(1, cap)
...
# --------------------------------------------------------------------------
# Backup-Quelle: Dukascopy (kostenlos, keine Keys, Jahre an 1m-Historie).
# Genutzt, wenn die Primärquelle keine ältere Historie mehr hat (Bitunix:
# Kerzen erst ab Kontrakt-Listing; Yahoo-Forex: nur ~30 Tage). Ein Tages-File
# (BID_candles_min_1.bi5, LZMA-komprimiert) enthält die 1m-Kerzen eines Tages;
# Zeilenformat big-endian: (Sekunden ab Tagesbeginn, open, close, low, high
# als Ganzzahl × point, Volumen float32). ACHTUNG: Monat in der URL 0-basiert.
# --------------------------------------------------------------------------
DUKA_URL = ("https://datafeed.dukascopy.com/datafeed/"
            "{ref}/{y}/{m:02d}/{d:02d}/BID_candles_min_1.bi5")
DUKA_ROW = struct.Struct(">3i2if")
_FX_POINT = 100_000.0
# App-Symbol -> (Dukascopy-Instrument, point-Faktor der Preis-Ganzzahlen)
DUKA_REFS: Dict[str, tuple] = {
    "GOLD": ("XAUUSD", 1_000.0),
    "SILVER": ("XAGUSD", 1_000.0),
    "OIL": ("LIGHTCMDUSD", 1_000.0),          # WTI
    "QQQUSDT": ("USATECHIDXUSD", 1_000.0),    # Nasdaq-100-Index (skaliert auf QQQ)
    "SPYUSDT": ("USA500IDXUSD", 1_000.0),     # S&P-500-Index (skaliert auf SPY)
    "EURUSD": ("EURUSD", _FX_POINT),
    "GBPUSD": ("GBPUSD", _FX_POINT),
    "USDJPY": ("USDJPY", 1_000.0),
    "AUDUSD": ("AUDUSD", _FX_POINT),
    "USDCAD": ("USDCAD", _FX_POINT),
    "USDCHF": ("USDCHF", _FX_POINT),
    "NZDUSD": ("NZDUSD", _FX_POINT),
}
BACKUP_MAX_DAYS = 365          # Deckel wie das Backtester-Maximum
DUKA_EMPTY_DAY_STOP = 12       # so viele leere Tage in Folge = Historien-Ende


def has_backup(symbol: str) -> bool:
...
def decode_duka_day(raw: bytes, day_start_ms: int, point: float) -> Optional[np.ndarray]:
    ...
    try:
        ...
        return None
    n = len(data) // DUKA_ROW.size
    if not n:
        return None
    rows = []
    for i in range(n):
        sec, o, c, lo, hi, vol = DUKA_ROW.unpack_from(data, i * DUKA_ROW.size)
        o, c, lo, hi = o / point, c / point, lo / point, hi / point
        if o <= 0 or c <= 0:
            continue
        rows.append([day_start_ms + sec * 1000, o, hi, lo, c,
                     float(vol) if vol and vol > 0 else activity_volume(hi, lo, c)])
    return np.array(rows, dtype=np.float64) if rows else None


def scale_to_anchor(m: np.ndarray, anchor_price: float) -> np.ndarray:
    """Backup-Serie auf das Preisniveau der Primärquelle skalieren (rein).

    Index-Perps (QQQ/SPY) laufen auf ETF-Niveau, Dukascopy liefert den Index
    selbst – der Splice-Faktor macht den Übergang nahtlos; prozentuale
    Bewegungen (und damit alle Setups/Indikatoren) bleiben identisch."""
    if m is None or not m.shape[0] or not anchor_price:
        return m
    last_close = float(m[-1, 4])
    if last_close <= 0:
        return m
    ratio = float(anchor_price) / last_close
    if not (0.001 <= ratio <= 1000.0):   # unplausibel -> lieber unskaliert
        return m
    out = m.copy()
    out[:, 1:5] *= ratio
    return out
...
async def _duka_day(session, ref: str, day_dt: datetime) -> Optional[bytes]:
    ...
    logger.warning(f"dukascopy {ref} {day_dt:%Y-%m-%d}: {err}")
    return None


async def fetch_backup(session, symbol: str, start_ms: int, end_ms: int,
                       job: Dict = None, pace: float = 0.3) -> List[np.ndarray]:
    """Backup-Historie (Dukascopy) rückwärts tageweise laden.

    Bricht bei dauerhaften Fehlern ab statt Lücken zu hinterlassen (der Kopf
    bleibt zusammenhängend, der Rest wird beim nächsten Lauf nachgeladen) und
    endet nach DUKA_EMPTY_DAY_STOP leeren Tagen in Folge (Historien-Ende)."""
    key = (symbol or "").upper()
    if key not in DUKA_REFS or end_ms <= start_ms:
        return []
    ref, point = DUKA_REFS[key]
    day = datetime.fromtimestamp(end_ms / 1000, timezone.utc).replace(
        hour=0, minute=0, second=0, microsecond=0)
    first = datetime.fromtimestamp(start_ms / 1000, timezone.utc).replace(
        hour=0, minute=0, second=0, microsecond=0)
    blocks: List[np.ndarray] = []
    empty_streak = 0
    ...
    while day >= first:
        ...
        if raw is None:
            logger.warning(f"dukascopy {ref}: Download endet bei {day:%Y-%m-%d} – "
                           "bereits geladene Tage bleiben gültig")
            break
        m = decode_duka_day(raw, int(day.timestamp() * 1000), point)
        if m is None:
            empty_streak += 1
            if empty_streak >= DUKA_EMPTY_DAY_STOP:
                break
        else:
            empty_streak = 0
            blocks.append(m)
