...
class AutoTradeManager:
    ...
    async def sync_position_state(self, local: Dict, pos: Dict) -> List[str]:
        ...
        if ex_margin > 0 and notional > 0 and \
            ...
            # (Das 'leverage'-Feld ist nur das Setting – nach Margen-Anpassung
            # weicht der effektive Hebel ab, sonst würde der Sync flattern.)
            new_lev = round(ex_lev, 2)
            new_margin = round(notional / max(ex_lev, 0.01), 4)
        if new_lev is not None:
            old_lev = float(local.get("leverage") or 0)
            liq = self.liq_price_for(local["side"], entry, new_lev)
            updates.update({"leverage": new_lev, "margin_used": new_margin,
                            "liq_price": liq})
            msg = (f"SYNC: Marge/Hebel extern geändert – Hebel {old_lev}x -> "
                   f"{new_lev}x, Marge {round(cur_margin, 2)} -> {new_margin} "
                   f"USDT, Liq {liq}")
            events.append(msg)
            changes.append(msg)
