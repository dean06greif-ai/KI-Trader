...
def fee_guard_check(ai_cfg: Dict, cfg: Dict, entry: float, sl: float,
    ...
    if sl_dist_pct + 1e-9 < min_pct:
        ...
        return False, (
            ...
            f"({mult:g}× (Roundtrip-Fees {2 * fee:.2f}%{funding_note})) – Kosten "
...
class AutoTradeManager:
    ...
    async def _on_signal_impl(self, signal: Dict, candles: List[Dict]) -> Optional[Dict]:
        ...
        if not guard_ok:
            ...
            return None
        # ---- Marktphasen-Filter (Regime-Gate): neue Trades in blockierten
        # Phasen (z.B. Seitwärtsmarkt) überspringen. Fail-open bei Fehlern.
        if cfg.get("regime_filter_enabled") and not collection:
            from services import regime_gate
            rg_ok, rg_reason = await regime_gate.check_signal_allowed(cfg, symbol)
            if not rg_ok:
                logger.info(f"AutoTrade blockiert {symbol} {side}: {rg_reason}")
                signal["_reject_reason"] = rg_reason
                return None
        ...
        if strategy_id == "ai_trader":
            ...
            if not fg_ok:
                ...
                try:
                    ...
                    fee_pct = effective_fee_percent(cfg)
        ...
        lev_used = max(1.0, min(lev_used, self._coin_max_lev(symbol)))
        mode = eff_mode
        ...
        if isinstance(sizing, dict) and sizing.get("mode") == "risk":
            try:
        ...
        if free_alloc is not None:
            ...
            if free_alloc < 5.0:
                logger.info(f"{symbol}: Kapital-Limit erreicht "
                            f"({used:.2f}/{cap_str} USDT belegt, "
                            f"frei {free_alloc:.2f}) -> kein Trade")
                signal["_reject_reason"] = (f"Kapital-Limit erreicht: "
                                            f"{used:.2f}/{cap_str} USDT belegt")
                await self._notify_reject(
                    symbol, side,
                    f"Kapital-Limit erreicht: {used:.2f}/{cap_str} USDT belegt")
                return None
        ...
        # ---- LIVE MODE (IBKR-Forex): Bracket-Order (Market + SL/TP) über das
        # Client-Portal-Gateway. Live-Forex nutzt den VOLLEN TP (tpf) – IBKR-
        # Brackets tragen genau einen TP; das Monitoring/Verbuchen übernimmt
        # services/ibkr_trade.run_loop(). Nur bei Erfolg persistieren.
        if mode == "live" and use_ibkr:
            from services import ibkr_trade
            ib_res = await ibkr_trade.open_live_forex(
                symbol=symbol, side=side, entry=entry, sl=sl, tp=tpf,
                notional_usd=capital * lev_used)
            ...
            tp1 = tpf  # Live-Forex: ein TP (Bracket), kein Partial-TP1
        ...
        # Forex: IBKR-Kommissionsmodell (inkl. Mindestkommission) statt der
        # Krypto-Futures-Gebühr – getrennt einstellbar in den Haupteinstellungen.
        from services import fee_model
        fee_pct_used = fee_model.fee_percent_for(symbol, fee_pct_used,
    ...
    async def monitor(self, prices: Dict[str, float]):
        ...
        async for t in cursor:
            if t.get("external_adopted"):
                ...
                continue
            if t.get("broker") == "ibkr" and t.get("mode") == "live":
                # IBKR-Forex-Live: SL/TP liegen als Bracket bei IBKR, das
                # Verbuchen übernimmt services/ibkr_trade.run_loop().
                continue
            symbol = t["symbol"]
    ...
    async def _live_flash_close(self, t, qty):
        ...
        IBKR-Forex-Trades werden über das IBKR-Gateway geschlossen (Bracket-
        Orders stornieren + Gegen-Market-Order); das Verbuchen des Abschlusses
        übernimmt danach der IBKR-Sync-Loop bzw. der Aufrufer."""
        if t.get("broker") == "ibkr":
            from services import ibkr_trade
            res = await ibkr_trade.close_live_forex(t)
            return {"ok": bool(res.get("ok")),
                    "detail": res.get("error") or "IBKR-Close gesendet"}
