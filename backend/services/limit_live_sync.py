...
def should_arm(free: Optional[float], margin: float, dist_pct: Optional[float],
    ...
    return (dist_pct <= float(cfg.get("arm_dist_pct", 1.5) or 0)
            and margin <= (capital_fit.usable_free(free, True) or 0.0))
...
def plan_size(autotrader, row: Dict, equity: Optional[float] = None,
              free: Optional[float] = None) -> Optional[Dict]:
    ...
    `free` = freies Live-Kapital -> Marge wird auf den Rest begrenzt."""
    ...
    if isinstance(sizing, dict) and sizing.get("mode") == "risk" and sl > 0:
        try:
            ...
            rs = position_sizing.compute(sizing, cfg, entry, sl, equity,
                                         ...
                                         preset_lev=lev,
                                         lev_source="Trade-Einstellungen (Limit-Fill)")
            if rs:
    ...
    if capital <= 0:
        return None
    # Rest-Kapital-Trading: bei knappem Konto mit dem verfügbaren Rest armieren
    # (Untergrenze/Börsen-Minimum via services/capital_fit.py) statt nie live zu gehen.
    fit_note = None
    if free is not None:
        from services import capital_fit
        min_qty = 0.0
        try:
            min_qty = float((autotrader.client.contract_meta(
                autotrader.client.to_bitunix_symbol(symbol)) or {}).get("min_qty") or 0)
        except Exception:  # noqa: BLE001
            min_qty = 0.0
        fit = capital_fit.fit_margin(capital, free, lev, entry, min_qty=min_qty, live=True)
        if fit["reject"]:
            return None
        capital, fit_note = fit["margin"], fit["note"]
...
async def sync(db, autotrader, prices: Dict[str, float]) -> None:
    ...
    for row in rows:
        ...
        if retry_at and str(retry_at) > now_iso:
            continue
        plan = plan_size(autotrader, row, equity=(fc or {}).get("allocated"), free=free)
        if not plan:
            continue
        if should_arm(free, plan["margin"], dist_pct, cfg):
            if plan.get("fit_note"):
                logger.info(f"{symbol}: Limit-Arm mit Rest-Kapital – {plan['fit_note']}")
