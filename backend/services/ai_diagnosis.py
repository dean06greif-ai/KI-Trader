...
# Frische-Schwelle (min) je Markt: Krypto-Feeds liefern minütlich; Yahoo-Kurse
# (Forex/Rohstoffe) hinken regulär 1-3 min hinterher -> weiteres Fenster,
# identisch zur Standard-Schwelle des Stale-Price-Guards (stale_price_max_min).
STALE_MIN_CRYPTO = 5.0
STALE_MIN_PAUSED_MARKETS = 10.0


def stale_limit(row: Dict) -> float:
    return STALE_MIN_PAUSED_MARKETS if row.get("has_pauses") else STALE_MIN_CRYPTO
...
def build_findings(live: Dict, paper: Dict, per_setup_live: Dict, per_setup_paper: Dict,
    ...
    # Nur bei OFFENEM Markt ist eine alte Kerze ein Feed-Fehler. Forex/Rohstoffe/
    # Indizes am Wochenende bzw. in der täglichen Handelspause liefern zu Recht
    # keine neuen Kerzen (Bug-Report 09/2026: 7 Forex-Paare samstags als
    # "KRITISCH veraltet" gemeldet – der Markt war schlicht geschlossen).
    stale_syms = [d for d in data_quality
                  if d.get("last_candle_age_min", 0) > stale_limit(d) and not d.get("market_closed")]
    if stale_syms:
        names = ", ".join(f"{d['symbol']} ({d['last_candle_age_min']:.0f} min)" for d in stale_syms[:5])
        add("kritisch", "daten",
            f"Veraltete Kursdaten bei {len(stale_syms)} Symbol(en) trotz geöffnetem Markt: "
            f"{names} – der Stale-Price-Guard blockt hier zu Recht; Datenfeed prüfen "
            "(Krypto: Bitunix-WS/Backfill · Forex/Rohstoffe: Yahoo-Feed).")
    closed_syms = [d for d in data_quality
                   if d.get("market_closed") and d.get("last_candle_age_min", 0) > stale_limit(d)]
    if closed_syms:
        names = ", ".join(d["symbol"] for d in closed_syms[:7])
        add("info", "daten",
            f"{len(closed_syms)} Symbol(e) ohne frische Kerzen, weil der Markt geschlossen ist "
            f"({names}) – normal (Wochenende/Handelspause), kein Datenfehler. "
            "Der Wochenend-Guard verhindert dort Einstiege.")
...
def data_quality_rows(scanner) -> List[Dict]:
    ...
    for sym, candles in buf.items():
        ...
        try:
            ...
            of_real = None
        inst = _instr.get(sym)
        closed, closed_why = market_hours.is_market_closed(sym)
        rows.append({"symbol": sym, "candles": len(candles),
                     "last_candle_age_min": round(max(age_min, 0), 1),
                     "orderflow_real": of_real,
                     "has_pauses": bool(inst is not None and inst.has_trading_pauses),
                     "market_closed": closed,
                     "market_closed_reason": closed_why or None})
    rows.sort(key=lambda r: (r["market_closed"], -r["last_candle_age_min"]))
    return rows
...
async def build(db, scanner, config: Dict, days: int = 14) -> Dict:
    ...
    # ---- Setup-Katalog + Live-Reife (Statistik = aktive Variante je Setup) ----
    setups: List[Dict] = []
    try:
        from services import ai_playbook
        stats = await ai_playbook.variant_setup_stats(db)
        for sid, desc in ai_playbook.SETUPS.items():
            st = stats.get(sid) or {}
            ok, why = ai_playbook.live_ready_for(sid, st if st else None)
            setups.append({"setup": sid, "beschreibung": desc,
                           ...
                           "variant_since": (str(st.get("variant_since"))[:10]
                                             if st.get("variant_since") else None),
                           "live_ready": ok, "live_reason": why,
                           "disabled": ai_playbook.disabled_reason(sid),
                           "demoted": ai_playbook.live_block_reason(sid)})
    except Exception as e:
