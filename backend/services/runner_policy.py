...
# Runner-Test bei Trendfolge-Setups (09/2026): auch OHNE News darf ein Teil der
# Trendfolge-Scalps als Runner laufen (A/B-Anteil `runner_trend_share`, deter-
# ministisch über die Entscheidungs-ID) – so entsteht eine vergleichbare
# Statistik Runner vs. festes Ziel je Setup (Trade-Feld ai_runner).
TREND_RUNNER_SETUPS = ("trend_follow", "trend_follow2")


def _ab_pick(dec: Dict, share: float) -> bool:
    """Deterministischer A/B-Anteil aus der Entscheidungs-ID (rein)."""
    key = str(dec.get("id") or dec.get("symbol") or "")
    if not key:
        return False
    bucket = sum(ord(ch) for ch in key) % 100
    return bucket < int(round(max(0.0, min(1.0, share)) * 100))


def trend_runner_allowed(dec: Dict, config: Dict) -> bool:
    cfg = config or {}
    if not cfg.get("runner_trend_setups", True):
        return False
    if str(dec.get("setup") or "") not in TREND_RUNNER_SETUPS:
        return False
    return _ab_pick(dec, float(cfg.get("runner_trend_share", 0.5) or 0))
...
def runner_allowed(dec: Dict, config: Dict) -> bool:
    ...
    if cfg.get("runner_scalp_news_only", True):
        if str(dec.get("news_impact") or "neutral") in ("positive", "negative"):
            return True
        return trend_runner_allowed(dec, cfg)
    return True
