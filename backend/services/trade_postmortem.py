    ...
    async def _candles(self, symbol: str, start_ms: int) -> List[Dict]:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as s:
                arr = await candle_cache.get_candles(s, symbol, min(days, MAX_AGE_DAYS + 1))
            rows = _c.as_list(arr) if arr is not None else []
            if rows:
                return rows
        except Exception as e:
            logger.debug(f"Nachanalyse {symbol}: candle_cache fehlgeschlagen: {e}")
        try:
