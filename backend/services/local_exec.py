...
# ---------------- Worker-Registry ----------------
REQUIRED_WORKER_VERSION = (1, 10, 0)
REQUIRED_WORKER_VERSION_STR = "1.10.0"
...
async def apply_progress(job_id: str, data: Dict, db=None) -> Dict:
    ...
    if job is None:
        LOCAL_JOBS.pop(job_id, None)
        return {"cancel": True}
    _revive_if_conn_lost(job_id, job)
...
def _revive_if_conn_lost(job_id: str, job: Dict):
    ...
    if job.get("status") == "error" and job.get("_conn_lost"):
        ...
        LOCAL_JOBS.setdefault(job_id, {"kind": None, "enqueued_at": _now()})
...
def check_stale():
    for jid, meta in list(LOCAL_JOBS.items()):
        ...
        if meta.get("state") == "queued":
            ...
            silent_for = _now() - meta.get("last_update", 0)
            if silent_for > OFFLINE_JOB_TIMEOUT:
                ...
                LOCAL_JOBS.pop(jid, None)
            elif offline and silent_for > OFFLINE_HINT_AFTER and not meta.get("offline_hint"):
                meta["offline_hint"] = True
                job["phase"] = ("⚠ Verbindung zum lokalen Worker unterbrochen – der Worker "
