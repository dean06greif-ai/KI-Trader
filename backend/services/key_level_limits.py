...
Paper-Orders bleiben lokal (Fill = Market am Level, paper_execution rechnet
Spread/Slippage an). LIVE-Orders werden zusätzlich von
services/limit_live_sync.py als ECHTE Limit-Orders nach Bitunix gespiegelt
(sofort bei genug freiem Kapital, sonst "smart" erst nahe am Level), damit
schnelle Wicks direkt an der Börse füllen. Rows mit `live_order_id` werden
hier beim Touch-Fill übersprungen – die Börse ist dann die Wahrheit.
"""
import logging
import uuid
...
MIN_VALID_MIN = 5
MAX_VALID_MIN = 720
MAX_VALID_MIN_SWING = 2880   # Swing-Setups an Haupt-S/R-Zonen dürfen bis 48h warten
...
def _max_valid(horizon) -> int:
    """Swing-Limit-Orders an 4h/1d-Zonen dürfen deutlich länger warten."""
    return MAX_VALID_MIN_SWING if str(horizon or "").lower() == "swing" else MAX_VALID_MIN
...
def parse_decision_fields(d: Dict) -> Dict:
    ...
    try:
        ...
        vm = 0
    vm = max(MIN_VALID_MIN, min(_max_valid(d.get("horizon")), vm)) if vm > 0 \
        else DEFAULT_VALID_MIN
...
def _md(text) -> str:
    """Markdown-kritische Zeichen entschärfen (wie notifications._md_safe)."""
    return (str(text or "").replace("*", "").replace("_", " ").replace("`", "'")
            .replace("[", "(").replace("]", ")"))


def _wait_str(row: Dict) -> str:
    try:
        start = datetime.fromisoformat(str(row.get("created_at")))
        mins = max(0, int((_now() - start).total_seconds() // 60))
        return f"{mins // 60}h {mins % 60}m" if mins >= 60 else f"{mins}m"
    except (ValueError, TypeError):
        return "?"


def _tg_fill_text(row: Dict, opened: bool, reject: Optional[str] = None) -> str:
    """Detaillierte Telegram-Nachricht für einen (versuchten) Limit-Fill."""
    sig = row.get("signal") or {}
    side, sym = row.get("side"), row.get("symbol")
    lp = row.get("limit_price")

    def _lvl(label, key, sign):
        v = sig.get(key)
        if v is None:
            return f"{label}: n/a"
        try:
            pct = abs((float(v) - float(lp)) / float(lp) * 100)
            return f"{label}: `{_fmt_price(v)}` ({sign}{pct:.2f}%)"
        except (TypeError, ValueError, ZeroDivisionError):
            return f"{label}: `{_fmt_price(v)}`"

    if opened:
        emoji = "🟢" if side == "LONG" else "🔴"
        header = f"{emoji} *LIMIT-ORDER GEFÜLLT* – Trade eröffnet"
    else:
        header = "⚠️ *LIMIT ERREICHT – Trade abgelehnt*"
    lines = [header, "",
             f"💰 *{sym}* · {side} @ `{_fmt_price(lp)}`",
             _lvl("🛑 SL", "stop_loss", "-"),
             _lvl("🎯 TP1", "take_profit_1", "+"),
             _lvl("🚀 TP", "take_profit_full", "+")]
    size_bits = []
    try:
        cap = float(sig.get("ai_max_capital") or 0)
        pct = float(sig.get("ai_capital_pct") or 100)
        if cap > 0:
            size_bits.append(f"Einsatz ~{cap * pct / 100:.0f} USDT")
    except (TypeError, ValueError):
        pass
    try:
        lev = float(sig.get("ai_leverage") or 0)
        if lev > 0:
            size_bits.append(f"Hebel {lev:g}x")
    except (TypeError, ValueError):
        pass
    if size_bits:
        lines.append(f"💵 {' · '.join(size_bits)}")
    meta = f"🧠 Konfidenz {row.get('confidence') or '-'} · {row.get('horizon') or 'scalp'}"
    if row.get("setup"):
        meta += f" · Setup {_md(row['setup'])}"
    lines.append(meta)
    reason = _md(row.get("reason") or sig.get("ai_reasoning") or "")[:220]
    if reason:
        lines.append(f"📝 {reason}")
    if not opened and reject:
        lines.append(f"❌ Grund: {_md(reject)[:160]}")
    lines.append(f"⏳ gewartet: {_wait_str(row)}")
    return "\n".join(lines)


def _tg_expired_text(row: Dict, cur_price: Optional[float]) -> str:
    """Telegram-Nachricht für eine verfallene Limit-Order."""
    sig = row.get("signal") or {}
    lp = row.get("limit_price")
    lines = ["⌛ *LIMIT-ORDER VERFALLEN* (nicht gefüllt)", "",
             f"💰 *{row.get('symbol')}* · {row.get('side')} @ `{_fmt_price(lp)}`",
             f"⏳ Gültigkeit: {row.get('valid_min')} Min – Preis hat das Level nicht erreicht"]
    try:
        if cur_price and float(lp) > 0:
            dist = abs(float(cur_price) - float(lp)) / float(lp) * 100
            lines.append(f"📍 Aktueller Preis: `{_fmt_price(cur_price)}` "
                         f"({dist:.2f}% vom Level entfernt)")
    except (TypeError, ValueError, ZeroDivisionError):
        pass
    lines.append(f"🧠 Konfidenz {row.get('confidence') or '-'} · "
                 f"{row.get('horizon') or 'scalp'}")
    reason = _md(row.get("reason") or sig.get("ai_reasoning") or "")[:220]
    if reason:
        lines.append(f"📝 {reason}")
    lines.append("♻️ Wird im nächsten Analyse-Zyklus neu bewertet")
    return "\n".join(lines)


async def _tg_notify(db, text: str) -> None:
    """Telegram-Versand über den zentralen Toggle `limit_orders` (Einstellungen)."""
    try:
        from core import state
        from services import notifications
        await notifications.telegram_notify(db, state.telegram, "limit_orders", text)
    except Exception as e:
        logger.warning(f"limit order telegram notify failed: {e}")
...
async def place(db, signal: Dict, dec: Dict) -> Optional[str]:
    ...
    valid_min = max(MIN_VALID_MIN, min(_max_valid(dec.get("horizon")), valid_min))
    ...
    for row in rows:
        if str(row.get("expires_at") or "") < now_iso:
            ...
            if res.modified_count:
                ...
                await _tg_notify(db, _tg_expired_text(row, prices.get(row["symbol"])))
            continue
        if row.get("live_order_id"):
            # Liegt als ECHTE Limit-Order an der Börse – Fill/Stornierung
            # verwaltet services/limit_live_sync.py (Börse = Wahrheit).
            continue
        ...
        if opened:
            ...
            await _tg_notify(db, _tg_fill_text(row, True))
        else:
            ...
            await _tg_notify(db, _tg_fill_text(row, False, reject))
...
async def pending_context(db) -> str:
    ...
    for r in rows:
        live_tag = "LIVE an der Börse, " if r.get("live_order_id") else ""
        lines.append(
            f"- {r['side']} {r['symbol']} LIMIT @ {r['limit_price']} "
            f"({live_tag}{r.get('dist_pct')}% vom damaligen Preis, Setup {r.get('setup') or '-'}, "
