...
async def recent_1m(symbol: str, start_ms: int, max_days: int) -> List[Dict]:
    """1m-Kerzen ab `start_ms` (candle_cache, Fallback Scanner-Puffer) – gemeinsame
    Hilfsfunktion für Nachanalyse und Lektions-Gegenprobe. Fail-open: [] bei Fehlern."""
    import logging
    import time
    days = int((time.time() * 1000 - start_ms) / 86400000) + 1
    try:
        import aiohttp
        from services import candle_cache
        async with aiohttp.ClientSession() as s:
            arr = await candle_cache.get_candles(s, symbol, min(days, max_days + 1))
        rows = as_list(arr) if arr is not None else []
        if rows:
            return rows
    except Exception as e:  # noqa: BLE001
        logging.getLogger(__name__).debug(f"recent_1m {symbol}: candle_cache fehlgeschlagen: {e}")
    try:
        from core import state
        return list((getattr(state.scanner, "candle_buffer", {}) or {}).get(symbol) or [])
