...
def compute(params: Dict, cfg: Dict, entry: float, sl: float, equity: Optional[float],
            coin_max_lev: float = 200.0, preset_lev: Optional[float] = None,
            lev_source: str = "") -> Optional[Dict]:
    """Marge/Hebel aus Risiko-Budget. None = nicht berechenbar -> Legacy-Pfad.

    preset_lev: Hebel aus den Trade-Einstellungen des Assets (Auto-Leverage
    der Coin-Config, fester Hebel oder KI-Wahl) – hat Vorrang vor dem internen
    Risiko-Hebel (Fix 06/2026: vorher wurde IMMER der eigene Hebel-Pfad mit
    risk_max_leverage=15 genutzt -> gefühlter 'fester 15x-Hebel'). Die
    KI-Trader-Obergrenzen (Swing-Cap, Coin-Max) gelten weiterhin."""
    try:
    ...
    if preset_lev and float(preset_lev) > 0:
        lev = float(preset_lev)
        swing_cap = float(params.get("swing_cap") or 0)
        if params.get("is_swing") and swing_cap > 0:
            lev = min(lev, swing_cap)
        if coin_max_lev and coin_max_lev > 0:
            lev = min(lev, float(coin_max_lev))
        lev = round(max(1.0, lev), 2)
        lev_note = lev_source or "Hebel aus den Trade-Einstellungen"
        if params.get("is_swing") and swing_cap > 0 and lev == swing_cap:
            lev_note += f" (Swing-Cap {swing_cap:g}x greift)"
    else:
        lev = risk_leverage(cfg, params, entry, sl, coin_max_lev)
        lev_note = (f"Risiko-Hebel (Liq hinter SL, max "
                    f"{float(params.get('max_leverage') or 0):g}x)")
    notional = risk_usdt / sl_dist
    margin = notional / lev
    notes = [f"Risiko {risk_usdt:.2f} USDT ({float(params.get('risk_pct') or 2.0):g}% × {scale:g}), "
             f"SL {sl_dist * 100:.2f}%, Hebel {lev:g}x", scale_note, f"Hebel-Quelle: {lev_note}"]
    ...
    return {"margin": round(margin, 6), "leverage": lev, "leverage_source": lev_note,
