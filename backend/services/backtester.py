        ...
        if open_t is not None:
            ...
            if sl_hit or liq_hit:
                ...
                if is_liq:
                    ...
                    # Label nach REALISIERTEM PnL: Nach TP1/Break-Even/Trailing ist
                    # der Stop-Exit je nach Lage ein Gewinn (getrailter SL), ein
                    # Break-Even (SL an Entry+Gebühren) oder ein Verlust (Smart-BE
                    # unter Entry) – vorher pauschal "breakeven"/"win".
                    close_trade(t, level, "loss", c_ts)
                    if t["be_moved"] or t["tp1_done"] or t["secured"]:
                        t["result"] = ("win" if t["pnl"] > 1e-6
                                       else "breakeven" if t["pnl"] >= -1e-6
                                       else "loss")
                open_t = None
            else:
        ...
        if open_t is None:
            ...
            if sig and sig.get("type") in ("LONG", "SHORT"):
                ...
                if sig.get("signal_class") == "PRE_SIGNAL" and not cfg.get("trade_pre_signals"):
                    continue
                if require_all and sig.get("signal_class") != "PRE_SIGNAL" \
                        and sig.get("rules_total") \
...
def create_job(params: Dict) -> str:
    ...
    # Trade-Rohdaten älterer Jobs ebenfalls freigeben (DB-Fallback greift)
...
async def _simulate_all_parallel(job, strategy_ids, symbols, days, cfg, registry,
    ...
    try:
        ...
        while pending:
            ...
            if job_control.pause_requested(job):
                # Pool-Prozesse kennen das Flag nicht: laufende Paare enden,
                # danach steht der Job bis zum Fortsetzen.
                await job_control.wait_if_paused(job)
