...
class FastSeries:
    ...
    def get(self, name: str, d: Dict) -> np.ndarray:
        ...
        elif name in ("supertrend", "supertrend_dir"):
            key = (name, p("supertrend_period", 10), pf("supertrend_mult", 3.0))
        elif name == "ema_slope_pct":
            key = ("ema_slope_pct", p("ema_slow_period", 50), p("slope_lookback", 5))
        elif name == "roc":
            key = ("roc", p("roc_period", 10))
        elif name == "chop":
            key = ("chop", p("chop_period", 14))
        elif name in ("aroon_up", "aroon_down"):
            key = (name, p("aroon_period", 25))
        elif name == "williams_r":
            key = ("williams_r", p("williams_period", 14))
        elif name == "atr_pct":
    ...
    def _compute(self, name: str, key: tuple) -> np.ndarray:
        ...
        if key[0] in ("keltner_upper", "keltner_middle", "keltner_lower"):
            ...
            return self._cache[key]
        if key[0] in ("supertrend", "supertrend_dir"):
            line, direction = vec.supertrend(self.high, self.low, self.close, key[1], key[2])
            self._cache[("supertrend", key[1], key[2])] = line
            self._cache[("supertrend_dir", key[1], key[2])] = direction
            return self._cache[key]
        if key[0] == "ema_slope_pct":
            e = vec.ema(self.close, key[1])
            prev = np.concatenate([np.full(key[2], np.nan), e[:-key[2]]]) if key[2] < self.n else np.full(self.n, np.nan)
            with np.errstate(invalid="ignore", divide="ignore"):
                out = (e - prev) / prev * 100
            return out
        if key[0] == "roc":
            k = key[1]
            prev = np.concatenate([np.full(k, np.nan), self.close[:-k]]) if k < self.n else np.full(self.n, np.nan)
            with np.errstate(invalid="ignore", divide="ignore"):
                out = (self.close - prev) / prev * 100
            return out
        if key[0] == "chop":
            return vec.choppiness(self.high, self.low, self.close, key[1])
        if key[0] in ("aroon_up", "aroon_down"):
            up, dn = vec.aroon(self.high, self.low, key[1])
            self._cache[("aroon_up", key[1])] = up
            self._cache[("aroon_down", key[1])] = dn
            return self._cache[key]
        if key[0] == "williams_r":
            return vec.williams_r(self.high, self.low, self.close, key[1])
