...
ALLOWED_MODELS = {
    ...
    "openrouter": [
        ...
        # 09/2026 entfernt (OpenRouter: "unavailable for free"): openai/gpt-oss-20b:free,
        # nvidia/nemotron-nano-9b-v2:free -> MODEL_MIGRATIONS (gleiches Modell auf Groq).
...
MODEL_MIGRATIONS = {
    ...
    # 09/2026: OpenRouter bietet die OpenAI-OSS-Modelle nicht mehr gratis an
    # ("This model is unavailable for free") -> gleiches Modell gratis auf Groq.
    "openai/gpt-oss-20b:free": ("groq", "openai/gpt-oss-20b"),
    "nvidia/nemotron-nano-9b-v2:free": ("groq", "openai/gpt-oss-20b"),
}
...
# Bezahlte Modelle (Guthaben vorhanden): ein 429 ist dort fast immer ein
# TRANSIENTES Minuten-Limit des Anbieters, KEIN Tages-Kontingent. Statt sofort
# auf ein Free-Fallback auszuweichen, wird derselbe Key nach kurzen Wartezeiten
# erneut probiert – das Haupt-Modell wird nicht umgangen, solange Guthaben da ist.
PAID_RATE_LIMIT_RETRIES = (6, 15, 30)
...
def record_result(provider: str, model: str, status: str, detail: str = "",
    ...
    `role` kommt IMMER explizit vom Aufrufer (generate_chain/stream_chain
    fixieren sie am Start). Der frühere Fallback auf das globale _current_role
    ordnete parallele Rollen-Calls dem falschen Assistenten zu und wurde
    entfernt (Backlog P2); ohne Rolle bleibt der Eintrag bewusst unzugeordnet."""
    ...
    if status == "ok":
        # Fallback = ANDERES Modell als gewünscht. Ein Backup-Key desselben
        # Modells ist normale Key-Rotation (Kontingent-Verteilung) und KEIN
        # Fallback – vorher löste jeder Backup-Key eine "Fallback aktiv ·
        # Backup-Key"-Warnung aus (Trader-Entscheid 09/2026: nur warnen, wenn
        # wirklich ein Ersatz-Modell läuft oder alle Keys erschöpft sind).
        _last_call.update({
            "provider": provider, "model": model, "role": role,
            "requested_model": requested, "key_index": key_index,
            "fallback": bool(requested and requested != model),
            "ts": _now(),
        })
...
async def generate_chain(chain: List[Tuple[str, str]], prompt: str, system: str,
                         ...
                         priority: str = "normal",
                         role: Optional[str] = None) -> Tuple[str, str, str]:
    ...
    `role`: Rolle EINMAL beim Start festhalten statt zur Laufzeit aus dem
    globalen _current_role zu lesen – parallele Rollen-Calls (z.B. News-Wächter
    triggert Deep-Analyse als Task) überschrieben sich sonst gegenseitig und
    Ausfälle/Fallbacks wurden dem FALSCHEN Assistenten zugeordnet (Bug-Report:
    News-Wächter zeigte OpenRouter-Fallback, obwohl OpenRouter nicht in seinem
    Team ist – der Call kam vom Deep-Analysten)."""
    ...
    for provider, model in chain:
        ...
        if budget and est_tokens > budget:
            ...
            record_result(provider, model, "skipped_too_large",
                          f"Prompt ~{est_tokens} Tokens > Budget {budget}", role=role)
            ...
            try:
                ...
                try:
                    ...
                    if is_empty_response_error(e1):
                        ...
                        # Bezahl-Modell mit Guthaben: 429 = transientes Minuten-Limit.
                        # Kurz warten und denselben Key erneut nutzen, statt das gute
                        # Haupt-Modell durch ein Fallback zu umgehen.
                        text = None
                        paid_err = e1
                        for wait_s in PAID_RATE_LIMIT_RETRIES:
                            logger.info(f"{provider}/{model}: Rate-Limit auf Bezahl-Modell – "
                                        f"Retry in {wait_s}s (Haupt-Modell wird nicht umgangen)")
                            ...
                            try:
                                ...
                                paid_err = e2
                                if not is_rate_limit_error(e2) or is_payment_error(e2):
                                    raise
                        if text is None:
                            raise paid_err
                    else:
                        raise
                if i > 0:
                    logger.debug(f"AI: Key {i + 1}/{len(keys)} für {provider} genutzt ({model})")
                ...
                if is_rate_limit_error(e) or is_payment_error(e):
                    ...
                    paid_transient = (model in PAID_MODELS_NO_FALLBACK
                                      and not is_payment_error(e))
                    if paid_transient:
                        # Minuten-Limit eines BEZAHL-Modells: Key NICHT in den
                        # 10-min-Cooldown schicken (würde auch Free-Calls dieses
                        # Keys blockieren) – nächster Key (evtl. anderes Konto
                        # mit eigenem Guthaben) direkt probieren.
                        logger.info(f"{provider}/{model} {reason} trotz Retries "
                                    f"(Key {i + 1}/{len(keys)}) – nächster Key…")
                    else:
                        mark_key_limited(provider, i, str(e))
                        logger.info(f"{provider}/{model} {reason} (Key {i + 1}/{len(keys)}), weiter…")
                    if not paid_transient and not is_payment_error(e):
                        ...
                        streak_429 += 1
                    streak_abort = streak_429 >= SAME_ORG_429_STREAK and i != last_idx
                    # Ein einzelner limitierter Key ist KEIN Ausfall, solange
                    # weitere Keys übernehmen: erst wenn das Modell für diesen
                    # Aufruf aufgegeben wird (letzter Key bzw. Streak-Abbruch),
                    # landet es im KI-Status (Trader-Entscheid 09/2026 – keine
                    # Backup-Key-Warnungen, solange die Backups reichen).
                    if i == last_idx or streak_abort:
                ...
                if is_empty_response_error(e):
                    ...
                    record_result(provider, model, "error",
                                  ...
                                  "überlastet, Fallback übernimmt", key_index=i, role=role)
                    if i == last_idx:
                        failed_models.append(f"{provider}/{model}")
                        failed_providers.add(provider)
                ...
                record_result(provider, model, "error", str(e), key_index=i, role=role)
    ...
    try:
        ...
        await notify_ai_failure(role or "KI-Anfrage",
...
async def stream_chain(chain: List[Tuple[str, str]], prompt: str, system: str,
                       temperature: float = 0.6, role: Optional[str] = None):
    """Streaming über die Kette. Yields ('token', str) Chunks, danach einmal
    ('meta', (provider, model)). Bei komplettem Scheitern ('error', msg).
    `role`: einmal beim Start fixiert (siehe generate_chain) – verhindert
    Fehl-Zuordnung bei parallel laufenden Rollen-Calls."""
    role = role or _current_role.get("role")
    last_err: Optional[Exception] = None
    tried_any = False
    est_tokens = estimate_tokens(system) + estimate_tokens(prompt)
            ...
            try:
                ...
                if is_rate_limit_error(e) or is_payment_error(e):
                    ...
                    if not paid_transient:
                        # Bezahl-Modell: Minuten-Limit, Key nicht poisonen
                        mark_key_limited(provider, i, str(e))
                    logger.info(f"{provider}/{model} chat {reason} (Key {i + 1}), weiter…")
                    if paid_transient:
                        if i == idxs[-1]:
                            record_result(provider, model, "rate_limited", str(e),
                                          key_index=i, role=role)
                        continue
                    streak_429 += 1
                    streak_abort = streak_429 >= SAME_ORG_429_STREAK
                    # Wie generate_chain: Ausfall erst melden, wenn das Modell
                    # für diesen Aufruf aufgegeben wird (Backup-Keys reichen nicht).
                    if i == idxs[-1] or streak_abort:
                        record_result(provider, model, "rate_limited", str(e),
                                      key_index=i, role=role)
                    if streak_abort:
