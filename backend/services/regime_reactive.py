...
def detect(f: Dict, cfg: Dict) -> Dict:
    ...
    if det == "jump":
        from services.regime_jump import detect_jump
        return detect_jump(f, cfg)
    ...
    for i in range(1, n + 1):
        ...
        if d3[s] in (0, 2) and i - s >= 2:
            if causal:
                st = 0
                vsum = 0.0
                # Frische-Fenster: "stark" erlischt, wenn das jüngste Tempo
                # (Sichtfenster des Klassifikators) unter die Hälfte der
                # Einstiegs-Schwelle fällt – sonst bleibt ein längst
                # abgeflauter Trend auf altem Momentum "stark" hängen.
                w = max(int((cfg.get("horizon_bars") or [7])[0]), 2)
                for j in range(s, i):
                    vsum += float(dvol[j])
                    v = vsum / (j - s + 1)
                    ...
                    if st == 1 and j - s + 1 > w:
                        j0 = j - w + 1
                        ldays = max(w / bpd, 0.5)
                        lnet = abs(close[j] / max(close[j0], 1e-12) - 1.0) * 100.0
                        if lnet / ldays / max(v, 1e-9) < 0.5 * enter:
                            st = 0
                    sub[j] = st
            else:
...
def full_payload(f: Dict, cfg: Dict, candles) -> Dict:
    ...
    return {"live_labels": [None if r < 0 else int(r) for r in ids_live],
            ...
            # AP13: kausaler Score je Kerze (additiv, für Unsicherheits-
            # Kalibrierung) – heuristisch, KEINE kalibrierte Wahrscheinlichkeit
            "live_conf": [round(float(c), 4) for c in _conf],
