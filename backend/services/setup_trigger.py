...
def paper_wanted(setup: str, has_edge: bool, paper_all: bool, live_blocked: bool) -> bool:
    """Soll ein Detektor-Treffer einen LLM-freien Paper-Sammel-Trade auslösen? (rein)
      * Backtest-Edge oder paper_all          -> ja (Reife-Gate-Daten)
      * momentum_news (kein Backtest-Detektor) -> ja
      * Setup in der Klasse RÜCKGESTUFT        -> ja: die Rückstufung verlangt
        Paper-Trades seit Rückstufung – ohne Trigger kamen die nie zusammen
        (Befund 09/2026: 'Revision v1, 0 Paper-Trades' dauerhaft)."""
    return bool(has_edge or paper_all or setup == "momentum_news" or live_blocked)
...
class SetupTrigger:
    ...
    async def _handle_hit(self, symbol: str, asset_class: str, hit: Dict) -> Dict:
        ...
        try:
            from services import ai_playbook
            blocked = bool(ai_playbook.live_block_reason(hit["setup"], asset_class=asset_class))
        except Exception:  # noqa: BLE001
            blocked = False
        paper_ok = (self.cfg.get("setup_trigger_paper", True)
                    and bool(self.cfg.get("enabled"))
                    and bool(self.cfg.get("collection_enabled", True))
                    and paper_wanted(hit["setup"], bool(hit.get("has_edge")),
                                     bool(self.cfg.get("setup_trigger_paper_all", False)), blocked)
