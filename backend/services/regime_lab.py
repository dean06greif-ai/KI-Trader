...
async def fetch_histories(symbols: List[str], days: int, timeframe: str,
                          ...
                          dataset: Dict = None, skipped: Dict[str, Dict] = None) -> Dict[str, List[Dict]]:
    ...
    `skipped` (optional, out-Parameter): Symbole, die wegen zu weniger Kerzen
    (<= MIN_HISTORY_BARS) NICHT in die Analyse eingehen, mit Grund – damit die
    Oberfläche z.B. „Öl: nur 80 Kerzen“ zeigen kann statt den Knopf still wegzulassen.
...
def _train_segment_days(segments: List[Dict], train_end_ts, bpd: float):
    """AP07-Härtung (Kombi-Kalibrierung): mittlere FINAL-Phasendauer NUR aus
    Segmenten, die vollständig im Trainingsfenster liegen (to_ts <= Trainings-
    ende). Der Holdout ist finaler Test und darf die Kandidatenauswahl nicht
    mehr beeinflussen – auch nicht über den Dauer-Term des Scores."""
    if not segments:
        return None
    if train_end_ts:
        segs = [s for s in segments if int(s.get("to_ts") or 0) <= int(train_end_ts)]
    else:
        segs = list(segments)
    if not segs:
        return None
    return sum(int(s.get("bars") or 0) for s in segs) / len(segs) / max(bpd, 1e-9)
...
def _ideal_payload(candles: List[Dict], labels: List, model: Dict,
                   ideal: List = None) -> Optional[Dict]:
...
def _reference_payload(candles: List[Dict], live_labels: List, model: Dict,
                       train_end_ts=None, inner_start_ts=None):
    """Live-Sicht gegen die detektor-UNABHÄNGIGE Referenz (zentrierte Rückblick-
    Labels). Ergänzt Live=Final, das nur Selbst-Übereinstimmung misst.
    Rückgabe: (reference_dict | None, ideal_labels | None)."""
    if not rg.is_v2(model):
        return None, None
    try:
...
def _symbol_payload(model: Dict, candles, timeframe: str, conf_min: float,
    ...
    if reactive:
        ...
        ref, ideal = _reference_payload(candles, pay["live_labels"], model,
                                        train_end_ts, inner_start_ts)
        entry = {"segments": _segments_payload(candles, labels),
                 ...
                 "live_agreement": _live_agreement(candles, pay["live_labels"],
                                                   ...
                                                   inner_start_ts),
                 # Live-Sicht vs. detektor-unabhängige Referenz (vergleichbar
                 # zwischen reactive/ema/kombi – Live=Final ist es nicht)
                 "reference": ref,
                 ...
                 "ideal": (_ideal_payload(candles, labels, model, ideal)
                           if with_ideal else None)}
        # AP13: Unsicherheitskalibrierung (heuristischer Score vs. tatsächliche
        # Live=Final-Trefferquote) – additiv, nur reaktiver Detektor liefert
        # einen kausalen Score je Kerze.
        mode_m = (model.get("config") or {}).get(
            "regime_mode", model.get("regime_mode", eng.DEFAULT_REGIME_MODE))
        entry["uncertainty"] = research_calibration.symbol_bins(
            research_calibration.hit_points(pay["live_labels"], labels,
                                            pay.get("live_conf"), mode_m))
        return labels, entry
    labels = rg.classify_series(model, candles, timeframe, conf_min, min_hold_days)
    ref, ideal = _reference_payload(candles, labels, model, train_end_ts, inner_start_ts)
    entry = {"segments": _segments_payload(candles, labels),
             "reference": ref,
             "validation": _validation_payload(candles, labels, model),
             "current": _current_payload(candles, model, timeframe,
    ...
    try:
        ...
        if not histories:
            raise RuntimeError("Zu wenig Daten für diesen Timeframe/Zeitraum"
                               + (" – " + "; ".join(f"{s}: {v['reason']}" for s, v in skipped.items())
                                  if skipped else ""))
        ...
        bounds = {}
        train_hist = {}
        inner_anchor: Dict[str, Optional[int]] = {}
        for sym, candles in histories.items():
            cut = int(len(candles) * train_pct / 100.0)
            cut = min(max(cut, 100), len(candles))
            train_hist[sym] = candles[:cut]
            # AP07/R10: innere Validierung (letzter Teil des Trainings) auch für
            # die gespeicherte Analyse – bisher nur in EMA-/Kombi-Vergleich.
            ...
            bounds[sym] = {"start_ts": int(candles[0]["timestamp"]),
                           ...
                           "inner_start_ts": inner_anchor[sym],
                           "bars": len(candles)}
        ...
        if scope in ("both", "combined"):
            ...
            if model:
                ...
                combined = {"model": model, "per_symbol": per_symbol,
                            ...
                            # AP13: Score-Kalibrierung – assetspezifisch nur bei
                            # Datenstärke, sonst Anlageklassen-Pooling (additiv)
                            "uncertainty": research_calibration.calibration_report(
                                {s: (v.get("uncertainty") or {})
        ...
        doc = {"id": aid,
               ...
               "symbols_requested": [s for s in symbols if isinstance(s, str)],
    ...
    try:
        ...
        for pi, period in enumerate(periods):
            ...
            agg = {"direction_pct": [], "holdout_direction_pct": [],
                   ...
                   "reference_pct": [], "inner_reference_pct": [],
                   "holdout_reference_pct": [], "reference_lag_days": [],
                   "reference_missed_pct": []}
            for sym, candles in histories.items():
                if job.get("cancel"):
                    raise JobCancelled()
                ...
                for src, dst in (("direction_pct", "reference_pct"),
                                 ("inner_direction_pct", "inner_reference_pct"),
            ...
            rows.append({"period": period,
                         ...
                         "passed": agg["passed"],
                         # Referenz (detektor-unabhängig) – Auswahlbasis
                         "reference_pct": mean(agg["reference_pct"]),
                         "inner_reference_pct": mean(agg["inner_reference_pct"]),
                         "holdout_reference_pct": mean(agg["holdout_reference_pct"]),
                         "reference_lag_days": mean(agg["reference_lag_days"]),
                         "reference_missed_pct": mean(agg["reference_missed_pct"])})
        ...
        # NEU: bevorzugt die Referenz-Treffer (Live vs. zentrierte Rückblick-
        # Labels). Live=Final bevorzugt systematisch LANGE EMAs, weil sich
        # Live- und Final-Steigung dort kaum unterscheiden (Selbst-Übereinstimmung
        # ~98 %, obwohl ganze Trends verpasst werden).
        best, selection_basis = research_validation.select_best_row(
            rows, chain=[("inner_reference_pct", "inner_validation_reference"),
...
def _variant_metrics(agg: Dict) -> Dict:
    mean = lambda xs: round(sum(xs) / len(xs), 1) if xs else None  # noqa: E731
    return {"direction_pct": mean(agg["direction_pct"]),
            "holdout_direction_pct": mean(agg["holdout_direction_pct"]),
            "inner_direction_pct": mean(agg["inner_direction_pct"]),
            "holdout_bars": agg["holdout_bars"],
            "trend_hit_pct": mean(agg["trend_hit_pct"]),
...
async def run_ablation(job_id: str, body: Dict, db):
    """Indikator-Ablation (AP13/Zielbild §6B): derselbe Datensatz, der
    konfigurierte Detektor voll vs. ohne je eine Bestätigungs-Komponente vs.
    einfache Alternative. Auswahl/Bewertung auf der INNEREN Validierung
    (AP07); je Variante zusätzlich Anlageklassen-Pooling mit sichtbarer
    Symbol-Abweichung (AP13-Pooling)."""
    job = JOBS[job_id]
    try:
        symbols = body.get("symbols") or []
        timeframe = body.get("timeframe") or "15m"
        ...
        conf_min = float(engine_config.get("confidence_min") or 0.55)
        min_hold = float(engine_config.get("min_hold_days") or 0)
        variants = research_ablation.ablation_variants(engine_config)
        ...
        for vi, var in enumerate(variants):
            if job.get("cancel"):
                raise JobCancelled()
            job["phase"] = f"Ablation rechnen: {var['name']}"
            job["progress"] = 10 + round(vi / len(variants) * 85)
            model = await asyncio.to_thread(
                rg.detect_regimes, train_hist, timeframe, 5, 3.0, 5.0,
                engine="v2", engine_config=var["engine_config"])
            if not model:
        ...
        result = {"kind": "ablation", "rows": rows,
                  ...
                  "attempt_no": attempt_no,
                  "manifest": research_validation.experiment_manifest(
                      "ablation", {"detector": engine_config.get("detector")
                                   or "reactive",
        ...
        if db is not None:
            try:
                ...
                logger.warning(f"ablation persist failed: {e}")
        job["result"] = result
        job["status"] = "done"
        job["progress"] = 100
        job["phase"] = "Fertig"
        ...
        logger.exception(f"ablation {job_id} failed")
        job["status"] = "error"
        job["error"] = str(e)[:300]
        job["phase"] = "Fehler"
...
async def run_kombi_calibrate(job_id: str, body: Dict, db):
    ...
    try:
        ...
        for ci, (thr, slope) in enumerate(combos):
            ...
            for sym, candles in histories.items():
                ...
                if segs:
                    agg["avg_final_days"].append(
                        sum(s["bars"] for s in segs) / len(segs) / max(bpd, 1e-9))
                    d_train = _train_segment_days(segs, bounds.get(sym), bpd)
                    if d_train is not None:
                        agg["avg_final_days_train"].append(d_train)
                if lsegs:
                    agg["avg_live_days"].append(
                        sum(s["bars"] for s in lsegs) / len(lsegs) / max(bpd, 1e-9))
                agg["switches_final"] += max(len(segs) - 1, 0)
            ...
            # AP07-Härtung: Der Dauer-Term der AUSWAHL nutzt nur Trainings-
            # Segmente – vorher floss der Holdout über avg_final_days weiter
            # in die Kandidatenauswahl ein (Holdout-Leck).
            dur_sel = mean(agg["avg_final_days_train"])
            if dur_sel is None:
                dur_sel = dur
            ...
            if dur_sel is None:
                ...
                out_band = max(0.0, t_lo - dur_sel) + max(0.0, dur_sel - t_hi)
            ...
            rows.append({"thr": thr, "slope_days": slope,
                         ...
                         "avg_final_segment_days_train": rnd(dur_sel),
                         "avg_live_segment_days": rnd(mean(agg["avg_live_days"])),
                         "switches_final": agg["switches_final"],
                         "switches_live": agg["switches_live"],
                         "in_target": (dur_sel is not None
                                       and t_lo <= dur_sel <= t_hi),
                         "score": rnd(score, 2)})
        ...
        result = {"kind": "kombi_calibrate",
                  ...
                  "selection_basis": "inner_validation+train_duration",
                  "holdout_role": "final_test",
                  "evidence": research_validation.evidence_verdict(
                      (best or {}).get("holdout_bars")),
                  "attempt_no": await research_validation.register_attempt(
            ...
            try:
                await db.regime_calibrations.insert_one(
                    {"id": job_id, "created_at": datetime.now(timezone.utc).isoformat(),
                     # Regime-Brücke 1.1: Zuordnung zur Analyse (Nachweis-Gate)
                     "symbols": list(histories.keys()), "timeframe": timeframe,
                     "report": report})
            except Exception as e:  # noqa: BLE001
...
async def persist_analysis(db, doc: Dict):
    ...
    if n > MAX_ANALYSES:
        old = await db.regime_analyses.find({}, {"_id": 0, "id": 1}).sort("created_at", 1) \
...
# Regime-Brücke (PLAN_REGIME_BRUECKE 1.1): Nachweis-Gate lebt in services/regime_release.py,
# wird hier unter dem im Plan genannten Anker mit angeboten.
from services.regime_release import validate_release, validate_activation  # noqa: E402,F401
