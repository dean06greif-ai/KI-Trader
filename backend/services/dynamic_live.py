...
async def apply_configs(doc: Dict) -> list:
    ...
    for sym, st in per_symbol.items():
        ...
        if not plan["overrides"]:
            ...
            if prev_cfg.get("dynamic_keys"):
                cleaned = strategy_plan.merge_overrides(prev_cfg, {})
                for marker in ("dynamic_keys", "dynamic_applied", "dynamic_id",
...
async def _apply_strategy_params(strategy_id: str, symbol: str, params: Dict):
    ...
    for k in dyn_map.get(own_key, []) or []:
        if k not in params:
            cur.pop(k, None)
    cur.update(params)
