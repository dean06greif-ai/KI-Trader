...
async def apply_configs(doc: Dict) -> list:
    ...
    AP03/R02: Auflösung über den deterministischen Resolver
    (services/strategy_plan.py). Die effektive Konfiguration wird von der
    Basis abgeleitet – früher dynamisch gesetzte Keys, die im neuen Regime
    fehlen, werden entfernt statt kumulativ liegenzubleiben. Manuelle Keys
    bleiben erhalten."""
    ...
    for sym, st in per_symbol.items():
        ...
        if cfg_r is None:
            continue
        plan = strategy_plan.resolve_symbol_plan(doc, sym, st)
        # R01-Härtung: Discovery-Regeln/Regel-Varianten werden TATSÄCHLICH im
        # Scanner umgeschaltet (über coin_params['rules_override']), nicht nur
        # dokumentiert. Nur Custom-Strategien haben austauschbare Regeln.
        dyn_params = dict(plan["params"] or {})
        rules_switch = None
        if plan.get("rules_override"):
            if getattr(strategy, "IS_CUSTOM", False):
                dyn_params["rules_override"] = plan["rules_override"]
                rules_switch = "applied"
            else:
                rules_switch = "unsupported_builtin"
                logger.warning(
                    f"Dynamik {doc['id']}/{sym}: Regel-Umschaltung für Basis-Strategie "
                    f"{sid} nicht möglich (fest programmierte Regeln) – Trade-Overrides "
                    "werden trotzdem übernommen")
        info = {"symbol": sym, "regime": st.get("regime"),
                "label": st.get("label"), "confidence": st.get("confidence"),
                "plan_hash": plan["plan_hash"]}
        if rules_switch:
            info["rules_switch"] = rules_switch
        key = f"{sid}_{sym}"
        prev = await state.db.strategy_coin_configs.find_one({"_id": key})
        prev_cfg = dict((prev or {}).get("config", {}))
        if not plan["overrides"] and not dyn_params:
            # Baseline-Regime: R02 – Rückstände einer früheren dynamischen
            # Übernahme entfernen (A→B→Baseline == direkte Baseline);
            # gesicherte persönliche Basiswerte werden wiederhergestellt.
            if prev_cfg.get("dynamic_keys") or prev_cfg.get("dynamic_param_keys"):
                cleaned = strategy_plan.merge_overrides(prev_cfg, {}, params={})
                for marker in ("dynamic_keys", "dynamic_param_keys", "dynamic_prev",
                               "dynamic_prev_params", "dynamic_applied", "dynamic_id",
                               "dynamic_regime", "dynamic_sub_strategy"):
                    cleaned.pop(marker, None)
                await state.db.strategy_coin_configs.replace_one(
                    {"_id": key}, {"_id": key, "config": cleaned}, upsert=True)
                autotrader.config.setdefault("strategy_coin_configs", {})[key] = cleaned
            await _apply_strategy_params(sid, sym, {})
            applied.append({**info, "baseline": True})
            continue
        merged = strategy_plan.merge_overrides(prev_cfg, plan["overrides"],
                                               params=dyn_params)
        merged["dynamic_applied"] = now_iso
        merged["dynamic_id"] = doc["id"]
        merged["dynamic_regime"] = st.get("regime")
        if plan.get("sub_strategy"):
            # R01 (transparent): Discovery-Substrategie des Regimes am
            # Coin-Override sichtbar hinterlegen (additiv).
            merged["dynamic_sub_strategy"] = plan["sub_strategy"]
        else:
            merged.pop("dynamic_sub_strategy", None)
        await state.db.strategy_coin_configs.replace_one(
            {"_id": key}, {"_id": key, "config": merged}, upsert=True)
        autotrader.config.setdefault("strategy_coin_configs", {})[key] = merged
        # Live-Wirkung im Scanner (Params + Regel-Umschaltung) – R02: immer
        # anwenden (auch leer), damit Rückstände entfernt/wiederhergestellt werden.
        await _apply_strategy_params(sid, sym, dyn_params)
        applied.append(info)
...
async def _apply_strategy_params(strategy_id: str, symbol: str, params: Dict):
    ...
    R02: Ownership der dynamisch gesetzten Param-Keys wird separat gemerkt
    (settings['coin_params_dynamic_keys']); Keys, die das neue Regime nicht
    mehr setzt, werden entfernt statt liegenzubleiben. Manuelle Keys bleiben.
    Persönliche Basiswerte, die ein dynamischer Key überdeckt hat, werden in
    settings['coin_params_dynamic_prev'] gesichert und beim Zurückschalten
    wiederhergestellt (Befund: Basisparameter gingen verloren)."""
    if not strategy_id:
        return
    ...
    cur = dict(strat_cp.get(symbol, {}))
    dyn_map = dict(scanner.settings.get("coin_params_dynamic_keys", {}))
    prev_map = dict(scanner.settings.get("coin_params_dynamic_prev", {}))
    own_key = f"{strategy_id}_{symbol}"
    owned = list(dyn_map.get(own_key, []) or [])
    own_prev = dict(prev_map.get(own_key) or {})
    new_prev = {}
    for k in params:
        if k in owned:
            if k in own_prev:
                new_prev[k] = own_prev[k]
        elif k in cur:
            new_prev[k] = cur[k]
    for k in owned:
        if k not in params:
            if k in own_prev:
                ...
                cur.pop(k, None)
    cur.update(params)
    dyn_map[own_key] = sorted(params.keys())
    if new_prev:
        prev_map[own_key] = new_prev
    else:
        prev_map.pop(own_key, None)
    if cur:
        strat_cp[symbol] = cur
    else:
        strat_cp.pop(symbol, None)
    cp[strategy_id] = strat_cp
    await scanner.save_settings({"coin_params": cp,
                                 "coin_params_dynamic_keys": dyn_map,
                                 "coin_params_dynamic_prev": prev_map})
...
async def apply_regime_strategies(doc: Dict) -> list:
    ...
    for sym, st in per_symbol.items():
        ...
        if rid is None:
            continue
        plan = strategy_plan.resolve_symbol_plan(doc, sym, st)
        want = plan["strategy_id"]
        for sid in all_sids:
            await _set_toggle(sid, sym, sid == want)
        if want:
            # R02: Params IMMER anwenden (auch leer), damit Rückstände des
            # vorherigen Regimes entfernt werden.
            await _apply_strategy_params(want, sym, plan["params"])
            key = f"{want}_{sym}"
            prev = await state.db.strategy_coin_configs.find_one({"_id": key})
            prev_cfg = dict((prev or {}).get("config", {}))
            had_dynamic = bool(prev_cfg.get("dynamic_keys")
                               or prev_cfg.get("dynamic_param_keys"))
            if plan["overrides"] or plan["params"] or had_dynamic:
                merged = strategy_plan.merge_overrides(prev_cfg, plan["overrides"],
                                                       params=plan["params"])
        ...
        applied.append({"symbol": sym, "regime": rid, "label": st.get("label"),
                        ...
                        "strategy_params": plan["params"] or None,
                        "plan_hash": plan["plan_hash"],
...
def _owned_strategy_ids(doc: Dict) -> list:
    """Strategie-IDs, deren Trades dieser dynamischen Strategie gehören."""
    sids = set()
    if doc.get("strategy_id"):
        sids.add(doc["strategy_id"])
    for sid in (doc.get("regime_strategies") or {}).values():
        if sid:
            sids.add(sid)
    return sorted(sids)


def transition_close_query(doc: Dict, symbols: list) -> Dict:
    """R03: Übergangsschutz-Close wirkt NUR auf Trades der eigenen Strategien
    dieser dynamischen Strategie – nie symbolweit auf manuelle/fremde Trades
    (rein, testbar)."""
    return {"symbol": {"$in": list(symbols)}, "status": "open",
            "strategy_id": {"$in": _owned_strategy_ids(doc)},
...
async def transition_protect(doc: Dict, symbols: list, new_state: Dict) -> Optional[Dict]:
    ...
    AP01/R03: Wird nur noch als Teil einer tatsächlichen Übernahme aufgerufen
    (Auto-Apply oder bestätigter Wechsel), nie beim reinen Refresh. Der
    Close-Modus ist auf die eigenen Strategie-Trades gescoped."""
    s = doc.get("settings") or {}
    ...
    if mode == "close_open":
        rows = await state.db.auto_trades.find(
            transition_close_query(doc, symbols)).to_list(100)
        for t in rows:
            try:
...
def pending_claim_query(did: str, pending: Dict) -> Dict:
    """R13-Härtung (rein, testbar): Query für den atomaren Claim eines offenen
    Regime-Wechsels. Genau EINE parallele Bestätigung darf den Vorschlag
    beanspruchen (Compare-and-swap über 'claimed_at' in der DB, nicht nur im
    zuvor gelesenen Dokument)."""
    q = {"id": did, "pending_switch.at": pending.get("at"),
         "pending_switch.claimed_at": {"$exists": False}}
    if pending.get("command_id"):
        q["pending_switch.command_id"] = pending["command_id"]
    return q


async def claim_pending_switch(did: str, pending: Dict) -> bool:
    """Offenen Vorschlag atomar beanspruchen. False = bereits beansprucht
    oder inzwischen ersetzt/entfernt – Bestätigung ablehnen (409)."""
    doc = await state.db.dynamic_strategies.find_one_and_update(
        pending_claim_query(did, pending),
        {"$set": {"pending_switch.claimed_at": _now_iso()}})
    return doc is not None


async def release_pending_claim(did: str):
    """Claim nach fehlgeschlagener Übernahme freigeben (Retry möglich)."""
    await state.db.dynamic_strategies.update_one(
        {"id": did}, {"$unset": {"pending_switch.claimed_at": ""}})
...
async def unapply_dynamic(doc: Dict) -> Dict:
    ...
    for sid in _owned_strategy_ids(doc):
        for sym in doc.get("symbols") or []:
            ...
            for marker in ("dynamic_keys", "dynamic_param_keys", "dynamic_prev",
                           "dynamic_prev_params", "dynamic_applied",
            ...
            # Scanner-seitige coin_params (Regel-Umschaltung/Regime-Params)
            # ebenfalls entfernen bzw. persönliche Basiswerte wiederherstellen.
            try:
                await _apply_strategy_params(sid, sym, {})
            except Exception as e:  # noqa: BLE001
                logger.warning(f"Unapply coin_params {key} fehlgeschlagen: {e}")
            cleaned.append(key)
