...
from services import lesson_attribution
...
from services import setup_trigger as setup_trigger_mod
...
DEFAULT_AI_CONFIG = {
    ...
    # Sammel-Richtungs-Limit je Anlageklasse statt global (Sammel-Trades sind
    # Paper ohne Kapital – ein offener GBPUSD-Short soll keine Krypto-Short-
    # Daten blockieren). Cooldown je Symbol × Setup, damit seltene Setups
    # (z.B. vwap_reclaim, KI-eigene Setups) nicht von häufigen verdrängt werden.
    "collection_direction_per_class": True,
    ...
    **sweep_trigger.DEFAULTS,
    # Entry-Modus (services/entry_policy.py): ai (bisher) | aggressive (Limit am
    # Level bei Level-Setups) | conservative (immer Market nach Bestätigung).
    **entry_policy.DEFAULTS,
    ...
    **tf2_signal.DEFAULTS,
    ...
    # LLM-freie Paper-Sammel-Trades + gezielte Analyse bei live-reifen Setups.
    **setup_trigger_mod.DEFAULTS,
    ...
    # Lektions-Bilanz (PLAN_LEKTIONS_BILANZ): Attribution + HOLD-Gegenprobe sind
    # reine Datenerfassung (Default an); der Prompt-Block für Lernlauf/Neubewertung
    # bleibt aus, bis die Zahlen gesichtet wurden.
    "lesson_attribution_enabled": True,
    "lesson_counterfactual_enabled": True,
    "lesson_impact_in_prompt": False,
    # Regime-Brücke (PLAN_REGIME_BRUECKE 1.4): darf der KI-Trader den Wechsel
    # shadow -> active der freigegebenen Lab-Analyse vorschlagen (suggest) oder
    # mit 24 h Karenz selbst vollziehen (auto)? Bewusst getrennt von `autonomy`.
    "structural_regime_autonomy": "suggest",
...
ANALYSIS_SYSTEM = (
    ...
    "VWAP: Die 'VWAP(Tag)'-Zeile nennt je Asset den Abstand zum Tages-VWAP. 'frischer "
    "VWAP-RECLAIM/-VERLUST' + Volumen + nicht gegen den 1h-Trend + Abstand <= ~1 ATR5m = valides "
    "vwap_reclaim-Setup (Entry nahe VWAP, SL hinter Swing/VWAP, Ziel 2R). 'überdehnt' (> 2 ATR "
    "vom VWAP) = kein Einstieg in Bewegungsrichtung, eher mean_reversion. "
...
ANALYSIS_SYSTEM_LEAN = (
    ...
    "VWAP(Tag)-Zeile: 'frischer VWAP-RECLAIM/-VERLUST' + Volumen + mit 1h-Trend + <= ~1 ATR5m "
    "vom VWAP = valides vwap_reclaim-Setup (SL hinter Swing/VWAP, Ziel 2R); 'überdehnt' = eher "
    "mean_reversion. "
    "Antworte AUSSCHLIESSLICH mit validem JSON ohne Markdown, exakt in diesem Schema:\n"
...
def _with_structural(snap: Optional[Dict], ctx: Optional[Dict]) -> Optional[Dict]:
    """Regime-Brücke 3.2: Struktur-Kontext nur ab Stufe shadow anhängen –
    ohne Freigabe bleibt der Snapshot exakt wie bisher (auch None)."""
    if not ctx:
        return snap
    return {**(snap or {}), "structural": ctx}
...
class AIEngine(AIEngineContextMixin, AIEngineGovernanceMixin,
    ...
    async def update_config(self, updates: Dict) -> Dict:
        ...
        if "collection_direction_per_class" in updates:
            self.config["collection_direction_per_class"] = bool(updates["collection_direction_per_class"])
        ...
        entry_policy.clamp_updates(updates, self.config)
        tf2_signal.clamp_updates(updates, self.config)
        setup_trigger_mod.clamp_updates(updates, self.config)
        ...
        for flag in ("use_liquidation_data", "use_heatmap_data", "lean_prompt", "smart_skip",
                     "lesson_attribution_enabled", "lesson_counterfactual_enabled",
                     "lesson_impact_in_prompt"):
            if flag in updates:
        ...
        if "structural_regime_autonomy" in updates and \
                updates["structural_regime_autonomy"] in ("off", "suggest", "auto"):
            self.config["structural_regime_autonomy"] = updates["structural_regime_autonomy"]
    ...
    def _snapshot(self, symbol: str) -> Optional[Dict]:
        ...
        # Tages-VWAP-Abstand (jedes Asset mit Volumen): Datenbasis für vwap_reclaim
        vwap_info = None
        try:
            vwap_info = vwap_context.compute(candles, price)
            vw_line = vwap_context.context_line(candles, price)
            if vw_line:
                lines.append(vw_line)
        except Exception as e:
            logger.debug(f"VWAP-Kontext für {symbol} nicht berechenbar: {e}")
    ...
    def _trigger_hint(trigger: str) -> str:
        ...
        t = trigger.upper()
        if t.startswith("SETUP-TRIGGER"):
            return ("Prüfe NUR das genannte Symbol: Der regelbasierte Detektor dieses Setups hat auf "
                    "der letzten 5m-Kerze gefeuert (Backtest-Regel). Passt die Marktstruktur/das Regime "
                    "dazu? Dann setup = GENAU diese Setup-ID, Entry Market, SL/TP nahe an den genannten "
                    "Detektor-Levels (sl_pct/tp-Prozente entsprechend). Wenn nicht: HOLD mit kurzem Grund.")
        if t.startswith("TF2"):
    ...
    def _should_skip_group(self, g_label: str, g_syms, snaps, open_syms, manual: bool) -> bool:
        ...
        for s in g_syms:
            ...
            if move >= thr:
                return False
            # Regime-Brücke 3.1: Struktur-Wechsel (Richtung) zählt als "Markt verändert"
            if ((dec.get("entry_market_snapshot") or {}).get("structural") or {}).get("direction_changed"):
                return False
        return True
    ...
    async def run_analysis(self, manual: bool = False,
        ...
        try:
            ...
            # Lektions-Attribution (Baustein A): aktiver Bestand für die ID-Auflösung
            _attr_on = bool(self.config.get("lesson_attribution_enabled", True))
            _attr_lessons = []
            if _attr_on and self.learning:
                try:
                    _attr_lessons = ai_lessons.active_lessons(await self.learning.get_lessons())
                except Exception:
                    _attr_lessons = []
            for g_label, g_syms in groups:
                ...
                # Regime-Brücke 3.1: Struktur-Block nur für Klassen mit Stufe `active`
                g_struct = ""
                try:
                    g_struct = await structural_regime.prompt_block(list(g_syms))
                    if g_struct:
                        g_struct += "\n\n"
                ...
                try:
                    ...
                    # KI-eigene Setups (Playbook 'custom') ins Setup-Schema aufnehmen –
                    # das statische Enum kannte nur die Bibliothek, dadurch wurden
                    # eigene Setups nie gewählt und bekamen keine Daten.
                    sys_prompt = ai_playbook.with_dynamic_setup_enum(sys_prompt)
                    if _attr_on and _attr_lessons:
                        sys_prompt = sys_prompt + lesson_attribution.PROMPT_ADDENDUM
                    # Entry-Modus A/B (services/entry_policy.py) – leer im Default 'ai'
                    sys_prompt = sys_prompt + entry_policy.prompt_addendum(self.config)
                ...
                for d in data.get("decisions", []):
                    ...
                    dec = {
                        ...
                        **(lesson_attribution.attribution_fields(
                            ...
                            horizon == "swing") if _attr_on and _attr_lessons else {}),
                    }
            ...
            feed_entry = {
                ...
                "decisions": [{"symbol": x["symbol"], "action": x["action"],
                               ...
                               "setup": x.get("setup"),
                               "trade_opened": x.get("trade_opened"),
    ...
    async def _diversification_gate(self, sym: str, dec: Dict,
        ...
        if collection:
            ...
            if bool(self.config.get("collection_direction_per_class", True)):
                cls = setup_asset_class.asset_class_of(sym)
                rows_checked = [t for t in rows_checked
                                if setup_asset_class.asset_class_of(t.get("symbol")) == cls]
    ...
    def _collection_key(self, sym: str, setup: Optional[str]) -> str:
        """Sammel-Cooldown-Schlüssel: je Symbol × Setup (Config), sonst je Symbol."""
        if bool(self.config.get("collection_direction_per_class", True)) and setup:
            return f"{sym}:{setup}"
        return sym
    ...
    async def _emit_signal(self, dec: Dict, collection: bool = False) -> bool:
        ...
        if collection:
            ...
            ck = self._collection_key(sym, dec.get("setup"))
            if cd and (time.time() - self._last_collection_ts.get(ck, 0)) < cd:
                dec["blocked_by"] = (f"Sammel-Cooldown: {sym}/{dec.get('setup') or '-'} vor "
                                     f"< {cd // 60} min bereits gesammelt")
                return False
            cooldown = 0
        else:
        ...
        if cooldown:
            ...
            if max_per_coin > 1:
                ...
                dec["blocked_by"] = f"Cooldown: {sym} vor < {cooldown // 60:g} min bereits gehandelt"
                return False
        ...
        limit_req = False
        # Entry-Modus (Trader-Einstellung): konservativ zwingt Market nach Bestätigung
        ep_type, ep_note = entry_policy.apply(dec, self.config)
        if ep_note:
            dec["entry_mode_note"] = ep_note
            if ep_type != str(dec.get("entry_type") or "market"):
                dec["entry_type"] = ep_type
                logger.info(f"AI-Signal {sym}: {ep_note}")
        ...
        signal = {
            ...
            "applied_lessons": dec.get("applied_lessons"),
        ...
        try:
            ...
            if ok:
                if collection:
                    self._last_collection_ts[self._collection_key(sym, dec.get("setup"))] = time.time()
                else:
                    self._last_signal_ts[sym] = time.time()
                dec["signal_id"] = signal.get("id")
                # Ergebnis fürs KI-Trader-Panel: wurde wirklich ein Trade eröffnet (Modus/Setup)?
                dec["trade_opened"] = bool(signal.get("_trade_opened"))
                if signal.get("trade_mode"):
                    dec["trade_mode"] = signal["trade_mode"]
            return bool(ok)
        except Exception as e:
        ...
        while self.running:
            ...
            try:
                ...
                for name, mod_attr in (("market observer", "ai_market_observer.market_observer"),
                                       ...
                                       ("move scanner", "ai_move_scanner.move_scanner"),
                                       ("setup trigger", "setup_trigger.setup_trigger"),
                                       ("activity guard", "activity_guard.activity_guard")):
                    try:
