...
ANALYSIS_SYSTEM = (
    ...
    '"capital_pct": 10-100, '
    '"entry_type": "market|limit", "limit_price": null, "limit_valid_min": 15-480, '
    '"cancel_limit": false, '
    ...
    "Gültigkeit in Minuten (15-480) – wähle sie selbst passend zum Setup (Scalp kurz, Swing lang); "
...
ANALYSIS_SYSTEM_LEAN = (
    ...
    "sl/tp-Prozente gelten dann ab limit_price, limit_valid_min (15-480) wählst du selbst – danach "
...
class AIEngine:
    ...
    async def _emit_signal(self, dec: Dict, collection: bool = False) -> bool:
        ...
        try:
            max_age_min = float(self.config.get("stale_price_max_min", 10) or 0)
        except (TypeError, ValueError):
            max_age_min = 10.0
    ...
    async def run_deep_analysis(self, manual: bool = False, trigger: str = "schedule",
                                news_event: Optional[Dict] = None) -> Dict:
        ...
        try:
            ...
            if lean_news:
                affects = {str(a).upper() for e in (news_event.get("events") or [])
                           for a in (e.get("affects") or [])}
                hit = [s for s in symbols if s in affects]
                if hit:
            ...
            news_block = "(News deaktiviert)"
            if self.config.get("news_enabled"):
                news = await news_feed.get_headlines(10 if lean_news else 25)
            ...
            doc = {
                ...
                "outlook": [o for o in (data.get("outlook") or []) if isinstance(o, dict)][:40],
