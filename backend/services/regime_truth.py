...
def calibration_grid(detector: str, base: Dict, d: float) -> Dict[str, List[float]]:
    ...
    if detector == "jump":
        return {
            "jump_fast_days": _uniq_clip([base["jump_fast_days"], 0.5, 1.0, 2.0], 0.1, 30.0),
            "jump_slow_days": _uniq_clip([base["jump_slow_days"], 7.0, 14.0, 21.0], 0.5, 90.0),
            "jump_center": _uniq_clip([base["jump_center"], 0.4, 0.6, 0.8], 0.02, 5.0),
            "jump_penalty_days": _uniq_clip([base["jump_penalty_days"], 0.75, 1.5, 3.0], 0.0, 30.0),
            **phase, **hold,
        }
