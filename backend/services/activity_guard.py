...
DEFAULTS: Dict = {
    ...
    "idea_rounds": True,          # Stufe 4: LLM-Ideen-Runde bei anhaltender Flaute
    "idea_gap_hours": 24,         # Mindestabstand zwischen Ideen-Runden
}
IDEAS_KEEP = 10
...
def normalize(raw: Optional[Dict]) -> Dict:
    ...
    cfg["idea_rounds"] = bool(raw.get("idea_rounds", True))
...
def idea_round_due(state: Dict, rate: float, now: Optional[datetime] = None) -> bool:
    """Stufe 4 fällig? Nur wenn alle Lockerungs-Stufen ausgereizt sind, die
    Aktivität weiter unter dem Ziel liegt und der Ideen-Abstand eingehalten ist (rein)."""
    if not state.get("idea_rounds", True):
        return False
    if int(state.get("steps") or 0) < int(state.get("max_steps") or 0):
        return False
    if rate >= state.get("target_trades_per_day", 3):
        return False
    last = state.get("last_idea_at")
    if not last:
        return True
    try:
        ...
        return True
    return ((now or datetime.now(timezone.utc)) - dt) >= timedelta(hours=int(state.get("idea_gap_hours") or 24))
...
class ActivityGuard:
    ...
    async def check(self, manual: bool = False) -> Dict:
        ...
        if result["action"] == "none" and idea_round_due(state, rate):
            await self._idea_round(state, result, rate, target, cur)
        if result["action"] != "none":
            hist = list(state.get("history") or [])
            hist = ([{k: result.get(k) for k in ("at", "rate", "action", "changes", "steps")}] + hist)[:20]
            state["history"] = hist
        await self._save(state)
        self.last_result = result
        return result

    async def _idea_round(self, state: Dict, result: Dict, rate: float, target: int, cur: Dict) -> None:
        """Stufe 4: Setup-Lücke per LLM suchen (services/activity_ideas)."""
        from services import activity_ideas
        state["last_idea_at"] = _now_iso()
        try:
            idea = await activity_ideas.run(self.engine, rate, target, cur)
        except Exception as e:  # noqa: BLE001
            logger.warning(f"ActivityGuard Ideen-Runde: {e}")
            idea = None
        if not idea:
            return
        result.update({"action": "idea_round", "idea": idea})
        state["ideas"] = ([idea] + list(state.get("ideas") or []))[:IDEAS_KEEP]
        await self._post_chat(activity_ideas.chat_text(idea))

    async def run_idea_round(self) -> Dict:
        """Manuell (API): Ideen-Runde unabhängig von Stufen/Abstand ausführen."""
        state = await self._load()
        rate = await self.trades_per_day(state["window_hours"])
        cur = {k: self.engine.config.get(k) for k in ADJUST_KEYS}
        result: Dict = {"status": "ok", "rate": rate, "target": state["target_trades_per_day"],
                        "action": "none", "at": _now_iso()}
        await self._idea_round(state, result, rate, state["target_trades_per_day"], cur)
        await self._save(state)
        if result["action"] != "idea_round":
