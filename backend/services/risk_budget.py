...
DEFAULT_CONFIG = {
    ...
    # Rest-Budget-Trading (09/2026, analog capital_fit): passt das geplante
    # Risiko nicht mehr ins Budget, wird die Marge auf den freien Rest
    # verkleinert statt den Trade abzulehnen (Untergrenze capital_fit.MIN_MARGIN_USDT).
    "scale_to_fit": True,
}
...
def headroom(symbol: str, open_total: float, by_cluster: Dict[str, float],
             equity: Optional[float], cfg: Dict) -> Optional[float]:
    """Maximal zulässiges Risiko (USDT) für EINEN neuen Trade (rein).
    None = kein Limit wirksam (Budget aus, Equity unbekannt -> entscheidet check())."""
    if not cfg.get("enabled", True) or equity is None or equity <= 0:
        return None
    room: Optional[float] = None
    pmax = float(cfg.get("max_portfolio_risk_pct") or 0)
    if pmax > 0:
        room = equity * pmax / 100 - float(open_total or 0)
    cmax = float(cfg.get("max_cluster_risk_pct") or 0)
    if cmax > 0:
        c_room = equity * cmax / 100 - float(by_cluster.get(cluster_of(symbol), 0.0) or 0)
        room = c_room if room is None else min(room, c_room)
    return None if room is None else round(max(0.0, room), 6)


def fit_scale(new_risk_usdt: float, room: Optional[float]) -> Optional[float]:
    """Faktor (<1), um den die Position verkleinert werden muss, damit ihr Risiko
    ins Rest-Budget passt – None, wenn nichts zu verkleinern ist (rein)."""
    if room is None:
        return None
    try:
        new_risk = float(new_risk_usdt or 0)
    except (TypeError, ValueError):
        return None
    if new_risk <= 0 or new_risk <= room + 1e-9:
        return None
    return round(max(0.0, room) / new_risk, 6)
...
async def update_config(db, updates: Dict) -> Dict:
    ...
    for k, v in (updates or {}).items():
        if k not in DEFAULT_CONFIG:
            continue
        if k in ("enabled", "fail_open_no_equity", "scale_to_fit"):
            clean[k] = bool(v)
        else:
            try:
...
async def scale_for_new_trade(db, mode: str, symbol: str, new_risk_usdt: float,
                              equity: Optional[float]) -> Tuple[Optional[float], Optional[float]]:
    """(Verkleinerungs-Faktor, Rest-Budget USDT) für den Rest-Budget-Pfad.
    Faktor None = Trade passt (oder Skalierung deaktiviert) -> nichts ändern."""
    cfg = await get_config(db)
    if not cfg.get("enabled", True) or not cfg.get("scale_to_fit", True):
        return None, None
    u = await usage(db, mode)
    room = headroom(symbol, u["open_risk_usdt"], u["by_cluster"], equity, cfg)
    return fit_scale(new_risk_usdt, room), room
