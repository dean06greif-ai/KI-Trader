...
def event_inactive_ok(sid: str, entry: Optional[Dict]) -> bool:
    """Event-Setups nur überarbeiten, wenn sie nach der Event-Regel inaktiv sind
    (Event fand statt, aber kein Trade) – Alt-Einträge nach der Wochen-Regel zählen nicht."""
    from services import setup_lifecycle
    return not setup_lifecycle.is_event_setup(sid) or bool((entry or {}).get("event"))
        ...
        for sid in library:
            ...
            if why:
                ...
                candidates.append((sid, "chronisch negativ", why))
                    ...
                    and event_inactive_ok(sid, scope["inactive"].get(sid)):
