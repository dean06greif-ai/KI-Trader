...
MOVE_SYSTEM = (
    ...
    "Setup. Antworte AUSSCHLIESSLICH mit validem JSON ohne Markdown:\n"
    ...
    async def scan(self, manual: bool = False, symbol: Optional[str] = None) -> Dict:
        ...
        for sym in symbols:
            ...
            event = {"id": str(uuid.uuid4()), "ts": _now_iso(), "symbol": sym,
                     ...
                     "caught": caught_info["caught"]}
            analysis = None
            if self.engine and self.engine.key \
                    and self._llm_budget_left(cfg["max_llm_per_day"]):
                # Erwischte Bewegungen nur analysieren, wenn Budget großzügig übrig
                if not caught_info["caught"] or self._llm_used < cfg["max_llm_per_day"] // 2:
                    self._llm_used += 1
                    try:
