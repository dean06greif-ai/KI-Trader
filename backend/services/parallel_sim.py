...
_PAUSE_EVT = None                  # mp.Event: gesetzt = Kind-Prozesse pausieren (job_control)


def set_paused(flag: bool) -> None:
    """Pause an alle Kind-Prozesse des aktuellen Pools durchreichen."""
    if _PAUSE_EVT is None:
        return
    if flag:
        _PAUSE_EVT.set()
    else:
...
def _child_should_stop() -> bool:
    """should_stop für simulate_pair im Kind-Prozess: wartet, solange die
    Pause gesetzt ist (alle 0,5 s prüfen); Abbruch läuft weiter über den Pool."""
    if _PAUSE_EVT is not None and _PAUSE_EVT.is_set():
...
def _init_spawn(data: Dict[str, List[Dict]], pause_evt=None):
    global _DATA, _FS, _PAUSE_EVT
    _DATA = data
    _FS = {}
    _PAUSE_EVT = pause_evt
...
def close_pool(pool: ProcessPoolExecutor, kill: bool = False):
    ...
    global _DATA, _FS, _PAUSE_EVT
    set_paused(False)  # pausierte Kinder freigeben, sonst hängt shutdown(wait=True)
    try:
...
def sim_pair_task(spec: Dict, key: str, symbol: str, settings: Dict, cfg: Dict,
    ...
    try:
        ...
        return simulate_pair(strat, candles, symbol, settings, cfg, None,
                             collect_trades, _child_should_stop, provider)
    except Exception as e:  # noqa: BLE001
...
def evaluate_task(spec: Dict, symbols: List[str], settings: Dict, cfg: Dict) -> Dict:
    ...
    return _evaluate(strat, hist, settings, cfg, fs, _child_should_stop)
