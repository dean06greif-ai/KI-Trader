...
async def discover_regime_strategy(segments: Dict[str, List[Dict]], rid: int,
                                   ...
                                   tf_options: List[str] = None,
                                   soft_stop=None) -> Dict:
    """Vollständige eigene Strategie für EIN Regime entdecken.

    `soft_stop` (optional, Callback -> bool): sanfter Stop ("Suche beenden &
    Beste behalten") – die Greedy-Suche endet nach dem laufenden Kandidaten
    und liefert die bis dahin beste Regel-Kombination (kein JobCancelled).
    ...
    for round_i in range(max_rules):
        ...
        if round_best is None or round_best[0] <= best_score + 1e-9:
            steps.append({"round": round_i + 1, "added": None,
                          "info": ("Suche vom Nutzer beendet – bestes Ergebnis behalten"
                                   if stopped_early else
                                   "Keine weitere Regel verbessert diese Marktphase")})
            break
...
async def _deep_regime_search(cands, definition, segments, rid, settings, base_cfg,
                              ...
                              best_score, best_metrics, has_val,
                              soft_stop=None) -> Dict:
    ...
    `deep_report` mit Beitrag/Synergie je Regel. `soft_stop`: sanfter Stop –
    weitere Kandidaten werden nicht mehr bewertet (None), alle Phasen laufen
