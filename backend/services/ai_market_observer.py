...
class MarketObserver:
    ...
    async def collect(self, persist: bool = True) -> List[Dict]:
        ...
        for sym in self._symbols():
            ...
            if not feats:
                continue
            # Wochenend-Guard: geschlossene reale Märkte liefern eingefrorene
            ...
            label_hit = self._label_hit_pct(sym, feats.get("regime"))
            if label_hit is not None:
                feats["regime_label_hit_pct"] = label_hit
    ...
    def _label_hit_pct(self, symbol: str, label: Optional[str]) -> Optional[float]:
        """Vorwärts-Trefferquote des AKTUELLEN Regime-Labels (Cockpit-Cache, 14 d):
        je Symbol UND je Label – damit das ML-Gate schwache Labels gezielt erkennt.
        Nur belastbare Labels (genug Punkte); None wenn nichts vorliegt."""
        if not label:
            return None
        try:
            from services import regime_cockpit
            ent = regime_cockpit._overview_cache.get(f"{symbol}:14")
            row = ((ent or {}).get("row") or {}).get("observer_per_label") or {}
            lab = row.get(str(label)) or {}
            if lab.get("reliable") and lab.get("hit_pct") is not None:
                return float(lab["hit_pct"])
        except Exception:  # noqa: BLE001
            pass
        return None
