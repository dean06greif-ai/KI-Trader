...
DEFAULTS: Dict = {"enabled": False, "interval_hours": 24, "days": 90, "mode": "loop",
                  "asset_classes": list(ac.CLASSES), "ai_revise": True, "ai_rounds": 3,
                  "target_passed": 3}
MODES = ("single", "loop", "ai_loop")
SAVE_KEYS = ("enabled", "interval_hours", "days", "mode", "asset_classes",
             "ai_revise", "ai_rounds", "target_passed")
HISTORY_KEEP = 10
CHECK_EVERY_S = 60
...
def normalize(raw: Optional[Dict]) -> Dict:
    ...
    try:
        ...
        pass
    cfg["mode"] = raw.get("mode") if raw.get("mode") in MODES else "loop"
    cfg["ai_revise"] = bool(raw.get("ai_revise", True))
    for key, lo, hi in (("ai_rounds", 1, 10), ("target_passed", 1, 20)):
        try:
...
async def save(db, raw: Dict) -> Dict:
    ...
    merged = normalize({**cur, **{k: v for k, v in (raw or {}).items() if k in SAVE_KEYS}})
...
async def _record(db, cfg: Dict, summary: Optional[Dict], error: Optional[str]) -> None:
    ...
    if summary is not None:
        upd["auto.last_summary"] = {k: summary.get(k) for k in
                                    ("passed", "tested", "finished_at", "mode", "days", "ai_proposals", "ai_rounds")}
...
async def run_once(db, cfg: Optional[Dict] = None) -> Optional[Dict]:
    ...
    if ram_queue.free_mb() < ram_queue.MIN_FREE_MB:
        ...
        return None
    ai = runner.ai_options(cfg)
    params = {"kind": "ai_seed", "asset_classes": cfg["asset_classes"], "days": cfg["days"],
              "mode": cfg["mode"], "setups": None, "trigger": "auto", **ai}
