...
INT_KEYS = ("lookback", "lookback_h", "min_bars", "slope_bars", "pivot_k", "sl_lookback",
...
def build_prompt(asset_class: str, setup: str, desc: str, base: Dict, history: List[Dict],
    ...
    return (
        ...
        f"{rules.get('min_oos_trades')} Trades (adaptiv: erwartet werden ~{rules.get('expected_trades') or '?'} "
        f"Signale über alle Assets der Klasse im Zeitraum; zu wenige Trades = kein belastbarer Edge, "
        f"`min_trades_factor` nur begründet ändern), PnL in BEIDEN Fenstern > 0 nach Gebühren, "
