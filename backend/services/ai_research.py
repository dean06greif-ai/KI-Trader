...
RESEARCH_SYSTEM = (
    ...
    '"regime_release_recommendation": {"asset_class": "crypto|indices|resources|forex", '
    '"verdict": "activate|keep_shadow|revoke", "confidence": 0-100, "reason": "kurz"}}\n'
    "regime_release_recommendation NUR, wenn der Block STRUKTUR-REGIME (SHADOW) Daten zeigt: "
    "'activate' nur bei >= 30 Trades je Struktur-Regime UND klar unterschiedlichen Ø-Rewards; "
    "'revoke' nur, wenn die wirksame Struktur-Ebene Ergebnisse verschlechtert; sonst weglassen."
)
...
class ResearchAnalyst:
    ...
    async def reset(self) -> Dict:
        """Forschungs-Daten zurücksetzen: Report + Zustand wandern in den
        Papierkorb (lab_trash) und können wiederhergestellt werden."""
        from services import lab_trash
        try:
            rep = await self.db.settings.find_one({"_id": "ai_research_report"})
            st = await self.db.settings.find_one({"_id": "ai_research_state"})
            if rep or st:
                for d in (rep, st):
                    if d:
                        d.pop("_id", None)
                await lab_trash.put(
                    self.db, "research", {"report": rep, "state": st},
                    label=f"Forschung (Stand {str((rep or {}).get('ts') or '?')[:16]})")
        ...
        self._last_counts = {}
        logger.info("Forschungs-Analyst zurückgesetzt (Report & Zustand in den Papierkorb)")
        return {"status": "ok"}

    async def restore(self, payload: Dict) -> Dict:
        """Papierkorb-Eintrag wiederherstellen: Report + Zustand zurückschreiben."""
        rep = payload.get("report")
        st = payload.get("state")
        if not rep and not st:
            return {"status": "error", "detail": "Papierkorb-Eintrag ohne Daten"}
        if rep:
            await self.db.settings.replace_one({"_id": "ai_research_report"}, dict(rep), upsert=True)
            self.report = dict(rep)
            self.last_run = rep.get("ts")
        if st:
            await self.db.settings.replace_one({"_id": "ai_research_state"}, dict(st), upsert=True)
            self._last_counts = st.get("counts") or {}
        self.last_error = None
        logger.info("Forschungs-Daten aus dem Papierkorb wiederhergestellt")
        return {"status": "ok"}
    ...
    async def run(self, manual: bool = False, trigger: str = "manual") -> Dict:
        ...
        try:
            ...
            try:
                ...
                pass
            # Regime-Brücke 1.4: Shadow-Stichprobe je Struktur-Regime als Kontext
            try:
                from services import ai_rewards, regime_release
                rels = await regime_release.all_releases(self.db)
                if rels:
                    rows = await ai_rewards.by_structural_regime(self.db, 90)
                    lines = [f"- {r['regime']}: {r['trades']} Trades, Ø Reward {r['avg_reward']:+.2f}, "
                             f"WR {r['win_rate']} %" for r in rows]
                    stages = ", ".join(f"{','.join((r.get('release') or {}).get('asset_classes') or [])}: "
                                       f"{(r.get('release') or {}).get('stage')} ({r['id']})" for r in rels)
                    extra += ("\n\n=== STRUKTUR-REGIME (SHADOW) – Rewards je Lab-Regime, 90 Tage ===\n"
                              f"Freigaben: {stages}\n" + ("\n".join(lines) or "(noch keine Trades)"))
            except Exception as re_:
                logger.debug(f"Struktur-Regime-Kontext: {re_}")
            ...
            self.last_error = None
            # Regime-Brücke 1.4: KI-Empfehlung -> Proposal (Gate wird dort geprüft)
            try:
                from services import regime_release
                rec = parsed.get("regime_release_recommendation")
                if isinstance(rec, dict):
                    doc["regime_release_recommendation"] = rec
                    await regime_release.handle_recommendation(self.db, self.engine, rec)
            except Exception as re_:
                logger.warning(f"regime_release_recommendation: {re_}")
