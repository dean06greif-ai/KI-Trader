...
GATE_FEATURES = [
    ...
    "regime_hit_pct",
    # Trefferquote des konkreten Regime-Labels zum Entry (je Symbol × Label) –
    # 50 = neutral/unbekannt. Alte Modelle nutzen ihre gespeicherte Feature-Liste.
    "regime_label_hit_pct",
]
...
DEFAULT_SETTINGS = {"threshold": 0.45, "shadow_enabled": True,
                    ...
                    "retrain_min_new": 50,
                    # ML-Positionssteuerung: OOS-AUC unter auc_min ->
                    # Positionsgröße des KI-Traders × auc_risk_factor.
                    # Audit 2.10: Default AUS – Skalierung nur bewusst
                    # aktivieren, solange das Shadow-Training beweist, dass
                    # die AUC ehrlich (leckagefrei, 2.2) gemessen ist.
                    "auc_risk_scaling": False, "auc_min": 0.55,
                    "auc_risk_factor": 0.5}
...
def gate_feature_row(side: str, confidence: float, sl_pct: float, tp1_pct: float,
    ...
    return {
        ...
        "regime_label_hit_pct": float(f.get("regime_label_hit_pct")
                                      if f.get("regime_label_hit_pct") is not None else 50.0),
    }
...
class MLGate:
    ...
    async def update_settings(self, updates: Dict) -> Dict:
        ...
        if "auc_risk_scaling" in updates:
            self.settings["auc_risk_scaling"] = bool(updates["auc_risk_scaling"])
        if "auc_min" in updates:
            self.settings["auc_min"] = min(0.75, max(0.50, float(updates["auc_min"])))
        if "auc_risk_factor" in updates:
            self.settings["auc_risk_factor"] = min(1.0, max(0.1, float(updates["auc_risk_factor"])))
    ...
    def size_factor(self) -> Tuple[float, str]:
        """(Faktor, Grund) für die KI-Positionsgröße. 1.0 = keine Reduktion.

        Fällt die Out-of-Sample-AUC des aktiven Gate-Modells unter auc_min,
        liefert dies auc_risk_factor (Default 0.5 = -50% Größe). Blockt nie."""
        if not self.settings.get("auc_risk_scaling", False) or not self.model_meta:
            return 1.0, ""
        try:
            auc = (self.model_meta.get("metrics") or {}).get("oos_auc")
            if auc is None:
                return 1.0, ""
            auc_min = float(self.settings.get("auc_min", 0.55))
            if float(auc) < auc_min:
                f = min(1.0, max(0.1, float(self.settings.get("auc_risk_factor", 0.5))))
                return f, (f"Gate-OOS-AUC {float(auc):.3f} < {auc_min:g} – "
                           f"Positionsgröße auf {f * 100:.0f}% reduziert")
        except (TypeError, ValueError):
            pass
        return 1.0, ""
    ...
    def status(self) -> Dict:
        ...
        sf, sf_why = self.size_factor()
        return {
            "size_factor": sf,
            "size_factor_reason": sf_why or None,
            # Audit 2.10: sichtbare Kennzeichnung, ob die ML-Skalierung aktiv ist
            "risk_scaling_active": bool(self.settings.get("auc_risk_scaling", False)),
            "mode": "shadow",
