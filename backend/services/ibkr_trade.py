...
class GatewayWatch:
    ...
    * Logout: authenticated war True und wird False -> 'logout'; zurück -> 'login'.
    * readyz rot (≠200) länger als RED_AFTER_SEC -> 'readyz_red' (einmalig),
      wieder grün nach Alarm -> 'readyz_green'."""
    RED_AFTER_SEC = 300
    ...
    def evaluate(self, now: float, authenticated: Optional[bool],
        ...
        if readyz == 200:
            ...
            if self.red_since is None:
                self.red_since = now
            elif not self.red_alerted and now - self.red_since >= self.RED_AFTER_SEC:
        ...
        try:
            if state.db is not None:
                ...
                await website_notify(state.db, "ibkr_gateway", title,
                                     ...
                                     cooldown_min=1, source="IBKR-Monitor")
        except Exception as e:
