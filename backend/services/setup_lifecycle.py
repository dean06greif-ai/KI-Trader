...
# ---- Event-Setups (FOMC/CPI/NFP/PPI/PCE): nur Event-Fenster zählen ----
# Event-Setups dürfen NUR im Event-Fenster handeln – "wenig Trades pro Woche"
# ist bei ihnen normal. Überarbeitet wird ausschließlich, wenn ein Event
# stattfand und das Setup in dessen Fenster KEINEN Trade gemacht hat.
def event_of(sid: str):
    """(label, pre_min, post_min, termine) des Event-Setups – None für normale Setups."""
    from services import econ_event, fomc_event
    if sid == fomc_event.SETUP_ID:
        return "FOMC", fomc_event.PRE_MIN, fomc_event.POST_MIN, fomc_event.all_decisions_utc()
    ev = econ_event.by_setup(sid)
    if ev is not None:
        return (str(getattr(ev, "label", "") or sid.replace("_event", "").upper()),
                econ_event.PRE_MIN, econ_event.POST_MIN, ev.all_releases_utc())
    return None


def is_event_setup(sid: str) -> bool:
    return event_of(sid) is not None


def event_inactivity_reason(sid: str, since_iso, trade_times, now=None) -> Optional[str]:
    """Event-Setup inaktiv? (rein & testbar) Nur wenn das LETZTE abgeschlossene
    Event-Fenster seit `since_iso` ohne Trade des Setups blieb. Kein Event seit
    Anlage/Revision -> nie inaktiv; letztes Event mit Trade -> aktiv."""
    info = event_of(sid)
    if info is None:
        return None
    label, pre_min, post_min, dates = info
    try:
        since = datetime.fromisoformat(str(since_iso).replace("Z", "+00:00"))
    except (TypeError, ValueError):
        return None
    ...
    windows = [(t - timedelta(minutes=pre_min), t + timedelta(minutes=post_min), t)
               for t in dates]
    done = [w for w in windows if w[0] >= since and w[1] < now]
    if not done:
        return None
    times = []
    for ts in trade_times or []:
        try:
            d = datetime.fromisoformat(str(ts).replace("Z", "+00:00"))
        except (TypeError, ValueError):
            continue
        times.append(d if d.tzinfo else d.replace(tzinfo=timezone.utc))
    start, end, t_ev = done[-1]
    if any(start <= d <= end for d in times):
        return None
    missed = sum(1 for s, e, _ in done if not any(s <= d <= e for d in times))
    return (f"{label}-Event am {t_ev.strftime('%d.%m.%Y %H:%M')} UTC ohne Trade im Event-Fenster "
            f"({missed}/{len(done)} Event-Fenster seit Anlage/Revision ohne Trade)")


async def event_trade_times(db, sid: str, since_iso: str, symbols: Optional[List[str]] = None) -> List[str]:
    """Eröffnungszeiten der KI-Trades eines Event-Setups seit `since_iso`."""
    match = {"strategy_id": "ai_trader", "setup": sid, "opened_at": {"$gte": str(since_iso)}}
    if symbols is not None:
        match["symbol"] = {"$in": list(symbols)}
    rows = await db.auto_trades.find(match, {"_id": 0, "opened_at": 1}).to_list(2000)
    return [r.get("opened_at") for r in rows if r.get("opened_at")]
