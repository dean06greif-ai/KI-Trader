...
class AIEngineContextMixin:
    ...
    async def _open_trades_text(self, allowed: Optional[List[str]] = None) -> str:
        ...
        def _fmt(t: Dict) -> str:
            flags = []
            if t.get("data_collection"):
                flags.append("SAMMLUNG·Daten-Trade")
            ...
            if isinstance(fr, (int, float)) and abs(fr) >= 0.01:
                pnl_txt += f", Funding {fr:+.2f}USDT"
            return (
        ...
        if any(t.get("data_collection") for t in rows):
            lines.append(
                "(Hinweis: SAMMLUNG = Daten-Sammel-Trade des KI-Traders – nur Paper, "
                "zählt NICHT als Paper-/Live-Performance und erscheint auf der Website "
                "nur im Tab 'Sammlung'. Bei Meldungen an den Trader immer als "
                "Sammlungs-Trade kennzeichnen, nie als regulären Paper-/Live-Trade ausgeben.)"
            )
        return "\n".join(lines)
    ...
    async def _context_brief(self, coins=None) -> str:
        ...
        btc_blk = self._btc_corr_block()
        if btc_blk:
            parts.append(btc_blk)
    ...
    def _btc_corr_block(self) -> str:
        ...
        damit Altcoin-Entries gegen die BTC-Richtung vermieden werden."""
        try:
            snap = self._snapshot("BTC")
        except Exception:
            snap = None
        if not snap:
            return ""
        segs = [s.strip() for s in (snap.get("text") or "").split(" | ")
                if s.strip().startswith(("15m:", "1h:"))]
        if not segs:
            return ""
        return ("=== BTC-KORRELATIONS-FILTER (Leitwährung) ===\n"
                "BTC-Trend: " + " | ".join(segs) + "\n"
                "Weiche Regel (KEIN hartes Verbot): In normalen Phasen folgen Altcoins "
                "BTC – eröffne dann keine Altcoin-Position GEGEN den BTC-15m/1h-Trend, "
                "außer mit explizitem, coin-spezifischem Grund (News, klar stärkere "
                "eigene Struktur, Divergenz).\n"
                "AUSNAHME Altseason/Entkopplung: Laufen Altcoins sichtbar stärker als "
                "BTC (eigene Trends trotz schwachem/seitwärts BTC), ist die Korrelation "
                "gebrochen – dann gilt diese Regel NICHT und du tradest die Stärke der "
                "Alts normal. Prüfe das an den Marktdaten (Δ% der Alts vs. BTC), statt "
                "die Regel blind anzuwenden. Bei BTC-Trend 'unklar' keine Einschränkung.")
    ...
    async def _macro_block(self) -> str:
        ...
        if fo:
            try:
                from services import funding_fade
                lines.extend(funding_fade.context_lines(fo))
            except Exception as e:
                logger.debug(f"funding fade block: {e}")
    ...
    async def _liquidity_block(self, symbols: Optional[List[str]] = None) -> str:
        ...
        `symbols` = nur diese (z.B. Trade-Review: Symbole der offenen Trades).
        """
        ...
        if symbols is not None:
            ...
            syms = [s for s in syms if str(s).upper() in wanted]
            if not syms:
                return ""
        try:
    ...
    async def _capital_risk_block(self) -> str:
        ...
        return ("=== KAPITAL & RISIKO-STATUS (live) ===\n" + "\n".join(lines) +
                ...
                "Rest-Kapital (Untergrenze ~1 USDT, Börsen-Minimum wird beachtet) – "
                "lehne gute Setups deshalb NICHT wegen knappen Kapitals ab.")

    async def _analysis_extra_blocks(self, purpose: str = "analysis",
                                     symbols: Optional[List[str]] = None) -> str:
        ...
        purpose:
          ...
          "trade_review"  – schlanker Kontext für den Trade-Manager (Positions-
                            ...
                            `symbols` (Symbole der offenen Trades) begrenzt den
                            Liquiditäts-Block auf diese Assets (Token-Sparen).
        """
        ...
        if not review:
            ...
            try:
                from core.state import scanner as _scanner
                from services import session_open
                so = session_open.context_block(_scanner)
                if so:
                    parts.append(so)
            except Exception as e:
                logger.warning(f"AI session-open block failed: {e}")
        if purpose != "analysis_base":
            try:
                liq = await self._liquidity_block(symbols if review else None)
                if liq:
                    parts.append(liq)
            except Exception as e:
                logger.warning(f"AI liquidity block failed: {e}")
        if not review:
            # Setup-Trigger (services/setup_trigger.py): frische regelbasierte
            # Detektor-Treffer der Backtest-Setups – kompakt, nur wenn vorhanden.
            try:
                from services.setup_trigger import setup_trigger
                st_txt = setup_trigger.context_text()
                if st_txt:
                    parts.append(st_txt)
            except Exception as e:
                logger.debug(f"AI setup trigger block failed: {e}")
        try:
            if self.learning:
        ...
        if not review:
            ...
            # Nachanalyse (services/trade_postmortem.py): nur robuste, mehrfach
            # geprüfte Was-wäre-wenn-Befunde – leer, solange nichts robust ist.
            try:
                from services.trade_postmortem import postmortem
                pm = await postmortem.context_text()
                if pm:
                    parts.append("=== NACHANALYSE DEINER TRADES (Was-wäre-wenn, robust gefiltert) ===\n" + pm)
            except Exception as e:
                logger.warning(f"AI postmortem block failed: {e}")
        if not review:
