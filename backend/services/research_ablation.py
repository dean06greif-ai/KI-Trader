...
def ablation_variants(engine_config: Optional[Dict]) -> List[Dict]:
    ...
    if det == "reactive":
        ...
        if float(ec.get("jump_penalty_days", 1.5) or 0) > 0:
            add("no_jump_penalty", "ohne Sprungkosten (reines Nächstes-Zentrum)",
                "jump_penalty_days", jump_penalty_days=0.0)
        add("alt_ema", "einfache Alternative: Detektor 'ema'",
            "detector:jump", detector="ema")
    else:  # regression
        add("alt_ema", "einfache Alternative: Detektor 'ema'",
