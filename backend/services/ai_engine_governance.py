...
class AIEngineGovernanceMixin:
    ...
    async def _macro_gate(self, prop: Dict, macro_keys: List[str], current: Dict,
        ...
        # Audit 2.8: Evidenz-Basis = geschlossene Trades im Fenster – frühere
        # Vorschläge zählen nur mit neuer Evidenz dazwischen (real_confirmations).
        trade_ts = await self._evidence_trade_ts(scope, prop.get("symbol"), since)
        min_new = int(validation_gate.settings.get("macro_evidence_min_trades", 3))
        worst = None
        clamped_any = False
        ...
        for key in macro_keys:
            ...
            try:
                prior = await self.db.ai_proposals.find({
                    "scope": scope,
                    "symbol": prop["symbol"],
                    f"changes.{key}": {"$exists": True},
                    "macro_direction": direction,
                    "ts": {"$gte": since},
                }, {"ts": 1, "_id": 0}).to_list(300)
                confirmations, evidence_since = ai_validation.real_confirmations(
                    [p.get("ts") for p in prior], trade_ts, min_new)
            except Exception:
                confirmations, evidence_since = 1, None
            gate = validation_gate.macro(sample, confirmations)
            gate["key"] = key
            gate["direction"] = direction
            gate["evidence_since_ts"] = evidence_since
            gate["evidence_min_trades"] = min_new
    ...
    async def _evidence_trade_ts(self, scope: str, symbol: Optional[str], since: str) -> List[str]:
        """Zeitstempel geschlossener Trades im Bestätigungs-Fenster (Audit 2.8)."""
        try:
            if scope == "candidate" and symbol:
                flt = {"ai_candidate_id": symbol, "status": "closed",
                       "closed_at": {"$gte": since}}
            else:
            ...
            rows = await self.db.auto_trades.find(
                flt, {"closed_at": 1, "_id": 0}).to_list(2000)
            return sorted(str(r.get("closed_at")) for r in rows if r.get("closed_at"))
        except Exception as e:
            logger.debug(f"Evidenz-Trades nicht ladbar: {e}")
            return []
    ...
    async def decide_proposal(self, pid: str, approve: bool) -> Optional[Dict]:
        ...
        if not prop or prop.get("status") not in ("pending", "needs_data",
            ...
            return None
        if approve:
            symbol = None if prop.get("scope") in ("engine", "regime_release") else prop.get("symbol")
