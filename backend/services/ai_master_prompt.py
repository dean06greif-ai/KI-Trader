...
DEFAULT_LESSON_POLICY = (
    ...
    "4. Risiko nur vorsichtiger: Regeln zu Stop-Loss, Marge/Kapital und Positionsgröße dürfen "
    "strenger, nie aggressiver werden. Der Hebel ist davon ausgenommen – er wird technisch "
    "über Marge und SL-Abstand gesteuert (siehe 9).\n"
    ...
    "festgelegten – Lektionen. Ergänzt eine neue Erkenntnis eine bestehende Lektion nur, wird "
    "die bestehende geschärft statt eine zweite angelegt.\n"
    "7. Vorläufig markieren, solange die Stichprobe klein ist; erst mit mehr Daten schärfen.\n"
    "8. KEINE pauschalen Richtungs-Lektionen: 'Longs bevorzugen' oder 'Shorts meiden' ohne "
    "Kontext ist verboten – ob Longs oder Shorts funktionieren, hängt vom aktuellen "
    "Marktregime ab. Richtungs-Erkenntnisse IMMER an eine prüfbare Bedingung binden "
    "(z.B. 'solange BTC im 4h-Abwärtstrend unter der EMA200 liegt') und mit "
    "Gültigkeitsdauer versehen – sie verfallen, wenn das Regime dreht.\n"
    "9. KEINE Hebel-Deckel-Lektionen: Das Auto-Leverage-System wählt bewusst den maximal "
    "möglichen Hebel, weil die Liquidation dadurch HINTER dem Stop-Loss liegt und jeder "
    "Trade das Maximum aus der Marge holt. Ein pauschal reduzierter Hebel verbessert "
    "nichts – bei schlechten Trades entscheidet der SL, nicht der Hebel. Risiko wird "
    "über die MARGE (capital_pct) oder den SL-ABSTAND gesteuert, niemals über "
    "Hebel-Empfehlungen.\n"
    "10. KEINE Coin-Ranglisten aus vergangenem PnL: 'Coin A priorisieren, Coin B meiden' "
    "allein weil A zuletzt Gewinn und B Verlust brachte, ist verboten – das ist Zufall und "
    "Regime, keine Erkenntnis, und es verfälscht die Datensammlung. Asset-Lektionen brauchen "
    "einen benennbaren Mechanismus (z.B. Spread/Slippage, Handelszeiten, Volatilität, "
    "Liquidität) und eine prüfbare Bedingung.\n"
    "11. Lektionen sind Handlungsregeln, keine Meta-Kommentare: Aussagen wie 'Paper-PnL nicht "
    "überbewerten' oder 'Datenbasis noch dünn' gehören in die Einschätzung, nicht in den "
    "Lektionsbestand. Jede Lektion muss sagen, WANN die KI WAS anders macht."
)

# Frühere Standard-Fassungen: wurde die Policy nie vom Trader angepasst (Text ist
# 1:1 eine alte Vorlage), wird sie beim Laden auf die aktuelle Vorlage gehoben.
_LEGACY_LESSON_POLICY_V1 = (
    "1. Datenbasis: Eine Lektion beruht auf ausgewerteten Trades, nicht auf einer Vermutung. "
    "Nenne Bedingung, Konsequenz und Belege (z.B. „bei ATR% > 0.8 war der 0.4%-SL in 12 von 15 "
    "Trades zu eng“).\n"
    "2. Vorrang: Keine Lektion darf den MasterPrompt aufweichen, umgehen oder relativieren.\n"
...
LEGACY_LESSON_POLICIES = {_LEGACY_LESSON_POLICY_V1.strip()}

DEFAULT_RULES: Dict = {
    # 0 = keine Obergrenze. Bewusst KEIN Default-Limit mehr: das Auto-Leverage-
    # System setzt die Liquidation hinter den Stop-Loss – Risiko wird über
    # Marge und SL gesteuert, nicht über einen pauschalen Hebel-Deckel.
    # (Bug-Report: die KI meldete eine "harte Regel Max. Hebel 25x", die der
    # Trader nie gesetzt hatte – das war dieser frühere Code-Default.)
    "max_leverage": 0,
    ...
    # Lektions-Qualität (maschinell erzwungen):
    # 1. Keine Lektionen, die pauschale Hebel-Deckel empfehlen – Risiko wird
    #    über Marge/SL gesteuert (Auto-Leverage legt die Liquidation hinter
    #    den SL, ein kleinerer Hebel bringt dann nichts).
    "block_leverage_lessons": True,
    # 2. Keine pauschalen Richtungs-Lektionen ("Shorts meiden") ohne
    #    benannten Marktkontext – Richtungs-Performance ist regimeabhängig.
    "require_direction_context": True,
    # 3. Keine Coin-Ranglisten aus vergangenem PnL ("AVAX priorisieren, SOL
    #    meiden"): reiner Rückspiegel/Zufall, verfälscht die Datensammlung.
    "block_coin_ranking_lessons": True,
}
...
RULE_LABELS = {
    ...
    "block_leverage_lessons": "Keine Hebel-Deckel-Lektionen (Marge/SL statt Hebel)",
    "require_direction_context": "Richtungs-Lektionen nur mit Marktkontext",
    "block_coin_ranking_lessons": "Keine Coin-Ranglisten aus vergangenem PnL",
}

# Erkennt Hebel-Angaben in Lektionen ("Hebel 40x", "leverage 40")
_LEV_RE = re.compile(r"(?:hebel|leverage)\D{0,12}(\d{1,3})", re.IGNORECASE)

# Erkennt Lektionen, die pauschale Hebel-Deckel/-Reduktionen empfehlen
# ("Hebel max 20x", "nicht mehr als 10x Hebel", "Hebel reduzieren/begrenzen").
_LEV_CAP_RE = re.compile(
    r"(?:hebel|leverage)\W{0,3}\w{0,12}\W{0,3}"
    r"(?:max|höchstens|maximal|deckel|begrenz|limitier|reduzier|senk|verringer|drossel|nicht (?:mehr|über|höher))"
    r"|(?:max\.?|maximal|höchstens|nicht mehr als)\s*\d{1,3}\s*x?\s*(?:er)?\W{0,3}(?:hebel|leverage)"
    r"|(?:hebel|leverage)\s*(?:auf|unter|<=?)\s*\d{1,3}",
    re.IGNORECASE)

# Erkennt pauschale Richtungs-Aussagen ("Shorts meiden", "nur Longs", …)
_SIDE_BIAS_RE = re.compile(
    r"\b(?:longs?|shorts?)\b[^.\n]{0,70}\b(?:meiden|vermeiden|verbieten|verboten|"
    r"bevorzug\w*|priorisier\w*|stoppen|pausieren|aussetzen|deaktivier\w*|generell|grundsätzlich)"
    r"|\b(?:keine|nur(?: noch)?)\s+(?:longs?|shorts?)\b",
    re.IGNORECASE)

# Kontext-Marker: Lektion ist an einen benennbaren Marktzustand gebunden
_CONTEXT_TOKENS = (
    "trend", "uptrend", "downtrend", "abwärts", "aufwärts", "seitwärts", "regime",
    "range", "solange", "sofern", "falls", "wenn ", "bei ", "während", "über ",
    "unter ", "oberhalb", "unterhalb", "ema", "sma", "vwap", "level", "widerstand",
    "unterstützung", "support", "resistance", "news", "volatil", "atr", "session",
    "uhrzeit", "uhr", "montag", "dienstag", "mittwoch", "donnerstag", "freitag",
    "samstag", "sonntag", "wochenende", "funding", "dominanz", "korrelation",
    "momentum", "volumen", "orderflow", "sweep", "liquid", "kerze", "timeframe",
    "1m", "5m", "15m", "1h", "4h", "konfidenz",
)


def _has_market_context(text: str) -> bool:
    t = str(text or "").lower()
    return any(tok in t for tok in _CONTEXT_TOKENS)


# Coin-Ranglisten: Rangfolge-Verb + mindestens drei Ticker (z.B. "AVAX, ETH, POL
# priorisieren – ADA, SOL meiden"). Geprüft wird nur der TITEL, damit Belege im
# Detail ("SILVER -11 USDT, OIL -2 USDT") nicht fälschlich anschlagen.
_RANK_VERB_RE = re.compile(
    r"\b(?:priorisier\w*|bevorzug\w*|meiden|vermeiden|ausschließen|sperren|"
    r"favorisier\w*|fokussier\w*|konzentrier\w*|nur noch)\b", re.IGNORECASE)
_TICKER_RE = re.compile(r"\b[A-Z]{2,6}(?:USDT|USD)?\b")
_TICKER_STOP = {"ATR", "SL", "TP", "PNL", "USDT", "USD", "CRV", "EMA", "SMA", "VWAP",
                "RSI", "MACD", "KI", "OK", "CVD", "OI", "FVG", "LONG", "SHORT", "HOLD",
                "NEWS", "TP1", "BE", "ML", "AI", "UTC", "CET", "MIN", "MAX"}
COIN_RANKING_MIN_TICKERS = 3


def _ticker_count(text: str) -> int:
    return len({m for m in _TICKER_RE.findall(str(text or "")) if m not in _TICKER_STOP})


def is_coin_ranking(title: str, detail: str = "") -> bool:
    """Lektion ist eine reine Coin-Rangliste (rein, testbar)."""
    t = str(title or "")
    return bool(_RANK_VERB_RE.search(t)) and _ticker_count(t) >= COIN_RANKING_MIN_TICKERS
...
def normalize_rules(raw: Optional[Dict]) -> Dict:
    ...
    for key in ("block_leverage_lessons", "require_direction_context",
                "block_coin_ranking_lessons"):
        if raw and key in raw:
...
def check_lesson_rules(rules: Dict, title: str, detail: str) -> Tuple[bool, str]:
    ...
    if r.get("block_leverage_lessons") and _LEV_CAP_RE.search(text):
        return False, ("Hebel-Deckel-Lektionen sind nicht erlaubt: das Auto-Leverage-"
                       "System legt die Liquidation hinter den Stop-Loss – Risiko "
                       "über Marge (capital_pct) oder SL-Abstand steuern, nicht über "
                       "einen pauschalen Hebel-Deckel")
    if r.get("require_direction_context") and _SIDE_BIAS_RE.search(text) \
            and not _has_market_context(text):
        return False, ("Pauschale Richtungs-Lektion ohne Marktkontext: Long/Short-"
                       "Performance hängt vom aktuellen Regime ab – Bedingung "
                       "benennen (z.B. 'solange BTC im 4h-Downtrend'), sonst keine "
                       "Richtungs-Regel")
    if r.get("block_coin_ranking_lessons") and is_coin_ranking(title, detail):
        return False, ("Coin-Rangliste aus vergangenem PnL ist keine Lektion: "
                       "'Coin A priorisieren, Coin B meiden' bildet nur Zufall und "
                       "Regime ab und verfälscht die Datensammlung – Asset-Regeln "
                       "brauchen einen Mechanismus (Spread, Session, Volatilität) "
                       "und eine prüfbare Bedingung")
    return True, ""
...
def rules_text(rules: Dict) -> str:
    ...
    if r.get("block_leverage_lessons"):
        parts.append("Lektionen mit pauschalen Hebel-Deckeln: VERBOTEN "
                     "(Risiko über Marge/SL steuern, Auto-Leverage bleibt maximal)")
    if r.get("require_direction_context"):
        parts.append("Richtungs-Lektionen (Long/Short bevorzugen/meiden): nur mit "
                     "benanntem Marktkontext + Gültigkeitsbedingung")
    if r.get("block_coin_ranking_lessons"):
        parts.append("Coin-Ranglisten aus vergangenem PnL (Coin A priorisieren / "
                     "Coin B meiden): VERBOTEN")
...
class MasterPromptStore:
    ...
    async def load(self) -> Dict:
        ...
        if self.lesson_policy.strip() in LEGACY_LESSON_POLICIES:
            # Lektions-Grundregeln sind noch 1:1 eine alte Vorlage (nie vom Trader
            # angepasst) -> auf die aktuelle Fassung heben (Widerspruch Regel 4/9,
            # Coin-Ranglisten, Meta-Lektionen). Eigene Texte bleiben unberührt.
            self.lesson_policy = DEFAULT_LESSON_POLICY
            await self.db.settings.update_one(
                {"_id": DOC_ID},
                {"$set": {"lesson_policy": self.lesson_policy, "updated_at": _now_iso()}})
            logger.info("MasterPrompt: Lektions-Grundregeln auf aktuelle Vorlage gehoben")
        if not doc.get("editor"):
            # Noch nie vom Trader gespeichert -> aktuelle Vorlage übernehmen,
            # damit Verbesserungen an der Standard-Vorlage ankommen. Gilt auch
            # für die Regeln: der alte Code-Default max_leverage=25 stand sonst
            # als unsichtbare "harte Regel" in jedem Prompt (Bug-Report).
            if self.text != DEFAULT_TEXT or self.lesson_policy != DEFAULT_LESSON_POLICY \
                    or self.rules != normalize_rules(DEFAULT_RULES):
                self.text = DEFAULT_TEXT
                self.lesson_policy = DEFAULT_LESSON_POLICY
                self.rules = normalize_rules(DEFAULT_RULES)
                await self.db.settings.update_one(
                    {"_id": DOC_ID},
                    {"$set": {"text": self.text, "lesson_policy": self.lesson_policy,
                              "rules": self.rules, "updated_at": _now_iso()}})
