...
# Mindest-1m-Puffer, sobald der KI-Trader aktiv ist: 60 geschlossene 1h-Kerzen
# (SMC-Zonen order_block/fvg_fill auf 1h, services/smc_zones.py)
AI_TRADER_MIN_BUFFER = 60 * 60
...
DEFAULT_SETTINGS = {
    ...
    # R02: Ownership dynamisch gesetzter coin_params-Keys ({"sid_sym": [keys]})
    "coin_params_dynamic_keys": {},
    # Gesicherte persönliche Basiswerte überdeckter coin_params ({"sid_sym": {k: v}})
    "coin_params_dynamic_prev": {},
    "strategy_timeframes": {},                      # {strategy_id: "5m" | "1h" | ...}
    "deleted_strategies": [],                       # permanently hidden/removed strategy ids
    "strategy_names": {},                           # Umbenennungen von Built-in-Strategien {id: name}
    ...
    # Futures-Gebühren (Haupteinstellungen, in % je Seite). Gelten nur für
    # NEUE Trades – bestehende behalten ihre gespeicherte fee_percent.
    "futures_taker_fee_pct": 0.06,
    "futures_maker_fee_pct": 0.02,
    # Forex-Gebühren (Interactive Brokers) – GETRENNT von den Krypto-Futures-
    # Gebühren: Kommission je Seite in % des Notionals (IBKR IdealPro ~0.2
    # Basispunkte) + Mindestkommission in USD. Gilt für Live, Paper & Backtest
    # auf allen FOREX-Symbolen (services/fee_model.py).
    "forex_commission_pct": 0.002,
    "forex_min_commission_usd": 2.0,
}
...
class StrategyScanner:
    ...
    def buffer_limit(self) -> int:
        ...
        if "ai_trader" in self.enabled_strategies():
            # KI-Trader: SMC-Zonen (order_block/fvg_fill) brauchen >= 30 GESCHLOSSENE
            # 1h-Kerzen (smc_zones) – ohne 1h-Strategie im Set schrumpfte der Puffer
            # auf 220x5m = 18h und die 1h-Order-Blocks verschwanden aus dem Prompt
            # (Befund 09/2026: order_block feuerte nicht mehr).
            limit = max(limit, AI_TRADER_MIN_BUFFER)
        return limit
    ...
    async def save_settings(self, updates: Dict) -> List[str]:
        """update_settings + gezielte Persistenz NUR der übergebenen Top-Level-Keys.

        Ersetzt das frühere Komplett-Überschreiben des scanner_settings-Dokuments
        ($set: scanner.settings): Bei parallelen Schreibern (Optimizer, KI-Labor,
        Dynamic-Live, UI) gingen dadurch Änderungen verloren (Lost-Update).
        Rückgabe: Liste der tatsächlich übernommenen Keys."""
        self.update_settings(updates)
        keys = [k for k in updates if k in DEFAULT_SETTINGS]
        from core import state
        if state.db is not None and keys:
            payload = {k: self.settings.get(k) for k in keys}
            # update_settings kann active_strategy korrigiert haben -> mitschreiben
            payload["active_strategy"] = self.settings.get("active_strategy")
    ...
    def _maybe_signal(self, symbol, strategy, res) -> Optional[Dict]:
        ...
        return {
            ...
            "rules_met": rules_met,
            # Richtungs-bewusst zählen (Parität zu services/fast_sim): bei einem
            # LONG-Signal zählen nur die LONG-Regeln. Vorher wurden Long+Short
            # gemischt gezählt (rules_total = alle Regeln) – Custom-Strategien
            # mit beiden Richtungen wurden dadurch von require_all_rules IMMER
            # blockiert (Live 0 Trades, Backtest voll – Paritäts-Bug).
            "rules_met_count": (res.get("long_count") if res["signal_type"] == "LONG"
                                else res.get("short_count"))
            if res.get("long_count") is not None or res.get("short_count") is not None
            else sum(1 for v in rules_met.values() if v),
            "rules_total": res.get("rules_total") or len(rules_met),
