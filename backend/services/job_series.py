...
KINDS = ("backtest", "optimizer", "regime_analysis", "regime_calibration",
         "regime_kombi", "regime_ablation", "regime_ema_compare", "regime_autopilot")
# Regime-Lab-Werkzeuge: Serien-Kind -> Start-Funktion im Router (gleiche
# Validierung wie beim direkten Start) + lesbarer Name.
REGIME_STARTERS = {"regime_analysis": "start_analysis",
                   "regime_calibration": "start_calibration",
                   "regime_kombi": "start_kombi_calibrate",
                   "regime_ablation": "start_ablation",
                   "regime_ema_compare": "start_ema_compare",
                   "regime_autopilot": "start_autopilot"}
REGIME_LABELS = {"regime_analysis": "Regime-Analyse",
                 "regime_calibration": "Regime-Kalibrierung",
                 "regime_kombi": "Kombi-Auto-Kalibrierung",
                 "regime_ablation": "Indikator-Ablation",
                 "regime_ema_compare": "EMA-Vergleich",
...
def default_label(kind: str, body: Dict) -> str:
    ...
    return (f"{REGIME_LABELS.get(kind, 'Regime-Analyse')} {body.get('name') or ''} · {sym_txt} · "
            f"{body.get('timeframe') or ''} · {days}d").replace("  ", " ")
...
def summarize(kind: str, result: Optional[Dict]) -> Dict:
    ...
    if kind == "optimizer":
        ...
        return out
    if kind == "regime_autopilot":
        best = result.get("best") or {}
        m = best.get("metrics") or {}
        out["holdout_pct"] = m.get("holdout_direction_pct")
        out["inner_pct"] = m.get("inner_direction_pct")
        out["detector"] = best.get("detector")
        out["improvements"] = result.get("improvements")
        out["tested"] = result.get("tested")
        out["improved"] = result.get("improved")
        return out
    if kind == "regime_calibration":
        rep = result.get("report") or result
        out["best_pct"] = (rep.get("best") or {}).get("balanced_direction_pct")
        out["baseline_pct"] = (rep.get("baseline") or {}).get("balanced_direction_pct")
        out["improved"] = rep.get("improved")
        return out
    if kind in ("regime_kombi", "regime_ablation", "regime_ema_compare"):
        best = result.get("best") or {}
        out["holdout_pct"] = best.get("holdout_direction_pct")
        out["best"] = (result.get("best_variant") or result.get("best_period")
                       or (f"thr {best.get('thr')} · {best.get('slope_days')}d"
                           if best.get("thr") is not None else None))
        return out
...
async def add_item(db, kind: str, body: Dict, label: Optional[str] = None) -> Dict:
    if kind not in KINDS:
        raise ValueError("kind muss einer von " + "|".join(KINDS) + " sein")
...
async def _start_job(kind: str, body: Dict) -> str:
    ...
    if kind == "backtest":
        ...
        starter = getattr(rl, REGIME_STARTERS.get(kind, "start_analysis"))
        res = await starter(body, True)
