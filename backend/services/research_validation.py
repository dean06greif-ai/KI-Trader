...
def select_best_row(rows: List[Dict], metric: str = "inner_direction_pct",
                    ...
                    key_field: str = None,
                    chain: List[Tuple[str, str]] = None) -> Tuple[Optional[Dict], str]:
    ...
    `chain` (optional): geordnete Liste (metrik, basis) – die erste Metrik, für
    die es Werte gibt, entscheidet (z.B. Referenz vor Live=Final)."""
    chain = chain or [(metric, "inner_validation"), (fallback, "train_only")]
    for m, basis in chain:
        cand = [r for r in rows if r.get(m) is not None]
        if cand:
            return max(cand, key=lambda r: r[m]), basis
    return None, "none"
