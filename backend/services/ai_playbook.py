...
automatischer Wieder-Freischaltung.
...
Setup eine Statistik (Trades, Winrate, PnL) mit Urteil:
  ...
  schwach  – wird NICHT mehr gesperrt, sondern in die Paper-Datensammlung
             ZURÜCKGESTUFT (kein Live-Einstieg, Paper läuft weiter, um das
             Setup zu verbessern). Wieder live, sobald seit der Rückstufung
             genug gute Paper-Trades vorliegen (setup_lifecycle.promotion_ok)
             – spätestens beim Re-Test nach RETEST_DAYS, wenn die Gesamt-
             statistik die Freischaltung wieder rechtfertigt.

Historie: Bis 06/2026 wurden 'schwache' Setups 14 Tage hart gesperrt
(`disabled`), auch für die Datensammlung. Das blockierte Setups wie
momentum_news/breakout, obwohl sie sich zwischenzeitlich wieder positiv
entwickelt hatten, und verhinderte genau die Daten, die zur Verbesserung
nötig sind. `disabled` bleibt aus Kompatibilität (API/UI) als leeres Feld
erhalten; Alt-Einträge werden beim ersten refresh() in `live_blocked`
migriert.
...
SETUPS: Dict[str, str] = {
    ...
    # Neue Setups 06/2026 (Empfehlungen aus STRATEGIEN_BEWERTUNG.md). Datenbasis:
    # funding_fade -> FUNDING-FADE-RADAR (services/funding_fade.py, Makro-Block),
    # session_open -> SESSION-OPEN-Block (services/session_open.py). Beide laufen
    # wie jedes neue Setup zuerst über das Reife-Gate (Paper-Datensammlung).
    "funding_fade": "Funding-Fade: extrem überhitzte Funding-Rate (siehe FUNDING-FADE-RADAR) GEGEN die Crowd handeln – Einstieg erst nach Bestätigung (tieferes Hoch/höheres Tief bzw. 15m-Strukturbruch), SL hinter dem letzten Extrem, Ziel Mean-Reversion-Zone; konservative Größe.",
    "session_open": "Session-Open: London-/US-Open-Volatilität über die Opening-Range (erste 15 min, siehe SESSION-OPEN-Block) – Vol-Regime hoch: Range-BREAKOUT (Einstieg Ausbruch/Retest, SL Range-Mitte), Vol-Regime niedrig: FADE des Fehlausbruchs zurück in die Range (SL hinter dem Fakeout-Extrem, Ziel Gegenseite).",
    "divergence": "Divergenz: RSI/Preis-Divergenz am Extrempunkt (höheres Hoch bei fallendem RSI bzw. tieferes Tief bei steigendem RSI), Einstieg nach Bestätigungskerze gegen den erschöpften Move, SL hinter dem Extrem, enges Mean-Reversion-Ziel.",
}
...
# Rückgestufte Setups (Paper-Datensammlung statt Live): schwaches Gesamt-Urteil
# ODER Paper/Live-Divergenz (Setup läuft im Paper ok, live schwach)
_live_blocked_cache: Dict[str, Dict] = {}
# Reife-Status je Setup aus dem letzten refresh(): setup -> (live_ok, Grund)
...
def live_block_reason(setup: Optional[str]) -> Optional[str]:
    ...
    refresh(). Der Trade läuft dann als Paper-Datensammlung weiter."""
    if not setup:
        return None
    d = _live_blocked_cache.get(setup)
    if d:
        return (f"Setup '{setup}' rückgestuft ({d.get('reason', '')}) – läuft als "
...
async def refresh(db) -> Dict:
    """Statistik neu berechnen, schwache Setups zurückstufen (Paper-Datensammlung),
    Wieder-Freischaltungen prüfen.

    Läuft einmal pro Analyse-Zyklus (über context_text) und hält den
    Rückstufungs-Cache für das Live-Gate (ai_engine._setup_live_gate) aktuell."""
    ...
    library = all_setups()
    # ---- Lebenszyklus: Rückstufung & Wieder-Freischaltung (setup_lifecycle) ----
    live_blocked: Dict[str, Dict] = dict(doc.get("live_blocked") or {})
    # Migration: harte Alt-Sperren -> Rückstufung (keine Sperren mehr)
    disabled, live_blocked, migrated = migrate_disabled(
        dict(doc.get("disabled") or {}), live_blocked, _now_iso())
    if migrated:
        changed = True
        logger.info("Playbook: Alt-Sperren in Rückstufungen migriert – "
                    f"{', '.join(k for k in live_blocked)}")
    ...
    # (die guten Paper-Trades, die zur Freischaltung führten, inklusive).
    eval_since: Dict[str, str] = dict(doc.get("eval_since") or {})
    lifted: set = set()
    ...
    for sid in list(live_blocked.keys()):
        ...
        if ok_paper or ok_retest:
            ...
            logger.info(f"Playbook: Rückstufung '{sid}' aufgehoben ({how})")
            try:
                await db.ai_chat.insert_one({
                    "id": str(uuid.uuid4()), "role": "playbook", "setup": sid,
            ...
            # KI-eigenes Setup bleibt auch nach dem Re-Test-Datum schwach ->
            # ausmustern (Platz für die nächste Idee, Historie in custom_retired)
            live_blocked.pop(sid)
            changed = True
            await retire_custom_setup(
                db, sid, f"nach Re-Test weiterhin schwach: "
    ...
    try:
        ...
        live_stats = {}
    # Rückstufung: Gesamt-Urteil 'schwach' ODER Paper/Live-Divergenz. Kein
    # Setup wird gesperrt – die Paper-Datensammlung läuft weiter, damit sich
    # das Setup verbessern und wieder freischalten kann. Beurteilt wird nur
    # die Statistik SEIT der letzten Freischaltung (sonst würde ein gerade
    # wieder freigeschaltetes Setup wegen alter Verluste sofort erneut
    # zurückgestuft – Endlosschleife).
    judge_stats: Dict[str, Dict] = dict(stats)
    for sid, since_iso in eval_since.items():
        if sid not in library or sid in live_blocked:
            continue
        try:
            part = await setup_stats(db, since=since_iso, setup=sid)
            judge_stats[sid] = part.get(sid) or {"trades": 0, "wins": 0, "pnl": 0.0,
                                                 "margin": 0.0, "verdict": "test"}
        except Exception as e:  # noqa: BLE001
            logger.debug(f"Playbook: Statistik seit Rückstufung '{sid}': {e}")
    for sid, why in demotion_candidates(judge_stats, live_stats, library).items():
        ...
        live_blocked[sid] = {"at": _now_iso(), "reason": why,
                             ...
                             "paper_since": {"trades": 0}}
        changed = True
        logger.warning(f"Playbook: Setup '{sid}' rückgestuft in die Paper-Datensammlung ({why})")
        try:
            await db.ai_chat.insert_one({
                "id": str(uuid.uuid4()), "role": "playbook", "setup": sid,
                "text": (f"Setup '{sid}' läuft schwach ({why}) – Live-Einstiege pausiert, "
                         f"zurückgestuft in die Paper-Datensammlung (keine Sperre: Paper-"
                         f"Trades laufen weiter, um das Setup zu verbessern). Wieder live nach "
    ...
    # Nach einer Rückstufung zählt die Statistik seit der Rückstufung
    # (judge_stats), sonst die Gesamtstatistik – so kann ein wieder frei-
    # geschaltetes Setup trotz alter 30-Tage-Verluste tatsächlich live gehen.
    ...
    _disabled_cache = disabled  # immer {} – keine harten Sperren mehr
    ...
    return {"stats": stats, "disabled": disabled, "live_ready": ready_now,
            ...
            "judge_stats": judge_stats, "lifecycle": lc_state,
            "custom": dict(_custom_cache)}


def live_ready_for(setup: Optional[str], stats: Optional[Dict]) -> Tuple[bool, str]:
    """Live-Gate-Entscheidung für den Analyse-Loop: bevorzugt den von refresh()
    berechneten Reife-Status (berücksichtigt Rückstufungen und die Statistik
    seit der Rückstufung), Fallback: live_ready(stats)."""
...
async def context_text(db) -> str:
    ...
    if stats:
        for sid, st in sorted(stats.items(), key=lambda x: -x[1]["pnl"]):
            ...
            if sid in disabled:
                ...
                mark += (f" – RÜCKGESTUFT ({live_blocked[sid].get('reason', 'schwach')}): "
                         f"kein Live, nur Paper-Datensammlung; wieder live nach "
                         f"{lifecycle.MIN_TRADES_PROMOTE} guten Paper-Trades "
    ...
    if live_blocked:
        lines.append("RÜCKGESTUFTE SETUPS (nur Paper-Datensammlung, KEINE Sperre – nutze sie "
                     "weiter bewusst für Paper-Trades, damit sie sich verbessern können): "
                     + ", ".join(f"{s} (Re-Test ab {str(d.get('retest_at', ''))[:10]})"
                                 for s, d in live_blocked.items()))
    lines.append("REGELN: Wähle das Setup passend zur Marktphase. Bevorzuge BEWÄHRTE "
                 "Setups. Setups ohne Daten bewusst klein antesten "
                 ...
                 "statt immer dasselbe Muster. REGIME-FILTER: breakout, squeeze_breakout und "
                 "momentum_news NUR im Trend-/High-Vol-Regime handeln – in Chop-/Seitwärts-"
                 "phasen sind das die typischen Verlierer, nutze dort range_fade/mean_reversion/"
                 "htf_range. funding_fade NUR bei einer Meldung im FUNDING-FADE-RADAR, "
                 "session_open NUR im aktiven SESSION-OPEN-Fenster. MULTI-TIMEFRAME: Zone/Struktur auf 15m/1h "
                 ...
                 "LEBENSZYKLUS: Live-Setups werden weiter gemessen; läuft ein Setup "
                 f"nach ≥{lifecycle.DEMOTE_MIN_LIVE_TRADES} Trades schwach, geht es "
                 "zurück in die Paper-Datensammlung (keine Sperre) und wird dort weiter "
                 "optimiert, bis es sich erneut qualifiziert.")
    return "\n".join(lines)
...
def maturity_overview(stats: Dict[str, Dict], disabled: Dict[str, Dict],
                      ...
                      judge_stats: Optional[Dict[str, Dict]] = None) -> List[Dict]:
