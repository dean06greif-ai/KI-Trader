...
SETUPS: Dict[str, str] = {
    ...
    "vwap_reclaim": ("VWAP-Reclaim (5m, 'VWAP(Tag)'-Zeile): frischer RECLAIM/VERLUST des Tages-VWAP mit Volumen, "
                     "Entry nahe VWAP, nie gegen 1h-Trend, SL hinter Swing/VWAP, 2R."),
    ...
    "fomc_event": ("FOMC-Event (nur im Event-Fenster um den Zinsentscheid): Whipsaw-FADE des "
                   "Erst-Spikes zurück in die Pre-Range ODER DRIFT-Folge nach der Pressekonferenz; "
                   "mehrere Trades im Fenster erlaubt, SL an Spike-Extrem/Range-Mitte."),
    # CPI-/NFP-Events (services/econ_event.py): gleiches Muster wie fomc_event
    # für die US-Datenveröffentlichungen (BLS, 8:30 ET).
    "cpi_event": ("CPI-Event (nur im Event-Fenster um die US-Inflationsdaten): Whipsaw-FADE des "
                  "Erst-Spikes zurück in die Pre-Range ODER DRIFT-Folge nach den Zahlen; "
                  "mehrere Trades im Fenster erlaubt, SL an Spike-Extrem/Range-Mitte."),
    "nfp_event": ("NFP-Event (nur im Event-Fenster um den US-Arbeitsmarktbericht): gleiche "
                  "Regeln wie CPI – Whipsaw-FADE oder DRIFT nach den Zahlen, SL an "
                  "Spike-Extrem/Range-Mitte."),
    "ppi_event": ("PPI-Event (nur im Event-Fenster um die US-Erzeugerpreise): gleiche Regeln "
                  "wie CPI – Whipsaw-FADE oder DRIFT nach den Zahlen, SL an "
                  "Spike-Extrem/Range-Mitte."),
    "pce_event": ("PCE-Event (nur im Event-Fenster um den Core-PCE-Preisindex, Fed-Lieblingsmaß): "
                  "gleiche Regeln wie CPI – Whipsaw-FADE oder DRIFT nach den Zahlen, SL an "
                  "Spike-Extrem/Range-Mitte."),
}
...
_ALIASES = (
    ...
    ("fomc", "fomc_event"),
    ("cpi", "cpi_event"), ("nfp", "nfp_event"),
    ("ppi", "ppi_event"), ("pce", "pce_event"),
    ("erzeugerpreis", "ppi_event"), ("inflation", "cpi_event"),
    ("payroll", "nfp_event"), ("arbeitsmarkt", "nfp_event"),
...
def setup_enum() -> str:
    """Setup-Enum fürs JSON-Schema des Prompts inkl. aktiver KI-eigener Setups (rein)."""
    extra = [sid for sid in _custom_cache if sid not in SETUPS]
    return "|".join(list(SETUPS.keys()) + extra)


def with_dynamic_setup_enum(prompt: str) -> str:
    """Ersetzt das statische SETUP_ENUM im Systemprompt durch das aktuelle (rein)."""
    return prompt.replace(SETUP_ENUM, setup_enum()) if _custom_cache else prompt
...
def revision_allowed(asset_class: str, sid: str, scope: Dict, library: Dict,
    ...
    if lifecycle.is_event_setup(sid):
        # Event-Setups: nur nach verpasstem Event (kein Trade im Fenster) oder
        # echter Rückstufung wegen schwacher Ergebnisse – nie "auf Verdacht".
        ev_inactive = bool(((scope.get("inactive") or {}).get(sid) or {}).get("event"))
        lb = (scope.get("live_blocked") or {}).get(sid)
        weak = lb is not None and not lifecycle.revision_demotion(lb)
        if not (ev_inactive or weak):
            return False, ("Event-Setup: Überarbeitung nur, wenn ein Event ohne Trade blieb "
                           "oder die Event-Trades schwach liefen")
...
def class_live_stats(asset_class: str, setup: Optional[str] = None) -> Optional[Dict]:
    """NUR-Live-Statistik der Klasse aus dem letzten refresh() (rein, Audit 2.5)
    – Basis für den Live-Vorrang der Setup-Gewichtung (setup_weighting)."""
    st = (_class_cache.get(asset_class) or {}).get("live_stats") or {}
    return st.get(setup) if setup else st
...
def revision_versions() -> Dict[str, Dict[str, int]]:
    """{Anlageklasse: {setup: Revisions-Version}} aus dem Klassen-Cache –
    Playbook-Stand für den Policy-Fingerprint (Audit 3.1)."""
    out: Dict[str, Dict[str, int]] = {}
    for cls, cc in _class_cache.items():
        vers = {sid: int((r or {}).get("version") or 0)
                for sid, r in ((cc or {}).get("revisions") or {}).items()}
        if vers:
            out[cls] = vers
    return out
...
def _world_fields(prefix: str, st: Optional[Dict]) -> Dict:
    """n / Winrate / PnL einer Teil-Welt für die Setup-Reife-Tabelle (rein)."""
    st = st or {}
    n = int(st.get("trades") or 0)
    return {f"{prefix}_trades": n,
            f"{prefix}_winrate": round(int(st.get("wins") or 0) / n * 100) if n else 0,
            f"{prefix}_pnl": round(float(st.get("pnl") or 0), 2)}


def _stats_row(r: Dict) -> Dict:
    n, wins = int(r["trades"]), int(r["wins"])
    ...
    return {"trades": n, "wins": wins,
            ...
            "risk_usdt": round(float(r.get("risk_usdt") or 0), 2),
            "verdict": verdict_for(n, wins, float(r.get("pnl") or 0)),
            "wr_ci": [lo, hi], "uncertain": lifecycle.is_uncertain(n)}
...
_GROUP_FIELDS = {"trades": {"$sum": 1},
                 ...
                 # Audit 2.6: Summe riskiertes Kapital (risk_usdt, seit 2.3 am
                 # Trade) – Basis für den Erwartungswert je riskiertem USDT
                 "risk_usdt": {"$sum": {"$ifNull": ["$risk_usdt", 0]}}}


def _group_stage(split_world: bool) -> Dict:
    """$group je Setup – optional zusätzlich nach Welt (Paper-Logik vs. Sammlung)."""
    if not split_world:
        return {"$group": {"_id": "$setup", **_GROUP_FIELDS}}
    return {"$group": {"_id": {"setup": "$setup",
                               "dc": {"$eq": [{"$ifNull": ["$data_collection", False]}, True]}},
                       **_GROUP_FIELDS}}


def _rows_to_stats(rows: List[Dict], split_world: bool) -> Dict[str, Dict]:
    """Aggregat-Zeilen -> {setup: stats}. Mit split_world bleibt die Summe
    (Paper + Sammlung) unverändert, dazu kommen Teil-Statistiken
    'paper' (Paper-Logik, ohne Sammel-Trades) und 'collection' (nur Sammel-Trades)."""
    if not split_world:
        return {str(r["_id"]): _stats_row(r) for r in rows}
    acc: Dict[str, Dict] = {}
    for r in rows:
        rid = r.get("_id")
        sid = str(rid.get("setup") if isinstance(rid, dict) else rid)
        a = acc.setdefault(sid, {"parts": {}, "sum": {k: 0 for k in _GROUP_FIELDS}})
        if isinstance(rid, dict):
            a["parts"]["collection" if rid.get("dc") else "paper"] = _stats_row(r)
        for k in _GROUP_FIELDS:
            a["sum"][k] += r.get(k) or 0
    return {sid: {**_stats_row(a["sum"]), **a["parts"]} for sid, a in acc.items()}
...
async def setup_stats(db, days: int = LOOKBACK_DAYS, live_only: bool = False,
                      ...
                      split_world: bool = False) -> Dict[str, Dict]:
    ...
    rows = await db.auto_trades.aggregate([
        ...
        _group_stage(split_world),
    ]).to_list(100)
    return _rows_to_stats(rows, split_world)
...
async def _refresh_scope(db, scope: Dict, library: Dict[str, str], cls: str) -> Tuple[Dict, Dict]:
    ...
    try:
        collect_stats = await setup_stats(db, paper_only=True, asset_class=cls, split_world=True)
    except Exception as e:  # noqa: BLE001
    ...
    # Setups, deren Revisions-Rückstufung per Backtest-Edge aufgehoben wurde:
    # für sie darf das Backtest-Seeding auch nach eval_since weiter zählen
    # (bis zu einer erneuten Rückstufung wegen schwacher Ergebnisse).
    bt_validated: Dict[str, str] = {k: v for k, v in dict(scope.get("bt_validated") or {}).items()
                                    if k in library}
    ...
    for sid in list(live_blocked.keys()):
        ...
        # Revisions-Rückstufung ('Validierung neu gestartet'): der bestätigte
        # Backtest-Edge der Klasse zählt gewichtet mit (wie beim ersten Schritt
        # Sammeln -> Live) – echte Paper-Trades seit Revision bleiben Pflicht
        # (bt_weights.MIN_REAL_TRADES, profitabel). Schwache Setups: unverändert.
        ok_bt, why_bt = False, ""
        if not (ok_paper or ok_retest) and lifecycle.revision_demotion(live_blocked[sid]):
            merged = bt_weights.merge_stats(paper_since, bt_stats.get(sid))
            if merged:
                ok_bt, why_bt = lifecycle.promotion_ok(merged)
                if ok_bt:
                    why_bt = (f"backtest-validiert: {why_bt} (davon {merged['backtest_weighted']:g} "
                              f"gewichtet aus {merged['backtest_trades']} Backtest-Trades, "
                              f"{merged['real_trades']} echte Paper-Trades seit Revision)")
        live_blocked[sid]["paper_since"] = paper_since or {"trades": 0}
        if ok_paper or ok_retest or ok_bt:
            ...
            changed = True
            if ok_bt:
                bt_validated[sid] = _now_iso()
            how = (f"seit Rückstufung {why_paper}" if ok_paper
                   else "Re-Test-Datum erreicht" if ok_retest else why_bt)
    ...
    for sid, why in demotion_candidates(judge_stats, live_stats, library, per_symbol).items():
        ...
        live_blocked[sid] = {"at": _now_iso(), "reason": why,
                             ...
                             "paper_since": {"trades": 0}}
        bt_validated.pop(sid, None)   # schwach -> wieder echte Paper-Trades nötig
        changed = True
    ...
    for sid in library:
        ...
        if lifecycle.is_event_setup(sid):
            # Event-Setups: nur "Event fand statt, aber kein Trade" zählt
            try:
                times = await lifecycle.event_trade_times(db, sid, since_iso, ac.symbols_of(cls))
            except Exception as e:  # noqa: BLE001
                logger.debug(f"Event-Trades '{sid}' nicht geladen: {e}")
                continue
            why_inactive = lifecycle.event_inactivity_reason(sid, since_iso, times)
        else:
    ...
    try:
        ...
        variant_collect = await setup_stats_since_map(db, variant_since, paper_only=True,
                                                      asset_class=cls, split_world=True)
    except Exception as e:  # noqa: BLE001
    ...
    for sid in library:
        ...
        if not ok and sid not in live_blocked and sid in bt_stats \
                and (sid not in eval_since or sid in bt_validated):
            # Backtest-Beschleunigung für den ersten Schritt Sammeln -> Live
            # sowie nach einer per Backtest aufgehobenen Revisions-Rückstufung
...
async def refresh(db, force: bool = False) -> Dict:
    ...
    collect_stats = await setup_stats(db, paper_only=True, split_world=True)
    ready_now: Dict[str, bool] = {}
    ready_why: Dict[str, str] = {}
...
def live_ready_for(setup: Optional[str], stats: Optional[Dict],
    ...
    if setup and asset_class:
        ...
        # FOMC-Bonus: Backtest-Validierung + Trader-Opt-in ersetzen die Pflicht-
        # Paper-Trades (nur solange nicht rückgestuft – live_block_reason wird im
        # Live-Gate VOR diesem Aufruf geprüft und gewinnt immer).
        if setup == "fomc_event":
            from services import fomc_event
            ov = fomc_event.live_override(asset_class)
            if ov:
                return ov
        # CPI-/NFP-/PPI-/PCE-Bonus: gleiches Muster (Backtest-Validierung + Opt-in)
        elif setup in ("cpi_event", "nfp_event", "ppi_event", "pce_event"):
            from services import econ_event
            ov = econ_event.live_override_for_setup(setup, asset_class)
            if ov:
                return ov
...
async def context_text(db, classes: Optional[List[str]] = None) -> str:
    ...
    lines.append("GEWICHTUNG: Setups sind Werkzeuge, keine Fesseln – deine EIGENE Analyse "
                 "(inkl. Tiefenanalyse-Outlook) entscheidet. Die gelernte Erfolgsbilanz des "
                 "gewählten Setups justiert deine Konfidenz automatisch (bewährt hebt, schwach "
                 "senkt, max. ±10 Punkte). Passt kein Setup wirklich, erzwinge keines – schlage "
                 "per 'new_setups' ein passendes vor.")
    for c in classes:
        lim = ac.LIMITS[c]
    ...
    for c in classes:
        ...
        wl = setup_weighting.context_line(ac.LABELS[c], cd["stats"])
        if wl:
            lines.append(wl)
        try:
...
def maturity_overview(stats: Dict[str, Dict], disabled: Dict[str, Dict],
    ...
    for sid in library:
        ...
        # Globale Tabelle hat keine Varianten-Sammelstatistik -> Gesamtstatistik
        # (vorher leer "—" für alle Setups mit aktiver Variante)
        cs = ((variant_collect_stats.get(sid) or {}) if has_variant and variant_collect_stats
              else (collect_stats.get(sid) or {}))
        ...
        out.append({
            ...
            "wr_ci": list(lifecycle.wilson_interval(wins, trades)),
            "uncertain": lifecycle.is_uncertain(trades),
            "pnl": round(float((st or {}).get("pnl") or 0), 2),
            "live_trades": int(ls.get("trades") or 0),
            "live_wr_ci": list(lifecycle.wilson_interval(int(ls.get("wins") or 0),
                                                         int(ls.get("trades") or 0))),
            ...
            # Aufteilung der Nicht-Live-Welt: Paper-Logik (echte Live-Logik,
            # nur simuliert) vs. Datensammlung (Sammel-Trades, feste Marge)
            **_world_fields("paper", cs.get("paper")),
            **_world_fields("coll", cs.get("collection")),
