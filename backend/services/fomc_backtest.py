...
def simulate_event(candles: List[Dict], t0: int, p: Optional[Dict] = None) -> List[Dict]:
...
# ---------------------------------------------------------------------------
async def _event_candles(symbol: str, t0_ms: int) -> List[Dict]:
    start = t0_ms - int(FIXED["pre_range_hours"] * 3600 * 1000) - 600_000
    end = t0_ms + (FIXED["timeout_drift_min"] + 30) * 60_000
    return await fetch_klines_range(symbol, "5m", start_ms=start, end_ms=end, limit=200)


async def run(db, years: float = 2.0, symbols: Optional[List[str]] = None) -> Dict:
    global _running
    if _running:
    ...
    try:
        ...
        for dt in decisions:
            ...
            ev_trades = []
            for sym in symbols:
                candles = await _event_candles(sym, t0 * 1000)
                if len(candles) < 20:
                    continue
                for t in simulate_event(candles, t0):
        ...
        result = {
            ...
            "params_fixed": FIXED, "aggregate": agg, "per_event": per_event,
...
async def last_result(db) -> Optional[Dict]:
