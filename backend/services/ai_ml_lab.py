...
class MLLab:
    ...
    async def reset(self) -> Dict:
        """ML-Trainingsdaten zurücksetzen: Modell + Status wandern in den
        ...
        try:
            ...
            if doc:
                doc.pop("_id", None)
                await lab_trash.put(
                    self.db, "ml", {"model": doc},
                    label=f"ML-Modell (trainiert {str(doc.get('trained_at') or '?')[:16]})")
        ...
        self._last_train_ts = 0.0
        self._last_labeled = 0
        logger.info("ML-Labor zurückgesetzt (Modell & Status in den Papierkorb)")
        return {"status": "ok"}

    async def restore(self, payload: Dict) -> Dict:
        """Papierkorb-Eintrag wiederherstellen: Modell-Doc zurückschreiben + laden."""
        doc = dict(payload.get("model") or {})
        if not doc:
            return {"status": "error", "detail": "Papierkorb-Eintrag ohne Modell"}
        await self.db.settings.replace_one({"_id": "ai_ml_model"}, doc, upsert=True)
        self._restore_model(doc)
        self.last_error = None
        logger.info("ML-Modell aus dem Papierkorb wiederhergestellt")
        return {"status": "ok"}
