...
async def fit_risk_budget(db, mode: str, symbol: str, new_risk_usdt: float,
                          equity: Optional[float]) -> Tuple[Optional[float], Optional[float]]:
    """Rest-Budget-Trading: Faktor (<1), um den die Marge verkleinert werden
    muss, damit das Risiko ins Budget passt (services/risk_budget.py).
    None = nichts anpassen; fail-open bei internen Fehlern."""
    try:
        from services import risk_budget
        return await risk_budget.scale_for_new_trade(db, mode, symbol, new_risk_usdt, equity)
    except Exception as e:  # noqa: BLE001
        logger.warning(f"{symbol}: Risikobudget-Anpassung fehlgeschlagen (unverändert): {e}")
        return None, None
