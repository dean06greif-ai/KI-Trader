...
DEFAULT_SETTINGS = {
    ...
    # Audit 2.8: eine frühere Bestätigung zählt nur, wenn seit der zuletzt
    # gezählten Bestätigung mind. so viele NEUE geschlossene Trades vorlagen
    # (nicht überlappende Evidenz-Fenster – Vorschlags-Spam zählt nicht).
    "macro_evidence_min_trades": 3,
}
...
def real_confirmations(prior_ts, trade_ts, min_new_trades: int = 3):
    """Echte Bestätigungen (rein & testbar, Audit 2.8).

    Frühere Vorschläge derselben Richtung zählen nur als Bestätigung, wenn
    zwischen ihnen NEUE Evidenz lag: seit der zuletzt gezählten Bestätigung
    müssen mind. `min_new_trades` geschlossene Trades vorliegen. Auch der
    aktuelle Vorschlag zählt nur mit frischer Evidenz seit der letzten
    Bestätigung – mehrfaches Vorschlagen ohne neue Trades erhöht nichts.
    Rückgabe: (Bestätigungen, ts der letzten gezählten Bestätigung | None)."""
    need = max(1, int(min_new_trades))
    trades = sorted(str(t) for t in (trade_ts or []) if t)
    counted, last = 0, None
    for ts in sorted(str(t) for t in (prior_ts or []) if t):
        fresh = sum(1 for t in trades if (last is None or t > last) and t <= ts)
        if last is None or fresh >= need:
            counted += 1
            last = ts
    fresh_now = sum(1 for t in trades if last is None or t > last)
    if last is None or fresh_now >= need:
        counted += 1
    return max(1, counted), last
