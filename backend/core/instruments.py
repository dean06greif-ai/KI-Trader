...
# Bitunix-Kontrakte (Metalle/Öl/Indizes): Historie ab Listing, wächst täglich.
# Der Deckel entspricht dem Backtester-Maximum (365 Tage) – nicht dem Angebot.
BITUNIX_HIST_CAP_DAYS = 365
...
class Instrument:
    ...
    # IBKR-Paar (z.B. "EUR.USD") – Forex wird live über Interactive Brokers
    # gehandelt (services/ibkr_client.py); Krypto/Rohstoffe/Indizes über Bitunix.
    ibkr: Optional[str] = None

    @property
    def tradable(self) -> bool:
        """Echte Live-Orders nur mit vorhandenem Bitunix-Kontrakt.
        (IBKR-Forex hat einen eigenen Live-Pfad – siehe broker/live_tradable.)"""
        return bool(self.bitunix)

    @property
    def broker(self) -> Optional[str]:
        """Zuständiger Live-Broker: 'bitunix' | 'ibkr' | None."""
        if self.bitunix:
            return "bitunix"
        if self.ibkr:
            return "ibkr"
    ...
    def live_tradable(self) -> bool:
        """Live handelbar über irgendeinen Broker (Bitunix ODER IBKR)."""
        return bool(self.bitunix or self.ibkr)
...
def _resource(symbol: str, name: str, yahoo: str, bitunix: str, days: int) -> Instrument:
    ...
    `days` = OBERGRENZE der angeforderten Historie, nicht das tatsächliche
    Angebot: Bitunix liefert Kerzen ab Kontrakt-Listing, d.h. die verfügbare
    Historie WÄCHST täglich (Stand 09/2026: XAU ~217 d, XAG ~249 d, CL ~176 d).
    Die alten festen Werte (110/150/180) haben den Backtester dauerhaft auf
    weniger Tage begrenzt, als die Quelle längst hatte – die Quelle beendet den
    Download selbst, wenn keine älteren Kerzen mehr kommen."""
...
def _index(symbol: str, name: str, days: int) -> Instrument:
    """US-Indizes/Index-ETFs: bei Bitunix als USDT-Perp gelistet (Historie ab
    Listing, wächst täglich – siehe _resource)."""
...
def _forex(symbol: str, name: str) -> Instrument:
    ...
    LIVE-Trading läuft über Interactive Brokers (ibkr-Paar, services/ibkr_trade.py);
    ohne konfiguriertes IBKR-Gateway fällt Live automatisch auf Paper zurück."""
...
INSTRUMENTS: List[Instrument] = [
    ...
    Instrument(symbol="HYPEUSDT", name="Hyperliquid", group=GROUP_CRYPTO,
               ...
               hist_source="bitunix", hist_ref="HYPEUSDT", max_hist_days=365),
    # ---- Rohstoffe ---- (Bitunix-Historie ab Listing, wächst täglich; Deckel = Backtester-Maximum)
    _resource("GOLD", "Gold", "GC=F", "XAUUSDT", BITUNIX_HIST_CAP_DAYS),
    _resource("SILVER", "Silver", "SI=F", "XAGUSDT", BITUNIX_HIST_CAP_DAYS),
    _resource("OIL", "Oil (WTI)", "CL=F", "CLUSDT", BITUNIX_HIST_CAP_DAYS),
    # ---- Indizes ----
    _index("QQQUSDT", "Nasdaq 100 (QQQ)", BITUNIX_HIST_CAP_DAYS),
    _index("SPYUSDT", "S&P 500 (SPY)", BITUNIX_HIST_CAP_DAYS),
    # ---- Forex ----
...
def kline_available(symbol: str) -> bool:
    """Der Trade-Chart nutzt die Bitunix-Kline-API: ohne Bitunix-Kontrakt
    (Forex) gibt es keine Kerzen. Unbekannte Symbole (extern adoptierte
    Bitunix-Kontrakte) gelten als verfügbar."""
    inst = get(symbol)
    return True if inst is None else bool(inst.bitunix)
...
def groups() -> List[Dict]:
    ...
    for name in GROUP_ORDER:
        ...
        out.append({
            "name": name,
            "symbols": [{"symbol": i.symbol, "name": i.name,
                         "tradable": i.live_tradable, "broker": i.broker,
