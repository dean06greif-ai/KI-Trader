...
# Bandbreite: zuletzt per WebSocket gesendeter Stand je Symbol – unveränderte
# Kerzen/Regel-Zustände (Markt zu, keine Bewegung) werden nicht erneut gesendet.
_ws_last_candle: dict = {}
_ws_last_rules: dict = {}


def _rules_sig(states) -> str:
    try:
        import json
        return json.dumps(states, sort_keys=True, default=str)
    except (TypeError, ValueError):
        return str(time.time())  # nicht serialisierbar -> immer senden
...
# Feed-Wächter für Märkte mit Handelspausen (Forex/Rohstoffe/Indizes via Yahoo):
# hängt der Feed bei OFFENEM Markt länger als das, wird gewarnt – am Wochenende
# bzw. in der täglichen Handelspause ist eine alte Kerze dagegen normal.
PAUSED_MARKET_LAG_WARN_MS = 15 * 60 * 1000


def feed_lag_warning(symbol: str, forming_ts: int, now_ms: int) -> Optional[str]:
    """Warntext, wenn ein Markt mit Handelspausen bei geöffnetem Markt keine
    frischen Kerzen mehr liefert (Datenfeed prüfen). None = alles in Ordnung
    oder Markt geschlossen (rein & testbar)."""
    inst = get_instrument(symbol)
    if inst is None or not inst.has_trading_pauses:
        return None
    age_ms = now_ms - int(forming_ts)
    if age_ms <= PAUSED_MARKET_LAG_WARN_MS:
        return None
    now_dt = datetime.fromtimestamp(now_ms / 1000, tz=timezone.utc)
    closed, _why = market_hours.is_market_closed(symbol, now_dt)
    if closed:
        return None
    return (f"{symbol}: Marktdaten veraltet ({age_ms / 60000:.0f} min) trotz geöffnetem "
            f"Markt – Datenfeed ({inst.live_source}) prüfen")
    ...
    while scanner_running.is_set():
        ...
        for symbol in ALL_SYMBOLS:
            ...
            try:
                ...
                fresh = not _is_stale(symbol, forming["timestamp"], now_ms)
                if fresh:
                    prices[symbol] = price
                    lag = feed_lag_warning(symbol, forming["timestamp"], now_ms)
                    if lag and time.time() - _stale_warned.get(symbol, 0) > 600:
                        _stale_warned[symbol] = time.time()
                        logger.warning(lag)
                else:
                    now = time.time()
                ...
                candle_sig = (forming.get("timestamp"), forming.get("close"),
                              forming.get("high"), forming.get("low"), forming.get("volume"))
                if _ws_last_candle.get(symbol) != candle_sig:
                    _ws_last_candle[symbol] = candle_sig
                    await broadcast({"type": "candle", "symbol": symbol, "data": forming})
                states = scanner.rule_states.get(symbol)
                if states:
                    rules_sig = _rules_sig(states)
                    if _ws_last_rules.get(symbol) != rules_sig:
                        _ws_last_rules[symbol] = rules_sig
                        await broadcast({"type": "rule_states", "symbol": symbol, "data": states})
            except Exception as e:
                logger.error(f"Scan error for {symbol}: {e}")
            await asyncio.sleep(0.1)
        # Preview-Guard: SL/TP-Überwachung und Live-Limit-Spiegelung laufen nur
        # auf der Prod-Instanz – zwei Instanzen auf derselben DB/Börse würden
        # Trades doppelt schließen (core/config.local_engine_disabled).
        ...
        # KI-Limit-Orders (Key-Level-Entries): Fills und Abläufe gegen die
        # frischen Preise dieses Ticks prüfen (services/key_level_limits.py).
        try:
            from services import key_level_limits
            await key_level_limits.check_fills(state.db, prices)
        except Exception as e:
            logger.error(f"ki limit order check error: {e}")
        # Live-Spiegelung der KI-Limit-Orders an die Börse (echte Limit-Orders,
        # smartes Armieren/Parken bei Kapital-Knappheit, Fill-Verbuchung).
        try:
            from services import limit_live_sync
            await limit_live_sync.sync(state.db, autotrader, prices)
        except Exception as e:
            logger.error(f"limit live sync error: {e}")
        await asyncio.sleep(POLL_INTERVAL)
...
async def perform_daily_reset(prev_date: str):
    ...
    try:
        # Datensammel-Signale/-Trades (data_collection) zählen NICHT in die
        # Tages-Statistik – sie würden Winrate/PnL verfälschen.
        pipeline = [
            {"$match": {"trade_date": prev_date, "data_collection": {"$ne": True}}},
