...
async def lifespan(app: FastAPI):
    ...
    # Adaptive Bausteine: Aktivitäts-Wächter (Schwellen-Regelkreis) +
    # Bewegungs-Scanner (starke Moves -> Ursache/Lücken-Analyse)
    from services.activity_guard import activity_guard
    from services.ai_move_scanner import move_scanner
    activity_guard.setup(ai_engine)
    move_scanner.setup(ai_engine)
    # Setup-Trigger: Backtest-Detektoren live (LLM-freie Paper-Sammel-Trades)
    from services.setup_trigger import setup_trigger
    setup_trigger.setup(ai_engine)
    ...
    # DB-Retention: täglicher Sweep gegen die Atlas-512-MB-Quota (services/retention.py)
    from services import retention
    asyncio.create_task(retention.run_loop(app.mongodb))
    # Tägliches Backup der kritischen Collections nach Supabase Storage (services/backup.py)
    from services import backup
    asyncio.create_task(backup.run_loop(app.mongodb))
    ...
    # Lektions-Bilanz (PLAN_LEKTIONS_BILANZ): HOLD-Gegenprobe + Bilanz-Cache
    from services.lesson_counterfactual import counterfactual
    from services.lesson_impact import lesson_impact
    counterfactual.setup(app.mongodb)
    lesson_impact.setup(app.mongodb)
    if ai_engine.config.get("lesson_counterfactual_enabled", True):
        asyncio.create_task(counterfactual.run_loop())
    # Regime-Brücke (PLAN_REGIME_BRUECKE): Struktur-Resolver (Cache-Refresh, Karenz-Vollzug)
...
# Bandbreite (Render-Quota): JSON-Antworten gzip-komprimieren (~70-85 % kleiner).
# Browser senden Accept-Encoding automatisch; Clients ohne Header bekommen
# weiterhin unkomprimierte Antworten.
