...
async def list_calibrations(limit: int = 20):
    """Verlauf der wissenschaftlichen Kalibrierungen (Cloud + lokaler Worker):
    kompakt, ohne per_symbol-Rohdaten – zum Nachschlagen und erneuten Übernehmen."""
    rows = await state.db.regime_calibrations.find(
        {}, {"_id": 0, "report.per_symbol": 0}) \
        .sort("created_at", -1).to_list(max(1, min(int(limit), 100)))
...
async def list_releases():
    ...
    act_ok, act_reasons, act_info = regime_release.validate_activation(rewards)
...
async def release_check(aid: str, stage: str = "shadow", scope: str = None, symbol: str = None):
    ...
    if stage == "active":
        ...
        if rel.get("stage") not in ("shadow", "active"):
            reasons.append("erst Stufe Shadow (Beobachten) freigeben")
        a_ok, a_reasons, _ = regime_release.validate_activation(
            await ai_rewards.by_structural_regime(state.db, 90))
        reasons += a_reasons
        ok = ok and a_ok and rel.get("stage") in ("shadow", "active")
