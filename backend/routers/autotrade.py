...
async def ibkr_status():
    """Status der Interactive-Brokers-Anbindung (Forex-Live-Trading) fürs
    Master-Panel: Gateway konfiguriert? Session authentifiziert? Konto-ID?"""
    from services import ibkr_trade
    return await ibkr_trade.status()
...
@router.get("/api/autotrade/coin/{symbol}/meta")
async def get_coin_meta(symbol: str):
    """Kontrakt-Metadaten des Coins (u.a. Max-Hebel laut Bitunix-Katalog)."""
    return {"symbol": symbol, "max_leverage": trade_client.max_leverage_for(symbol)}
...
async def get_trade_chart(trade_id: str):
    ...
    if not kline_available(t.get("symbol") or ""):
        # Forex & Co.: Bitunix listet keinen Kontrakt -> Grund explizit melden
        # (das Frontend blendet den Chart-Button zusätzlich aus).
        return {"symbol": t["symbol"], "interval": None, "candles": [],
                "chart_available": False,
                "reason": f"Für {t['symbol']} listet Bitunix keinen Kontrakt "
                          f"(z.B. Forex) – keine Kerzendaten verfügbar.",
                "trade": trade_payload}
...
async def get_slippage_stats(days: int = 30):
    ...
    days = int(days)
    days = 30 if days == 0 else max(1, min(days, 365))
    ...
    # Klarnamen statt kryptischer IDs (Bug-Report: Custom-Strategien erschienen
    # als "custom_24456534"): Registry (Built-ins + geladene Customs) und alle
    # Strategie-Collections abfragen.
    names = {"ai_trader": "KI Trader", "external": "Manuell (Bitunix)"}
    ...
    for meta in strategy_registry.list_all():
        if meta.get("id") and meta.get("name"):
            names.setdefault(meta["id"], meta["name"])
    for coll in ("strategies", "custom_strategies", "dynamic_strategies"):
        try:
            async for s in state.db[coll].find({}, {"id": 1, "name": 1}):
                if s.get("id") and s.get("name"):
                    names.setdefault(s["id"], s["name"])
        except Exception:
            pass
    # Letzter Fallback: Name direkt vom Trade – nur für noch unaufgelöste IDs
    unresolved = [g.get("strategy_id") for g in groups
                  if g.get("strategy_id") and g.get("strategy_id") not in names]
    trade_names = {}
    if unresolved:
        trade_names = {t.get("strategy_id"): t.get("strategy_name")
                       for t in await state.db.auto_trades.find(
                           {"strategy_id": {"$in": unresolved},
                            "strategy_name": {"$ne": None}},
                           {"_id": 0, "strategy_id": 1, "strategy_name": 1}
                       ).sort("opened_at", -1).to_list(200)}
    for g in groups:
        sid = g.get("strategy_id")
        g["strategy_name"] = names.get(sid) or trade_names.get(sid) or sid
    return {"days": days, "measured_trades": len(trades), "groups": groups}
...
async def manual_trade_action(trade_id: str, body: Dict, _: bool = Depends(require_admin)):
    ...
    if res.get("status") in ("blocked", "rejected"):
        raise HTTPException(status_code=409, detail=res.get("detail") or "Aktion blockiert")
    if res.get("status") == "error":
        raise HTTPException(status_code=400, detail=res.get("detail") or "Aktion fehlgeschlagen")
    return {"status": "success", "result": res}
...
async def get_watchdog_status():
    ...
    doc.pop("_id", None)
    # Feste Feld-Garantie: Zähler auch liefern, wenn der Watchdog noch nie lief
    # (z.B. Bitunix nicht konfiguriert) – stabiler API-Vertrag fürs Frontend.
    defaults = {"positions": 0, "adopted": 0, "sl_fixed": 0, "sl_missing": 0,
                "emergency_closed": 0, "dust_closed": 0, "state_synced": 0,
                "adopt_deferred": 0, "deduped": 0,
                "errors": [], "last_run_at": None}
    from services import entry_inflight
    return {"configured": trade_client.configured(),
            "settings": dict(watchdog.settings), **defaults, **doc,
            "inflight_entries": entry_inflight.active()}
...
async def run_watchdog(_: bool = Depends(require_admin)):
    ...
    res = {"status": "success", "manage": manage,
           "result": await watchdog.check(manage=manage)}
    if not manage:
        res["reason"] = "Watchdog ausgeschaltet – nur Sichtbarkeits-Sync gelaufen"
    return res
...
async def set_watchdog_config(body: Dict, _: bool = Depends(require_admin)):
    ...
    max_sl_retries, emergency_close, adopt_unknown, adopt_grace_sec)."""
    from services.position_watchdog import watchdog
    return {"status": "success", "settings": await watchdog.update_settings(body)}


@router.get("/api/autotrade/pnl-reconcile/status")
async def get_pnl_reconcile_status():
    """Letzter Lauf des PnL-Abgleichs (echter Bitunix-PnL nach dem Close)."""
    doc = await state.db.settings.find_one({"_id": "pnl_reconcile_status"}) or {}
    doc.pop("_id", None)
    return {"configured": trade_client.configured(), **doc}


@router.post("/api/autotrade/pnl-reconcile/run")
async def run_pnl_reconcile(_: bool = Depends(require_admin)):
    """Manueller PnL-Abgleich: geschlossene Live-Trades der letzten 48 h mit der
    Bitunix-Positions-Historie abgleichen (realisierter PnL inkl. Fees/Funding)."""
    from services import pnl_reconcile
    if not trade_client.configured():
        return {"status": "skipped", "detail": "Keine Bitunix-API-Keys konfiguriert"}
    return {"status": "success", **await pnl_reconcile.reconcile_pending(autotrader)}
...
async def get_capital_allocation(_: bool = Depends(require_admin)):
    ...
    for scope in ("live", "paper"):
        ...
        fc = await autotrader.free_capital(
            scope, total=total if scope == "live" else None)
        out[scope] = {
            **a,
            "allocated": round(fc["allocated"], 2) if fc["allocated"] is not None else None,
            "used_margin": round(fc["used"], 2),
            "free": round(fc["free"], 2) if fc["free"] is not None else None,
        }
        if fc.get("exchange_available") is not None:
            out[scope]["exchange_available"] = round(fc["exchange_available"], 2)
...
async def get_balance(_: bool = Depends(require_admin)):
    ...
    # Paper-Kennzahlen zählen zusätzlich nur Strategie×Asset-Kombinationen,
    # die derzeit freigeschaltet sind (services/active_scope).
    active_keys = await active_scope.load_active_keys(state.db, autotrader)

    def _counts(rows, paper: bool = False):
        rows = [t for t in rows
                if not is_stale_strategy(t.get("strategy_id") or "external")]
        return active_scope.filter_active(rows, active_keys) if paper else rows

    proj = {"_id": 0, "strategy_id": 1, "symbol": 1, "manual_trade": 1,
            "external_adopted": 1, "realized_pnl": 1}
    base_q = {"mode": mode, "data_collection": {"$ne": True}}
    open_rows = await state.db.auto_trades.find(
        {**base_q, "status": "open"}, proj).to_list(500)
    closed = _counts(await state.db.auto_trades.find(
        {**base_q, "status": "closed"}, proj).to_list(5000), paper=(mode == "paper"))
    open_ct = len(_counts(open_rows, paper=(mode == "paper")))
    ...
    # ---- IBKR (Forex-Live-Konto) fürs Header-Badge (Doppelklick-Umschalter) ----
    try:
        from services import ibkr_trade
        result["ibkr"] = await ibkr_trade.account_summary()
    except Exception as e:
        result["ibkr"] = {"configured": False, "error": str(e)[:120]}
    ...
    if mode == "live":
        try:
            ...
            # nur derzeit freigeschaltete Strategie×Asset-Kombinationen zählen.
            pq = {"mode": "paper", "data_collection": {"$ne": True}}
            paper_open = len(_counts(await state.db.auto_trades.find(
                {**pq, "status": "open"}, proj).to_list(500), paper=True))
            paper_closed = _counts(await state.db.auto_trades.find(
                {**pq, "status": "closed"}, proj).to_list(5000), paper=True)
...
# ---- Regime-Gate: aktuelle Marktphase eines Coins (Anzeige im Setup) -------
@router.get("/api/autotrade/regime_phase/{symbol}")
