...
async def get_klines_history(symbol: str, days: int = 7, timeframe: str = None):
    ...
    is_crypto = inst is None or not inst.yahoo_ref
...
async def update_settings(settings: Dict, _: bool = Depends(require_admin)):
    applied = await scanner.save_settings(settings)
    ignored = [k for k in settings if k not in applied]
    if ignored:
        logger.warning(f"POST /api/settings: unbekannte Keys ignoriert: {ignored}")
    return {"status": "success", "settings": scanner.settings, "ignored_keys": ignored}
...
async def system_ram():
    ...
    from services import ram_guard, ram_queue
    return {
        ...
        # Container-Limit (cgroup, auf Render das echte Plan-Limit) + erkanntes
        # Profil: 'big' = 2-GB-Limits aktiv (Kerzen-Cache, Orderflow, ML)
        "container": {**ram_guard.profile(), "free_mb": round(ram_queue.free_mb())},
        "candle_cache": {
