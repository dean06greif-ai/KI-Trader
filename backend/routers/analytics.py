...
from core.state import scanner, open_signal_evals, autotrader
from core.pipeline import broadcast
from core.utils import _clean, _enrich_trade
from services import trade_trash, active_scope, setup_variant
...
@router.get("/api/analytics/policy-report")
async def policy_report(days: int = 90):
    """Netto-Erfolgsbericht je Policy-Version (Audit 3.4): PnL netto je Version,
    Live/Paper/Sammel getrennt; Betriebskosten (Token/Analyse-Zyklen) SEPARAT."""
    ...
    trades = await state.db.auto_trades.find(
        ...
        {"_id": 0, "policy_version": 1, "realized_pnl": 1, "fees_paid": 1,
         "slippage_usdt": 1, "risk_usdt": 1, "risk": 1, "qty": 1, "result": 1,
         "mode": 1, "data_collection": 1, "opened_at": 1}).limit(3000).to_list(3000)
    rows = setup_variant.policy_report_rows(trades)
    # Entscheidungs-Aufwand je Policy (wie oft hat diese Policy analysiert?)
    try:
        dec = await state.db.ai_decisions.aggregate([
            {"$match": {"ts": {"$gte": cutoff}}},
            {"$group": {"_id": "$policy_version.combined", "n": {"$sum": 1}}},
        ]).to_list(200)
        dec_map = {str(d.get("_id") or ""): int(d.get("n") or 0) for d in dec}
    except Exception as e:  # noqa: BLE001
        logger.warning(f"policy-report: Decision-Aggregation fehlgeschlagen: {e}")
        dec_map = {}
    for r in rows:
        r["decisions"] = dec_map.get(r["combined"] or "", 0)
    # Betriebskosten separat: Token-Schätzung + Zyklen aus dem Analyse-Feed
    try:
        ops = await state.db.ai_chat.aggregate([
            {"$match": {"role": "analysis", "ts": {"$gte": cutoff}}},
            {"$group": {"_id": None, "cycles": {"$sum": 1},
                        "tokens": {"$sum": {"$ifNull": ["$token_estimate", 0]}}}},
        ]).to_list(1)
        ops = ops[0] if ops else {}
    except Exception as e:  # noqa: BLE001
        logger.warning(f"policy-report: Ops-Aggregation fehlgeschlagen: {e}")
        ops = {}
    return {"days": days, "rows": rows,
            "ops": {"analysis_cycles": int(ops.get("cycles") or 0),
                    "token_estimate_total": int(ops.get("tokens") or 0)}}
...
async def reaggregate_daily_stats() -> Dict[str, int]:
    ...
    for doc in daily_docs:
        ...
        if not date:
            continue
        pipeline = [
            {"$match": {"trade_date": date, "data_collection": {"$ne": True}}},
    ...
    for doc in tstat_docs:
        ...
        tstats = await state.db.auto_trades.aggregate([
            {"$match": {"trade_date": date, "status": "closed", "data_collection": {"$ne": True}}},
...
# ---------------- Strategie-Vergleich ----------------
def is_stale_strategy(sid: str, registry=None) -> bool:
    """'Karteileiche': Strategie-ID, die es nicht mehr gibt (gelöschte Custom-
    Strategie, umbenannte/alte IDs) – rein & testbar. KI-Trader und manuelle
    Pseudo-Strategie zählen nie als Karteileiche."""
    if sid in ("ai_trader", "manual", "external"):
        return False
    reg = registry if registry is not None else strategy_registry
    return reg.get(sid) is None
...
async def strategy_comparison(mode: str = "all", days: int = 0,
    ...
    eröffnete Trades) sind KEINE Strategie und werden standardmäßig NICHT
    mitverglichen (include_manual=true zeigt sie als Pseudo-Strategie).
    Datensammel-Trades des KI-Traders (data_collection) sind KEINE Paper-
    Performance und 'Karteileichen' (Trades gelöschter/nicht mehr existierender
    Strategien) keine vergleichbare Strategie – beide bleiben IMMER ausgeblendet.
    Die Parameter include_collection/include_stale werden nur noch aus
    Rückwärtskompatibilität akzeptiert und ignoriert.
    only_active (Standard an): nur Trades von Strategie×Asset-Kombinationen,
    die derzeit freigeschaltet sind (Trade-Modus paper/live) – Trades
    deaktivierter Assets werden nicht eingerechnet (`inactive_hidden`)."""
    ...
    active_keys = await active_scope.load_active_keys(state.db, autotrader) \
        if only_active else None
    inactive_hidden = 0
    if active_keys is not None:
        kept = active_scope.filter_active(trades, active_keys)
        inactive_hidden = len(trades) - len(kept)
        trades = kept
    ...
    async for t in state.db.auto_trades.find(
            {"status": "open"},
            {"_id": 0, "strategy_id": 1, "symbol": 1, "manual_trade": 1, "external_adopted": 1}):
        sid = MANUAL_ID if _is_manual(t) else (t.get("strategy_id") or "unknown")
        if sid == "unknown" or (sid == MANUAL_ID and not include_manual):
            continue  # Trades ohne Strategie-Zuordnung nicht als Pseudo-Strategie zeigen
        if active_keys is not None and sid != MANUAL_ID \
                and not active_scope.is_active(active_keys, sid, t.get("symbol")):
            continue
    ...
    for t in trades:
        ...
        if manual and not include_manual:
            continue  # Manuelle Trades sind keine Strategie (User-Vorgabe)
        sid = MANUAL_ID if manual else (t.get("strategy_id") or "unknown")
        if sid == "unknown":
            continue  # 'unknown' aus dem Vergleich ausblenden (Alt-/Import-Daten)
        if not include_stale and is_stale_strategy(sid):
            stale_ids.add(sid)
            continue  # Karteileiche: Strategie existiert nicht mehr
    ...
    if include_manual and open_counts.get(MANUAL_ID) and MANUAL_ID not in by_strat:
        by_strat[MANUAL_ID] = {
    ...
    for sid, e in by_strat.items():
        ...
        e["is_stale"] = is_stale_strategy(sid)
        out.append(e)
    out.sort(key=lambda x: -x["pnl"])
    counted = sum(e["trades"] for e in out)
    return {"mode": mode, "days": days, "comparison": out,
            "total_trades": counted, "loaded_trades": len(trades),
            "include_collection": include_collection, "include_stale": include_stale,
            "only_active": bool(only_active), "inactive_hidden": inactive_hidden,
            "stale_strategies": sorted(stale_ids)}
