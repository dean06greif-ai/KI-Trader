...
async def start_backtest(body: Dict, _: bool = Depends(require_admin)):
    ...
    if seed_runner.running_job():
        raise HTTPException(status_code=409, detail="Es läuft bereits ein KI-Trader Backtest-Seeding")
...
async def backtest_status(job_id: str):
    ...
    if not job and state.db is not None:
        # Nach Server-Neustart: fertigen Lauf aus der DB liefern bzw.
        # lokal laufenden Job wiederherstellen (statt 404 -> UI-Flackern).
        doc = await state.db.backtests.find_one({"id": job_id})
        if doc:
...
@router.post("/api/backtest/pause/{job_id}")
async def backtest_pause(job_id: str, _: bool = Depends(require_admin)):
    """Backtest anhalten (z.B. Rechner abkühlen lassen) – Fortsetzen jederzeit."""
...
@router.post("/api/backtest/resume/{job_id}")
async def backtest_resume(job_id: str, _: bool = Depends(require_admin)):
    from services import job_control
    job = bt.JOBS.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job nicht gefunden")
...
async def backtest_active():
    ...
    if not running and state.db is not None:
        from services import local_exec
        if await local_exec.restore_running_jobs(state.db, "backtest"):
            running = [j for j in bt.JOBS.values() if j["status"] == "running"]
    if running:
