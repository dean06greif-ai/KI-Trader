...
async def optimizer_status(job_id: str):
    job = opt.JOBS.get(job_id)
    if not job and state.db is not None:
        # Nach einem Server-Neustart (Render OOM/Deploy) ist die In-Memory-
        # Job-Liste leer. Statt 404 (UI-Balken flackerte weg): fertigen Lauf
        # aus der DB liefern bzw. lokal laufenden Job wiederherstellen.
        doc = await state.db.optimizer_runs.find_one({"id": job_id})
        if doc:
            return {"id": job_id, "status": "done", "progress": 100,
                    "phase": "Fertig", "result": doc.get("result"),
                    "params": doc.get("params"),
                    "created_at": doc.get("created_at"), "error": None}
        from services import local_exec
        if await local_exec.restore_job(state.db, job_id):
            job = opt.JOBS.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job nicht gefunden")
...
@router.post("/api/optimizer/pause/{job_id}")
async def optimizer_pause(job_id: str, _: bool = Depends(require_admin)):
    """Optimierung/Endlos-Suche anhalten – Zwischenstand bleibt im Speicher
    (lokaler Worker: überlebt auch Server-Neustarts), Fortsetzen jederzeit."""
...
@router.post("/api/optimizer/resume/{job_id}")
async def optimizer_resume(job_id: str, _: bool = Depends(require_admin)):
    from services import job_control
    job = opt.JOBS.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job nicht gefunden")
...
async def optimizer_active():
    ...
    running = [j for j in opt.JOBS.values() if j["status"] == "running"]
    if not running and state.db is not None:
        # Nach Server-Neustart: lokal laufende Optimierungen wiederherstellen
        from services import local_exec
...
async def _exclude_symbols(sid: str, symbols, reasons: Dict) -> list:
    """Ausreißer-Assets (services/optimizer_outliers.py): Strategie für diese
    Coins auf AUS stellen (strategy_coin_configs.mode=off, höchste Priorität)."""
    done = []
    now_iso = datetime.now(timezone.utc).isoformat()
    for sym in [x for x in (symbols or []) if x in ALL_SYMBOLS]:
        key = f"{sid}_{sym}"
        doc = await state.db.strategy_coin_configs.find_one({"_id": key})
        cfg_sc = dict((doc or {}).get("config", {}))
        cfg_sc.update({"mode": "off", "optimizer_excluded": now_iso,
                       "optimizer_excluded_reason": str((reasons or {}).get(sym) or "Ausreißer-Asset")[:200]})
...
async def optimizer_apply(body: Dict, _: bool = Depends(require_admin)):
    ...
    excl = [x for x in (body.get("exclude_symbols") or []) if isinstance(x, str)]
    excl_reasons = body.get("exclude_reasons") or {}
    if apply_type == "params":
        sid = body.get("strategy_id")
        if not sid or not strategy_registry.get(sid):
            raise HTTPException(status_code=400, detail="Gültige strategy_id erforderlich")
        excluded = await _exclude_symbols(sid, excl, excl_reasons) if excl else []
        ...
        if scope == "coins":
            symbols = [s for s in (body.get("symbols") or []) if s in ALL_SYMBOLS and s not in excl]
            if not symbols:
                raise HTTPException(status_code=400,
        ...
        updates: Dict = {"strategy_params": sp}
