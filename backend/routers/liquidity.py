...
from services import sr_zones as srz
...
@router.get("/sr-zones/{symbol}")
async def sr_zones(symbol: str):
    """4h/1d-Haupt-S/R-Zonen fürs Chart-Overlay (Pivot-/Cluster-Analyse, 5min-Cache)."""
    try:
        out = await srz.get_sr_zones(symbol.upper())
    except Exception as e:  # noqa: BLE001
        logger.error(f"sr zones failed {symbol}: {e}")
        raise HTTPException(status_code=502, detail=str(e)[:200])
    if not out.get("zones"):
        raise HTTPException(status_code=502, detail="Keine Kerzendaten für S/R-Zonen verfügbar")
    return out
