...
async def _copy_strategy_settings(strategy_id: str, new_id: str, definition_tf: str) -> None:
    """Parameter, Timeframe, Zeitfenster, Backtest-Config und (deaktivierten)
    Trade-Override einer Strategie auf eine neue ID übertragen."""
    s = scanner.settings
    updates: Dict = {}
    ...
    tfs[new_id] = tfs.get(strategy_id) or definition_tf or "1m"
    updates["strategy_timeframes"] = tfs
...
async def duplicate_strategy(strategy_id: str, body: Dict = None, _: bool = Depends(require_admin)):
    ...
    Timeframe, Zeitfenster und Backtest-Einstellungen).
    Custom/Discovery -> Kopie der Regel-Definition (weiter editierbar).
    Built-in -> Variante: gleicher Code, eigene ID/Name/Parameter (strategy_variants)."""
    body = body or {}
    strat = strategy_registry.get(strategy_id)
    if not strat:
        raise HTTPException(status_code=404, detail="Strategie nicht gefunden")
    if strategy_id == "ai_trader":
        raise HTTPException(status_code=400, detail="Der KI Trader kann nicht dupliziert werden")
    ...
    return {"status": "success", "id": new_id, "name": new_name, "definition": definition,
            "is_custom": bool(getattr(strat, "IS_CUSTOM", False)),
            "is_variant": not getattr(strat, "IS_CUSTOM", False)}


@router.post("/api/strategies/{strategy_id}/rename")
async def rename_strategy(strategy_id: str, body: Dict, request: Request,
                          _: bool = Depends(require_admin)):
    """Strategie umbenennen – Custom, Variante UND Built-in. Built-in-Namen
    werden als Override in scanner.settings.strategy_names gespeichert und beim
    Boot wieder angewendet (Code bleibt unverändert)."""
    ...
    if not strat:
        raise HTTPException(status_code=404, detail="Strategie nicht gefunden")
    old = getattr(strat, "STRATEGY_NAME", strategy_id)
    strategy_registry.set_name(strategy_id, name)
    name = strat.STRATEGY_NAME
    if getattr(strat, "IS_CUSTOM", False):
        await state.db.custom_strategies.update_one({"id": strategy_id}, {"$set": {"name": name}})
        try:
            await state.db.ai_strategy_candidates.update_one(
                {"custom_strategy_id": strategy_id}, {"$set": {"name": name}})
        except Exception:
            pass
    elif getattr(strat, "IS_VARIANT", False):
        await state.db.strategy_variants.update_one({"id": strategy_id}, {"$set": {"name": name}})
    else:
        names = dict(scanner.settings.get("strategy_names", {}))
        names[strategy_id] = name
        await scanner.save_settings({"strategy_names": names})
    await log_action(request, "strategy_rename", {"strategy_id": strategy_id,
                                                  "from": old, "to": name})
    return {"status": "success", "id": strategy_id, "name": name, "previous": old}
...
async def delete_strategy(strategy_id: str, request: Request,
    ...
    is_variant = strategy_registry.is_variant(strategy_id)
    await log_action(request, "strategy_delete", {"strategy_id": strategy_id,
                                                  "custom": is_custom, "variant": is_variant})
    if is_custom:
        await state.db.custom_strategies.delete_one({"id": strategy_id})
        strategy_registry.remove_custom(strategy_id)
        await _unlink_candidate(strategy_id)
    elif is_variant:
        await state.db.strategy_variants.delete_one({"id": strategy_id})
        strategy_registry.remove_variant(strategy_id)
    else:
        deleted = list(scanner.settings.get("deleted_strategies", []))
        if strategy_id not in deleted:
            deleted.append(strategy_id)
        await scanner.save_settings({"deleted_strategies": deleted})
    enabled = [s for s in scanner.settings.get("enabled_strategies", []) if s != strategy_id]
    await scanner.save_settings({"enabled_strategies": enabled})
...
async def restore_default_strategies(_: bool = Depends(require_admin)):
    ...
    await scanner.save_settings({"deleted_strategies": []})
...
async def export_strategy(strategy_id: str):
    ...
    return {
        ...
        "is_custom": strategy_id in strategy_registry._custom_ids,
        "is_variant": strategy_registry.is_variant(strategy_id),
        "variant": dict(getattr(strat, "variant_definition", None) or {}) or None,
...
async def import_strategy(body: Dict, _: bool = Depends(require_admin)):
    ...
    variant = body.get("variant") if isinstance(body.get("variant"), dict) else None
    ...
    if is_custom and isinstance(definition, dict):
        ...
        await state.db.custom_strategies.update_one({"id": sid}, {"$set": definition}, upsert=True)
        strategy_registry.upsert_custom(definition)
    elif variant and variant.get("base_id"):
        variant = {**variant, "kind": "variant", "id": sid,
                   "name": body.get("name") or variant.get("name"),
                   "timeframe": effective_tf or variant.get("timeframe")}
        if strategy_registry.upsert_variant(variant) is None:
            raise HTTPException(status_code=404,
                                detail="Basis-Strategie der Variante existiert in dieser Version nicht")
        await state.db.strategy_variants.update_one({"id": sid}, {"$set": variant}, upsert=True)
    ...
    # Umbenannte Built-ins: Name aus dem Backup übernehmen
    if not is_custom and not variant and body.get("name") \
            and body["name"] != getattr(strategy_registry.get(sid), "STRATEGY_NAME", None):
        strategy_registry.set_name(sid, body["name"])
        names = dict(scanner.settings.get("strategy_names", {}))
        names[sid] = strategy_registry.get(sid).STRATEGY_NAME
        updates["strategy_names"] = names
