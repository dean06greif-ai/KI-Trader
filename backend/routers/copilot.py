...
# ---- Hintergrund-Chat (Fix: Render/Ingress kappt HTTP-Requests nach ~60s,
# wodurch langsame Modelle wie DeepSeek Flash mitten in der Antwort abbrachen).
# Der Chat läuft jetzt als Server-Task, das Frontend pollt das Ergebnis –
# Modelle dürfen sich bis ~110s Zeit nehmen, ohne dass die Antwort verloren geht.
_chat_jobs: Dict[str, Dict] = {}
_CHAT_JOB_TTL = 900  # 15 min


def _cleanup_chat_jobs():
    import time as _t
    cutoff = _t.time() - _CHAT_JOB_TTL
    for jid in [j for j, v in _chat_jobs.items() if v.get("ts", 0) < cutoff]:
        _chat_jobs.pop(jid, None)


async def _run_chat_job(job_id: str, message: str, ctx: Dict):
    try:
        result = await copilot.chat(message, ctx, deadline=110.0, per_call=45.0)
        _chat_jobs[job_id].update({"status": "done", "result": result})
    except Exception as e:
        logger.warning(f"Copilot-Chat-Job fehlgeschlagen: {e}")
        _chat_jobs[job_id].update({"status": "error", "error": str(e)[:250]})


@router.post("/api/copilot/chat/start")
async def copilot_chat_start(body: Dict, _: bool = Depends(require_admin)):
    import asyncio
    import time as _t
    message = (body.get("message") or "").strip()
    if not message:
        raise HTTPException(status_code=400, detail="message erforderlich")
    _cleanup_chat_jobs()
    if any(v.get("status") == "running" for v in _chat_jobs.values()):
        raise HTTPException(status_code=409,
                            detail="Der Copilot arbeitet noch an einer Antwort – bitte kurz warten")
    job_id = str(uuid.uuid4())
    _chat_jobs[job_id] = {"status": "running", "ts": _t.time()}
    asyncio.create_task(_run_chat_job(job_id, message, body.get("context") or {}))
    return {"status": "started", "job_id": job_id}


@router.get("/api/copilot/chat/job/{job_id}")
async def copilot_chat_job(job_id: str):
    job = _chat_jobs.get(job_id)
    if not job:
        raise HTTPException(status_code=404, detail="Job nicht gefunden (evtl. abgelaufen)")
