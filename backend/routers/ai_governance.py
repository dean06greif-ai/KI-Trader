...
async def lessons_impact(days: int = 90):
    ...
    Trades mit/ohne, verhinderte Trades (HOLD-Gegenprobe), Urteil + Vorschlag.
    Reine Information; ändert keine Lektion. 10-min-Cache."""
    from services.lesson_impact import lesson_impact
    from services.lesson_counterfactual import counterfactual
    data = await lesson_impact.report(days)
    return {**data,
            "flags": {k: bool(ai_engine.config.get(k)) for k in
                      ("lesson_attribution_enabled", "lesson_counterfactual_enabled",
                       "lesson_impact_in_prompt")},
            "counterfactual": {"last_run": counterfactual.last_run,
                               "last_error": counterfactual.last_error}}


@router.post("/api/ai/lessons/impact/run")
async def lessons_impact_run(_: bool = Depends(require_admin)):
    """Gegenprobe sofort ausführen + Bilanz-Cache verwerfen (Admin)."""
    from services.lesson_impact import lesson_impact
    from services.lesson_counterfactual import counterfactual
    res = await counterfactual.run_pending()
    lesson_impact.invalidate()
    return {"status": "ok", "counterfactual": res}
...
@router.post("/api/ai/lessons/reevaluate")
async def reevaluate_lessons(body: Optional[Dict] = None,
                             _: bool = Depends(require_admin)):
    """Lern-Reset: alle Lektionen gegen die AKTUELLE Strategie neu bewerten.

    Für den Trader-Knopf nach Strategie-Änderungen – veraltete Lektionen
    (aus altem Kontext) werden entfernt/angepasst, LOCKED-Lektionen bleiben."""
    if not ai_engine.learning:
        raise HTTPException(status_code=503, detail="Lern-Modul nicht initialisiert")
    reason = str((body or {}).get("reason") or "Strategie-Änderung (Trader-Knopf)")
    return await ai_engine.learning.reevaluate_lessons(reason=reason)
...
@router.post("/api/ai/strategies/develop")
async def strategy_develop(body: Dict = None, _: bool = Depends(require_admin)):
    """Trainingslager: Der KI-Trader entwickelt selbst ein neues Setup aus den
    Vergleichsdaten der anderen Strategien, eigenen Lektionen und Labor-Ergebnissen."""
    res = await strategy_lab.develop(focus=str((body or {}).get("focus") or ""))
