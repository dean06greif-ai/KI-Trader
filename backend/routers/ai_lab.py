...
@router.get("/api/policy-lab/status")
async def policy_lab_status():
    """Aktiver Kandidat + Trial-Statistik + Labor-Einstellungen."""
    return await policy_lab.status()


@router.get("/api/policy-lab/trials")
async def policy_lab_trials(limit: int = 50):
    """Jüngste Shadow-Trials (alle Kandidaten, neueste zuerst)."""
    return {"trials": await policy_lab.recent_trials(limit)}


@router.post("/api/policy-lab/candidate")
async def policy_lab_create(spec: Dict, _: bool = Depends(require_admin)):
    """Kandidaten-Policy anlegen (name + system_suffix und/oder config_overrides)."""
    res = await policy_lab.create_candidate(spec, source="trader")
    if res.get("status") == "rejected":
        raise HTTPException(status_code=400, detail=res.get("reason"))
    return res


@router.post("/api/policy-lab/candidate/discard")
async def policy_lab_discard(body: Optional[Dict] = None, _: bool = Depends(require_admin)):
    """Aktiven Kandidaten verwerfen (Trials bleiben zur Nachanalyse erhalten)."""
    res = await policy_lab.discard_candidate(str((body or {}).get("reason") or ""))
    if res.get("status") == "rejected":
        raise HTTPException(status_code=400, detail=res.get("reason"))
    return res


@router.post("/api/policy-lab/config")
async def policy_lab_config(updates: Dict, _: bool = Depends(require_admin)):
    """Labor-Einstellungen (shadow_every_k, trial_timeout_min, max_open_trials, fee_pct,
    promotion={min_trials, champ_min_trades, bootstrap_alpha, dd_tolerance, ...})."""
    return {"settings": await policy_lab.update_settings(updates)}


@router.get("/api/policy-lab/promotion-report")
async def policy_lab_promotion_report():
    """Aktuelle Promotion-Prüfung (Audit 3.3): Kandidat vs. Champion in R-Multiples."""
    return await policy_lab.promotion_report()


@router.post("/api/policy-lab/promote")
async def policy_lab_promote(body: Optional[Dict] = None, _: bool = Depends(require_admin)):
    """Kandidat wird Champion (Overrides -> Engine-Config, Suffix -> MasterPrompt);
    Rollback-Fenster wird automatisch überwacht."""
    res = await policy_lab.promote(force=bool((body or {}).get("force")))
    if res.get("status") == "rejected":
        raise HTTPException(status_code=400, detail=res.get("reason"))
    return res


@router.post("/api/policy-lab/rollback")
async def policy_lab_rollback(body: Optional[Dict] = None, _: bool = Depends(require_admin)):
    """Promotion manuell zurücknehmen (stellt Config + MasterPrompt wieder her)."""
    res = await policy_lab.rollback(str((body or {}).get("reason") or "manuell"))
    if res.get("status") == "rejected":
        raise HTTPException(status_code=400, detail=res.get("reason"))
    return res
...
async def ml_reset(_: bool = Depends(require_admin)):
    """ML-Trainingsdaten (gespeichertes Modell + Status) in den Papierkorb."""
    return await ml_lab.reset()


# ---------------- Papierkorb (Forschung + ML) ----------------
@router.get("/api/ai/lab/trash")
async def lab_trash_list():
    """Papierkorb-Einträge (ohne Payload) – wiederherstellbare Resets."""
    from services import lab_trash
    return {"items": await lab_trash.list_items(ai_engine.db)}


@router.post("/api/ai/lab/trash/restore")
async def lab_trash_restore(body: Dict, _: bool = Depends(require_admin)):
    """Papierkorb-Eintrag wiederherstellen (kind: ml -> Modell, research -> Report)."""
    from services import lab_trash
    doc = await lab_trash.pop(ai_engine.db, str(body.get("id") or ""))
    if not doc:
        raise HTTPException(status_code=404, detail="Papierkorb-Eintrag nicht gefunden")
    payload = doc.get("payload") or {}
    res = (await ml_lab.restore(payload) if doc.get("kind") == "ml"
           else await research_analyst.restore(payload))
    if res.get("status") != "ok":
        # Eintrag zurücklegen, damit nichts verloren geht
        await ai_engine.db.lab_trash.insert_one(dict(doc))
        raise HTTPException(status_code=400,
                            detail=res.get("detail") or "Wiederherstellen fehlgeschlagen")
