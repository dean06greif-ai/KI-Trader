...
class BaseStrategy(ABC):
    ...
    IS_CUSTOM = False
    IS_VARIANT = False          # Kopie einer Built-in-Strategie (registry.upsert_variant)
    ...
    def check_signal(self, candles: List[Dict], symbol: str, settings: Dict) -> Optional[Dict]:
        ...
        return {
            ...
            # Richtungs-bewusst (Parität zu fast_sim/Live-Scanner): nur die
            # Regeln der Signal-Richtung zählen, sonst blockiert
            # require_all_rules Custom-Strategien mit Long+Short-Regeln immer.
            "rules_met_count": (result.get("long_count")
                                if result["signal_type"] == "LONG"
                                else result.get("short_count"))
            if result.get("long_count") is not None or result.get("short_count") is not None
            else sum(1 for v in rules_met.values() if v),
            "rules_total": result.get("rules_total") or len(rules_met),
            "rules_met": rules_met,
            "used_params": params,
        }
