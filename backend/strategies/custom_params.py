...
INDICATOR_PERIODS = {
    ...
    "supertrend": ["supertrend_period", "supertrend_mult"],
    "supertrend_dir": ["supertrend_period", "supertrend_mult"],
    "ema_slope_pct": ["ema_slow_period", "slope_lookback"],
    "roc": ["roc_period"],
    "chop": ["chop_period"],
    "aroon_up": ["aroon_period"],
    "aroon_down": ["aroon_period"],
    "williams_r": ["williams_period"],
}
...
PERIOD_RANGES = {
    ...
    "supertrend_period": (10, 5, 30, 1, "Supertrend ATR-Periode"),
    "supertrend_mult": (3.0, 1.0, 5.0, 0.25, "Supertrend Multiplikator"),
    "slope_lookback": (5, 2, 30, 1, "EMA-Steigung Lookback"),
    "roc_period": (10, 3, 50, 1, "ROC Periode"),
    "chop_period": (14, 7, 40, 1, "Choppiness Periode"),
    "aroon_period": (25, 10, 60, 5, "Aroon Periode"),
    "williams_period": (14, 5, 30, 1, "Williams %R Periode"),
}
...
BOUNDED_THRESHOLDS = {
    ...
    "ema_slope_pct": (-3.0, 3.0, 0.05), "roc": (-10.0, 10.0, 0.1),
    "chop": (20, 80, 1), "aroon_up": (0, 100, 5), "aroon_down": (0, 100, 5),
    "williams_r": (-100, 0, 2),
}
# Binär (0/1) bzw. Zeit – nicht optimierbar
BINARY_INDICATORS = {"ha_color", "hour", "supertrend_dir"}
...
def apply_params(definition: Dict, params: Dict) -> Dict:
    ...
    for side in ("long_rules", "short_rules"):
        ...
        for i, r in enumerate(rules):
            ...
            new_rules.append(nr)
        out[side] = new_rules
    # R01-Härtung: dynamische Regel-Umschaltung je Coin/Regime. Eine
    # Discovery-Substrategie ERSETZT die Regel-Listen (long_rules/short_rules),
    # eine Regel-Variante ergänzt zusätzliche Regeln (add_long_rules/
    # add_short_rules). Wirkt identisch im Live-Scanner und im Fast-Path.
    ro = params.get("rules_override")
    if isinstance(ro, dict):
        if ro.get("long_rules") is not None or ro.get("short_rules") is not None:
            out["long_rules"] = [dict(r) for r in (ro.get("long_rules") or [])
                                 if isinstance(r, dict)]
            out["short_rules"] = [dict(r) for r in (ro.get("short_rules") or [])
                                  if isinstance(r, dict)]
        for side, add_key in (("long_rules", "add_long_rules"),
                              ("short_rules", "add_short_rules")):
            extra = [dict(r) for r in (ro.get(add_key) or []) if isinstance(r, dict)]
            if extra:
                out[side] = list(out.get(side) or []) + extra
    return out
