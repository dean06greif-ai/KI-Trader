...
USER_MATCH = re.search(r"Benutzer\s*`([^`]+)`", CREDENTIALS)
LOGIN_PAYLOAD = {"password": PASSWORD}
if USER_MATCH:
    LOGIN_PAYLOAD["username"] = USER_MATCH.group(1)
...
def api_client():
    ...
    yield session
    if PASSWORD:
        login = session.post(f"{BASE_URL}/api/auth/login", json=LOGIN_PAYLOAD, timeout=30)
        if login.ok:
...
class TestRegimeLabWorkflow:
    ...
    def test_03_seeded_analysis_schema_labels_and_persistence(self, api_client):
        ...
        if not rows:
            pytest.skip("Keine Regime-Analysen in der DB (frische Dev-DB)")
