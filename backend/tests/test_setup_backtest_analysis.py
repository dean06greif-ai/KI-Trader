...
def test_apply_filters_hours_wrap_midnight():
    # Audit 2.9: hour_from > hour_to = Fenster über Mitternacht (z. B. 22–6)
    f = _features()
    sigs = [Signal(i, "LONG", 100.0, 99.0, 101.0, 102.0) for i in range(300, 1500, 7)]
    night = detectors.apply_filters(f, sigs, {"hour_from": 22, "hour_to": 6})
    day = detectors.apply_filters(f, sigs, {"hour_from": 6, "hour_to": 22})
    bl = f.berlin()
    assert all(bl[s.idx].hour >= 22 or bl[s.idx].hour < 6 for s in night)
    assert all(6 <= bl[s.idx].hour < 22 for s in day)
    assert len(night) + len(day) == len(sigs)
