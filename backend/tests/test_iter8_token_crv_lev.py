...
def test_default_trade_frame_backwards_compatible():
    # crv_max Standard 4.0 (User-Entscheid 15.06.: Deckel gegen unrealistische
    # CRV-Ausreißer; alte crv_max=0-Configs werden einmalig migriert).
    # lev_mode "coin" = Coin-Trade-Settings entscheiden.
    assert DEFAULT_AI_CONFIG["crv_min"] == 1.2
    assert DEFAULT_AI_CONFIG["crv_max"] == 4.0
...
def test_active_fallback_tracked_and_cleared():
    ...
    ai_providers.record_result("groq", "openai/gpt-oss-20b", "ok",
    ...
    assert any(f["role"] == "analyst"
               and f["model"] == "openai/gpt-oss-20b"
...
def test_backup_key_is_plain_key_rotation_not_fallback():
    """Seit 09/2026 (Trader-Entscheid): Ein Backup-Key desselben Modells ist
    normale Key-Rotation – KEIN aktiver Fallback, keine Warnung."""
    ai_providers._role_fallbacks.clear()
    ai_providers.record_result("groq", "openai/gpt-oss-120b", "ok",
    ...
    hs = ai_providers.health_status()
    fbs = hs.get("active_fallbacks") or []
    assert not any(f["role"] == "news_watcher" for f in fbs)
    assert hs["fallback_active"] is False
    ai_providers._role_fallbacks.clear()
