...
def test_record_result_without_role_stays_unassigned():
    """Backlog P2: record_result liest das globale _current_role NICHT mehr –
    ohne explizite Rolle bleibt der Fallback bewusst unzugeordnet (statt
    potenziell dem falschen, parallel laufenden Assistenten)."""
    ai_providers.set_current_role("chat")
    ai_providers.record_result("groq", "model-b", "ok", key_index=1,
                               requested="model-a")
    assert "chat" not in ai_providers._role_fallbacks
