...
class TestPaperStatsCollectionStale:
    ...
    def test_include_collection_delta(self, seeded):
        # Datensammel-Trades bleiben IMMER ausgeblendet – auch mit include_collection=true
        off = _cmp(mode="paper")
        on = _cmp(mode="paper", include_collection="true")
        ki_off, ki_on = _row(off, "ai_trader"), _row(on, "ai_trader")
        assert ki_on["trades"] == ki_off["trades"]
        assert round(ki_on["pnl"] - ki_off["pnl"], 2) == 0.0
        assert on["include_collection"] is False
        assert on["total_trades"] == off["total_trades"]

    def test_include_stale_shows_deleted_strategy(self, seeded):
        # Karteileichen bleiben IMMER ausgeblendet – auch mit include_stale=true
        d = _cmp(mode="paper", include_stale="true")
        assert _row(d, STALE_ID) is None, "Karteileiche darf nie erscheinen"
        assert STALE_ID in d["stale_strategies"]
        assert d["include_stale"] is False
