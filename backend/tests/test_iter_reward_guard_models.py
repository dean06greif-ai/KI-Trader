...
def test_too_large_error_detection():
    assert ai_providers.is_too_large_error(Exception(
        ...
        "`openai/gpt-oss-20b` on tokens per minute (TPM)'}}"))
...
def test_generate_chain_skips_over_budget_model(monkeypatch):
    ...
    big_prompt = "x" * (9000 * 3)  # ~9000 Tokens > Budget 7000 der gpt-oss-Modelle
    chain = [("groq", "openai/gpt-oss-20b"), ("groq", "llama-3.3-70b-versatile")]
    ...
    assert "groq/openai/gpt-oss-20b" not in called  # übersprungen, nicht aufgerufen
    health = ai_providers.health_status()
    skipped = {f"{m['provider']}/{m['model']}" for m in health["skipped_too_large"]}
    assert "groq/openai/gpt-oss-20b" in skipped
...
def test_reward_quick_stopout_penalized_confidence_decoupled():
    ...
    # Audit 2.7: Konfidenz-Bonus/Malus ist entkoppelt (Default aus) …
    assert not any("Konfidenz" in l for l in labels)
    assert "Sofort-Stop-Out (<15 min)" in fast["violations"]
    assert fast["confidence"] == 70
    # … und nur mit Schalter wieder aktiv (Alt-Verhalten)
    legacy = ai_rewards.compute_reward(_trade(-3.0, "loss", minutes=5),
                                       {"confidence": 70}, confidence_reward=True)
    assert any("Konfidenz" in c["label"] for c in legacy["components"])
