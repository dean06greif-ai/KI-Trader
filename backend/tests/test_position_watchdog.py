...
def make_watchdog(db, client, grace: int = 0):
    at = AutoTradeManager(client)
    at.set_db(db)
    wd = PositionWatchdog()
    wd.setup(db, client, at, telegram=None)
    # Doppel-Trade-Schutz (adopt_grace_sec) wird in test_watchdog_inflight_dedupe.py
    # getestet – hier sofortige Übernahme wie im Altverhalten.
