...
def test_comparison_groups_manual_bitunix_trades(manual_trade):
    """Manuelle/Watchdog-Trades erscheinen (mit include_manual=true) als EINE
    Pseudo-Strategie 'Manuell (Bitunix)' (id 'manual'), nie als rohe
    'external'-Strategie. Standardmäßig sind sie ausgeblendet."""
    r = requests.get(f"{BASE}/api/analytics/strategy-comparison",
                     params={"include_manual": "true"}, timeout=30)
    assert r.status_code == 200
    data = r.json()
    ids = [row["strategy_id"] for row in data["comparison"]]
    assert "external" not in ids
    manual_rows = [row for row in data["comparison"]
                   if row["strategy_id"] == "manual"]
    assert len(manual_rows) == 1
    assert manual_rows[0]["strategy_name"] == "Manuell (Bitunix)"
    assert manual_rows[0].get("is_manual") is True
