...
class TestConfirmCAS:
    ...
    def test_parallel_confirms_run_apply_only_once(self, monkeypatch):
        """R13-Härtung: der zuvor offene Bestätigungsfehler – zwei parallele
        Confirms desselben Vorschlags dürfen das Apply nur EINMAL ausführen."""
        doc = _doc()
        doc["pending_switch"] = _pending(rel.state_version(doc["last_state"]))
        db, calls = self._setup(monkeypatch, doc)

        async def slow_apply(d):
            await asyncio.sleep(0.05)
            calls.append(d["id"])
            return [{"symbol": "BTCUSDT"}]

        monkeypatch.setattr(dyn_router.dynamic_live, "apply_active", slow_apply)

        async def both():
            t1 = asyncio.ensure_future(
                dyn_router.dynamic_confirm("dyn1", {"command_id": "cmd_abc"}, True))
        ...
        assert errs[0].status_code == 409
        assert calls.count("dyn1") == 1
...
class TestApplyGateAndArchive:
    ...
    def test_manual_apply_blocked_for_draft_then_allowed_after_approve(self, monkeypatch):
        ...
        # R09-Härtung: Freigeben ohne Testnachweis wird ohne ausdrückliches
        # Override abgelehnt (409) – kein stiller Bypass mehr.
        with pytest.raises(HTTPException) as e:
            _run(dyn_router.dynamic_approve("dyn1", {"note": "test"}, True))
        assert e.value.status_code == 409
        # Override ohne Begründung: abgelehnt (400)
        with pytest.raises(HTTPException) as e:
            _run(dyn_router.dynamic_approve(
                "dyn1", {"override_missing_evidence": True}, True))
        assert e.value.status_code == 400
        res = _run(dyn_router.dynamic_approve(
            "dyn1", {"override_missing_evidence": True, "note": "bewusst"}, True))
        assert res["release_status"] == "approved"
        assert res["release"]["approved_without_evidence"] is True
