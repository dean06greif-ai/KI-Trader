...
def test_load_keeps_preset_when_user_never_configured():
    ...
    stored = {"_cost_migration_v1": True,
              "analyst": {"enabled": False, "provider": None, "model": None}}
...
def test_load_respects_user_choice():
    mgr = ai_roles.AIRoleManager()
    stored = {"_cost_migration_v1": True,
              "analyst": {"provider": "mistral", "model": "mistral-small-latest",
...
def test_chain_appends_available_providers(monkeypatch):
    ...
    # Seit der Kosten-Migration hat jede Rolle ein eigenes Preset – das bleibt erste Wahl.
    preset = ai_roles.DEFAULT_ROLES_CONFIG["analyst"]["provider"]
    assert providers[0] == preset, "Rollen-Preset bleibt erste Wahl"


def test_load_migration_keeps_enabled_flags():
    """Kosten-Migration setzt Modelle auf Presets, aber Nutzer-Ein/Aus bleibt."""
    mgr = ai_roles.AIRoleManager()
    stored = {"analyst": {"enabled": False, "provider": "mistral",
                          "model": "mistral-small-latest", "user_configured": True}}
    asyncio.run(mgr.load(_FakeDB(stored)))
    cfg = mgr.role_cfg("analyst")
    assert cfg["enabled"] is False
    assert cfg["model"] == ai_roles.DEFAULT_ROLES_CONFIG["analyst"]["model"]
