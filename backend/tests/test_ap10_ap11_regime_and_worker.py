...
def test_localworker_manifest(auth):
    ...
    assert r.status_code == 200, r.text
    m = r.json()
    assert m.get("required_version") == __import__("services.local_exec", fromlist=["x"]).REQUIRED_WORKER_VERSION_STR
...
def test_localworker_status(auth):
    ...
    assert r.status_code == 200, r.text
    data = r.json()
    assert data.get("required_version") == __import__("services.local_exec", fromlist=["x"]).REQUIRED_WORKER_VERSION_STR
