...
TIMEOUT = 120
# Pro Lauf eindeutig: eine feste ID blieb nach dem ersten Lauf als
# "cancelled" im In-Memory-Store des Servers liegen -> Restore-Test schlug
# bei jedem weiteren Lauf gegen denselben Serverprozess fehl.
RESTORE_ID = "tst_restore_" + uuid.uuid4().hex[:8]
...
class TestLocalWorker:
    def test_status_ok(self, client):
        ...
        d = r.json()
        assert "online" in d
        # 'online' hängt davon ab, ob gerade ein echter lokaler Worker pollt –
        # keine harte False-Erwartung (war flaky, sobald ein Worker lief).
