...
from services.ai_roles import AIRoleManager, in_active_hours, DEFAULT_ROLES_CONFIG, ROLE_PRESETS
...
def test_new_providers_in_catalog():
    for prov in ("gemini", "groq", "openrouter", "mistral", "cerebras"):
...
def test_provider_for_model():
    ...
    # GitHub Models wurde eingestellt -> Migration auf funktionierenden Ersatz
    assert ai_providers.migrate_model(None, "openai/gpt-4.1") == ("groq", "openai/gpt-oss-120b")
    assert ai_providers.provider_for_model("gpt-oss-120b") == "cerebras"
    assert ai_providers.provider_for_model("unbekannt") is None


def _clear_numbered_backups(monkeypatch, base="OPENROUTER_API_KEY"):
    import os as _os
    import re as _re
    pat = _re.compile(rf"^{base}_BACKUP\d+$")
...
def test_openrouter_backup_key_order(monkeypatch):
    _clear_numbered_backups(monkeypatch)
...
def test_role_chain_inherits_engine_default():
    rm = AIRoleManager()
    # Analyst hat ein eigenes Preset -> das steht vorn in der Kette
    chain = rm.chain("analyst", ENGINE_CFG)
    preset = ROLE_PRESETS["analyst"]
    assert chain[0] == (preset["provider"], preset["model"])
    # Ohne eigenes Rollen-Modell erbt die Rolle das Engine-Haupt-Modell
    rm.config["analyst"] = {**rm.config["analyst"], "provider": None, "model": None}
    chain = rm.chain("analyst", ENGINE_CFG)
    assert chain[0] == ("gemini", "gemini-3.5-flash")
...
def test_role_chain_dead_model_migrates():
    """Von Groq entfernte Modelle (llama-3.3/3.1) dürfen die Kette nicht
    brechen: es muss ein gültiges Groq-Modell an ihrer Stelle stehen."""
    from services import ai_providers
    rm = AIRoleManager()
    rm.config["deep_analyst"].update({
        "provider": "openrouter", "model": "nvidia/nemotron-3-ultra-550b-a55b:free",
    ...
    chain = rm.chain("deep_analyst", ENGINE_CFG)
    groq_models = [m for p, m in chain if p == "groq"]
    assert groq_models, "Groq-Fallback fehlt komplett"
    assert all(m in ai_providers.allowed_models("groq") for m in groq_models), \
        f"totes Groq-Modell in der Kette: {groq_models}"
...
def test_role_chain_outside_hours_uses_fallback_first():
    ...
    assert outside[0] == ("groq", "openai/gpt-oss-20b")
