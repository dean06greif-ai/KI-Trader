...
class FakeCollection:
    ...
    async def count_documents(self, q):
        return len([d for d in self.docs if self._query_ok(d, q)])

    def _query_ok(self, doc, q):
        for k, v in q.items():
    ...
    def find(self, q, proj=None):
        rows = [dict(d) for d in self.docs if self._query_ok(d, q)]
...
def _confirm(e, scope, symbol, key, direction="down", times=3):
    ...
    Audit 2.8: Bestätigungen zählen nur mit NEUER Evidenz – deshalb werden
    zwischen den Vorschlägen (und nach dem letzten) geschlossene Trades gesät."""
    def _seed_trades(day):
        for j in range(3):
            e.db.auto_trades.docs.append({
                "strategy_id": "ai_trader", "status": "closed", "symbol": symbol,
                "closed_at": f"2999-01-{day:02d}T0{j}:00:00+00:00"})
    for i in range(times):
        e.db.ai_proposals.docs.append({
            "id": f"seed{i}", "scope": scope, "symbol": symbol,
            "changes": {key: 1}, "macro_direction": direction,
            "ts": f"2999-01-{i * 2 + 1:02d}T12:00:00+00:00"})
        _seed_trades(i * 2 + 2)
