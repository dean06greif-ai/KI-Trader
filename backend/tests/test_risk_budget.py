...
def test_check_fail_open_without_equity_and_when_disabled():
    assert rb.check(999.0, "BTCUSDT", 0.0, {}, None, CFG) == (True, "")
...
def test_hook_in_on_signal_before_order_and_not_for_collection():
    ...
    i = src.index("risk_budget.check_new_trade")
    assert src.index("if not collection:", i - 800) < i
