...
def test_gateway_watch_logout_and_readyz_alarm():
    ...
    # readyz seit >5 Minuten rot -> einmaliger Alarm
    assert w.evaluate(1030 + 299, False, 503) == []
    assert w.evaluate(1030 + 301, False, 503) == ["readyz_red"]
    assert w.evaluate(1030 + 400, False, 503) == []
    # Wieder eingeloggt + grün -> Entwarnung für beides
    ev = w.evaluate(1030 + 500, True, 200)
    assert set(ev) == {"login", "readyz_green"}
