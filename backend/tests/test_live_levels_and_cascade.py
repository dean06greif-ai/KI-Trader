...
class FakeClient:
    ...
    def __init__(self, live_qty="1.0", pending_tpsl=None,
                 ...
                 modify_silent_noop=False):
        self.live_qty = live_qty
        self.pending = pending_tpsl or []          # [{"id": "...", "slPrice": ...}]
        self.modify_code = modify_code
        self.modify_silent_noop = modify_silent_noop
    ...
    def contract_meta(self, s):
        return {"price_tick": 0.01, "qty_step": 0.001, "min_qty": 0.001}
    ...
    async def get_pending_tpsl(self, symbol, position_id=None):
        ...
        return {"code": 0, "data": [dict(p) for p in self.pending]}
    ...
    async def modify_tpsl_order(self, symbol, order_id, side="LONG",
        ...
        self.calls.append(("tpsl_modify_order", order_id,
                           {"sl": sl_price, "tp": tp_price}))
        if self.modify_code == 0 and not self.modify_silent_noop:
            for p in self.pending:
                if str(p.get("id")) == str(order_id):
                    ...
                    if tp_price is not None:
                        p["tpPrice"] = tp_price
        return {"code": self.modify_code,
                "msg": "Success" if self.modify_code == 0 else "order not exist"}
    ...
    async def place_position_tp_sl(self, symbol, position_id, side,
        ...
        if code == 0:
            self.pending.append({"id": "tpsl-new", "slPrice": sl_price,
                                 "tpPrice": tp_price, "slQty": sl_qty,
                                 "tpQty": tp_qty})
...
def test_sync_levels_cancels_stale_tpsl_orders_before_placing():
    ...
    client = FakeClient(live_qty="1.0",
                        pending_tpsl=[{"id": "old-1", "slPrice": "94"},
                                      {"id": "old-2", "slPrice": "93"},
                                      {"id": "old-3", "slPrice": "92"}])
    m = _manager(client, [t])
...
def test_sync_levels_retries_without_qty_when_size_rejected():
    ...
    client = FakeClient(live_qty="1.0", modify_code=1, place_codes=[20015, 0],
                        pending_tpsl=[{"id": "o1", "slPrice": "95", "slQty": "1.0"}])
    m = _manager(client, [t])
    res = asyncio.run(m.sync_live_levels(t, sl=96.0))
    assert res["ok"], res
...
def test_sync_levels_prefers_modify_no_cancel_needed():
    """Modify per Order-ID erfolgreich -> weder Cancel noch neue Order."""
    t = _trade()
    client = FakeClient(
        live_qty="1.0", modify_code=0,
        pending_tpsl=[{"id": "tpsl-7", "slPrice": "95", "slQty": "1.0"}])
    m = _manager(client, [t])
    res = asyncio.run(m.sync_live_levels(t, sl=96.0))
    assert res["ok"]
    assert any(c[0] == "tpsl_modify_order" for c in client.calls)
    assert not any(c[0] == "cancel_tpsl" for c in client.calls)
    assert not any(c[0] == "tpsl_place" for c in client.calls)
    assert float(client.pending[0]["slPrice"]) == 96.0   # Börse hat's wirklich


def test_sync_levels_detects_silent_noop_and_replaces():
    """REGRESSION (XRP-Trail): Börse meldet code 0, ändert die Order aber
    NICHT -> Read-Back erkennt das und erzwingt Cancel+Replace."""
    t = _trade()
    client = FakeClient(
        live_qty="1.0", modify_code=0, modify_silent_noop=True,
        pending_tpsl=[{"id": "tpsl-7", "slPrice": "95", "slQty": "1.0"}])
    m = _manager(client, [t])
    res = asyncio.run(m.sync_live_levels(t, sl=96.0))
    assert res["ok"], res
    assert any(c[0] == "cancel_tpsl" for c in client.calls)
    assert any(c[0] == "tpsl_place" for c in client.calls)
    sl_now = [p for p in client.pending if p.get("slPrice") is not None]
    assert sl_now and float(sl_now[0]["slPrice"]) == 96.0
...
def test_cascade_switches_to_fallback1_first():
    ...
    assert switches and switches[0]["to"]["model"] == "openai/gpt-oss-120b"
    assert "Fallback 1" in switches[0]["to"]["via"]
    assert rm.config["analyst"]["model"] == "openai/gpt-oss-120b"
