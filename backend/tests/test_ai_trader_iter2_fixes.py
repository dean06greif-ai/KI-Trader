...
def test_live_be_uses_exact_fee_formula():
    # Formel lebt seit 06/2026 im geteilten Helper breakeven_price
    # (genutzt von _manage_trade UND sync_position_state/TP1-extern).
    from services.bitunix_trade import breakeven_price
    src = inspect.getsource(breakeven_price)
    assert "(1 + fee) / (1 - fee)" in src
    assert "(1 - fee) / (1 + fee)" in src
    assert "breakeven_price" in inspect.getsource(AutoTrader)
...
def _make_watchdog(local_trade, adopt_unknown=False, manage_external=False):
    ...
    class _AutoTrades:
        ...
        def find(self, flt):
            rows = [dict(local_trade)] if local_trade is not None else []
...
def test_watchdog_skips_adopted_external_position():
    wd = _make_watchdog(local_trade={"id": "ext1", "external_adopted": True,
...
def test_schedule_keeps_model_and_provider():
    ...
    assert sched[0]["model"] == "openai/gpt-oss-120b"
