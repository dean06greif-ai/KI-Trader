...
class TestMlGateSizeFactor:
    def _gate(self, auc, scaling=True):
        from services.ml_gate import MLGate
        g = MLGate()
        # Audit 2.10: Default ist AUS – für die Skalierungs-Tests explizit an
        g.settings["auc_risk_scaling"] = scaling
        if auc is not None:
            g.model_meta = {"version": 3, "metrics": {"oos_auc": auc}}
        return g

    def test_default_scaling_off(self):
        # Audit 2.10: ohne explizites Einschalten keine Skalierung
        from services.ml_gate import MLGate, DEFAULT_SETTINGS
        assert DEFAULT_SETTINGS["auc_risk_scaling"] is False
        g = MLGate()
        g.model_meta = {"version": 3, "metrics": {"oos_auc": 0.52}}
        assert g.size_factor() == (1.0, "")
        assert g.status()["risk_scaling_active"] is False
