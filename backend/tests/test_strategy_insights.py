...
def test_daily_quota_cooldown_until_utc_midnight():
    ...
    assert cd <= 86400 + 60
    # Minuten-Limit sperrt nur kurz (~65s), damit ein Burst nicht alle Keys
    # 10 min aus der Rotation nimmt (siehe _quota_cooldown_s-Docstring)
    from services.ai_providers import MINUTE_LIMIT_COOLDOWN_S
    assert _quota_cooldown_s("429 requests per minute") == MINUTE_LIMIT_COOLDOWN_S
    assert MINUTE_LIMIT_COOLDOWN_S < KEY_LIMIT_COOLDOWN_S
