...
def test_sync_no_flapping_with_leverage_setting():
    ...
    Margen-Anpassung ist der effektive Hebel anders. Die Marge bleibt die
    Wahrheit (margin_used + effective_leverage steuern gebundene Marge/Liq),
    aber ANGEZEIGT wird das Börsen-Setting (Bug-Report 26.08.: Karte zeigte
    unmöglichen 333x-Hebel statt 200x). Das Setting darf den Sync nicht
    zurückkippen (Flattern)."""
    ...
    saved = _run(m.db.auto_trades.find_one({"id": "t1"}))
    assert saved["margin_used"] == pytest.approx(25.0)
    assert saved["leverage"] == pytest.approx(200.0)  # Anzeige = Setting
    assert saved["effective_leverage"] == pytest.approx(40.0)  # Marge-Wahrheit
    # gebundene Marge rechnet mit dem effektiven Hebel weiter
    assert m.trade_bound_margin(saved) == pytest.approx(25.0)
