...
async def main():
    ...
    sigs = await db.signals.find({"timestamp": {"$gte": D7}, "strategy_id": "ai_trader"},
    ...
    for m in models:
        ...
        print(f"    v{m.get('version')} {str(m.get('trained_at'))[:16]} samples={m.get('samples')} "
              f"AUC={met.get('oos_auc')} brier={met.get('oos_brier_calibrated')} "
              f"vs base={met.get('baseline_brier')} trigger={m.get('trigger')}")
    ...
    if rew:
        w = sum(1 for r in rew if float(r.get("score") or 0) > 0)
        print(f"  Score>0: {w}/{len(rew)}")
        ...
        for r in rew:
            ...
            byreg[rg][0] += 1
            byreg[rg][1] += 1 if float(r.get("score") or 0) > 0 else 0
        print("  Nach Regime:")
