...
backend:
  - task: "Key-Level-Limit-Orders (KI legt Limit-Orders an Order-Blocks/POC, Ablauf + Neu-Bewertung je Zyklus)"
    implemented: true
    working: true
    file: "backend/services/key_level_limits.py"
    stuck_count: 0
    priority: "high"
    needs_retesting: false
    status_history:
      - working: true
        agent: "main"
        comment: >
          Neues Modul services/key_level_limits.py: KI waehlt pro Entscheidung entry_type
          market|limit + limit_price + limit_valid_min (15-480, KI-gewaehlt) + cancel_limit.
          Wartende Orders lokal in ai_limit_orders (synthetisch, identisch fuer Paper+Live),
          Fill-Check je Scanner-Tick (core/scheduler.py), Neu-Bewertung je Analyse-Zyklus
          (reevaluate in ai_engine), Prompt-Kontext WARTENDE LIMIT-ORDERS, Validierung
          (Seite + max 2.5%/8% Distanz). API: GET/DELETE /api/ai/limit-orders.
          8/8 Unit-Tests gruen (tests/test_key_level_limits.py), Lifecycle per API verifiziert.

  - task: "Ehrliches Paper-Trading (simulierter Spread + Slippage, Live-Orderbuch bevorzugt)"
    implemented: true
    working: true
    file: "backend/services/paper_execution.py"
    stuck_count: 0
    priority: "high"
    needs_retesting: false
    status_history:
      - working: true
        agent: "main"
        comment: >
          Neues Modul services/paper_execution.py: Paper-Fills adversarial mit Spread/2 +
          Slippage (Entry LONG teurer/SHORT billiger, Exits gespiegelt). Quelle: echter
          Top-of-Book-Spread Binance->OKX->Bybit (20s Cache), Fallback Tier-Schaetzwerte
          (Major 0.01/0.01, Crypto 0.04/0.03, Yahoo 0.06/0.05). Integriert in
          bitunix_trade.py: Entry (on_signal), SL/TP1/TPF (_manage_trade), manual_close,
          partial_close – NUR mode=paper, Live unveraendert. Abschaltbar via
          paper_realistic_fills. 6/6 Unit-Tests gruen, E2E: Trade eroeffnet mit
          paper_exec (Quelle orderbook) + PAPER-FILL-Events verifiziert.

frontend:
  - task: "KI-Limit-Orders Karte (KeyLevelLimitOrders) im Analytics-Bereich"
    implemented: true
    working: "NA"
    file: "frontend/src/components/KeyLevelLimitOrders.js"
    stuck_count: 0
    priority: "medium"
    ...
    status_history:
      - working: "NA"
        agent: "main"
        comment: >
          Neue Karte analog PendingEntryOrders in PerformanceAnalytics eingebunden:
          zeigt wartende KI-Limit-Orders (Level, Distanz, Countdown) + Stornieren-Button
          (DELETE /api/ai/limit-orders/{id}, Admin). Rendert nichts ohne offene Orders.
