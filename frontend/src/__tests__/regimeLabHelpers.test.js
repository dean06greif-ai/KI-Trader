import { autopilotDecision, autopilotReport, calibAppliedFromReport, calibrationDecision, metricLabel } from '../lib/regimeCalibration';
...
describe('calibrationDecision – schlechtere Kalibrierung wird nicht übernommen', () => {
  ...
  test('anderes Zeitfenster wird nicht verglichen – zählt als Erst-Kalibrierung des Timeframes', () => {
    const cur = { detector: 'ema', best_pct: 99, truth_source: 'centered', timeframe: '1d' };
    const d = calibrationDecision(cur, { ...report(80), timeframe: '4h' }, 'ema');
    expect(d).toMatchObject({ adopt: true, reason: 'other_timeframe' });
  });
  test('gleicher Timeframe wird weiterhin normal verglichen', () => {
    const cur = { detector: 'ema', best_pct: 99, truth_source: 'centered', timeframe: '4h' };
    expect(calibrationDecision(cur, { ...report(80), timeframe: '4h' }, 'ema').reason).toBe('worse');
  });
  test('ohne Timeframe-Angabe (Altbestand) bleibt der bisherige Vergleich', () => {
    const cur = { detector: 'ema', best_pct: 99, truth_source: 'centered' };
    expect(calibrationDecision(cur, { ...report(80), timeframe: '4h' }, 'ema').reason).toBe('worse');
  });
});

describe('autopilotDecision – Autopilot überschreibt nie mit einem schlechteren Ergebnis', () => {
  const result = (holdoutBest, holdoutBase = 96.4, improved = true) => ({
    improved, holdout_regressed: holdoutBest < holdoutBase,
    best_engine_config: { detector: 'ema', ema_regime_days: 20 },
    best: { detector: 'ema', score: 97, metrics: { holdout_direction_pct: holdoutBest } },
    baseline: { score: 96, metrics: { holdout_direction_pct: holdoutBase } },
    symbols: ['XAUUSDT'], timeframe: '1d',
  });
  test('Holdout 96,4 → 95,7 wird NICHT übernommen (der gemeldete Fall)', () => {
    const d = autopilotDecision(null, result(95.7));
    expect(d.adopt).toBe(false);
    expect(d.reason).toBe('holdout_regressed');
    expect(d.report.metric).toBe('holdout_direction_pct');
  });
  ...
  test('nicht unter die aktive Autopilot-Kalibrierung desselben Grundgerüsts', () => {
    const cur = { detector: 'ema', metric: 'holdout_direction_pct', best_pct: 98.0 };
    expect(autopilotDecision(cur, result(97.1))).toMatchObject({ adopt: false, reason: 'worse', curPct: 98 });
  });
  test('Autopilot: anderer Timeframe der aktiven Kalibrierung wird nicht verglichen (der gemeldete 1d/4h-Fall)', () => {
    const cur = { detector: 'ema', metric: 'holdout_direction_pct', best_pct: 99.9, timeframe: '1d' };
    expect(autopilotDecision(cur, { ...result(97.1), timeframe: '4h' }))
      .toMatchObject({ adopt: true, reason: 'other_timeframe' });
  });
  test('andere Metrik (wissenschaftliche Kalibrierung aktiv) wird nicht verglichen – Holdout-Regel entscheidet', () => {
    const cur = { detector: 'ema', best_pct: 99.0 }; // balanced_direction_pct (implizit)
    expect(autopilotDecision(cur, result(97.1)).adopt).toBe(true);
    expect(autopilotDecision(cur, result(95.0)).adopt).toBe(false);
  });
  test('calibAppliedFromReport trägt die Metrik mit', () => {
    const c = calibAppliedFromReport(autopilotReport(result(97.1)), 'j1', 'Regime-Autopilot', {});
    expect(c).toMatchObject({ metric: 'holdout_direction_pct', best_pct: 97.1, baseline_pct: 96.4, detector: 'ema' });
    expect(metricLabel(c)).toBe('Holdout Live=Final');
    expect(metricLabel({ best_pct: 1 })).toBe('Treffer (Referenz)');
  });
});
