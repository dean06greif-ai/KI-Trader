  ...
  return (
    <div className="gov-panel" data-testid="ai-governance-panel">
      ...
      <div className="gov-rules">
        ...
        {/* Lektions-Qualität (maschinell erzwungen, siehe ai_master_prompt.check_lesson_rules) */}
        {[
          ['block_leverage_lessons', 'Keine Hebel-Deckel-Lektionen (Risiko über Marge/SL steuern)'],
          ['require_direction_context', 'Richtungs-Lektionen (Long/Short) nur mit Marktkontext'],
          ['block_coin_ranking_lessons', 'Keine Coin-Ranglisten aus vergangenem PnL („A priorisieren, B meiden“)'],
        ].map(([key, label]) => (
          <label className="gov-check" key={key}>
            <input type="checkbox" checked={rules[key] !== false}
              onChange={e => setRules({ ...rules, [key]: e.target.checked })}
              data-testid={`master-rule-${key.replace(/_/g, '-')}`} />
            <span>{label}</span>
          </label>
        ))}
      </div>
