...
export const SetupMaturityTable = ({ rows, showAssets, diagnosis, mode = 'live' }) => {
  // Spalte je Welt: real = Echtgeld-Live, paper = Paper-Logik, collection = Sammel-Trades
  // ('live' bleibt als Alias für Echtgeld kompatibel)
  const world = mode === 'collection' ? 'coll' : mode === 'paper' ? 'paper' : 'live';
  const WORLD_LABEL = { live: 'Echtgeld', paper: 'Paper', coll: 'Sammlung' };
  const WORLD_TITLE = { live: 'Nur echte Live-Trades (ohne Sammel-Trades)', paper: 'Nur Paper-Trades mit Live-Logik (ohne Sammel-Trades)', coll: 'Nur Datensammel-Trades' };
  const modeCol = (r) => (r[`${world}_trades`]
    ? `${r[`${world}_trades`]} / ${r[`${world}_winrate`]}% / ${money(r[`${world}_pnl`])}` : '—');
  ...
  // + 95%-Wilson-Intervall der Winrate (Audit 2.5: ehrliche Unsicherheit)
  const ciText = (ci) => (ci && ci.length === 2 ? `\n95%-Intervall Winrate: ${ci[0]}–${ci[1]}%` : '');
  const variantTitle = (r) => (r.variant
    ...
    : 'Gesamtstatistik – Setup hat noch keine Variante (Rückstufung/Revision/Profil-Version)') + ciText(r.wr_ci);
    ...
    <thead>
      <tr style={{ opacity: 0.55, textAlign: 'left' }}>
        ...
        <th style={{ padding: '3px 6px' }} data-testid="ai-maturity-mode-col-header"
          title={WORLD_TITLE[world]}>
          {WORLD_LABEL[world]} n / WR / PnL</th>
    ...
    <tbody>
      {rows.map(r => (
        <tr key={r.setup} data-testid={`ai-maturity-row-${r.setup}`} style={{ borderTop: '1px solid #22252F' }} title={r.reason || ''}>
          ...
          <td style={{ padding: '4px 6px' }} title={variantTitle(r)} data-testid={`ai-maturity-trades-${r.setup}`}>
            {r.trades}
            {r.variant && r.variant.total?.trades > r.trades && (
              <span style={{ fontSize: 9.5, opacity: 0.55, marginLeft: 4 }} data-testid={`ai-maturity-total-${r.setup}`}>
                / ges. {r.variant.total.trades}
              </span>
            )}
          </td>
          ...
          <td style={{ padding: '4px 6px', opacity: 0.85 }} data-testid={`ai-maturity-live-${r.setup}`}
            title={world === 'live' && r.live_trades ? `Nur echte Live-Trades${ciText(r.live_wr_ci)}` : undefined}>
            {modeCol(r)}
          </td>
