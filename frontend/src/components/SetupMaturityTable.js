...
const PhaseCell = ({ r }) => {
  if (r.live_ready) return <span style={{ color: '#00FF66', fontWeight: 700 }}>✓ live</span>;
    ...
    <tbody>
      {rows.map(r => (
        <tr key={r.setup} data-testid={`ai-maturity-row-${r.setup}`} style={{ borderTop: '1px solid #22252F' }} title={r.reason || ''}>
          ...
          <td style={{ padding: '4px 6px', color: (r.pnl ?? 0) > 0 ? '#00FF66' : (r.pnl ?? 0) < 0 ? '#FF3366' : undefined }}>{r.trades ? money(r.pnl) : '—'}</td>
