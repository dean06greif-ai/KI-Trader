...
const AITradingPanel = ({ onClose, selectedCoin = 'BTCUSDT' }) => {
  ...
  const toggleLearn = () => {
    setShowLearn(prev => {
      const next = !prev;
      if (next) { loadInsights(); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleSetup = () => {
    setShowSetup(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleTeam = () => {
    setShowTeam(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleLab = () => {
  ...
  const toggleGov = () => {
    setShowGov(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowStrat(false); setShowCurve(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleStrat = () => {
    setShowStrat(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowCurve(false); closeChatFocus(); }
      return next;
    });
  };
  // Verlauf-Reiter (Equity-Kurve): lädt seine Daten erst beim Öffnen
  const toggleCurve = () => {
    setShowCurve(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); closeChatFocus(); }
      return next;
    });
  };
