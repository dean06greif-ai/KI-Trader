...
import AIEquityPanel from './AIEquityPanel';
...
import EventScheduleBadges from './EventScheduleBadges';
...
const AITradingPanel = ({ onClose, selectedCoin = 'BTCUSDT' }) => {
  ...
  const [lessonImpact, setLessonImpact] = useState(null);
  const [showLearn, setShowLearn] = useState(false);
  const [learning, setLearning] = useState(false);
  const [reevaluating, setReevaluating] = useState(false);
  // KI-Team (Rollen-Konfiguration)
  const [roles, setRoles] = useState(null);
  // Vom Modell-Wächter neu entdeckte Modelle (bestätigt = auswählbar)
  const [extraModels, setExtraModels] = useState([]);
  // Entdeckte, noch unbestätigte Modelle (Bestätigungs-Button im KI-Team)
  ...
  // Analyse-Unterreiter: nur der aktive Bereich wird gemountet/geladen
  const [diagTab, setDiagTab] = useState('diagnose');
  const [showFomc, setShowFomc] = useState(false);
  ...
  // Der KI-Chat ist die STANDARD-Ansicht: sobald kein Fenster (Lernen, Setup,
  // KI-Team, KI-Labor, MasterPrompt, Strategien) offen ist, zeigt das Panel
  // automatisch wieder den Chat. Erneutes Klicken auf einen aktiven Reiter
  // schließt ihn und führt damit zurück zum Chat.
  const closeChatFocus = () => {
    setShowChatFocus(false);
    try { localStorage.setItem(CHAT_FOCUS_STORE_KEY, '0'); } catch (e) { /* ignore */ }
  };
  // Asset-Fokus ist KEIN Vollfenster: der Chat bleibt sichtbar, es erscheint
  // nur ein Banner mit der Asset-Auswahl oberhalb des Chats.
  const toggleChatFocus = () => {
    setShowChatFocus(prev => {
      const next = !prev;
      try { localStorage.setItem(CHAT_FOCUS_STORE_KEY, next ? '1' : '0'); } catch (e) { /* ignore */ }
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowFomc(false); }
      return next;
    });
  };
  const toggleLearn = () => {
    setShowLearn(prev => {
      const next = !prev;
      if (next) { loadInsights(); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowDiag(false); setShowFomc(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleSetup = () => {
    setShowSetup(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowDiag(false); setShowFomc(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleTeam = () => {
    setShowTeam(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowDiag(false); setShowFomc(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleLab = () => {
    setShowLab(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowDiag(false); setShowFomc(false); closeChatFocus(); }
      return next;
    });
  };
  // Diagnose & Verlauf: Inline-Sektionen im Panel (wie Masterprompt-Tab) –
  // beim Öffnen schließen alle anderen Sektionen, nie zwei gleichzeitig.
  const toggleDiag = () => {
    setShowDiag(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowLab(false); setShowFomc(false); closeChatFocus(); }
      return next;
    });
  };
  // FOMC-Event-Setup: Status, Backtest & Live-Opt-in (Inline-Sektion)
  const toggleFomc = () => {
    setShowFomc(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowLab(false); setShowDiag(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleGov = () => {
    setShowGov(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowStrat(false); setShowCurve(false); setShowDiag(false); setShowFomc(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleStrat = () => {
    setShowStrat(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowCurve(false); setShowDiag(false); setShowFomc(false); closeChatFocus(); }
      return next;
    });
  };
  // Verlauf: Inline-Sektion (Equity-Kurve), lädt Daten erst beim Öffnen
  const toggleCurve = () => {
    setShowCurve(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowDiag(false); setShowFomc(false); closeChatFocus(); }
      return next;
    });
  };
  ...
  const loadInsights = useCallback(async () => {
    ...
    try {
      const li = await fetch(`${API_URL}/api/ai/lessons/impact?days=90`).then(r => r.json());
      setLessonImpact(li && typeof li === 'object' ? li : null);
    } catch (e) { /* silent */ }
  }, []);
  ...
  const loadCatalog = useCallback(async () => {
    try {
      const d = await fetch(`${API_URL}/api/ai/models/catalog`).then(r => r.json());
      setExtraModels((d.discovered || []).map(o => ({
        provider: o.provider, model: o.model,
        label: `NEU · ${o.provider} · ${o.model} (bestätigt)`,
      })));
      setPendingModels(d.pending || []);
    } catch (e) { /* silent */ }
  }, []);
  ...
  const approveModel = async (p, m) => {
    try {
      const res = await fetch(`${API_URL}/api/ai/models/approve`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({ provider: p, model: m }),
      });
      if (!res.ok) { toast.error(res.status === 401 ? 'Admin-Login erforderlich' : 'Bestätigen fehlgeschlagen'); return; }
      toast.success(`${p} · ${m} freigeschaltet – jetzt auswählbar`);
      loadCatalog();
    } catch (e) { toast.error('Verbindungsfehler'); }
  };

  const dismissModel = async (p, m) => {
    try {
      const res = await fetch(`${API_URL}/api/ai/models/dismiss`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({ provider: p, model: m }),
      });
      if (!res.ok) { toast.error(res.status === 401 ? 'Admin-Login erforderlich' : 'Verwerfen fehlgeschlagen'); return; }
      loadCatalog();
    } catch (e) { toast.error('Verbindungsfehler'); }
  };

  const allModelOptions = extraModels.length
    ? [...MODEL_OPTIONS, ...extraModels] : MODEL_OPTIONS;

  useEffect(() => {
    loadStatus(); loadHistory(); loadProposals(); loadInsights(); loadRoles();
    const iv = setInterval(() => {
      if (document.hidden) return; // Tab im Hintergrund: kein Polling (Bandbreite)
      loadStatus(); loadHistory(); loadProposals();
    }, 12000);
  ...
  const chatVisible = !showLearn && !showSetup && !showTeam && !showLab && !showGov && !showStrat && !showCurve && !showDiag && !showFomc;
  useEffect(() => {
  ...
  const reevaluateLessons = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/ai/lessons/reevaluate`, {
        ...
        body: JSON.stringify({ reason: 'Strategie-Änderung (Trader-Knopf)' }),
      });
      const data = await res.json();
      if (data.status === 'ok') {
        toast.success(`Neu bewertet: ${data.checked} geprüft, `
          + `${(data.removed || []).length} veraltet entfernt, `
          + `${(data.adjusted || []).length} angepasst`);
        if ((data.protected || []).length) {
          toast.info(`${data.protected.length} Trader-Lektion(en) blieben geschützt`);
        }
      } else {
        toast.error(data.detail || 'Neubewertung fehlgeschlagen');
      }
      loadInsights(); loadHistory();
    } catch (e) { toast.error('Verbindungsfehler'); }
  ...
  const saveRole = async (roleKey, updates) => {
    try {
      ...
      if (!res.ok) {
        toast.error(res.status === 401 ? 'Admin-Login erforderlich'
          : (data.detail || 'Speichern fehlgeschlagen'));
        return;
      }
      setRoles(data.roles || null);
    } catch (e) { toast.error('Verbindungsfehler'); }
  ...
  const resetRole = async (roleKey) => {
    try {
      ...
      if (!res.ok) {
        toast.error(res.status === 401 ? 'Admin-Login erforderlich'
          : (data.detail || 'Zurücksetzen fehlgeschlagen'));
        return;
      }
      setRoles(data.roles || null);
  ...
  return (
    <div className="ai-panel-overlay" onClick={onClose} data-testid="ai-trading-panel">
      <div className="ai-panel" onClick={e => e.stopPropagation()}>
            ...
            <div>
              ...
              {showHealth && (
                <div className="ai-health-dropdown" data-testid="ai-health-dropdown">
                  ...
                  {Object.entries(status?.providers_health?.key_status || {})
                    .filter(([, v]) => (v.limited || 0) > 0 && (v.limited || 0) >= (v.total || 0))
                    .map(([prov, v]) => (
                      <div className="ai-health-dd-row keys" key={`ks-${prov}`}
                        data-testid={`ai-keystatus-${prov}`}
                        title={(v.limited_keys || []).map(k => `Key ${k.index}: ${k.detail || 'Rate-Limit'}`).join('\n')}>
                        <span className="ai-health-dd-model">{PROVIDER_LABEL[prov] || prov}</span>
                        <span>
                          ALLE {v.total} Key{v.total > 1 ? 's' : ''} im Limit – Backups reichen nicht, Fallback-Modell übernimmt
                          {(() => {
        ...
        {/* Guthaben-Warnung für bezahlte OpenRouter-Keys (Paid-Modelle) */}
        {status?.key_credits?.worst_level && ['warning', 'critical'].includes(status.key_credits.worst_level) && (
          <div className={`ai-warning ${status.key_credits.worst_level === 'critical' ? 'ai-warning-critical' : ''}`} data-testid="ai-key-credits-banner">
            {status.key_credits.worst_level === 'critical' ? '🔴' : '🟡'} Key-Guthaben niedrig (Paid-Modelle):{' '}
            {(status.key_credits.keys || []).filter(k => ['warning', 'critical'].includes(k.level)).map(k => (
              <span key={k.key_masked}>{k.label} ({k.key_masked}): <b>{(k.remaining ?? 0).toFixed(2)} $</b>{' '}</span>
            ))}
            – Aufladen: openrouter.ai/settings/credits (Warnung &lt; {status.key_credits.warn_below_usd} $, kritisch &lt; {status.key_credits.critical_below_usd} $)
          </div>
        )}
        ...
        <div className="ai-status-row" {...statusDrag.props} data-testid="ai-status-row">
          ...
          <button
            className={`ai-setup-toggle ${showCurve ? 'active' : ''}`}
            onClick={toggleCurve}
            title="Verlauf: Equity-Kurve der KI-Trades – wird der KI Trader über die Zeit besser?"
            data-testid="ai-equity-toggle"
          >
          ...
          <button
            ...
            onClick={toggleDiag}
            title="Analyse: Diagnose (Warum laufen die Live-Trades schlecht?), Reward-Programm und Nachanalyse nach Trade-Close – auswertende Sicht auf echte Messdaten."
            data-testid="ai-diagnosis-toggle"
          >
            <Warning size={12} weight="bold" /> Analyse
          </button>
          <button
            className={`ai-setup-toggle ${showFomc ? 'active' : ''}`}
            onClick={toggleFomc}
            title="Event-Setups: FOMC-Zinsentscheid + CPI-/NFP-/PPI-/PCE-Daten mit Backtest vergangener Events, Validierung & Live-Opt-in"
            data-testid="ai-fomc-toggle"
          >
            <Bank size={12} weight="bold" /> Events
          </button>
          <button
        ...
        {showLearn && (
          <div className="ai-learn-panel" data-testid="ai-learn-panel">
            ...
            <div className="ai-learn-head" style={{ position: 'static', margin: '8px 0 4px', padding: 0, background: 'transparent', border: 'none' }}>
              ...
              <button className="ai-action-btn" onClick={reevaluateLessons} disabled={reevaluating}
                title="Lern-Reset: Lektionen gegen die aktuelle Strategie neu bewerten (nach Strategie-Änderungen). Trader-Lektionen bleiben geschützt."
                data-testid="ai-lessons-reevaluate-btn">
                <ArrowsClockwise size={13} weight="bold" className={reevaluating ? 'spin' : ''} />
                {reevaluating ? ' Bewerte…' : ' Neu bewerten'}
              </button>
            ...
            {(insights?.lessons || []).length > 0 ? (
              <>
              <LessonImpactSummary impact={lessonImpact} />
                    ...
                    {editLesson && l.id && editLesson.id === l.id ? (
                      ...
                      <div className="ai-lesson-row" style={(l.superseded || l.status === 'dormant') ? { opacity: 0.45 } : undefined}>
                        <div className="ai-lesson-body" style={l.superseded ? { textDecoration: 'line-through' } : undefined}>
                          ...
                          {lessonImpact?.rows && (
                            <LessonImpactBadge row={lessonImpact.rows.find(r => r.id === l.id)}
                              recent={lessonImpact.recent_prevented} onReload={loadInsights} />
                          )}
                        </div>
                        {l.status === 'dormant' ? (
        ...
        {/* Verlauf: Equity-Kurve – Inline-Sektion im Panel (wie Masterprompt-Tab) */}
        {showCurve && <AIEquityPanel />}

        {/* Analyse: Diagnose + Reward-Programm + Nachanalyse + Policy-Vergleich – Inline-Sektion */}
        {showDiag && (
          <div className="ai-analysis-section" data-testid="ai-analysis-section">
            {/* Unterreiter statt 5 schwerer Panels auf einmal: nur der aktive
                Bereich wird gemountet und lädt seine Daten (Bandbreite/Tempo) */}
            <div className="ai-analysis-tabs" data-testid="ai-analysis-tabs">
              {[['diagnose', 'Diagnose'],
                ['regime', 'Regime-Cockpit'],
                ['reward', 'Reward'],
                ['policy', 'Policy-Versionen'],
                ['postmortem', 'Nachanalyse']].map(([k, l]) => (
                <button key={k} type="button"
                  className={`ai-analysis-tab ${diagTab === k ? 'active' : ''}`}
                  onClick={() => setDiagTab(k)}
                  data-testid={`ai-analysis-tab-${k}`}>{l}</button>
              ))}
            </div>
            {/* Diagnose: Warum laufen die Live-Trades schlecht? */}
            {diagTab === 'diagnose' && <AIDiagnosisPanel />}
            {/* Regime-Cockpit: beide Regime-Ebenen live + KI-Trades + Vorwärts-Trefferquote (PLAN_REGIME_COCKPIT) */}
            {diagTab === 'regime' && <RegimeCockpit />}
            {/* Belohnungssystem: Reward-Verlauf + Auswertung pro Markt-Regime */}
            {diagTab === 'reward' && <AIRewardPanel />}
            {/* Policy-Versionen: Netto-Vergleich der Regelwerke (vom Dashboard hierher verschoben) */}
            {diagTab === 'policy' && <PolicyReportCard />}
            {/* Nachanalyse: Was-wäre-wenn nach Trade-Close (TP/SL/Runner/Limit) mit Overfitting-Schutz */}
            {diagTab === 'postmortem' && <AIPostmortemPanel />}
          </div>
        )}

        {/* Event-Zeitplan (nur Info) – Backtest & KI-Schleife im Backtester-Tab „Event-Setups“ */}
        {showFomc && (
          <div data-testid="ai-event-setups-section">
            <EventScheduleBadges />
          </div>
        )}
        ...
        {showSetup && (
          <div className="ai-setup" data-testid="ai-setup-panel">
            ...
            <label title="Autonomie-Leitplanke: Spanne, in der die KI den Richtungs-Guard (Max. gleiche Richtung) für optimalen Profit SELBST anpassen darf. Außerhalb der Spanne (oder 0/aus) wird jede Änderung nur ein Vorschlag, den du bestätigen musst. Im Datensammel-Modus ist der Guard ohnehin ausgesetzt. Die Spanne selbst kann die KI nie ändern.">
              <span>KI-Spanne Richtungs-Guard</span>
              <span style={{ display: 'inline-flex', gap: 4, alignItems: 'center' }}>
                <select value={cfg.tune_guard_min ?? 1} onChange={e => updateConfig({ tune_guard_min: Number(e.target.value) })} data-testid="ai-tune-guard-min-select">
                  {[1, 2, 3, 4].map(v => <option key={v} value={v}>{v}</option>)}
                </select>
                –
                <select value={cfg.tune_guard_max ?? 6} onChange={e => updateConfig({ tune_guard_max: Number(e.target.value) })} data-testid="ai-tune-guard-max-select">
            ...
            <label title="Stale-Price-Guard: blockt KI-Einstiege, wenn die letzte Kurskerze des Coins älter ist als das Limit (hängender Datenfeed, z.B. Forex-Feed am Wochenende). 0 = aus.">
              ...
              <select value={cfg.stale_price_max_min ?? 10} onChange={e => updateConfig({ stale_price_max_min: Number(e.target.value) })} data-testid="ai-stale-price-select">
                {[0, 3, 5, 10, 15, 30].map(v => <option key={v} value={v}>{v === 0 ? 'aus' : `${v} min`}</option>)}
              </select>
            </label>
            <label title="Slippage-Wächter: Momentum-Setups (momentum_news, breakout, squeeze_breakout) gehen nur live, wenn die zuletzt GEMESSENE Entry-Slippage des Coins (Ø der letzten 14 Tage, min. 3 Live-Trades) unter dem Limit liegt. Befund 02.09.: squeeze_breakout Paper 48 % / live 7 % Winrate – Ursache Slippage (DOGE 0,86 %, GOLD 1,16 %), nicht Strategie. Sammel-Trades laufen weiter. 0 = aus.">
              <span>Slippage-Wächter (Ø %)</span>
              <select value={cfg.slippage_guard_max_pct ?? 0.3} onChange={e => updateConfig({ slippage_guard_max_pct: Number(e.target.value), slippage_guard_enabled: Number(e.target.value) > 0 })} data-testid="ai-slippage-guard-select">
                {[0, 0.15, 0.2, 0.3, 0.5, 1.0].map(v => <option key={v} value={v}>{v === 0 ? 'aus' : `${v} %`}</option>)}
              </select>
            </label>
            <label title="Slippage-Wächter auf ALLE Setups anwenden (Standard: nur Momentum-Setups, weil dort in die Bewegung hinein gekauft wird).">
              <span>Slippage: alle Setups</span>
              <select value={cfg.slippage_guard_all_setups ? 'on' : 'off'} onChange={e => updateConfig({ slippage_guard_all_setups: e.target.value === 'on' })} data-testid="ai-slippage-guard-all-select">
                <option value="off">nur Momentum</option>
                <option value="on">alle</option>
              </select>
            </label>
            <label title="Regime-Sperrfilter: blockt LIVE-Neueinstiege, solange der Coin in einem unklaren Regime steckt (Standard-Liste: range_ruhig – dort historisch nur ~10% Winrate). Sammel-Trades laufen bewusst weiter, damit die Statistik den Filter beweisen kann. Empfehlung: erst nach 1–2 Wochen sauberer Nach-Guard-Daten aktivieren.">
              <span>Regime-Sperre</span>
              ...
              <>
                ...
                <label title="Hebel-Deckel im Risiko-Modus (1–200). Der Hebel wird pro Trade so gewählt, dass die Liquidation den Coin-'Liq-Abstand hinter SL' einhält – nie höher als dieser Deckel bzw. das Coin-Maximum.">
                  ...
                  <input type="number" min={1} max={200} step={1} style={{ width: 64 }}
                    key={`rml-${cfg.risk_max_leverage ?? 15}`}
                    defaultValue={cfg.risk_max_leverage ?? 15}
                    onBlur={e => {
                      const v = parseInt(e.target.value, 10);
                      if (Number.isNaN(v)) { e.target.value = String(cfg.risk_max_leverage ?? 15); return; }
                      if (v !== (cfg.risk_max_leverage ?? 15)) updateConfig({ risk_max_leverage: v });
                    }}
            ...
            <label title="Entry-Modus: KI entscheidet (bisher) = Limit nur bei klarem Key-Level, sonst Market. A – Aggressiv = bei Level-Setups (Order-Block, Range-Kante, Pullback, Liquidity-Sweep, Mean-Reversion) Limit direkt am erwarteten Reversal-Level: besserer Preis, keine Slippage, aber Fill-Risiko. B – Konservativ = immer Market erst nach Bestätigung (z.B. Sweep → Kerze schließt wieder in der Range): hohe Fill-Sicherheit, schlechterer Preis. Momentum-Setups (Breakout, Squeeze, News, TF2, Session-Open) bleiben in jedem Modus Market.">
              <span><Lightning size={13} /> Entry-Modus (Limit/Market)</span>
              <select value={cfg.entry_mode || 'ai'} onChange={e => updateConfig({ entry_mode: e.target.value })} data-testid="ai-entry-mode-select">
                <option value="ai">KI entscheidet (bisher)</option>
                <option value="aggressive">A – Aggressiv: Limit am Level</option>
                <option value="conservative">B – Konservativ: Market nach Bestätigung</option>
              </select>
            </label>
            ...
            <label className="ai-setup-check" title="Runner-Trades (nur Teil-TP, Rest läuft mit Trailing) sichern sich nach der Gewinnschwelle automatisch ab: SL in den Gewinn + Marge maximal freisetzen – der Hebel steigt bei gleicher Positionsgröße, der Trade wird risikofrei und das Kapital wird für andere Trades frei">
              <span>Runner risikofrei stellen</span>
              <input type="checkbox" checked={cfg.runner_secure_enabled !== false}
                onChange={e => updateConfig({ runner_secure_enabled: e.target.checked })} data-testid="ai-runner-secure-toggle" />
            </label>
            {cfg.runner_secure_enabled !== false && (
              <label title="Ziel-Hebel beim Marge-Freisetzen (200 = Marge maximal reduzieren)">
                <span>Runner Ziel-Hebel</span>
                <select value={cfg.runner_secure_max_leverage || 100}
                  onChange={e => updateConfig({ runner_secure_max_leverage: Number(e.target.value) })}
                  data-testid="ai-runner-secure-maxlev-select">
                  {[25, 50, 75, 100, 125, 150, 200].map(v => <option key={v} value={v}>{v}x</option>)}
                </select>
              </label>
            )}
            ...
            {/* Guard-Cockpit: welcher Wächter hat zuletzt wie viele Einstiege verhindert */}
            {guardStats && (
              <div style={{ gridColumn: '1 / -1' }} data-testid="ai-guard-cockpit">
                <div className="ai-learn-title" style={{ margin: '6px 0 4px' }}>
                  <Prohibit size={13} weight="fill" /> Guard-Cockpit – geblockte Einstiege ({guardStats.days} Tage)
                </div>
                {(guardStats.guards || []).filter(g => g.count > 0).length === 0 ? (
                  <div style={{ fontSize: 12, opacity: 0.7 }} data-testid="ai-guard-cockpit-empty">
                    Keine Blocks im Zeitraum.
                  </div>
                ) : (guardStats.guards || []).filter(g => g.count > 0).map(g => (
                  <div key={g.key} style={{ fontSize: 12, opacity: 0.9, marginBottom: 2 }}
                    title={g.last ? `Zuletzt: ${g.last.symbol || ''} ${g.last.action || ''} – ${g.last.reason || ''}` : ''}
                    data-testid={`ai-guard-row-${g.key}`}>
                    <b>{g.label}</b>: {g.count}×{g.collection ? ` (davon ${g.collection} Sammel)` : ''}
                  </div>
                ))}
                {guardStats.regime_shadow && !guardStats.regime_shadow.enabled && (
                  <div style={{ fontSize: 11, opacity: 0.6, marginTop: 2 }} data-testid="ai-guard-regime-shadow"
                    title="Schatten-Zähler: So viele LIVE-Einstiege hätte der (deaktivierte) Regime-Sperrfilter im Zeitraum geblockt. Hilft zu entscheiden, ob sich das Aktivieren lohnt.">
                    Regime-Sperre (aus): hätte {guardStats.regime_shadow.would_block_live} Live-Einstiege geblockt ({(guardStats.regime_shadow.list || []).join(', ')})
                  </div>
                )}
              </div>
            )}
                  ...
                  <div className="ai-role-grid">
                    ...
                    {rd.key === 'news_watcher' && (
                      <>
                        ...
                        <label className="ai-setup-check" title="Bei Medium/High-Impact-News zusätzlich eine Tiefenanalyse auslösen (analysiert Ergebnis vs. Erwartung + Prognose, max. 1x pro 45 min)">
                          <span>Tiefenanalyse bei News</span>
                          <input type="checkbox" checked={rc.deep_on_news !== false}
                            onChange={e => saveRole('news_watcher', { deep_on_news: e.target.checked })}
                            data-testid="ai-role-news-deepanalysis" />
                        </label>
                      </>
                    )}
                  </div>
        ...
        {proposals.length > 0 && !showLab && !showGov && !showStrat && !showTeam && !showCurve && (
        ...
        {/* Chat + Input – STANDARD-Ansicht: immer sichtbar, wenn kein
            Vollfenster (Lernen/Setup/Team/Labor/MasterPrompt/Strategien) offen ist.
            Beim Asset-Fokus bleibt der Chat offen (Banner erscheint darüber). */}
        {chatVisible && (
        <>
