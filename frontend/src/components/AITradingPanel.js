...
import { Robot, PaperPlaneRight, X, Trash, ArrowsClockwise, Lightning, CaretDown, CaretUp, Newspaper, PushPin, Brain, GraduationCap, CheckCircle, XCircle, Sliders, Coins, Flask, ArrowCounterClockwise, PencilSimple, Crown, Plus, FloppyDisk, Warning, ArrowDown, Prohibit, ChartLineUp } from '@phosphor-icons/react';
...
const AITradingPanel = ({ onClose, selectedCoin = 'BTCUSDT' }) => {
  ...
  const toggleChatFocus = () => {
    setShowChatFocus(prev => {
      ...
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); }
      return next;
    });
  };
  const toggleLearn = () => {
    setShowLearn(prev => {
      const next = !prev;
      if (next) { loadInsights(); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowDiag(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleSetup = () => {
    setShowSetup(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowDiag(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleTeam = () => {
    setShowTeam(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowCurve(false); setShowDiag(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleLab = () => {
  ...
  const toggleGov = () => {
    setShowGov(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowStrat(false); setShowCurve(false); setShowDiag(false); closeChatFocus(); }
      return next;
    });
  };
  const toggleStrat = () => {
    setShowStrat(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowCurve(false); setShowDiag(false); closeChatFocus(); }
      return next;
    });
  };
  ...
  const toggleCurve = () => {
    setShowCurve(prev => {
      const next = !prev;
      if (next) { setShowLearn(false); setShowSetup(false); setShowTeam(false); setShowLab(false); setShowGov(false); setShowStrat(false); setShowDiag(false); closeChatFocus(); }
      return next;
    });
  };
  ...
  return (
    <div className="ai-panel-overlay" onClick={onClose} data-testid="ai-trading-panel">
      <div className="ai-panel" onClick={e => e.stopPropagation()}>
        ...
        {/* Diagnose: Warum laufen die Live-Trades schlecht? – Inline-Sektion */}
