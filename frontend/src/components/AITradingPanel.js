...
const AITradingPanel = ({ onClose, selectedCoin = 'BTCUSDT' }) => {
  ...
  // Vom Modell-Wächter neu entdeckte Modelle (zusätzlich auswählbar)
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/ai/models/catalog`).then(r => r.json())
      .then(d => setExtraModels((d.discovered || []).map(o => ({
        provider: o.provider, model: o.model,
        label: `NEU · ${o.provider} · ${o.model} (vom Modell-Wächter entdeckt)`,
      }))))
      .catch(() => {});
  }, []);
