    ...
    <div className="ai-reward-panel" data-testid="ai-reward-panel">
      ...
      {hist.length > 1 ? (
        ...
        <div className="reward-empty">
          ...
          belohnt/bestraft (R-Basis, Win/Loss, Sofort-Stop-Outs, CRV-Disziplin).
        </div>
      )}
      {regimes.length > 0 && (
      ...
      {calib?.calibration_error !== null && calib?.calibration_error !== undefined && (
        <div className="reward-regimes" data-testid="reward-calibration">
          <div className="reward-sub" title="Getrennt vom Reward (Audit 2.7): |Ø-Konfidenz − echte Winrate| je Konfidenz-Bin. 0 = perfekt kalibriert; große Werte = die KI über-/unterschätzt sich.">
            Konfidenz-Kalibrierung · Ø-Fehler{' '}
            <b data-testid="reward-calibration-error"
              className={calib.calibration_error <= 15 ? 'pos' : 'neg'}>
              {calib.calibration_error.toFixed(1)} Pkt.
            </b>{' '}({calib.rated} Trades)
          </div>
          <table>
            <thead>
              <tr><th>Konfidenz-Bin</th><th>Trades</th><th>Ø Konfidenz</th><th>echte Winrate</th><th>Abweichung</th></tr>
            </thead>
            <tbody>
              {(calib.bins || []).map(b => (
                <tr key={b.bin} data-testid={`reward-calib-bin-${b.bin}`}>
                  <td>{b.bin}%</td>
                  <td>{b.trades}</td>
                  <td>{b.avg_confidence}%</td>
                  <td>{b.win_rate}%</td>
                  <td className={b.gap <= 15 ? 'pos' : 'neg'}>{b.gap.toFixed(1)}</td>
                </tr>
              ))}
            </tbody>
          </table>
