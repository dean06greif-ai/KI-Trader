...
const AILabPanel = () => {
  ...
  const openTrash = async () => {
    ...
    if (next) {
      try {
        const d = await fetch(`${API_URL}/api/ai/lab/trash`).then(r => r.json());
  ...
  const restoreTrash = async (id) => {
    setShowTrash(false);
    setBusy('Wiederherstellen');
    try {
      const res = await fetch(`${API_URL}/api/ai/lab/trash/restore`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
      ...
      toast.success(`Wiederhergestellt: ${data.label || data.kind}`);
      load();
  ...
  return (
    <div className="ai-lab-panel" data-testid="ai-lab-panel">
        ...
        <div className="ai-lab-actions">
          ...
          <div className="ai-lab-trash-wrap">
            <button className="ai-action-btn ai-lab-trash-btn" disabled={!!busy}
              ...
              title="Papierkorb: Forschungs- oder ML-Trainingsdaten zurücksetzen & gelöschte Stände wiederherstellen">
              <Trash size={13} weight="bold" />
            </button>
            {showTrash && (
              <div className="ai-lab-trash-menu" data-testid="ai-lab-trash-menu">
                <button disabled={research.running_now}
                  onClick={() => { setShowTrash(false); if (window.confirm('Forschungs-Daten (Report & Zustand) in den Papierkorb verschieben? Der nächste Lauf startet frisch – Wiederherstellen ist jederzeit über den Papierkorb möglich.')) post('/api/ai/research/reset', 'Forschung in den Papierkorb verschoben'); }}
                  data-testid="ai-lab-research-reset-btn">
                  Forschungsdaten zurücksetzen
                </button>
                <button disabled={ml.training_now}
                  onClick={() => { setShowTrash(false); if (window.confirm('ML-Trainingsdaten (Modell + Status) in den Papierkorb verschieben? Das nächste Training baut neu auf – Wiederherstellen ist jederzeit über den Papierkorb möglich.')) post('/api/ai/ml/reset', 'ML-Modell in den Papierkorb verschoben'); }}
                  data-testid="ai-lab-ml-reset-btn">
                  ML-Trainingsdaten zurücksetzen
                </button>
                {trashItems.length > 0 && (
                  <div className="ai-lab-trash-items" data-testid="ai-lab-trash-items">
                    <div className="ai-lab-trash-sub">Papierkorb – wiederherstellbar</div>
                    {trashItems.map(it => (
                      <div key={it.id} className="ai-lab-trash-item" data-testid={`ai-lab-trash-item-${it.kind}`}>
                        <span title={`Gelöscht am ${new Date(it.deleted_at).toLocaleString('de-DE')}`}>
                          {it.label || (it.kind === 'ml' ? 'ML-Modell' : 'Forschung')}
                        </span>
                        <button onClick={() => restoreTrash(it.id)}
      ...
      <div className="ai-lab-tabs">
        {[['research', 'Forschung'], ['ml', 'ML-Modell'], ['gate', 'Gate v1'], ['trades', 'Trade-Steuerung'], ['adaptive', 'Adaptiv'], ['memory', 'Gedächtnis'], ['market', 'Markt'], ['storage', 'Speicher']].map(([k, label]) => (
