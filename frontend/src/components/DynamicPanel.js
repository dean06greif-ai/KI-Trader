...
export default function DynamicPanel() {
  ...
  const confirmSwitch = async (id, action, commandId) => {
    if (!isAdmin()) { toast.error('Admin-Login erforderlich'); return; }
    setBusy(b => ({ ...b, [id]: action }));
    try {
      const r = await fetch(`${API_URL}/api/dynamic/${id}/${action}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: action === 'confirm' ? JSON.stringify({ command_id: commandId || null }) : '{}',
      });
      const d = await r.json();
      if (!r.ok) { toast.error(d.detail || 'Aktion fehlgeschlagen'); load(); return; }
  ...
  const approve = async (id) => {
    ...
    try {
      const r = await fetch(`${API_URL}/api/dynamic/${id}/approve`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: '{}',
      });
      const d = await r.json();
      if (!r.ok) {
        if (r.status === 409 && (d.detail || '').includes('Testnachweis fehlt')) {
          const note = window.prompt(
            `${d.detail}\n\nBegründung für die Freigabe OHNE Testnachweis eingeben (Abbrechen = keine Freigabe):`);
          if (note && note.trim()) {
            const r2 = await fetch(`${API_URL}/api/dynamic/${id}/approve`, {
              method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders() },
              body: JSON.stringify({ override_missing_evidence: true, note: note.trim() }),
            });
            const d2 = await r2.json();
            if (!r2.ok) { toast.error(d2.detail || 'Freigabe fehlgeschlagen'); return; }
            toast.success('Ohne Testnachweis freigegeben – Begründung protokolliert');
            load();
          }
          return;
        }
        toast.error(d.detail || 'Freigabe fehlgeschlagen'); return;
      }
      toast.success('Strategie ausdrücklich freigegeben');
      load();
    } catch { toast.error('Verbindungsfehler'); }
    finally { setBusy(b => ({ ...b, [id]: null })); }
  };
  ...
  const remove = async (id) => {
    ...
    if (!window.confirm(`Dynamische Strategie „${s?.name || id}“ wirklich löschen?\nRegime-Zuordnungen, Wechsel-Protokoll und Freigabe-Status gehen dauerhaft verloren – das kann NICHT rückgängig gemacht werden.`)) return;
    try {
            ...
            return (
              <div key={s.id} className="dyn-card" data-testid={`dyn-card-${s.id}`}>
                <div className="dyn-card-head">
                  ...
                  {(s.release_status === 'draft' || s.release_status === 'stale') && (
                    <span className="opt-badge bad" data-testid={`dyn-release-${s.id}`}
                      title={s.release_status === 'draft'
                        ? 'Entwurf ohne bestandene Validierung – wird nicht live aktiviert, bis Walkforward bestanden oder ausdrücklich freigegeben wurde'
                        : 'Definition wurde nach der Validierung/Freigabe geändert – erneut validieren oder freigeben'}>
                      {s.release_status === 'draft' ? 'Entwurf – nicht validiert' : 'Validierung veraltet'}
                    </span>
                  )}
                  {s.application_status?.status === 'failed' && (
                    <span className="opt-badge bad" data-testid={`dyn-apply-status-${s.id}`}
                      title={`Letzte automatische Übernahme FEHLGESCHLAGEN (Versuch ${s.application_status.attempts || 1}, ${fmtDateTime(s.application_status.at)}): ${s.application_status.error || 'unbekannter Fehler'} – wird im nächsten Prüf-Zyklus erneut versucht. Beobachten ist nicht Anwenden: das angezeigte Regime ist noch NICHT als Konfiguration aktiv.`}>
                      Übernahme fehlgeschlagen – Retry
                    </span>
                  )}
                  {s.application_status?.status === 'blocked' && (
                    <span className="opt-badge bad" data-testid={`dyn-apply-status-${s.id}`}
                      title={`Automatische Übernahme blockiert (${fmtDateTime(s.application_status.at)}): ${s.application_status.error || 'Release nicht freigegeben'}`}>
                      Übernahme blockiert
                    </span>
                  )}
                  <span style={{ flex: 1 }} />
                  ...
                  {(s.release_status === 'draft' || s.release_status === 'stale') && (
                    <button className="opt-chip" onClick={() => approve(s.id)} disabled={!!busy[s.id]}
                      data-testid={`dyn-approve-${s.id}`}
                      title="Ausdrückliche Freigabe: erlaubt Übernahme/Auto-Übernahme trotz fehlender bzw. veralteter Validierung">
                      {busy[s.id] === 'approve' ? 'Gebe frei...' : 'Freigeben'}
                    </button>
                  )}
                  ...
                  <button className="opt-chip" onClick={() => remove(s.id)} data-testid={`dyn-delete-${s.id}`}
                    title="Dynamische Strategie löschen (mit Sicherheitsabfrage)">
                    <Trash size={12} />
                  </button>
                </div>
                ...
                {s.pending_switch && (
                  <div className="dyn-pending" data-testid={`dyn-pending-${s.id}`}>
                    ...
                    <span style={{ flex: 1 }} />
                    <button className="opt-chip" onClick={() => confirmSwitch(s.id, 'confirm', s.pending_switch.command_id)}
                ...
                {s.last_applied && (
                  <div className="opt-small" data-testid={`dyn-last-applied-${s.id}`}>
                    Zuletzt übernommen: {fmtDateTime(s.last_applied)}
                    {s.application_status?.status === 'applied' && s.application_status?.desired_version
                      ? <span title="Der zuletzt beobachtete Regime-Zustand wurde tatsächlich als Konfiguration angewendet (Beobachten ≠ Anwenden)"> · Stand angewendet ✓</span>
                      : null}
                  </div>
                )}
