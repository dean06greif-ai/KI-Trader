...
import { JobProgress, useLabJobControls } from './RegimeJobProgress';
...
export default function RegimeOptimizePanel({ analysisId, scope, symbol, regime,
  ...
  // Pausieren / Fortsetzen / Suche beenden / Notfall-Reset – wie im Strategie-Optimizer
  const controls = useLabJobControls(job, {
    setJob, onReset: () => { clearInterval(pollRef.current); setJob(null); },
  });
  ...
  return (
    <div className="rl-opt-box" data-testid={`regime-opt-panel-${regime.id}`}>
      ...
      {running && (
        <JobProgress job={job} testId={`regime-opt-progress-${regime.id}`}
          onPause={controls.togglePause} onStop={controls.softStop}
          onCancel={cancel} onReset={controls.reset} />
      )}
      {result && (
        <div className="opt-result" data-testid={`regime-opt-result-${regime.id}`}>
          {result.stopped_early && (
            <div className="opt-small" style={{ marginBottom: 6, color: '#ffa502' }}
              data-testid={`regime-opt-stopped-early-${regime.id}`}>
              Suche vorzeitig beendet – angezeigt wird das bis dahin Beste.
            </div>
          )}
