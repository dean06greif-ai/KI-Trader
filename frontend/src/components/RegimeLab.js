...
function AnalysisDetail({ analysis, strategies, jobBlocked, execution, onChanged }) {
  ...
  // Je-Asset-Knöpfe für ALLE angefragten Symbole: fehlt ein Coin-Modell (zu wenig
  // Daten / Fehler), erscheint der Knopf deaktiviert MIT Grund – statt still zu fehlen.
  const missing = [];
  const requested = analysis.symbols_requested?.length
    ? Array.from(new Set([...(analysis.symbols || []), ...analysis.symbols_requested]))
    : (analysis.symbols || []);
  requested.forEach(s => {
    const pc = analysis.per_coin?.[s];
    if (pc && !pc.error) {
      scopes.push({ id: `coin:${s}`, label: s.replace('USDT', '') });
      return;
    }
    const skipped = analysis.symbols_skipped?.[s];
    const noData = !(analysis.symbols || []).includes(s);
    // Bewusst nur kombiniert gerechnet (Modell-Umfang) und Daten vorhanden: kein Hinweis nötig
    if (!skipped && !pc?.error && !noData && analysis.scope === 'combined') return;
    const reason = skipped?.reason || pc?.error
      || (noData ? 'keine ausreichenden Kerzendaten geladen' : 'kein Einzel-Modell vorhanden');
    missing.push({ symbol: s, reason });
  });
  ...
  return (
    <div data-testid="regime-analysis-detail">
      {(scopes.length > 1 || missing.length > 0) && (
        <div className="rl-scope-tabs" data-testid="regime-scope-tabs"
          ...
          {missing.map(m => (
            <button key={m.symbol} className="opt-chip rl-scope-tab-missing" disabled
              title={`${m.symbol.replace('USDT', '')}: ${m.reason} – für ein Einzel-Modell Zeitraum/Timeframe anpassen oder Modell-Umfang „Beides“ wählen und neu „Regime suchen & speichern“`}
              data-testid={`regime-scope-tab-missing-coin:${m.symbol}`}>
              {m.symbol.replace('USDT', '')} · kein Modell
            </button>
          ))}
        </div>
      )}
      {missing.length > 0 && (
        <div className="opt-small" style={{ margin: '2px 0 6px', opacity: 0.85 }} data-testid="regime-scope-missing-note">
          ⚠ Ohne Einzel-Modell: {missing.map(m => `${m.symbol.replace('USDT', '')} (${m.reason})`).join(' · ')}
        </div>
      )}
          ...
          ? ` Engine v2 · ${(model?.regime_mode || model?.config?.regime_mode || 9)}-Regime-Modus`
            + ((model?.config?.detector || 'reactive') === 'ema'
              ...
              ? ` · Kombi-Detektor (EMA ${fmt(model?.config?.kombi_ema_days ?? 14, 0)} Tage`
                ...
                + ` · Mini-Phasen-Filter ${model?.config?.min_phase_days ? `${fmt(model.config.min_phase_days, 1)}d` : 'auto'}`
              : (model?.config?.detector || 'reactive') === 'jump'
              ? ` · Jump-Modell (Features ${model?.config?.jump_fast_days ?? 1}d/${model?.config?.jump_slow_days ?? 14}d`
                + `, Zentrum ${model?.config?.jump_center ?? 0.6}×Vola`
                + `, Sprungkosten ${model?.config?.jump_penalty_days ?? 1.5}d)`
                + ` · Mini-Phasen-Filter ${model?.config?.min_phase_days ? `${fmt(model.config.min_phase_days, 1)}d` : 'auto'}`
...
export default function RegimeLab({ onClose }) {
  ...
  const [calibApplied, setCalibAppliedRaw] = useState(saved.calibApplied || null);
  const [historyRefresh, setHistoryRefresh] = useState(0);
  // Vor JEDER Übernahme (Kalibrierung, Autopilot, Verlauf) den bisherigen Stand
  // serverseitig sichern – so ist nichts mehr „weg“, wenn überschrieben wird.
  const engineStateRef = useRef({ engineConfig: saved.engineConfig || {}, calibApplied: saved.calibApplied || null });
  const setCalibApplied = useCallback((next) => {
    const prev = engineStateRef.current;
    saveEngineSnapshot(prev.engineConfig, prev.calibApplied, next?.source).then(ok => { if (ok) setHistoryRefresh(k => k + 1); });
  ...
  useEffect(() => {
    engineStateRef.current = { engineConfig, calibApplied };
    try {
  ...
  const removeAnalysis = async (aid, e) => {
    ...
    const rel = a?.release?.stage && a.release.stage !== 'none' ? `\n\nAchtung: Diese Analyse ist freigegeben (Stufe „${a.release.stage}“) – die Freigabe geht mit verloren.` : '';
    if (!window.confirm(`Regime-Analyse „${a?.name || aid}“ wirklich löschen?\nModell, Nachweise, Zuordnungen und Walk-Forward-Ergebnis werden dauerhaft entfernt – das kann NICHT rückgängig gemacht werden.${rel}`)) return;
    ...
    <SafeOverlay className="opt-overlay" onClose={onClose} testId="regime-lab-overlay">
      <div className="opt-panel" onClick={e => e.stopPropagation()} data-testid="regime-lab-modal">
        ...
        <div className="opt-row">
          ...
          {selected && detail && (
            <div className="rl-edge-banner neutral" data-testid="regime-base-banner"
              ...
              <span>
                <b>Ausgewählte Analyse (unten): „{detail.name || 'Unbenannt'}“</b>
        ...
        <RegimeAutopilot selCoins={selCoins} timeframe={timeframe} days={days} trainPct={trainPct}
          engineConfig={engineConfig} setEngineConfig={setEngineConfig}
          calibApplied={calibApplied} setCalibApplied={setCalibApplied}
        ...
        <div className="opt-row">
          ...
          {(analyses || []).map(a => (
            <div key={a.id} className={`rl-analysis-row ${selected === a.id ? 'on' : ''}`}
              ...
              <AnalysisRename analysis={a} onRenamed={() => { loadList(); if (selected === a.id) loadDetail(a.id); }} />
              <button className="opt-chip" onClick={(e) => removeAnalysis(a.id, e)} data-testid={`regime-analysis-delete-${a.id}`}
                title="Analyse löschen (mit Sicherheitsabfrage)">
                <Trash size={11} />
              </button>
            </div>
          ))}
