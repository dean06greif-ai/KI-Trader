...
function AblationCompare({ selCoins, timeframe, days, trainPct, engineConfig, jobBlocked }) {
  ...
  const start = async () => {
    ...
    try {
      const r = await fetch(`${API_URL}/api/regime-lab/ablation`, {
  ...
  return (
    <div className="opt-row" data-testid="ablation-section">
      <div className="opt-label">INDIKATOR-ABLATION (DETEKTOR-KOMPONENTEN)</div>
      <div className="opt-small" style={{ marginBottom: 6 }}>
        Derselbe Datensatz, der oben konfigurierte Detektor voll vs. ohne je eine
        Bestätigungs-Komponente vs. einfache Alternative. So siehst du, welche
        Komponente wirklich beiträgt und welche redundant ist – bewertet auf der
        inneren Validierung, der Holdout bleibt finaler Test.
      </div>
      <div className="opt-setup" style={{ alignItems: 'center' }}>
        <button className="opt-chip" onClick={start} disabled={!!job || jobBlocked}
          data-testid="ablation-run">
          <Play size={11} weight="fill" /> Ablation starten
        </button>
        {job && (
          <span className="opt-small" data-testid="ablation-status">
            {job.phase} · {job.progress ?? 0}%
            <button className="opt-chip" onClick={cancel} style={{ marginLeft: 6 }}
              data-testid="ablation-cancel">Abbrechen</button>
          </span>
        )}
      </div>
      {result && (
        <div style={{ overflowX: 'auto', marginTop: 6 }}>
          <table className="rl-compare-table" data-testid="ablation-table"
            ...
            <thead>
              <tr style={{ textAlign: 'left', opacity: 0.7 }}>
                <th style={{ padding: '3px 10px 3px 0' }}>Variante</th>
                ...
                <th style={{ padding: '3px 10px' }} title="Innere Validierung (letzter Teil des Trainingsfensters) – Bewertungsbasis">Innere Val.</th>
                ...
                <th style={{ padding: '3px 10px' }} title="Beitrag der entfernten Komponente: volle Konfiguration minus diese Variante (innere Validierung)">Beitrag</th>
              </tr>
            </thead>
            <tbody>
              {result.rows.map(r => (
                <tr key={r.variant_key} data-testid={`ablation-row-${r.variant_key}`}
                  style={r.variant_key === result.best_variant
                    ? { background: 'rgba(80,200,120,0.12)' } : undefined}>
                  <td style={{ padding: '3px 10px 3px 0' }}>
                    <b>{r.name}</b>{r.variant_key === result.best_variant ? ' ★' : ''}
                  </td>
                  {r.error ? <td colSpan={6} className="neg">{r.error}</td> : (
                    <>
                      ...
                      <td style={{ padding: '3px 10px' }}
                        className={verdictOf(r.variant_key)?.cls || ''}>
                        {r.variant_key === 'full' ? '–'
          ...
          <div className="opt-small" style={{ marginTop: 4 }} data-testid="ablation-selection-note">
...
export default function RegimeLab({ onClose }) {
  ...
  const queueAnalysisToSeries = async () => {
    if (!isAdmin()) { toast.error('Admin-Login erforderlich'); return; }
    if (!selCoins.length) { toast.error('Mindestens 1 Coin wählen'); return; }
    await addToSeries('regime_analysis', buildAnalysisBody());
  };
