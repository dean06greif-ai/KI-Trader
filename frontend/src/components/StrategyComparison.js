...
export default function StrategyComparison({ onClose }) {
  ...
  // Datensammel-Trades (KI-Trader) und Karteileichen (gelöschte Strategien)
  // sind standardmäßig ausgeblendet – auf Wunsch zuschaltbar
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const d = await fetch(`${API_URL}/api/analytics/strategy-comparison?mode=${mode}&days=${days}`
        + `&include_collection=${includeCollection}&include_stale=${includeStale}`).then(r => r.json());
      setData(d);
    } catch (e) { console.error(e); }
    setLoading(false);
  }, [mode, days, includeCollection, includeStale]);
    ...
    <SafeOverlay className="sc-overlay" onClose={onClose}>
      <div className="sc-panel" onClick={e => e.stopPropagation()} data-testid="strategy-comparison-modal">
        ...
        <div className="sc-filters">
          ...
          <label className="sc-check" title="Datensammel-Trades des KI-Traders (Setup-Reifung) sind keine Paper-Performance und normalerweise ausgeblendet">
            <input type="checkbox" checked={includeCollection} onChange={e => setIncludeCollection(e.target.checked)} data-testid="comparison-include-collection" /> Datensammlung
          </label>
          <label className="sc-check" title={`Trades gelöschter/nicht mehr existierender Strategien (Karteileichen)${data?.stale_strategies?.length ? `: ${data.stale_strategies.join(', ')}` : ''}`}>
            <input type="checkbox" checked={includeStale} onChange={e => setIncludeStale(e.target.checked)} data-testid="comparison-include-stale" /> Karteileichen{data?.stale_strategies?.length ? ` (${data.stale_strategies.length})` : ''}
          </label>
          <div className="sc-total" data-testid="comparison-total">{data?.total_trades ?? 0} geschlossene Trades</div>
        </div>
                ...
                {rows.map((r, i) => (
                  <React.Fragment key={r.strategy_id}>
                    <tr className={r.pnl === bestPnl && r.pnl > 0 ? 'sc-best' : ''} data-testid={`comparison-row-${r.strategy_id}`}>
                      ...
                      <td className="sc-name" title={r.is_manual ? 'Manuell eröffnete Trades (Bitunix-App oder Website) – als eigene Vergleichsgruppe' : r.strategy_name}>{r.strategy_name}{r.is_stale && <span className="sc-stale" title="Strategie existiert nicht mehr (Karteileiche)">gelöscht</span>}
