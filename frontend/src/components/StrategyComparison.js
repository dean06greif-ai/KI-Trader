...
export default function StrategyComparison({ onClose }) {
  ...
  // Nur Strategie×Asset-Kombinationen, die derzeit freigeschaltet sind (paper/live),
  // fließen in den Vergleich ein – deaktivierte Assets optional einblendbar.
  const [onlyActive, setOnlyActive] = useState(true);
  // Datensammel-Trades (KI-Trader) und Karteileichen (gelöschte Strategien)
  // sind KEINE vergleichbare Strategie-Performance und bleiben immer ausgeblendet.

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const d = await fetch(`${API_URL}/api/analytics/strategy-comparison?mode=${mode}&days=${days}&only_active=${onlyActive}`).then(r => r.json());
      setData(d);
    } catch (e) { console.error(e); }
    setLoading(false);
  }, [mode, days, onlyActive]);
    ...
    <SafeOverlay className="sc-overlay" onClose={onClose}>
      <div className="sc-panel" onClick={e => e.stopPropagation()} data-testid="strategy-comparison-modal">
        ...
        <div className="sc-filters">
          ...
          <label className="sc-toggle" title="Nur Trades von Assets, bei denen die Strategie derzeit aktiv ist (Paper oder Live). Ausschalten zeigt auch Trades deaktivierter Assets." data-testid="comparison-active-toggle">
            <input type="checkbox" checked={onlyActive} onChange={e => setOnlyActive(e.target.checked)} data-testid="comparison-active-checkbox" />
            nur aktive Assets
            {onlyActive && (data?.inactive_hidden ?? 0) > 0 && (
              <span className="sc-hidden-count" data-testid="comparison-inactive-hidden">({data.inactive_hidden} ausgeblendet)</span>
            )}
          </label>
