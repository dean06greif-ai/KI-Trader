...
import { calibrationDecision, calibAppliedFromReport, metricLabel } from '../lib/regimeCalibration';
...
export default function RegimeEngineSettings({ engine, setEngine, config, setConfig, calibrateCtx,
  calibApplied, setCalibApplied, onJobStarted, historyRefreshKey = 0 }) {
  ...
  // Kalibrierung übernehmen + als „aktuelle Kalibrierungsdaten“ merken (Metrik
  // bleibt am Eintrag: Referenz-Treffer vs. Holdout Live=Final werden nie vermischt)
  const applyCalibration = (report, id, source) => {
    if (!report?.best_config) return false;
    applyConfig(report.best_config);
    setCalibApplied?.(calibAppliedFromReport(report, id, source, calibrateCtx || {}));
    return true;
  };

  // Gesicherten Stand exakt zurückholen (Engine-Konfiguration + aktive Kalibrierung)
  const restoreSnapshot = (snap) => {
    if (!snap?.engine_config) return false;
    setConfig({ ...snap.engine_config });
    setCalibApplied?.(snap.calib_applied ? { ...snap.calib_applied, source: `${snap.calib_applied.source || 'Kalibrierung'} · wiederhergestellt` } : null);
    return true;
  };
      ...
      return (
        <label className="opt-field" key={m.key} title={m.help}>
          ...
          <select value={v || 'reactive'} onChange={e => set('detector', e.target.value)}
            ...
            <option value="jump">Jump-Modell (Sprungkosten, empfohlen)</option>
    ...
    <div className="rl-engine-box" data-testid="regime-engine-settings">
      ...
      {engine === 'v2' && calibrateCtx && (
        <div className="rl-calibrate-box" data-testid="regime-calibrate-box">
          <div className="rl-calibrate-target" data-testid="regime-calibrate-target">
            ...
            {calibApplied && calibApplied.detector === detector ? (
              <span className="rl-cal-active" data-testid="regime-calibrate-active"
                title={`Übernommen ${new Date(calibApplied.applied_at).toLocaleString('de-DE')} via ${calibApplied.source || '–'} · Metrik: ${metricLabel(calibApplied)}`}>
                ✓ Kalibrierung aktiv · {metricLabel(calibApplied)} {Number(calibApplied.baseline_pct ?? 0).toFixed(1)}% → {Number(calibApplied.best_pct ?? 0).toFixed(1)}%
                {calibApplied.source ? <span className="opt-small" style={{ color: 'inherit', opacity: 0.8 }}> · via {calibApplied.source}</span> : null}
              </span>
            ) : (
          ...
          <RegimeCalibrationHistory refreshKey={calRuns + historyRefreshKey} activeId={calibApplied?.id} detector={detector}
            onApply={(row) => {
              if (applyCalibration(row.report, row.id, row.source === 'autopilot' ? 'Kalibrierungs-Verlauf (Autopilot)' : 'Kalibrierungs-Verlauf')) {
                setCalReport(null); setCalDecision(null);
                toast.success(`${row.source === 'autopilot' ? 'Autopilot-Erkennung' : 'Kalibrierung'} vom ${new Date(row.created_at).toLocaleString('de-DE')} übernommen – jetzt „Regime suchen & speichern“ starten`);
              } else toast.error('Diese Kalibrierung enthält keine übernehmbaren Werte');
            }}
            onRestore={(snap) => {
              if (restoreSnapshot(snap)) {
                setCalReport(null); setCalDecision(null);
                toast.success(`Einstellungen vom ${new Date(snap.created_at).toLocaleString('de-DE')} wiederhergestellt`);
              } else toast.error('Snapshot enthält keine Einstellungen');
            }} />
        </div>
      )}
    </div>
