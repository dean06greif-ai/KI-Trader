          ...
          {(data.setups || []).length > 0 && (
            <div className="diag-block">
              ...
              <table className="diag-table" data-testid="diag-setup-table">
                <thead><tr><th>Setup</th><th>Trades</th><th title="PnL der aktiven Setup-Variante (seit letzter Rückstufung / Revision / Profil-Version); ohne Variante = Gesamt">PnL (akt. Variante)</th><th>Urteil</th><th>Status</th></tr></thead>
                <tbody>
                  {data.setups.map(s => (
                    <tr key={s.setup} data-testid={`diag-setup-row-${s.setup}`}>
                      <td title={s.beschreibung}>{s.setup}</td>
                      <td className="mono">{s.trades}</td>
                      <td className={`mono ${(s.pnl || 0) >= 0 ? 'pos' : 'neg'}`} title={s.variant_since ? `aktive Variante seit ${s.variant_since}` : 'Gesamt (keine Variante)'}>
                        {(s.pnl || 0).toFixed(2)}$
                        {s.variant_since && <span style={{ opacity: 0.55, fontSize: 10 }} data-testid={`diag-setup-variant-${s.setup}`}> seit {s.variant_since.slice(5).split('-').reverse().join('.')}</span>}
                      </td>
                      <td>{s.verdict || '–'}</td>
                      <td title={s.demoted || undefined}>{s.disabled ? `⛔ gesperrt` : s.demoted ? '↩ rückgestuft (Paper-Datensammlung)' : s.live_ready ? '✓ live-reif' : `⏳ ${s.live_reason}`}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {(data.data_quality || []).length > 0 && (
            <div className="diag-block">
              ...
              <table className="diag-table" data-testid="diag-data-table">
                ...
                <tbody>
                  {data.data_quality.map(d => {
                    const limit = d.has_pauses ? 10 : 5;
                    const old = d.last_candle_age_min > limit;
                    return (
                    <tr key={d.symbol} className={old && !d.market_closed ? 'stale' : ''} data-testid={`diag-data-row-${d.symbol}`}>
                      <td>{d.symbol}</td>
                      <td className="mono">{d.candles}</td>
                      <td className="mono" title={d.market_closed_reason || undefined}>
                        {old && d.market_closed
                          ? <span style={{ opacity: 0.6 }} data-testid={`diag-data-closed-${d.symbol}`}>⏸ Markt geschlossen · {d.last_candle_age_min} min</span>
                          : old ? `⚠ ${d.last_candle_age_min} min alt` : `${d.last_candle_age_min} min`}
                      </td>
