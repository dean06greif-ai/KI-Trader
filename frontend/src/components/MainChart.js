...
import useTradeMarkers, { sameSymbol } from '../hooks/useTradeMarkers';
...
const MainChart = ({ symbol, candleData, signal, onClearSignal }) => {
  ...
  // 4h/1d-Haupt-S/R-Zonen als halbtransparente Bänder (Canvas-Overlay, Standard: aus)
  const srCanvasRef = useRef(null);
  ...
  const { tradeMapRef, counts: tradeCounts, hoverDetail, openTrades: allOpenTrades, refresh: refreshTrades,
          togglePin, pinAtTime, pinnedId } = useTradeMarkers(
    candleSeriesRef, symbol, showClosed, RANGES[range].barSec, `${range}:${bars}`);
  // Sicherheitsnetz: Badges nur für den aktuell gewählten Coin rendern
  const openTrades = allOpenTrades.filter(t => sameSymbol(t.symbol, symbol));
    ...
    <div className="main-chart" data-testid="main-chart">
      <div className="chart-header">
        ...
        <div className="chart-indicators">
          ...
          <button
            className={`chart-liq-toggle sr ${srOn ? 'on' : ''}`}
            onClick={() => setSrOn(v => !v)}
            title={srOn
              ? 'S/R-Zonen ausblenden'
              : 'Haupt-Unterstützungen & -Widerstände (4h/1d) als Zonen im Chart einblenden (serverseitige Pivot-/Cluster-Analyse, standardmäßig aus)'}
            data-testid="chart-sr-toggle"
          >
            S/R {srOn && srZones.length ? `· ${srZones.length}` : ''}
          </button>
          <button
      ...
      {srOn && (srError || srZones.length > 0) && (
        <div className="chart-liq-legend" data-testid="chart-sr-legend">
          {srError
            ? <span className="chart-liq-err">{srError}</span>
            : (
              <>
                <span className="chart-liq-chip sr-sup" title="Grüne Zonen: Haupt-Unterstützungen – geclusterte Swing-Tiefs der 4h/1d-Kerzen (Berührungen + Aktualität = Stärke)">grün = Unterstützung</span>
                <span className="chart-liq-chip sr-res" title="Rote Zonen: Haupt-Widerstände – geclusterte Swing-Hochs der 4h/1d-Kerzen (Berührungen + Aktualität = Stärke)">rot = Widerstand</span>
                <span className="chart-liq-chip" title="1d-Zonen werden kräftiger mit durchgezogener Kante gezeichnet, 4h-Zonen dezenter mit gestrichelter Kante. Zahl im Chart = Anzahl Berührungen des Levels">1d = kräftig · 4h = gestrichelt</span>
              </>
            )}
        </div>
      )}
      ...
      <div className="chart-wrap">
        ...
        <canvas ref={srCanvasRef} className="chart-sr-canvas" data-testid="chart-sr-canvas" />
        {tradeTip && (
          <div
