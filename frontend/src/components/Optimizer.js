...
import { X, Play, MagicWand, Trophy, CheckCircle, FloppyDisk, ChartLine, Cloud, Desktop, Gear, ClockCounterClockwise, MoonStars } from '@phosphor-icons/react';
...
export default function Optimizer({ onClose }) {
  ...
  const applyCopilotSettings = (s) => {
    ...
    if (s.timeframe && Array.isArray(TIMEFRAMES) && TIMEFRAMES.includes(s.timeframe)) { setTimeframe(s.timeframe); n++; }
