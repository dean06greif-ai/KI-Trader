...
import { X, Play, Pause, MagicWand, Trophy, CheckCircle, FloppyDisk, ChartLine, Cloud, Desktop, Gear, ClockCounterClockwise, MoonStars } from '@phosphor-icons/react';
...
// ---- Warteschlange, direkt im Optimizer-Prozess integriert ----
// Läuft bereits eine Suche, reiht "Zur Warteschlange" den nächsten Auftrag mit
// den GEÄNDERTEN Einstellungen ein; sobald der laufende fertig ist, startet der
// nächste automatisch. Alle Ergebnisse bleiben gespeichert und können hier
// einzeln geladen und verglichen werden.
const QUEUE_STATUS_LABEL = { queued: 'wartet', running: 'läuft', done: 'fertig', error: 'Fehler', cancelled: 'abgebrochen' };

function QueueStrip({ refreshKey, onLoadResult }) {
  ...
  const items = (data?.items || []).filter(i => i.kind === 'optimizer');
  if (!items.length) return null;
  const queued = items.filter(i => i.status === 'queued').length;
  const loadResult = async (id) => {
    setBusyId(id);
    try {
      const r = await fetch(`${API_URL}/api/series/${id}/result`);
      const d = await r.json();
      if (!r.ok || !d.result) { toast.error('Ergebnis (noch) nicht verfügbar'); return; }
      onLoadResult(d.result);
      toast.success('Warteschlangen-Ergebnis geladen');
  ...
  return (
    <div className="opt-history" data-testid="opt-queue-strip" style={{ marginTop: 10 }}>
      <div className="opt-section-title">
        <MoonStars size={14} /> WARTESCHLANGE – {queued} wartend
        {queued > 0 && data?.external_job_running ? ' · startet automatisch nach dem laufenden Prozess' : ''}
      </div>
      {items.slice(-8).reverse().map(i => (
        <div key={i.id} data-testid="opt-queue-item"
          ...
          <span style={{ color: i.status === 'done' ? '#26d07c' : i.status === 'running' ? '#f0a020' : i.status === 'error' ? '#ff5b5b' : '#9aa4b2' }}>
            {QUEUE_STATUS_LABEL[i.status] || i.status}
          </span>
          {i.status === 'done' && (
            <button className="opt-chip" onClick={() => loadResult(i.id)} disabled={busyId === i.id}
              data-testid="opt-queue-load" title="Ergebnis dieses Auftrags in die Ergebnis-Ansicht laden">
              {busyId === i.id ? 'Lädt…' : 'Ergebnis laden'}
            </button>
          )}
          {i.status !== 'running' && (
            <button className="opt-chip" onClick={() => removeItem(i.id)} data-testid="opt-queue-del"
              title="Eintrag aus der Warteschlange entfernen"><X size={11} /></button>
          )}
        </div>
      ))}
    </div>
...
export default function Optimizer({ onClose }) {
  ...
  useEffect(() => {
    const check = () => fetch(`${API_URL}/api/localworker/status`).then(r => r.json())
      .then(d => {
        setLwOnline(!!d.online);
        // RAM-Entlastung Render: ohne gespeicherte Präferenz standardmäßig
        // auf den lokalen Worker ausführen, sobald einer online ist.
        if (d.online && !saved.execution) setExecution('local');
      }).catch(() => setLwOnline(false));
    check();
  ...
  const poll = useCallback((jobId) => {
    // Robustes Polling (Fix "Balken flackert"): kein überlappendes Fetch,
    // veraltete/fehlerhafte Antworten (404 nach Server-Neustart, 502 bei
    // RAM-Druck) verstecken den Fortschritt NICHT mehr – erst nach mehreren
    // Fehlversuchen in Folge wird der Server-Zustand über /active geklärt.
    if (pollRef.current) clearInterval(pollRef.current);
    let busy = false;
    let misses = 0;
    pollRef.current = setInterval(async () => {
      if (busy) return; // langsame Antwort? Nicht stapeln (sonst Out-of-Order-Flackern)
      busy = true;
      try {
        const r = await fetch(`${API_URL}/api/optimizer/status/${jobId}`);
        const j = await r.json().catch(() => null);
        if (!r.ok || !j || !j.status) {
          misses += 1;
          if (misses >= 8) {
            // Server bestätigt: läuft wirklich nichts mehr? Dann sauber beenden.
            const a = await fetch(`${API_URL}/api/optimizer/active`)
              .then(x => x.json()).catch(() => null);
            if (a && !a.active) {
              clearInterval(pollRef.current);
              setJob(prev => prev ? { ...prev, status: 'error',
                error: 'Verbindung zum Lauf verloren (Server neu gestartet?) – Verlauf prüfen' } : prev);
              toast.error('Optimierung: Verbindung zum Lauf verloren – bitte Verlauf prüfen');
            } else if (a?.active?.id && a.active.id !== jobId) {
              setJob(a.active);
              poll(a.active.id);
            } else {
              misses = 0; // Job existiert (wieder) – weiter pollen
            }
          }
          return;
        }
        misses = 0;
        // Fortschritt nie rückwärts anzeigen (Netz-Antworten können sich überholen)
        setJob(prev => (prev && prev.id === j.id && j.status === 'running'
          && prev.status === 'running' && (j.progress || 0) < (prev.progress || 0))
          ? { ...j, progress: prev.progress } : j);
        if (j.status === 'done') {
          clearInterval(pollRef.current);
          setResult(j.result);
      ...
      } catch { misses += 1; /* Netzwerkfehler: Zustand behalten, weiter pollen */ }
      finally { busy = false; }
    }, 1500);
  }, []);
  ...
  // Pause / Fortsetzen: Job bleibt "running" (belegt den Slot), Backend liefert pause/paused
  const togglePause = async () => {
    if (!job?.id) return;
    const wantPause = !(job.pause || job.paused);
    try {
      const r = await fetch(`${API_URL}/api/optimizer/${wantPause ? 'pause' : 'resume'}/${job.id}`, {
        method: 'POST', headers: authHeaders(),
      });
      const d = await r.json().catch(() => ({}));
      if (!r.ok) { toast.error(d.detail || 'Aktion fehlgeschlagen'); return; }
      setJob(prev => prev ? { ...prev, pause: wantPause } : prev);
      toast.info(wantPause ? 'Pause angefordert – Lauf hält am nächsten Checkpoint an' : 'Lauf wird fortgesetzt');
    } catch { toast.error('Verbindungsfehler'); }
  };
  ...
  // ---- Copilot: Einstellungs-Vorschläge direkt ins Formular übernehmen ----
  const applyCopilotSettings = (s) => {
    if (!s || typeof s !== 'object') return 0;
    let n = 0;
    const num = (v) => (v === null || v === undefined || Number.isNaN(Number(v)) ? null : Number(v));
    if (s.mode && MODES.some(m => m.id === s.mode)) { setMode(s.mode); n++; }
    if (s.strategy_id && strategies.some(x => x.id === s.strategy_id)) { setSelStrategy(s.strategy_id); n++; }
    if (s.base_strategy_id !== undefined) { setBaseStrategy(s.base_strategy_id || ''); n++; }
    if (Array.isArray(s.coins)) {
      const valid = s.coins.filter(c => allSymbols.includes(c));
      if (valid.length) { setSelCoins(valid); n++; }
    }
    if (num(s.days) !== null) { setDays(num(s.days)); n++; }
    // TIMEFRAMES ist eine Liste von {v,l} – der frühere includes(string)-Check
    // schlug immer fehl, deshalb wurde der Timeframe nie übernommen.
    const tfValid = (tf) => Array.isArray(TIMEFRAMES) && TIMEFRAMES.some(t => (t?.v ?? t) === tf);
    if (s.timeframe && tfValid(s.timeframe)) { setTimeframe(s.timeframe); n++; }
    if (typeof s.sessions === 'string') { setOptSessions(s.sessions.trim()); n++; }
    if (['cloud', 'local'].includes(s.execution)) { setExecution(s.execution); n++; }
    if (s.opt_groups && typeof s.opt_groups === 'object') {
      const clean = {};
      Object.entries(s.opt_groups).forEach(([k, v]) => { if (typeof v === 'boolean') clean[k] = v; });
      if (Object.keys(clean).length) { setOptFlags(prev => ({ ...prev, ...clean })); n++; }
    }
    const rtf = s.rule_timeframes;
    if (rtf && typeof rtf === 'object') {
      setRuleTf(prev => ({
        enabled: typeof rtf.enabled === 'boolean' ? rtf.enabled : true,
        min: RULE_TIMEFRAMES.some(t => t.v === rtf.min) ? rtf.min : prev.min,
        max: RULE_TIMEFRAMES.some(t => t.v === rtf.max) ? rtf.max : prev.max,
      }));
      n++;
    } else if (rtf === false) { setRuleTf(prev => ({ ...prev, enabled: false })); n++; }
    const rb = s.robustness;
    if (rb && typeof rb === 'object') {
      if ('dd_max_pct' in rb) {
        if (num(rb.dd_max_pct) !== null) { setDdEnabled(true); setDdMaxPct(num(rb.dd_max_pct)); } else setDdEnabled(false);
        n++;
      }
      if ('constancy' in rb) {
        const c = rb.constancy;
        if (c && typeof c === 'object') {
          setCtEnabled(true);
          if (num(c.chunk_days) !== null) setCtChunkDays(Math.max(1, num(c.chunk_days)));
          if (num(c.max_dev_pct) !== null) setCtMaxDev(Math.max(1, num(c.max_dev_pct)));
        } else setCtEnabled(false);
        n++;
      }
      if ('stress_fee_mult' in rb) {
        if (num(rb.stress_fee_mult) !== null) { setStEnabled(true); setStMult(num(rb.stress_fee_mult)); } else setStEnabled(false);
        n++;
      }
      if ('slippage_var_pct' in rb) {
        if (num(rb.slippage_var_pct) !== null) { setSbEnabled(true); setSbVar(num(rb.slippage_var_pct)); } else setSbEnabled(false);
        n++;
      }
      if ('monte_carlo_runs' in rb) {
        if (num(rb.monte_carlo_runs) !== null) { setMcEnabled(true); setMcRuns(Math.max(10, num(rb.monte_carlo_runs))); } else setMcEnabled(false);
        n++;
      }
      if (typeof rb.regime_check === 'boolean') { setRgEnabled(rb.regime_check); n++; }
    }
    if (s.objective && OBJECTIVES.some(o => o.v === s.objective)) { setObjective(s.objective); n++; }
    if (s.algorithm && ALGORITHMS.some(a => a.v === s.algorithm)) { setAlgorithm(s.algorithm); n++; }
    if (num(s.iterations) !== null) { setIterations(Math.max(1, num(s.iterations))); n++; }
    if (num(s.min_trades) !== null) { setMinTrades(Math.max(0, num(s.min_trades))); n++; }
    if (num(s.max_rules) !== null) { setMaxRules(Math.max(1, num(s.max_rules))); n++; }
    if (typeof s.deep_test === 'boolean') { setDeepTest(s.deep_test); n++; }
    if (['deep', 'extreme'].includes(s.deep_depth)) { setDeepDepth(s.deep_depth); n++; }
    if (Array.isArray(s.indicators)) {
      const valid = s.indicators.filter(id => INDICATOR_POOL.some(i => i.id === id));
      if (valid.length) { setIndicators(valid); n++; }
    }
    const wf = s.walk_forward;
    if (wf && typeof wf === 'object') {
      if (typeof wf.enabled === 'boolean') { setWfEnabled(wf.enabled); n++; }
      else { setWfEnabled(true); n++; }
      if (['single', 'rolling', 'anchored'].includes(wf.mode)) setWfMode(wf.mode);
      if (num(wf.windows) !== null) setWfWindows(Math.max(2, num(wf.windows)));
      if (num(wf.train_pct) !== null) setWfTrainPct(Math.min(95, Math.max(50, num(wf.train_pct))));
    } else if (wf === false || wf === null) {
      if (wf === false) { setWfEnabled(false); n++; }
    }
    const ex = s.explore;
    if (ex && typeof ex === 'object') {
      if (num(ex.champions) !== null) { setExploreChamps(Math.max(1, num(ex.champions))); n++; }
      if (num(ex.max_minutes) !== null) { setExploreMaxMin(Math.max(0, num(ex.max_minutes))); n++; }
    }
    const dy = s.dynamic;
    if (dy && typeof dy === 'object') {
      if (num(dy.max_regimes) !== null) { setDynMaxRegimes(Math.max(2, num(dy.max_regimes))); n++; }
      if (num(dy.conf_min) !== null) { setDynConfMin(Math.min(100, Math.max(0, num(dy.conf_min)))); n++; }
      if (num(dy.min_hold_days) !== null) { setDynMinHold(Math.max(0, num(dy.min_hold_days))); n++; }
      if (num(dy.lookback_days) !== null) { setDynLookback(Math.max(1, num(dy.lookback_days))); n++; }
      if (num(dy.train_pct) !== null) { setDynTrainPct(Math.min(95, Math.max(50, num(dy.train_pct)))); n++; }
      if (num(dy.max_rules) !== null) { setDynMaxRules(Math.max(1, num(dy.max_rules))); n++; }
      if (typeof dy.start_from_base === 'boolean') { setDynStartFromBase(dy.start_from_base); n++; }
      if (typeof dy.rule_variants === 'boolean') { setDynRuleVariants(dy.rule_variants); n++; }
      if (typeof dy.per_regime === 'boolean') { setDynPerRegime(dy.per_regime); n++; }
    }
    return n;
  };
  ...
  const validate = () => {
    ...
    if (!selCoins.length) { toast.error('Mind. 1 Coin wählen'); return false; }
    if ((mode === 'params' || mode === 'dynamic') && !selStrategy) { toast.error('Strategie wählen'); return false; }
    if ((mode === 'discovery' || mode === 'combo' || mode === 'explore') && indicators.length === 0) { toast.error('Mind. 1 Indikator anhaken'); return false; }
    return true;
  };

  const queueToSeries = async () => {
    if (!validate()) return;
    const item = await addToSeries('optimizer', buildBody());
  ...
  const run = async (overrides = null) => {
    ...
    if (execution === 'cloud' && lwOnline) {
      toast.info('Tipp: Dein lokaler Worker ist online – Ausführung „Lokal" schont den Server-RAM (Render 512 MB)');
    }
    try {
      ...
      if (overrides?.symbols) setSelCoins(overrides.symbols);
      setResult(null);
      setApplied(false);
  ...
  const applyParams = async (scope, exclude = null, entry = null) => {
    const best = entry || selEntry || result?.best;
    if (!best) return;
    setApplying(true);
    try {
      const res = await fetch(`${API_URL}/api/optimizer/apply`, {
        ...
        body: JSON.stringify({ type: 'params', strategy_id: result.strategy_id,
          ...
          exclude_symbols: exclude?.excluded, exclude_reasons: exclude?.reasons }),
      });
      const d = await res.json();
      if (!res.ok) { toast.error(d.detail || 'Übernahme fehlgeschlagen'); return; }
      if ((d.excluded_symbols || []).length) toast.info(`Nicht mehr gehandelt mit dieser Strategie: ${d.excluded_symbols.map(x => x.replace('USDT', '')).join(', ')}`);
      setApplied(true);
  ...
  const saveStrategy = async (exclude = null, entry = null) => {
    const src = entry || selEntry;
    const def = src?.definition || result?.definition;
    if (!def) return;
    const tp = src ? src.trade_params : result.trade_params;
    try {
      const res = await fetch(`${API_URL}/api/optimizer/apply`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
      ...
      if ((d.excluded_symbols || []).length) toast.info(`Strategie für ${d.excluded_symbols.map(x => x.replace('USDT', '')).join(', ')} auf AUS gestellt`);
      setApplied(true);
  ...
  return (
    <SafeOverlay className="opt-overlay" onClose={onClose}>
      <div className="opt-panel" onClick={e => e.stopPropagation()} data-testid="optimizer-modal">
        ...
        <button className="opt-chip opt-history-btn" onClick={queueToSeries} data-testid="opt-add-series"
          title={running
            ? 'Neuen Suchauftrag mit den aktuell geänderten Einstellungen einreihen – startet automatisch, sobald der laufende Prozess fertig ist. Alle Ergebnisse bleiben gespeichert.'
          ...
          <div className="opt-progress" data-testid="opt-progress">
            ...
            <div className="opt-progress-row">
              <div className="opt-progress-text" data-testid="opt-progress-text">
                ...
                {job.pause && !job.paused && <span className="opt-eta" data-testid="opt-pausing-hint"> · Pause angefordert…</span>}
                {job.paused && <span className="opt-eta" data-testid="opt-paused-tag"> · ⏸ pausiert{job.paused_total_s > 0 ? ` (${fmtEta(job.paused_total_s)})` : ''}</span>}
                {job.eta_seconds != null && !job.paused && <span className="opt-eta"> · Restzeit {fmtEta(job.eta_seconds)}</span>}
              </div>
              <button className="opt-cancel-run" onClick={togglePause}
                data-testid={(job.pause || job.paused) ? 'opt-resume' : 'opt-pause'}
                title={(job.pause || job.paused)
                  ? 'Lauf fortsetzen'
                  : 'Lauf anhalten (z.B. Rechner abkühlen lassen) – Zwischenstand bleibt erhalten, Fortsetzen jederzeit'}>
                {(job.pause || job.paused)
                  ? <><Play size={13} weight="bold" /> Fortsetzen</>
                  : <><Pause size={13} weight="bold" /> Pausieren</>}
              </button>
            ...
            {job.best?.metrics && (
              <div className="opt-best-live">
                ...
                {typeof job.best?.score === 'number' && job.best.score < -4e8 && (
                  <span className="opt-badge bad" data-testid="opt-best-filter-warning"
                    title="Der Score ist ein interner Straf-Wert (-500 Mio. + PnL bzw. -1 Mrd. + Trades), damit Filter-Ausfälle hinten einsortiert werden – keine echte Zahl.">
        ...
        <QueueStrip refreshKey={queueRefresh}
          onLoadResult={(res) => { setResult(res); setApplied(false); setSelTop(0); }} />
              ...
              <div className="opt-top5" data-testid="opt-top5">
                ...
                {top5.map((t, i) => (
                  <button key={i} type="button"
                    ...
                    <OutlierVariant ov={t.outlier_variant} idx={i} applying={applying}
                      onApplyExcluded={() => { setSelTop(i); if (result.mode === 'params') applyParams('global', t.outlier_variant, t); else saveStrategy(t.outlier_variant, t); }}
                      onReoptimize={() => run({ symbols: t.outlier_variant.kept })} />
