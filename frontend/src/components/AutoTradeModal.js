...
const AutoTradeModal = ({ symbol, onClose }) => {
  ...
  const load = async () => {
    const [c, t, m] = await Promise.all([
      ...
      fetch(`${API_URL}/api/autotrade/coin/${symbol}/meta`).then(r => r.json()).catch(() => ({})),
    ]);
    ...
    <SafeOverlay className="at-overlay" onClose={onClose}>
      <div className="at-panel" onClick={e => e.stopPropagation()} data-testid="autotrade-modal">
        ...
        <div className="at-section">
          ...
          <div className="at-field">
            <label>Hebel: <b>{Math.min(cfg.leverage, coinMaxLev)}x</b> <span style={{ opacity: 0.6 }}>(max. {coinMaxLev}x)</span></label>
            <input type="range" min={1} max={coinMaxLev} value={Math.min(cfg.leverage, coinMaxLev)}
