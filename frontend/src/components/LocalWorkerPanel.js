...
export default function LocalWorkerPanel({ onClose }) {
  ...
  const copy = async (text, label) => {
    if (!text) { toast.error('Kein Token geladen – bitte als Admin neu einloggen'); return; }
    try {
      ...
      toast.error('Kopieren fehlgeschlagen – Feld ist markierbar, bitte manuell kopieren (Strg+C)');
    }
  };
