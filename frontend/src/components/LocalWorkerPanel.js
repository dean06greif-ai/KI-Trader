...
export default function LocalWorkerPanel({ onClose }) {
  ...
  const regenToken = async () => {
    try {
      const r = await fetch(`${API_URL}/api/localworker/token/regenerate`, {
        method: 'POST', headers: authHeaders(),
      });
      const d = await r.json();
      if (!r.ok || !d.token) {
        toast.error(d.detail || 'Token erneuern fehlgeschlagen – Admin-Login abgelaufen? Bitte neu einloggen.');
        return;
      }
      setToken(d.token);
      setTokenError(false);
      ...
      <div className="lw-panel" onClick={e => e.stopPropagation()} data-testid="localworker-panel">
          ...
          <ol className="lw-steps">
            ...
            <li>
              ...
              <div className="lw-cmd" data-testid="lw-cmd">
                ...
                <button className="lw-mini" data-testid="lw-cmd-copy" disabled={!token}
                  onClick={() => {
                    if (!token) { toast.error('Kein Token geladen – bitte als Admin neu einloggen'); return; }
                ...
                {tokenError && (
                  <span className="lw-token-error" data-testid="lw-token-error"
                    style={{ color: 'var(--danger, #e5484d)', fontSize: 11, marginLeft: 6 }}>
                    Token nicht geladen – Admin-Login abgelaufen, bitte neu einloggen.
                  </span>
                )}
              </div>
            </li>
        ...
        {settings && (
          <div className="lw-grid" data-testid="lw-settings">
            ...
            <label title="0 = automatisch: der Worker nutzt den gesamten RAM des PCs minus 1 GB Reserve. Die 2-GB-Grenze gilt nur für die Website (Render), nicht für den Worker.">
              RAM-Limit Kerzen-Cache (MB, 0 = ganzer RAM)
              <NumInput int min={0} step={512} value={settings.ram_limit_mb}
                onCommit={(v) => set('ram_limit_mb', v || 0)}
