...
export const AIStrategyLabPanel = () => {
  ...
  const runDevelop = async () => {
    setDevelopBusy(true);
    try {
      const res = await fetch(`${API_URL}/api/ai/strategies/develop`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({}),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.detail || 'Setup-Entwicklung fehlgeschlagen');
      if (data.status === 'ok') {
        toast.success(`Neues Setup „${data.candidate?.name}“ entwickelt – startet als Ghost`
          + (data.backtestable ? ' (inkl. Backtest-Regeln)' : ''));
      } else {
        toast.message(data.reason || data.detail || 'Kein neues Setup entwickelt');
      }
      load();
    } catch (e) { toast.error(e.message); } finally { setDevelopBusy(false); }
  };
  ...
  return (
    <div className="gov-panel" data-testid="ai-strategy-lab-panel">
      ...
      <div className="gov-card-actions" style={{ margin: '6px 0 10px' }}>
        <button className="gov-btn primary" disabled={developBusy || !settings?.allow_ai_create}
          onClick={runDevelop} data-testid="strategy-develop-btn"
          title="Die KI wertet Parameter + Ergebnisse der anderen Strategien (Trendfolge, Scalping, …) aus und entwickelt daraus selbst ein neues Setup – startet sicher als Ghost.">
          <Flask size={13} weight="fill" />
          {developBusy ? 'KI entwickelt Setup…' : 'KI: Neues Setup aus Trainingslager entwickeln'}
        </button>
      </div>
