...
const DET = { reactive: 'Umkehrpunkte (reactive)', ema: 'EMA-Steigung (ema)', kombi: 'Kombi', jump: 'Jump-Modell (jump)', regression: 'Regression' };
...
function BestLine({ best, testId }) {
  ...
  return (
    <span className="opt-small" data-testid={testId}>
      ...
      Holdout-F1 (Referenz v2) <b>{fmt(m.holdout_reference_f1_pct)}%</b>
      {m.holdout_kappa_pct != null && <> · κ <b>{fmt(m.holdout_kappa_pct)}</b></>} ·
      Ø Richtungs-Phase <b>{fmt(m.live_direction_phase_days)}d</b> ·
      innere Val. {fmt(m.inner_direction_pct)}% · Live=Final Holdout {fmt(m.holdout_direction_pct)}% ·
