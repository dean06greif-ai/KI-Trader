...
const DEFAULT_CFG = {
  ...
  profit_secure_max_leverage: 100,
...
export default function StrategyAutoTradeModal({ strategyId, strategyName, symbol, onClose, onSaved }) {
  ...
  const [regimePhase, setRegimePhase] = useState(null);
  const [saving, setSaving] = useState(false);
  const [coinMaxLev, setCoinMaxLev] = useState(200);
  ...
  const load = async () => {
    ...
    try {
      const m = await fetch(`${API_URL}/api/autotrade/coin/${symbol}/meta`).then(r => r.json());
      setCoinMaxLev(Math.max(2, Math.round(m.max_leverage || 200)));
    } catch { /* Fallback 200 bleibt */ }
  };
  ...
  // Aktuelle Marktphase (Regime) für die Anzeige im Marktphasen-Filter laden
  useEffect(() => {
    if (!cfg?.regime_filter_enabled) { setRegimePhase(null); return undefined; }
    let stale = false;
    fetch(`${API_URL}/api/autotrade/regime_phase/${symbol}`)
      .then(r => r.json())
      .then(d => { if (!stale) setRegimePhase(d && !d.error ? d : null); })
      .catch(() => { if (!stale) setRegimePhase(null); });
    return () => { stale = true; };
  }, [cfg?.regime_filter_enabled, symbol]);
        ...
        {cfg.mode !== 'off' && (
          <>
            <div className="at-section">
              ...
              <div className="at-field">
                <label>Hebel: <b>{cfg.auto_leverage_enabled ? 'AUTO' : `${Math.min(cfg.leverage, coinMaxLev)}x`}</b> <span style={{ opacity: 0.6 }}>(max. {coinMaxLev}x)</span></label>
                <input
                  type="range"
                  min={1}
                  max={coinMaxLev}
                  value={Math.min(cfg.leverage, coinMaxLev)}
              ...
              {cfg.auto_leverage_enabled && (
                <>
                  ...
                  <div className="at-section" style={{ marginTop: 10 }}>
                    ...
                    <div className="at-field">
                      <label>Max. Hebel (Coin-Max: {coinMaxLev}x)</label>
                      <NumInput int
                        min={1}
                        max={coinMaxLev}
                        value={Math.min(cfg.auto_lev_max ?? 50, coinMaxLev)}
            ...
            <div className="at-block">
              ...
              {cfg.profit_secure_enabled && (
                <div className="at-section" style={{ marginTop: 10 }}>
                  <div className="at-field">
                    <label>Auslöser: Gewinn % auf Marge (min. 5%)</label>
                    <NumInput
                      step={5}
                      min={5}
                      value={Math.max(5, cfg.profit_secure_trigger_pct ?? 30)}
                      onCommit={(v) => update('profit_secure_trigger_pct', Math.max(5, v || 5))}
                  ...
                  <label className="at-check">
                    <input
                      type="checkbox"
                      checked={!!cfg.profit_secure_release_margin}
                      onChange={e => update('profit_secure_release_margin', e.target.checked)}
                      data-testid="sat-ps-release-margin"
                    />
                    ...
                    <>
                      <div className="at-field">
                        ...
                        <input type="range" min={10} max={100} step={5}
                          ...
                          style={{ width: '100%' }} data-testid="sat-ps-margin-reduce" />
                      </div>
                      <div className="at-field">
                        <label>Ziel-Hebel beim Freisetzen: {Math.min(Math.round(cfg.profit_secure_max_leverage ?? 100), coinMaxLev)}x (max. Hebel des Coins: {coinMaxLev}x)</label>
                        <input type="range" min={2} max={coinMaxLev} step={1}
                          value={Math.min(cfg.profit_secure_max_leverage ?? 100, coinMaxLev)}
                          onChange={e => update('profit_secure_max_leverage', Number(e.target.value))}
                          style={{ width: '100%' }} data-testid="sat-ps-max-lev" />
                      </div>
            ...
            {/* Marktphasen-Filter (Regime-Gate) */}
            <div className="at-block">
              <div className="at-block-title">MARKTPHASEN-FILTER (REGIME)</div>
              <label className="at-check" style={{ marginTop: 0 }}>
                <input
                  type="checkbox"
                  checked={!!cfg.regime_filter_enabled}
                  onChange={e => update('regime_filter_enabled', e.target.checked)}
                  data-testid="sat-regime-filter"
                />
                <span>Neue Trades in bestimmten Marktphasen blockieren (z.B. Seitwärtsmarkt).
                  Die Phase wird pro Coin automatisch erkannt (1h-Kerzen, 30 Tage,
                  alle 15 Min. aktualisiert). Offene Trades laufen normal weiter.</span>
              </label>
              {cfg.regime_filter_enabled && (
                <>
                  <div className="at-seg" style={{ marginTop: 8 }}>
                    {[['seitwärts', 'Seitwärts'], ['bulle', 'Aufwärts (Bulle)'], ['bär', 'Abwärts (Bär)']].map(([k, l]) => {
                      const list = cfg.regime_block_phases || ['seitwärts'];
                      const on = list.includes(k);
                      return (
                        <button key={k} className={on ? 'active off' : ''}
                          onClick={() => update('regime_block_phases', on ? list.filter(x => x !== k) : [...list, k])}
                          title={on ? `Phase "${l}" wird blockiert (klick zum Erlauben)` : `Phase "${l}" ist erlaubt (klick zum Blockieren)`}
                          data-testid={`sat-regime-block-${k}`}
                        >{on ? '⛔ ' : ''}{l}</button>
                      );
                    })}
                  </div>
                  <div className="at-possize" style={{ marginBottom: 0 }} data-testid="sat-regime-phase-info">
                    Markierte Phasen werden blockiert.
                    {regimePhase && regimePhase.label
                      ? <> Aktuell für {coinShort}: <b>{regimePhase.label}</b>{regimePhase.confidence != null ? ` (${regimePhase.confidence}% Sicherheit)` : ''}.</>
