    ...
    <div className={`rc-health ${data.level}`} data-testid="regime-bridge-health">
      {shown.map(c => (
        <div key={c.name} className="rc-health-row" data-testid={`regime-bridge-health-${c.name}`}>
          ...
          <span>{c.detail}</span>
          {c.items?.length > 0 && !['observer_low_hit', 'structural_short_history'].includes(c.name) && (
            <span className="opt-small"> ({c.items.map(i => i.name || i.id).join(', ')})</span>
          )}
          {c.name === 'structural_short_history' && c.items?.length > 0 && (
            <span className="opt-small"> ({c.items.map(i => `${i.symbol} ${i.history_bars}/${i.required} Kerzen`).join(', ')})</span>
          )}
