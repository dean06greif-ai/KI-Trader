...
const TradeDetailCard = ({ t, stratName, getCoinName, onChanged, onShowChart }) => {
  ...
  <span className="tdc-main">
    ...
    <span className="tdc-coin-wrap">
      ...
      {t.data_collection && <sup className="tdc-sup tag-daten" title="Datensammel-Modus: reiner Paper-Trade zum ML-Datensammeln – wird NICHT in PnL/Statistik mitberechnet" data-testid={`trade-collection-badge-${t.id}`}>D</sup>}
    </span>
      ...
      {open && (
        <div className="tdc-body" data-testid={`trade-card-body-${t.id}`}>
          ...
          <div className="tdc-meta">
            ...
            <div className="tdc-meta-item" title="Hebel-Setting der Börse (wie in der Bitunix-App). Steht daneben „effektiv“: Durch Margen-Entnahme/-Zugabe weicht der tatsächliche Hebel (Positionswert ÷ gebundene Marge) vom Setting ab."><span>Hebel</span><b className="mono">{t.leverage ? `${t.leverage}x` : '—'}{t.effective_leverage && t.leverage && Math.abs(t.effective_leverage - t.leverage) > 0.5 ? <span style={{ opacity: 0.65, fontWeight: 400 }}> (effektiv {t.effective_leverage}x)</span> : null}</b></div>
            <div className="tdc-meta-item" title="Aktuell gebundene Margin in $ – das für diesen Trade gebundene Kapital (nach Teil-Closes und Margen-Anpassungen)."><span>Kapital</span><b className="mono">{c.margin_used != null ? `${c.margin_used} $` : (t.max_capital ? `${t.max_capital} $` : '—')}</b></div>
