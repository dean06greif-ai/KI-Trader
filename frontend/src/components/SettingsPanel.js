...
const SettingsPanel = ({ onClose, focusStrategy, mode = 'all', controlState, onControlChanged }) => {
  ...
  const NOTIFY_LABELS = {
    ...
    limit_orders: 'KI-Limit-Order gefüllt / verfallen (Key-Level-Entries)',
    ...
    job_series: 'Nacht-Serie: Job fertig / Serie komplett',
  ...
  const [feeCfg, setFeeCfg] = useState({ taker: 0.06, maker: 0.02, fxComm: 0.002, fxMin: 2.0 });
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([settingsData, strategiesData]) => {
        ...
        setFeeCfg({
          taker: settingsData.futures_taker_fee_pct ?? 0.06,
          maker: settingsData.futures_maker_fee_pct ?? 0.02,
          fxComm: settingsData.forex_commission_pct ?? 0.002,
          fxMin: settingsData.forex_min_commission_usd ?? 2.0,
        });
    ...
    fetch(`${API_URL}/api/ibkr/status`).then(r => r.json())
      .then(setIbkr).catch(() => setIbkr(null));
  }, []);

  const saveFees = async () => {
    if (!isAdmin()) { toast.error('Admin-Login erforderlich zum Speichern'); return; }
    const taker = parseFloat(feeCfg.taker);
    const maker = parseFloat(feeCfg.maker);
    const fxComm = parseFloat(feeCfg.fxComm);
    const fxMin = parseFloat(feeCfg.fxMin);
    if (!Number.isFinite(taker) || taker < 0 || taker > 1 || !Number.isFinite(maker) || maker < 0 || maker > 1) {
      toast.error('Gebühren bitte als %-Wert zwischen 0 und 1 angeben (z.B. 0.06)');
      return;
    }
    if (!Number.isFinite(fxComm) || fxComm < 0 || fxComm > 0.5 || !Number.isFinite(fxMin) || fxMin < 0 || fxMin > 50) {
      toast.error('Forex: Kommission 0–0.5 % (z.B. 0.002) und Mindestkommission 0–50 USD');
      return;
    }
    setFeeSaving(true);
    try {
      const res = await fetch(`${API_URL}/api/settings`, {
        ...
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({
          futures_taker_fee_pct: taker, futures_maker_fee_pct: maker,
          forex_commission_pct: fxComm, forex_min_commission_usd: fxMin,
        }),
      });
      if (res.status === 401) { toast.error('Nicht autorisiert – bitte als Admin anmelden'); return; }
      if (!res.ok) throw new Error();
      toast.success('Gebühren gespeichert (Krypto + Forex) – gelten für alle NEUEN Trades & Backtests');
    } catch { toast.error('Fehler beim Speichern der Gebühren'); } finally { setFeeSaving(false); }
  };
          ...
          {activeTab === 'control' && (
            <>
              <div className="settings-section">
                ...
                <div className="control-card" data-testid="control-fees-card">
                  <div className="control-card-header">
                    <div className="control-card-info">
                      <div className="control-card-title">Futures-Gebühren (Maker / Taker)</div>
                      <div className="control-card-desc">
                        Gebührensatz pro Seite in % des Positionswerts – wird einmal beim Eröffnen
                        und einmal beim Schließen (auch Teil-Exits) berechnet. Der Bot handelt mit
                        Market-Orders, daher gilt die <b>Taker-Gebühr</b> für alle Berechnungen.
                        Änderungen gelten nur für <b>neue</b> Trades – bestehende behalten ihren
                        gespeicherten Satz.
                      </div>
                    ...
                    <button className="control-master-btn active" onClick={saveFees}
                      disabled={feeSaving} data-testid="fee-save-btn">
                      <span className="control-master-label">{feeSaving ? 'SPEICHERT…' : 'GEBÜHREN SPEICHERN'}</span>
                    </button>
                  </div>
                </div>

                <div className="control-card" data-testid="control-forex-fees-card">
                  <div className="control-card-header">
                    <div className="control-card-info">
                      <div className="control-card-title">Forex-Gebühren (Interactive Brokers)</div>
                      <div className="control-card-desc">
                        Forex wird über <b>Interactive Brokers</b> abgerechnet – getrennt von den
                        Bitunix-Krypto-Gebühren: <b>Kommission je Seite in % des Positionswerts</b>
                        {' '}(IBKR IdealPro ≈ 0.002 % = 0.2 Basispunkte) plus <b>Mindestkommission in USD</b>.
                        Gilt für Live-, Paper- und Backtest-Berechnungen auf allen FOREX-Paaren.
                        Änderungen gelten nur für <b>neue</b> Trades.
                        {ibkr && (
                          <div style={{ marginTop: '8px', fontSize: '11px' }} data-testid="ibkr-status">
                            IBKR-Gateway:{' '}
                            {!ibkr.configured
                              ? <span style={{ color: '#f0a020' }}>nicht konfiguriert – Forex-Live fällt auf Paper zurück (siehe IBKR_SETUP_ANLEITUNG.md)</span>
                              : ibkr.authenticated
                                ? <span style={{ color: '#26d07c' }}>verbunden & eingeloggt{ibkr.account_id ? ` (Konto ${ibkr.account_id})` : ''}</span>
                                : <span style={{ color: '#ff5b5b' }}>Gateway erreichbar, aber NICHT eingeloggt{ibkr.error ? ` – ${ibkr.error}` : ''}</span>}
                            {ibkr.hint && (
                              <div style={{ marginTop: '4px', color: '#f0a020' }} data-testid="ibkr-status-hint">
                                Diagnose: {ibkr.hint}
                              </div>
                            )}
                            {ibkr.configured && ibkr.probe && (
                              <div style={{ marginTop: '2px', color: '#8a90a8' }} data-testid="ibkr-status-probe">
                                Proxy-Probes: livez {ibkr.probe.livez ?? '–'} · readyz {ibkr.probe.readyz ?? '–'}
                                {!ibkr.token_set ? ' · kein IBKR_GATEWAY_TOKEN gesetzt' : ''}
                              </div>
                            )}
                          </div>
                        )}
                      ...
                      <div className="fee-inputs" style={{ display: 'flex', gap: '14px', marginTop: '10px', flexWrap: 'wrap' }}>
                        <label style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '11px' }}>
                          Kommission (%/Seite)
                          <NumInput step="0.001" min="0" max="0.5" value={feeCfg.fxComm}
                            onCommit={(v) => setFeeCfg(f => ({ ...f, fxComm: v }))}
                            data-testid="fee-forex-comm-input"
                            style={{ width: '110px' }} className="param-input" />
                        </label>
                        <label style={{ display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '11px' }}>
                          Mindestkommission (USD/Seite)
                          <NumInput step="0.5" min="0" max="50" value={feeCfg.fxMin}
                            onCommit={(v) => setFeeCfg(f => ({ ...f, fxMin: v }))}
                            data-testid="fee-forex-min-input"
                            style={{ width: '110px' }} className="param-input" />
                        </label>
                      </div>
                    </div>
                    <button className="control-master-btn active" onClick={saveFees}
                      disabled={feeSaving} data-testid="fee-forex-save-btn">
                      <span className="control-master-label">{feeSaving ? 'SPEICHERT…' : 'GEBÜHREN SPEICHERN'}</span>
                    </button>
                  </div>
                </div>
                  ...
                  <div className="control-card-header">
                    <div className="control-card-info">
                      ...
                      <div className="control-card-desc">
                        ...
                        <span className="sync-status-line" data-testid="watchdog-status">
                          {watchdogStatus?.last_run_at
                            ? <>Letzter Lauf: <b>{new Date(watchdogStatus.last_run_at).toLocaleString('de-DE', { timeZone: 'Europe/Berlin' })}</b>
                              ...
                              {watchdogStatus.adopt_deferred ? ` · ${watchdogStatus.adopt_deferred} zurückgestellt (Entry läuft/Karenz)` : ''}
                              {watchdogStatus.deduped ? ` · ${watchdogStatus.deduped} Duplikat(e) bereinigt` : ''}
                        ...
                        <br />
                        <span className="sync-status-line" data-testid="watchdog-dedupe-hint">
                          Doppel-Trade-Schutz: Positionen eines gerade laufenden KI-Entrys werden nicht
                          übernommen; unbekannte Positionen erst nach {watchdogStatus?.settings?.adopt_grace_sec ?? 90} s
                          Karenz. Der KI-Trade bleibt immer führend, Duplikate an derselben Position werden bereinigt.
                        </span>
                      </div>
                    </div>
                    <button
