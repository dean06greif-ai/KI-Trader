...
const SettingsPanel = ({ onClose, focusStrategy, mode = 'all', controlState, onControlChanged }) => {
  ...
  const [feeCfg, setFeeCfg] = useState({ taker: 0.06, maker: 0.02 });
  ...
  const saveFees = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/settings`, {
        ...
        body: JSON.stringify({ futures_taker_fee_pct: taker, futures_maker_fee_pct: maker }),
      });
      if (res.status === 401) { toast.error('Nicht autorisiert – bitte als Admin anmelden'); return; }
      if (!res.ok) throw new Error();
      toast.success('Futures-Gebühren gespeichert – gelten für alle NEUEN Trades');
