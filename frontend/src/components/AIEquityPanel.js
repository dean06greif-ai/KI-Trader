...
export const AIEquityPanel = () => {
  ...
  const loadMaturity = useCallback(async (retries = 3) => {
    try {
      ...
      setPlaybook(ok ? res : {});
      // Reife-Cache kann direkt nach einem (Neu-)Start noch leer sein -> kurz
      // erneut versuchen, damit die Tabelle beim Öffnen automatisch erscheint.
      const rows = ok ? (Array.isArray(res.maturity) ? res.maturity.length
        : Object.keys(res.classes || {}).length) : 0;
      if (!rows && retries > 0) setTimeout(() => loadMaturity(retries - 1), 1800);
    } catch (e) {
      setPlaybook(prev => prev || {});
      if (retries > 0) setTimeout(() => loadMaturity(retries - 1), 1800);
    }
  }, []);
