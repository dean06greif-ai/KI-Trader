...
const MODE_LABELS = { single: 'Einmal-Durchlauf', loop: 'Auto-Schleife', ai_loop: 'KI-Schleife', optimize: 'Setup-Optimierer' };
const STATUS = {
...
export default function AITraderSeeding() {
  ...
  // die Event-Setups nach oben, bis der nächste Lauf fertig ist (Bug 09/2026).
  const refreshInfo = () => fetch(`${API_URL}/api/ai/playbook/backtest`).then(r => r.json()).then(d => {
    setInfo(d);
    if (d.last_result) setResult(prev => prev || d.last_result);
  }).catch(() => {});
  ...
  const runEventsOnly = async () => {
    const evSel = eventRef.current?.getSelected() || [];
    if (!evSel.length) { toast.error('Mindestens ein Event-Setup anhaken (FOMC/CPI/NFP/PPI/PCE)'); return; }
    const ok = await eventRef.current.start({ aiRevise, aiRounds: mode === 'ai_loop' ? aiRounds : 2 });
    if (ok) toast.success(`Nur Event-Setups laufen: ${evSel.map(e => e.toUpperCase()).join(', ')}`);
  };
  ...
  return (
    <div data-testid="ai-seed-panel">
      <details className="bt-hint ai-seed-intro" data-testid="ai-seed-intro">
        ...
        <b> Nicht-Krypto:</b> Wochenend-/Handelspausen-Kerzen werden vor der Auswertung entfernt; Forex-Historie kommt bei eingeloggtem IBKR-Gateway
        aus IBKR (~2 Jahre), sonst Yahoo (~30 Tage – Zeitraum wird automatisch begrenzt und im Ergebnis vermerkt).
      </details>
      ...
      <div className="ai-seed-grid">
        ...
        <div className="bt-col">
          ...
          <div className="bt-exec" data-testid="ai-seed-mode" style={{ flexWrap: 'wrap' }}>
            ...
            <button className={`bt-exec-btn ${mode === 'optimize' ? 'on' : ''}`} onClick={() => setMode('optimize')} data-testid="ai-seed-mode-optimize"
              title="Optimiert NUR die bereits Edge-validierten Setups auf mehr Trades / höhere Winrate – ein neuer Parameter-Satz wird nur übernommen, wenn der Out-of-Sample-PnL mindestens gleich bleibt">
              <Lightning size={13} weight="bold" /> Optimierer
            </button>
          </div>
          <div className="ai-seed-kv" data-testid="ai-seed-mode-hint">
            ...
            {mode === 'optimize' && <span>Nur Edge-validierte Setups: Parameter werden vorsichtig variiert (±20%, geklemmt) – übernommen wird ein Satz nur bei mehr Trades bzw. höherer Winrate UND mindestens gleichem Out-of-Sample-PnL. Setups ohne Edge werden übersprungen, ein Downgrade ist ausgeschlossen.</span>}
          </div>
        </div>
      </div>
      ...
      <div className="bt-params" data-testid="ai-seed-run-row">
        ...
        {admin ? (
          <>
            ...
            <button className="bt-run" onClick={runEventsOnly} disabled={anyRunning}
              title="Startet AUSSCHLIESSLICH die angehakten Event-Setups (FOMC/CPI/NFP/PPI/PCE) – die Playbook-Setups laufen dabei nicht mit"
              data-testid="event-seed-run-only">
              <CalendarCheck size={15} weight="fill" /> Nur Events testen
            </button>
          </>
        ) : (
      ...
      {(result || eventRows.length > 0) && (
        <div style={{ marginTop: 10 }} data-testid="ai-seed-result">
          <div className="btc-sub">
            {result ? (
              <>ERGEBNIS · {MODE_LABELS[result.mode] || result.mode}{result.trigger === 'auto' ? ' (Automatik)' : ''} · {result.days} Tage ·
                ...
                {result.mode === 'optimize' ? ` · ${result.optimized ?? 0} verbessert` : ''}
                {result.ai_rounds ? ` · ${result.ai_rounds} KI-Revisionen` : ''}
                {result.ai_proposals ? ` · ${result.ai_proposals} für nächsten Lauf vorgemerkt` : ''}
                {running ? <span style={{ opacity: 0.65 }} data-testid="ai-seed-result-stale"> · letzter Stand (neuer Lauf aktiv)</span> : null}</>
            ) : 'ERGEBNIS'}
            {eventRows.length > 0 && (
          ...
          {(result?.data_notes || []).length > 0 && (
            <div className="bt-rule-warnings" data-testid="ai-seed-data-notes">
              <b>Datenhinweise:</b> {result.data_notes.join(' · ')}
            </div>
          )}
