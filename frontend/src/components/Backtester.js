...
export default function Backtester({ onClose }) {
  ...
  // Regel-Treffer: Strategien signalisieren nur bei 100 % erfüllten Regeln
  // (Teil-Treffer = Pre-Signal) – identisch zum Live-Trading (require_all_rules=true).
  // Die frühere Checkbox war deshalb wirkungslos und wurde entfernt; der Wert
  // bleibt für Presets/Copilot-Schema fest auf true.
  ...
  const poll = useCallback((jobId) => {
    // Robustes Polling (siehe Optimizer): kein überlappendes Fetch, transiente
    // Fehler (404 nach Server-Neustart, 502) verstecken den Balken nicht mehr.
    ...
    pollRef.current = setInterval(async () => {
      ...
      busy = true;
      try {
        const r = await fetch(`${API_URL}/api/backtest/status/${jobId}`);
        const j = await r.json().catch(() => null);
        if (!r.ok || !j || !j.status) {
          misses += 1;
          if (misses >= 8) {
            const a = await fetch(`${API_URL}/api/backtest/active`)
              .then(x => x.json()).catch(() => null);
            if (a && !a.active) {
              clearInterval(pollRef.current);
              setJob(prev => prev ? { ...prev, status: 'error',
                error: 'Verbindung zum Lauf verloren (Server neu gestartet?)' } : prev);
              toast.error('Backtest: Verbindung zum Lauf verloren');
            } else { misses = 0; }
          }
          return;
        }
  ...
  const togglePause = async () => {
    ...
    try {
      const r = await fetch(`${API_URL}/api/backtest/${wantPause ? 'pause' : 'resume'}/${job.id}`, {
        method: 'POST', headers: authHeaders(),
      });
      const d = await r.json().catch(() => ({}));
      if (!r.ok) { toast.error(d.detail || 'Aktion fehlgeschlagen'); return; }
      setJob(prev => prev ? { ...prev, pause: wantPause } : prev);
      toast.info(wantPause ? 'Pause angefordert – Backtest hält am nächsten Checkpoint an' : 'Backtest wird fortgesetzt');
  ...
  const validate = () => {
    ...
    if (!selStrats.length || !selCoins.length) { toast.error('Mind. 1 Strategie und 1 Coin wählen'); return false; }
    if (dateMode === 'custom' && !dateFrom) { toast.error('Von-Datum wählen'); return false; }
    return true;
  };
  ...
  const queueToSeries = async () => {
    ...
    await addToSeries('backtest', buildBody());
  };
    ...
    return (
      <div className="btc-panel" key={`cfg-${s.id}`} data-testid={`bt-cfg-panel-${s.id}`}>
        ...
        <div className="btc-grid">
          ...
          <label>ATR-Trailing nach TP1
            <select value={cfg.trail_after_tp1 === undefined ? '' : (cfg.trail_after_tp1 ? '1' : '0')}
              onChange={e => updateCfg(s.id, 'trail_after_tp1', e.target.value === '' ? '' : e.target.value === '1')}
              data-testid={`bt-cfg-trail-${s.id}`}>
              <option value="">Standard (An)</option>
              <option value="1">An</option>
              <option value="0">Aus</option>
            </select>
          </label>
          {cfg.trail_after_tp1 !== false && (
            <label>Trailing-Abstand (× ATR)
              <input type="number" step={0.1} min={0.1} placeholder="1.5" value={cfg.trail_atr_mult ?? ''}
                onChange={e => updateCfg(s.id, 'trail_atr_mult', e.target.value === '' ? '' : parseFloat(e.target.value))}
                data-testid={`bt-cfg-trailmult-${s.id}`} />
            </label>
          )}
    ...
    <SafeOverlay className="bt-overlay" onClose={onClose}>
      <div className="bt-panel" onClick={e => e.stopPropagation()} data-testid="backtester-modal">
        <div className="bt-header">
          <h2><ClockCounterClockwise size={20} weight="bold" style={{ color: '#00A8FF' }} /> BACKTESTER</h2>
          <div className="bt-tabs" role="tablist" data-testid="bt-tabs">
            <button className={`bt-tab ${!aiTab ? 'on' : ''}`} onClick={() => setTab('strategies')}
              role="tab" aria-selected={!aiTab} data-testid="bt-tab-strategies"
              title="Regelbasierte Strategien auf Vergangenheitsdaten testen">
              Strategien
            </button>
        ...
        <div className="bt-params">
          ...
          <button className="bt-tool-btn" onClick={queueToSeries} data-testid="bt-add-series"
            title="Diesen Backtest in die Nacht-Serie einreihen – läuft automatisch nacheinander mit den anderen eingeplanten Jobs">
            <MoonStars size={13} weight="bold" /> + Serie
          </button>
        </div>
          ...
          <div className="bt-progress" data-testid="bt-progress">
            ...
            <div className="bt-progress-row">
              <div className="bt-progress-text" data-testid="bt-progress-text">
                ...
                {job.pause && !job.paused && <span className="bt-eta" data-testid="bt-pausing-hint"> · Pause angefordert…</span>}
                {job.paused && <span className="bt-eta" data-testid="bt-paused-tag"> · ⏸ pausiert{job.paused_total_s > 0 ? ` (${fmtEta(job.paused_total_s)})` : ''}</span>}
                {job.eta_seconds != null && !job.paused && <span className="bt-eta"> · Restzeit {fmtEta(job.eta_seconds)}</span>}
              </div>
              <button className="bt-cancel" onClick={togglePause}
                data-testid={(job.pause || job.paused) ? 'bt-resume' : 'bt-pause'}
                title={(job.pause || job.paused) ? 'Backtest fortsetzen' : 'Backtest anhalten – Zwischenstand bleibt erhalten'}>
        ...
        <StrategyCopilot panel="backtester"
          ...
          getContext={() => ({
            settings: {
              ...
              capital, fee_percent: fee, require_all_rules: requireAll, execution,
            },
            result: result ? {
              days: result.days, date_from: result.date_from, date_to: result.date_to,
              config: result.config || null, benchmark: result.benchmark || null,
              warnings: result.strategy_warnings || [],
              // Ergebnis-Metriken pro Strategie (USDT-Beträge + %-Werte), damit der
              // Copilot Drawdown/PnL korrekt einordnen kann (Backend beschriftet Einheiten)
              per_strategy: (result.per_strategy || []).slice(0, 12).map(s => ({
                strategy_id: s.strategy_id, strategy_name: s.strategy_name,
                trades: s.trades, wins: s.wins, losses: s.losses, breakevens: s.breakevens,
                win_rate: s.win_rate, pnl: s.pnl, pnl_pct: s.pnl_pct, fees: s.fees,
                max_drawdown: s.max_drawdown, max_drawdown_pct: s.max_drawdown_pct,
                avg_pnl: s.avg_pnl, avg_duration_min: s.avg_duration_min,
                liquidations: s.liquidations, symbols: s.symbols, timeframe: s.timeframe,
              })),
            } : null,
          })} />
      </div>
