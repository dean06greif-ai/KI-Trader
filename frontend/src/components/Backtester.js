...
import { X, Play, Trophy, ClockCounterClockwise, Gear, DownloadSimple, ArrowCounterClockwise, Cloud, Desktop, MoonStars } from '@phosphor-icons/react';
...
export default function Backtester({ onClose }) {
  ...
  const [aiSeed, setAiSeed] = useState(!!saved.aiSeed);
  ...
  useEffect(() => {
    try {
      localStorage.setItem(STATE_KEY, JSON.stringify({
        ...
        fee, requireAll, fastPath, execution, aiSeed,
      }));
    } catch { /* ignore */ }
  }, [selStrats, selCoins, days, dateMode, dateFrom, dateTo, capital,
    fee, requireAll, fastPath, execution, aiSeed]);
        ...
        <div className="bt-setup">
          <div className="bt-col">
            ...
            <div className="bt-chips">
              <span className="btc-chipwrap">
                <button className={`bt-chip ${aiSeed ? 'on' : ''}`} onClick={() => setAiSeed(v => !v)}
                  data-testid="bt-strat-ai_trader"
                  title="KI Trader: Playbook-Setups regelbasiert auf Vergangenheitsdaten testen (Backtest-Seeding fürs Reife-Gate je Anlageklasse)">
                  🤖 KI Trader · Setups
                </button>
              </span>
              {strategies.map(s => (
            ...
            {aiSeed && <AITraderSeeding selCoins={selCoins} days={days} dateMode={dateMode} />}
          </div>
          <div className="bt-col">
