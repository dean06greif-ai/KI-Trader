...
import { X, Plus, Trash, FloppyDisk, PencilSimple, ArrowCounterClockwise, DownloadSimple, UploadSimple, Copy, TextAa } from '@phosphor-icons/react';
...
const StrategyBuilder = ({ strategies, enabledIds, onClose, onChanged }) => {
  ...
  const renameStrategy = async (s) => {
    if (!isAdmin()) { toast.error('Admin-Login erforderlich'); return; }
    const name = window.prompt(`Neuer Name für "${s.name}":`, s.name);
    if (name === null) return;
    if (!name.trim()) { toast.error('Name darf nicht leer sein'); return; }
    if (name.trim() === s.name) return;
    try {
      const res = await fetch(`${API_URL}/api/strategies/${s.id}/rename`, {
        method: 'POST', headers: jsonHeaders(), body: JSON.stringify({ name: name.trim() }),
      });
      const d = await res.json().catch(() => ({}));
      if (!res.ok) { toast.error(d.detail || 'Umbenennen fehlgeschlagen'); return; }
      toast.success(`"${d.previous}" → "${d.name}"`);
      if (editingId === s.id) setName(d.name);
      onChanged && onChanged();
    } catch { toast.error('Verbindungsfehler'); }
  };
  ...
  const copilotContext = () => ({
    editing_strategy_id: editingId,
    draft: {
      name, description,
      timeframe: baseDefinition.timeframe || '1m',
      indicators: periods,
      long_rules: serializeRules(longRules),
      short_rules: serializeRules(shortRules),
      sl_mode: slMode, sl_percent: slPercent, sl_ticks: slTicks, crv_target: crv,
    },
    preview,
  });
      ...
      <div className="sb-panel" onClick={e => e.stopPropagation()} data-testid="strategy-builder">
        ...
        <div className="sb-content">
          ...
          <div className="sb-section">
            ...
            {strategies.map(s => (
              <div key={s.id} className="sb-custom-item" data-testid={`strategy-item-${s.id}`}>
                <div>
                  <b data-testid={`strategy-name-${s.id}`}>{s.name}</b>
                  {s.is_custom ? <span className="sb-badge">CUSTOM</span>
                    : s.is_variant ? <span className="sb-badge" title={`Kopie der Strategie ${s.base_id}`}>VARIANTE</span>
                      : <span className="sb-badge sb-badge-preset">VOREINGESTELLT</span>}
                ...
                <div className="sb-item-actions">
                  ...
                  <button className="sb-edit" onClick={() => renameStrategy(s)} data-testid={`rename-strategy-${s.id}`} title="Umbenennen (auch voreingestellte Strategien)">
                    <TextAa size={15} />
                  </button>
                  {s.id !== 'ai_trader' && (
                    <button className="sb-edit" onClick={() => duplicateStrategy(s)} data-testid={`duplicate-strategy-${s.id}`}
                      title={s.is_custom ? 'Strategie duplizieren – Kopie zum Weiterentwickeln, Original bleibt erhalten'
                        : 'Variante anlegen – gleiche Logik, eigene Parameter/Timeframe/Trade-Einstellungen'}>
                      <Copy size={15} />
                    </button>
                  )}
                  {s.is_custom && (
          ...
          <div className="sb-section sb-form-anchor">
            ...
            <StrategyCopilot panel="builder" getContext={copilotContext}
              onApplied={() => onChanged && onChanged()} />
          </div>
        </div>
      </div>
    </SafeOverlay>
