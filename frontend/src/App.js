...
function App() {
        ...
        if (message.type === 'signal') {
          ...
          if (notifyEnabled && signal.signal_class !== 'PRE_SIGNAL') {
            ...
            toast.success(`${signal.type} · ${signal.symbol.replace('USDT','')}`, {
              description: `${signal.strategy_name} · Entry $${signal.entry_price}`
                + (signal.trade_mode ? ` · ${({ live: 'LIVE', paper: 'PAPER', collection: 'DATENSAMMLUNG' })[signal.trade_mode] || signal.trade_mode}` : '')
                + (signal.trade_setup ? ` · Setup ${signal.trade_setup}` : ''),
              duration: 5000,
            });
          }
  ...
  useEffect(() => {
    const iv = setInterval(() => { if (!document.hidden) loadPerformance(); }, 60000);
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => { if (!document.hidden) fetchSession(); }, 30000);
    return () => clearInterval(iv);
  }, [showSettings]);
  ...
  <StrategyAutoTradeModal
    ...
    onSaved={loadAutotrade}
      ...
      {showSeries && (
        <JobSeriesPanel onClose={() => setShowSeries(false)} />
      )}
      {showLiquidity && (
