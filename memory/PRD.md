...
## Iteration 26.06.2026 – Plan-Fortsetzung: 1.6-Nachtrag + Audit 2.1 (Zentraler Entry-Guard)
Branch `conflict_120926_2234` 1:1 nach /app übernommen (lokale .env: lokale Mongo,
AI_TRADER_LOCAL_DISABLE=1, keine Exchange-/LLM-Keys). Baseline-Unit-Suite verifiziert.
- **1.6 Risikobudget**: war bereits umgesetzt (risk_budget.py + Hook + 6 Tests grün),
  Protokoll in UMSETZUNG_FORTSCHRITT.md nachgetragen.
- **2.1 Zentraler Entry-Guard FERTIG**: `services/entry_guard.py` (neu) – `EntryChecks`-Protokoll,
  `check_entry()` (trade_guard: Kill-Switch/Lernpflicht/Übergangsschutz/Anti-Stacking + Regime-Gate),
  `check_risk_budget()` (fail-open). In `bitunix_trade._on_signal_impl` eingehängt; Fee-Wächter,
  Low-Vol-ATR-Block, Kapital-Limit werden an bestehender Stelle protokolliert. Ergebnis als
  `entry_checks[]` am Trade-Dokument. Kein Verhaltensbruch (Reihenfolge/Reject-Gründe identisch).
  Tests: `tests/test_entry_guard.py` (6 neu), Wiring-Test in test_risk_budget.py angepasst.
- **Test-Hygiene**: 2 veraltete Tests repariert (Minute-Cooldown 65s; ScannerStub.save_settings).
  Unit-Suite 1300 passed; Rest-Fails nur test_iter38_* (braucht Dev-Server :8055 + Live-Keys).
- Testing-Agent iteration_59: Backend 100% (69/69 Unit, 11/11 API-Smoke inkl. Auth/401/Risk-Budget-Roundtrip).
- API-Hinweis: Risk-Budget/Trade-Guard-Pfade sind `/api/risk-budget`, `/api/risk-budget/config`, `/api/trade-guard`.
- **NÄCHSTER SCHRITT: 2.2 ML-Leckage schließen** (nearest_snapshot allow_future=False,
  purged_walk_forward mit label_ts=closed_at, nested Kalibrierung) in ai_ml_lab.py/ml_gate.py.
