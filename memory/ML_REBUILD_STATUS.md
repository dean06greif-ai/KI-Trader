...
## Aktueller Stand (Datum: 2026-09-02, 11. Handover – Render Pro 2 GB, Risiko-Sizing, SMC-Setups, Setup-Entdeckung)
Branch-Basis conflict_020926_0652. Vollständiger Diff gegen Upstream: memory/changes_020926_risk_sizing.patch.
**Prod-Befund (read-only, backend/scripts/sizing_check_prod.py)**: Equity ~700 USDT (Bitunix
available 298 + margin 403), KI-Live-Trades 30 d n=40: Marge Median 8,2 USDT, Hebel Median 8,5x,
Risiko/Trade ~0,1–0,5 USDT (!). Kette: Coin-max_capital 30 × capital_pct (LLM 20–30 %, durch
"defensive" Lektionen) × ml_risk_scale 0,5 (990/1605 Entscheidungen). squeeze_breakout live 7 % Win
(n=15) vs. 48 % Paper. Upstream-Bugs: orderflow.py + ai_ml_lab.py ohne `import os` (ImportError →
kein echter Orderflow im Prompt, kein ML-Auto-Training).
**Umgesetzt**: services/position_sizing.py (Modus risk, Boot-Migration risk_sizing_v1), ai_engine
(Defaults/Klemmen/Prompt-Block/ai_sizing im Signal), bitunix_trade.on_signal (Marge+Hebel aus
Risiko-Budget, Trade-Feld `sizing`), ai_playbook (order_block/fvg_fill/htf_range, Live-Divergenz-
Gate `live_blocked`, KI-Setups `new_setups` → propose_custom_setup, Shadow/Ausmusterung),
smc_zones.py (OB/FVG 5m/15m/1h im Prompt), ram_guard.profile()/Start-Log, ram_queue inactive_file,
/api/system/ram.container, UI (AITradingPanel Sizing-Controls, AIEquityPanel Live-Spalte + KI-Badge).
**Tests**: tests/test_risk_sizing_and_playbook_v2.py (12 grün), backend/tests/test_iter35_… (8 grün,
Testing-Agent), Suite 934 grün (3 Failures upstream-identisch). Frontend E2E grün (Iteration 35).
**Nach Render-Deploy prüfen**: Log-Zeilen `RAM-Profil: 2048 MB … GROSS` und `Boot-Migration:
KI-Positionsgröße -> Risiko-Modus`; erste Trades: Feld `sizing` im Trade-Dokument, Marge ~40–100 USDT
@ bis 50x statt 3–9 USDT @ 7x. Risiko-Regler im KI-Panel (Positionsgröße).
**Offen/Empfehlung**: Sweep-Trigger (Latenz), Per-Setup-Risiko-Faktor, Coin-Auto-Hebel-Migration
(leverage_proposals_v1 hatte Auto-Hebel je Coin deaktiviert – im Risiko-Modus irrelevant).


## Aktueller Stand (Datum: 2026-08-26, 10. Handover / Schritt 4 — Manuell-Trade-Anzeige, Momentum-News-Bremse, Slippage-UI, Latenz-Assessment)
**User-Bug (Screenshot, manueller BTC-Trade via Watchdog übernommen)**: Karte
zeigte unmöglichen Hebel 333.33x (Bitunix-Max 200x) und PnL +335,81% statt
Bitunix-ROI +158,33%. RCA: `sync_position_state` speicherte den aus der Marge
abgeleiteten EFFEKTIVEN Hebel als Anzeige-Hebel (nach Margen-Entnahme 9.076
USDT: 3025/9.076=333x); PnL-% rechnete auf die geschrumpfte Rest-Marge.

**Fixes (alle getestet, 4/4 + 892 Unit grün)**:
1. Sync: `leverage` = Börsen-SETTING (wie Bitunix-App), `effective_leverage`
   separat (Marge-Wahrheit); `trade_bound_margin` nutzt effective_leverage
   (Kapital-Berechnung + Anti-Flattern bleiben korrekt, Test angepasst:
   test_capital_sync_fixes verlangte explizit das ALTE Verhalten).
2. PnL-%-Basis wie Bitunix (core/utils._enrich_trade): GESCHLOSSEN → Initial-
   Marge (Notional/Hebel-Setting, wie Bitunix-Position-History-ROI: 161,5% ≈
   158,33%, Rest = Fees/Funding); OFFEN → aktuell gebundene Marge (wie deren
   Live-Anzeige). Normale Trades (Marge==Notional/Hebel) unverändert.
3. Boot-Migration `manual_lev_display_v1`: Alt-Trades mode=live mit leverage
   >200 → effective_leverage übernommen, Anzeige auf 200 gedeckelt (nur
   unmögliche Werte, Paper unangetastet, idempotent).
4. Frontend TradeDetailCard: Hebel zeigt "200x (effektiv 333.33x)" wenn
   abweichend; "Kapital" zeigt die aktuell gebundene Marge (computed.margin_used)
   statt des veralteten max_capital.
5. **Momentum-News-Bremse (datenbasiert)**: Analyse ergab: momentum_news ist
   GESAMT das stärkste Setup (n=206, 52% win, +190 USDT), verliert aber exakt
   im Band 70-74 (n=75, 43%, −201; 60-64: +185, 75-79: +157) → KEIN genereller
   Setup-Drossel! Neuer Guard `_momentum_brake_block`: blockt NUR Live-Entries
   von momentum_news im Config-Band `momentum_news_conf_block` (Default [70,74],
   []=aus, UI-klemmbar); Sammel-Trades messen weiter, ob das Band schlecht bleibt.
6. **Slippage-Karte neu (UI-Wunsch)**: standardmäßig EINGEKLAPPT (Kopfzeile mit
   n Messungen + Σ Kosten), aufklappbar; je Strategie eigener aufklappbarer
   Block mit KLARNAMEN (Endpoint liefert jetzt strategy_name aus db.strategies,
   ai_trader="KI Trader"); saubere Spalten (Order-Art | Modus | Fills |
   Ø Slippage | Σ Kosten | Ø MFE | Ø MAE) statt kryptischer Inline-Zeile.

**Latenz-Frage des Users (15-min-Intervall verpasst 1m/5m-Wick-Sweeps) —
ehrliches Assessment**: Möglich, aber NICHT als Schnellschuss. Das Intervall
einfach zu senken (UI erlaubt bis 2 min) würde die Analyst-Calls ver-7-fachen
(~1979/Woche → ~14k, nemotron-Free-Limit 1000 Req/Tag wäre gerissen). Richtige
Lösung = **Sweep-Trigger** (eigener Schritt): billiger lokaler Detektor (ohne
LLM) auf den 1m-Candles im Scanner-Buffer erkennt Wick-Sweeps (langer Docht +
Reclaim) und stößt eine GEZIELTE Einzel-Symbol-Analyse an (Budget-Deckel z.B.
20 Extra-Calls/Tag + Cooldown je Symbol). Benötigt Refactor run_analysis →
symbols-Parameter. Als nächster Schritt vorgeschlagen, Aufwand ~1 Session.

**Nächster geplanter Schritt**: Render-Deploy (Schritte 1-4), danach
leverage_effect_check + Slippage-Karte live prüfen. OFFEN: User-Go für
Sweep-Trigger (Latenz) als eigener Schritt.


## Aktueller Stand (Datum: 2026-08-26, 10. Handover / Schritt 3 — Getrennte Welten für Guards + Proposal-Aufräumer + Hebel-Wirkungs-Check)
**Bug-Report des Users bestätigt (Richtungs-Guard)**: "bereits 10 offene LONG-Risiken
… Limit 3" obwohl KEIN Live-Trade offen war – Sammel-Trades (data_collection=true)
wurden an DREI Stellen mit echten Trades vermischt:
1. `_diversification_gate` (Richtungs-/Korrelations-/Cluster-Guard) zählte ALLE
   offenen ai_trader-Trades → Sammel-LONGs verbrauchten die Live-Slots.
2. MasterPrompt-`max_open_trades`-Zähler in `_emit_signal` zählte alle Trades.
3. `_today_risk` (max_daily_loss/max_trades_per_day) zählte Sammel-Paper-PnL mit.
NEBENBEFUND: `collection_max_same_direction` (Default 5) war eine TOTE Einstellung
(definiert + UI-Klemme, aber nirgends benutzt) – Sammel-Richtung war unbegrenzt.

**Fix (Getrennte Welten, alle drei Stellen)**:
- Live-Einstiege prüfen nur echte Risiko-Trades (dc≠true); Sammel-Einstiege prüfen
  nur andere Sammel-Trades mit eigenem Limit `collection_max_same_direction`
  (jetzt verdrahtet). Korrelations-Guard + Playbook-Sperren weiterhin nur Live.
- Neuer Helper `_open_ai_trades_count(collection)` für den MasterPrompt-Zähler.
- `_today_risk` zählt nur noch echte Trades (Sammel verbraucht keine Tageslimits).
- Einschätzung zur User-Frage "ist der Richtungs-Guard sinnvoll?": JA für echte
  Trades (begrenzt gleichgerichtetes Klumpenrisiko, Hedge bleibt immer erlaubt) –
  er war nur durch die Vermischung kaputt. Duplikat-Guard (Schritt 2) bleibt
  bewusst weltübergreifend (identisches Signal doppelt ist immer ein Duplikat).

**Proposal-Aufräumer (dauerhaft)**: neuer `_insert_proposal`-Pfad in
ai_engine_governance – beim Anlegen eines Vorschlags werden ältere OFFENE
Duplikate (gleicher Scope+Symbol+Änderungs-Keys) automatisch 'superseded'.
Blockierte/fehlerhafte neue Vorschläge verdrängen nichts. Kein Stapeln von
hunderten identischen Vorschlägen mehr (Befund: ~285).

**Hebel-Wirkungs-Check (Werkzeug)**: `scripts/leverage_effect_check.py`
(read-only; --prod aus Dev, ohne Flag auf Render). Lauf 26.08. gegen Prod:
15/16 Symbolen weichen ab bzw. Auto-Hebel aktiv – bestätigt die RCA numerisch;
nur POLUSDT (fix 5x, auto aus) läuft exakt nach Config. Nach Deploy der
Boot-Migration (Schritt 2) erneut laufen lassen: Senkungs-Symbole müssen
'fix …x / OK ✓' zeigen.

**Tests**: neu tests/test_guard_separation.py (7 Blöcke grün: Bug-Repro 10
Sammel-LONGs blocken Live nicht mehr, Live-Limit 3 nur Risiko-Trades, Sammel-
Limit 5 verdrahtet, Cluster je Welt, Zähler je Welt, Tagesrisiko ohne Sammel,
Aufräumer inkl. blocked_master-Schutz). Regression: 892 Unit-Tests grün,
Schritt-1/2-Suiten (3/3, 7/7) weiter grün.

**Nächster geplanter Schritt**: Render-Deploy (enthält Schritt 1–3), danach
leverage_effect_check erneut + Guard-Verhalten im Log beobachten. Backlog
unverändert (momentum_news/Bucket 70–74, Gate-Neubewertung nach Messphase).


## Aktueller Stand (Datum: 2026-08-26, 10. Handover / Schritt 2 — Klick-Liste automatisiert als Boot-Migrationen + neue Guards)
**User-Entscheidungen dieses Schritts**: (1) Analyst-Kette: nemotron:free als Primär
offiziell, gpt-oss-20b:free Fallback 1, deepseek-v4-flash (paid) bleibt Sicherheits-
netz — KEIN neuer Provider nötig (OpenRouter-Free: 1000 Req/Tag bei ≥10$ Guthaben,
Analyst braucht 125–414/Tag, 8 Tage belegt). WICHTIGE KORREKTUR: Groq wäre als
Analyst-Primär falsch (Free: 200k Tokens/Tag PRO KONTO, 8k TPM < unsere ~15k/Call).
(2) Hebel-Proposals automatisch abarbeiten: JA. (3) Trade-Manager-Kappen 200 bleiben
ABSICHTLICH (Deans Taktik: Puffer SL↔Liq + hoher Hebel) — NICHT anfassen.

**Umgesetzt (alle einmaligen Migrationen laufen beim nächsten Render-Deploy von
selbst; Marker in settings._id='boot_migrations' verhindern Wiederholung und
respektieren spätere UI-Änderungen)**:
1. `services/boot_migrations.py` (neu) + Aufruf in server.py-Startup nach scc-Load:
   - cerebras_shutdown_v1: Analyst-Kette = nemotron:free → gpt-oss-20b:free →
     deepseek-v4-flash; übrige tote Cerebras-Slots (Chat-/Observer-Fallback
     gemma-4-31b) → openrouter/gpt-oss-20b:free. Gesunde Slots unangetastet.
   - heatmap_off_v1: use_heatmap_data True→False (RCA erfundene Formel-Level);
     User kann es danach im UI wieder einschalten, Migration läuft nie erneut.
   - slippage_artifacts_v1: Phantom-Slippage (|slip|>2%, market/taker) aus
     auto_trades entfernt (ersetzt das manuelle Render-Skript; Skript bleibt
     als Fallback bestehen).
   - leverage_proposals_v1: reine Hebel-SENKUNGEN angewendet (neuester je Symbol,
     Vergleich gegen EFFEKTIVEN Hebel = auto_lev_max bei Auto-Hebel), ältere als
     'superseded', ERHÖHUNGEN bleiben zur manuellen Entscheidung liegen,
     max_same_direction/cooldown_min-Vorschläge abgelehnt ("zu restriktiv für
     Messphase").
2. Hebel-Wirksamkeit dauerhaft: `_apply_changes` (ai_engine_governance) setzt bei
   jedem Hebel-Change automatisch auto_leverage_enabled=false mit (gilt für
   Auto-Apply, User-Bestätigung UND Migration); explizit mitgeschicktes Flag wird
   respektiert. Damit blockieren sich Auto-Hebel und Hebel-Proposals nie mehr.
3. Duplikat-Guard (neu, ai_engine._dup_entry_block + Aufruf in _emit_signal nach
   Stale-Guard): blockt identischen Symbol+Richtung-Einstieg, solange ein offener
   KI-Trade jünger als `dup_entry_window_min` (Default 30, 0=aus) ist. DB-basiert
   (überlebt Deploys, anders als der In-Memory-Sammel-Cooldown). Gilt für Live-
   UND Sammel-Trades.
4. Rohstoff-Stale-Limit: `stale_price_max_min_commodity` (Default 20 min) für
   GOLD/SILVER/OIL (group RESOURCES) — behebt die systematischen "11 min alt
   (Limit 10)"-Blocks. Übrige Symbole weiter `stale_price_max_min`.
   Beide neuen Keys mit update_config-Klemmen (0–120 bzw. 0–720).

**Tests**: neu tests/test_boot_migrations_and_guards.py (7 Blöcke, alle grün:
Cerebras-Migration/Idempotenz, Heatmap+User-Wahl-Respekt, Artefakt-Bereinigung
(Maker bleibt), Proposal-Abarbeitung inkl. Erhöhung-bleibt-liegen, _apply_changes-
Regel, Duplikat-Guard-Fenster/Richtung/0=aus, Stale-Limits). Regression:
892 Unit-Tests grün. Dev-Boot verifiziert: Migrationen laufen, Marker gesetzt,
API zeigt neue Keys (dup 30, commodity 20, heatmap False).

**Beweis für User-Zweifel dokumentiert**: ai_token_usage Rolle analyst zeigt seit
18.08. zu 100% nemotron:free (2,2–6,2M Tokens/Tag, 125–414 Calls/Tag, ~33M gesamt);
deepseek-fallback2 hat 0 Analyst-Zeilen, weil Fallback 1 nie hart versagte.

**Nächster geplanter Schritt**: Nach Render-Deploy Migrations-Log prüfen (eine
Zeile je Migration), dann Messphase: momentum_news/Konfidenz-70-74-Analyse als
Kandidat. OFFEN beim User: Deploy anstoßen; Erhöhungs-Proposals (z.B. AVAX 9.6x)
manuell entscheiden.


## Aktueller Stand (Datum: 2026-08-26, 10. Handover — Großer Check + Cerebras-402-RCA + smart_skip/Slippage-Fix)
**Kontext**: Neuer Agent hat übernommen. Diese Datei war NIE im Repo committet und
ging mit der alten Umgebung verloren — wiederhergestellt aus dem User-Repo
AntonHeinrich05/ml-implementation-ML_REBUILD_STATUS.md (Branch 0.83-ki-verlauf-chart,
Stand 9. Handover 17.08). AB JETZT: Datei liegt in /app/memory/ und wird mit jedem
Commit gepusht (memory/ ist nicht ignoriert, nur test_credentials.md).
Der Code-Stand (Branch conflict_250826_1035, 25.08) ist NEUER als das 9. Handover:
zusätzlich Iteration 15–17 einer parallelen Session (Korrelations-Guard-Hoheit,
402-Cooldown-Fix, Lektionen-Lebenszyklus dormant/reaktivierbar, Key-Status-Panel,
ai_engine.py-Split in ai_engine_context/_governance/_housekeeping + regime_core).

1. **Cerebras-Token-Rätsel WIRKLICH gelöst (Korrektur des 9.-Handover-Befunds)**:
   Es ist KEIN Quota-Sharing-/TPM-Problem. Direkter API-Test aller 16 Keys aus Dev:
   **alle liefern 402 payment_required**. Web-Recherche bestätigt: **Cerebras hat
   das Free-Tier zum 17.08.2026 eingestellt** (Kreditkarten-Pflicht, $5 Promo-Credits,
   30 Tage gültig) — exakt seit diesem Tag läuft der Analyst zu 100% auf Fallback
   nemotron-3-super:free (OpenRouter). Mehr Gratis-Konten bringen NICHTS mehr.
   Alle anderen Keys gesund (Tiny-Call-Test): Groq 3/3, OpenRouter 5/5, Mistral 2/2 OK.
   → Empfehlung an User: Analyst-Primär auf groq/gpt-oss-120b (gleiches Modell wie
   bei Cerebras!) umstellen, nemotron:free als Fallback lassen; Chat-Fallback
   (cerebras/gemma) ebenfalls ersetzen. Erledigt der User im UI (KI-Team-Panel).
2. **Hebel-RCA — warum bestätigte Hebel-Senkungen oft NICHT wirken**: In
   bitunix_trade.on_signal gilt: `auto_leverage_enabled=true` ⇒ effective_leverage()
   (Liquidation-hinter-SL-Formel, gedeckelt von auto_lev_max) — das bestätigte
   `leverage`-Feld wird dann IGNORIERT. Prod-Beweis: BTC 7.68 auto_applied 24.08,
   Trades danach mit lev=25 (=auto_lev_max BTC); EURUSD bestätigt 6x, Trades mit 50x
   (=auto_lev_max EURUSD); SILVER 67.57x (auto_lev_max 100). Nur Symbole mit
   auto_leverage_enabled=false traden den bestätigten Hebel (DOT 7.2 ✓, AVAX 8 ✓,
   POL 5 ✓). → Die ~285 offenen Hebel-Proposals sind großteils wirkungslos, solange
   auto_lev an bleibt. Klick-Liste unten; sauberer Code-Fix (Proposal-Apply setzt
   auto_leverage_enabled=false mit, ODER Hebel-Proposals deckeln auto_lev_max) ist
   OFFENE ENTSCHEIDUNG des Users.
3. **Slippage-RCA custom_23a30b65 (Ø ~4,95% "Slippage" live)**: MESSARTEFAKT, kein
   echter Fill-Verlust. parse_order_fill fiel bei MARKET-Orders aufs generische
   'price'-Feld zurück (Bitunix liefert dort den Schutz-/Limitpreis ~±5% vom Mark,
   solange avgPrice noch leer ist). Beweis: alle Messwerte clustern 4,2–5,04%, Entry
   blieb = Signalpreis, PnL unauffällig. **FIX umgesetzt (dieser Schritt)**:
   - parse_order_fill(allow_price_fallback=False) für die Messung (Maker-Fluss
     unverändert mit Fallback, dort ist Limitpreis=Fill korrekt);
   - neue reine Funktion fill_price_for_slippage(): nur avg-Fill gefüllter Orders
     (Status FILL*/filled_qty>0), Plausibilitäts-Grenze |slip|>2% ⇒ verwerfen+warnen;
   - 1 Retry nach 2s, falls Fill noch nicht registriert.
   - Alte Artefakte in Prod: scripts/cleanup_slippage_artifacts.py (Dry-Run-Default,
     --apply verweigert PROD_MONGO_URL hart; AUF RENDER ausführen).
4. **smart_skip_move_pct 0.02 → 0.10 (User-Freigabe, umgesetzt)**: Einmalige
   idempotente Migration in load_config (Marker smart_skip_migrated_v1, Muster
   crv_max_migrated_v1); bewusste spätere User-Werte bleiben. Dev-E2E verifiziert
   (Restart-Log "0.02 -> 0.10 migriert", API zeigt 0.1). Wirkt in Prod nach Deploy.
5. **Rewards-Entwarnung**: Befund "Reward>0: 0/315" im 9. Handover-Umfeld war ein
   BUG IM CHECK-SKRIPT (las Feld `reward`, es heißt `score`). Real: 169/316 Scores
   positiv, Ø +6.9 — Lernsignal ist gesund. big_check_prod.py gefixt (score,
   Gate-Metrik-Keys oos_auc/oos_brier_calibrated, Signals-Feld timestamp statt ts).
6. **Tests**: neue tests/test_smart_skip_and_fill_source.py (3 Blöcke: Fallback-
   Schalter, Mess-Guards, Migration idempotent). Regression: **892 Unit-Tests grün**
   (pytest -m unit). Live-Tests schlagen in frischer Dev-DB teils fehl
   (umgebungsbedingt: keine Seeds; test_iter12 days=0-Klemme erwartet 30, Code
   klemmt auf 1 — vorbestehende Diskrepanz, NICHT von diesem Schritt).

### Großer Check (read-only Prod, 26.08) — Kernzahlen
- KI 7d: 900 Decisions, 132 geschlossene Trades, Winrate 42%, PnL −60 USDT
  (Sammel −45, Live-Logik −15). Konfidenz-Buckets: <65: +221 (55% win, n=195),
  65–69: −25, **70–74: −185 (45% win, n=118 — Verlust-Bucket, v.a. momentum_news
  n=~60 und XRP/DOGE/DOT)**, 75–79: +139 (63% win, n=59), 80+: n klein.
- Guards 7d: Korrelations-Guard 149 Blocks; **Stale-Price-Guard blockt SILVER/OIL/
  GOLD systematisch mit "11 min alt (Limit 10)"** — Commodity-Feed-Takt ~11 min
  kollidiert mit 10-min-Limit ⇒ Empfehlung: stale_price_max_min 15 (UI) oder
  Asset-Klassen-Limit (Code, offene Entscheidung).
- Offene Trades: 10 (EURUSD LONG 2x seit 20.08 mit 50x + EURUSD SHORT-Hedge 2x,
  USDJPY SHORT 2x fast identische Entries 15 min auseinander ⇒ Anti-Stacking prüfen).
- Gate v37 (Shadow): 2425 Samples, **oos_auc 0.504** (v6: 0.597 bei 219 Samples),
  Kalibrierung kollabiert auf 1 Bin (~0.47) ⇒ prognostiziert quasi Konstante;
  would_block 24–29%, blockt anteilig MEHR Gewinner (31%) als Verlierer (26%).
  Gate darf so NICHT scharf geschaltet werden. Ehrliche Einschätzung unten.
- Token 7d: Analyst 25M tok/1979 Calls (nemotron:free), Learner 5.6M (deepseek-v4-pro),
  TM 5.6M (deepseek-v4-flash), deep_analyst 1.9M (nemotron-ultra:free). Kein Kosten-
  Notstand; smart_skip-Fix senkt Analyst-Calls deutlich.
- use_heatmap_data in Prod WEITER AN (war laut RCA auszuschalten — wurde nie
  ausgeschaltet, kein Proposal-Flip gefunden; Code-Default ist False, aber
  gespeicherte Prod-Config gewinnt). Trade-Manager max_leverage=200 (!) und
  profit_lock_max_leverage=200 — Empfehlung im Settings-Check war 25/50.

### Klick-Liste für den User (PROD-UI, nach Deploy dieses Stands)
1. KI-Setup: use_heatmap_data AUS (use_liquidation_data AN lassen).
2. KI-Team: Analyst Primär = groq / gpt-oss-120b (Fallback nemotron:free lassen);
   Chat-Fallback cerebras/gemma-4-31b ersetzen (z.B. groq gpt-oss-120b).
3. Proposals: Hebel-Senkungen bestätigen NUR wo auto_leverage_enabled=false gesetzt
   werden kann — sonst zuerst je Coin auto_lev AUS bzw. auto_lev_max senken
   (mind.: EURUSD/USDJPY/GBPUSD 50→~6–10, SILVER 100→~15, GOLD 50→~15, BTC 25→~10).
   max_same_direction=2 und cooldown=30 ABLEHNEN (zu restriktiv in Messphase);
   sl_mode=atr erst NACH der Messphase.
4. Trade-Manager: max_leverage 200→25, profit_lock_max_leverage 200→50.
5. Stale-Price-Guard: Limit 10→15 min (entblockt SILVER/OIL/GOLD).
6. Auf Render einmalig: python scripts/cleanup_slippage_artifacts.py --apply
   (entfernt die ~5%-Phantom-Slippage-Messwerte; Dry-Run vorher möglich).

### Ehrliche Einschätzung: ML-Gate vs. besserer KI-Trader (User-Frage)
Das ML-Gate ist aktuell WERTLOS (AUC 0.504 = Münzwurf) — nicht weil ML unsinnig
ist, sondern weil die Trainingsdaten heterogen sind: Sammel-Trades mit erfundenen
Leveln, Wochenend-Forex, wechselnde Configs, Regime-Rauschen. Reihenfolge daher:
(1) Prozess-Hürden beheben (Hebel-Wirksamkeit, Heatmap aus, Guards messen lassen),
(2) 2–4 Wochen SAUBERE Daten sammeln (die Guards von Iteration 39/40 laufen erst
seit Kurzem), (3) DANN Gate neu bewerten: Kriterium oos_auc ≥ 0.60 stabil über
2 Retrains UND blockt nachweislich mehr Verlierer als Gewinner im Schatten.
Bis dahin bleibt Shadow-Modus genau richtig. Konfidenz-Bucket 70–74 ist derweil
ein PROMPT/Kalibrierungs-Problem des Analysten (Konfidenz trennt nicht monoton) —
kurzfristig wirksamer: momentum_news-Setup drosseln/prüfen statt ML scharf schalten.

### Nächster geplanter Schritt
- User-Antworten zur Klick-Liste + offene Entscheidung Hebel-Proposal-Apply
  (auto_leverage_enabled mitsetzen? Code-Fix nur nach Freigabe).
- Danach Kandidaten (klein, isoliert): (a) Proposal-Apply-Fix Hebel,
  (b) Stale-Limit je Asset-Klasse, (c) momentum_news-Analyse/Drossel,
  (d) days=0-Klemme in slippage-stats an Test angleichen (30 statt 1).


## Aktueller Stand (Datum: 2026-08-17, 9. Handover — Hebel-Bestätigung Prod + Verlauf-Reiter)
1. **Hebel-Vorschläge in PROD bestätigt (User-Auftrag "tu das")**: Über das
   offizielle Prod-Admin-API (POST /api/ai/proposals/{id} action=approve auf
   crypto-scanner-backend-hil2.onrender.com, Präzedenzfall Iteration 39) wurden
   die 6 REINEN Hebel-Senkungs-Vorschläge der KI bestätigt und read-only in
   Atlas verifiziert (landen als strategy_coin_configs `ai_trader_<SYM>.config`
   — NUR der KI-Trader betroffen, andere Strategien unberührt):
   BTCUSDT 8x, QQQUSDT 8x, GBPUSD 6x, EURUSD 6x, ADAUSDT 7.2x, DOTUSDT 6x.
   BEWUSST NICHT angefasst: XRPUSDT 22x (wäre ERHÖHUNG), SOLUSDT 14x (13→14,
   keine Senkung), DOT-Mischvorschlag mit sl_mode=atr, alle sl_mode-/
   min_confidence-/cooldown-Vorschläge (nicht beauftragt, liegen weiter in
   needs_confirmation). Hinweis: /api/ai/proposals/actionable ist in Prod leer,
   weil autonomy=auto — needs_confirmation-Vorschläge sind nur per API/DB sichtbar.
2. **NEUER Reiter "Verlauf" (Equity-Kurve der KI-Trades)** — testing_agent
   iteration_42 alles grün (inkl. Lazy-Load-Nachweis):
   - Backend: `GET /api/ai/equity-curve?days=0..365&mode=all|live|collection`
     (routers/ai.py): kumulierter realisierter PnL der geschlossenen
     ai_trader-Trades, zeitlich sortiert; summary mit trades/wins/winrate/
     total_pnl/peak_equity/max_drawdown/fees. Leere DB (Reset) → leere Kurve.
   - Frontend: `AIEquityPanel.js` (SVG-Kurve im Stil der DeepAnalytics-Equity,
     Mode-Buttons Alle/Live-Logik/Sammel, Zeitraum Gesamt/7/30/90 Tage, Reload,
     KPIs, Empty-State). Tab-Button "Verlauf" (`ai-equity-toggle`) neben
     "Strategien"; exklusiv zu den anderen Reitern (toggleCurve, chatVisible,
     Proposals-Banner um !showCurve erweitert). Daten laden ERST beim Öffnen
     des Reiters (Komponente mountet erst dann).
   - Tests: `tests/test_ai_equity_curve.py` (4 Blöcke) + vom testing_agent
     ergänzt `backend/tests/test_ai_equity_curve_api.py`.

## Aktueller Stand (Datum: 2026-08-17, 9. Handover — Guard-Cockpit + Regime-Qualitäts-Bewertung)
1. **Guard-Cockpit (umgesetzt, testing_agent iteration_41 grün)**:
   - Backend: `GET /api/ai/guard-stats?days=7` (routers/ai.py) aggregiert
     `ai_decisions.blocked_by` in 10 Kategorien (weekend/stale/regime/correlation/
     direction/cluster/playbook/master/fee/other; fee aus `fee_guard_blocks`),
     je mit count, Sammel-Anteil und letztem Block-Beispiel.
   - **Regime-Schatten-Zähler**: Der Endpoint zeigt zusätzlich, wie viele
     LIVE-Einstiege der (deaktivierte) Regime-Sperrfilter geblockt HÄTTE
     (signaled-Decisions mit entry_market_snapshot.regime in regime_block_list).
     → Damit lässt sich die Aktivierungs-Entscheidung datenbasiert treffen.
   - Frontend: Block "Guard-Cockpit" unten im KI-Setup (`ai-guard-cockpit`,
     Zeilen `ai-guard-row-<key>`, leerer Zustand `ai-guard-cockpit-empty`,
     Schatten-Zeile `ai-guard-regime-shadow`), Tooltip je Zeile = letzter Grund.
   - Tests: `tests/test_guard_cockpit.py` (3 Blöcke, geseedete Kategorisierung);
     testing_agent legte zusätzlich `backend/tests/test_guard_cockpit_api.py` an.
2. **Regime-Qualitäts-Bewertung (Antwort auf User-Frage "ist die Erkennung gut
   genug für die Sperre?")**: NEIN, noch nicht beweisbar — Aktivierung NICHT
   empfohlen. Belege (scripts/analyze_regime_quality.py, read-only Prod):
   (a) 14 der 20 range_ruhig-Verluste waren NICHT-Krypto NACH Marktschluss
   (Fr-Nacht/Sa/So) — die 10%-Winrate ist durch genau die Trades kontaminiert,
   die der neue Wochenend-Guard jetzt verhindert; (b) "ruhig"-Anteil 82% der
   Snapshots (Soll ~30%) — Forex-Wochenend-Stale + dünne Percentile-Historie
   (vol_basis fallback ~50%); (c) alle anderen Regime-Buckets n<15.
   → Vorgehen: Guards deployen, 1–2 Wochen saubere Daten, dann Schatten-Zähler
   + analyze_regime_quality erneut prüfen. Aktivierungs-Kriterium (Vorschlag):
   range_ruhig n>=25 saubere Trades und Winrate <25% und Schatten-Zähler zeigt
   relevante Trefferzahl.

## Aktueller Stand (Datum: 2026-08-17, 9. Handover — Stale-Price-Guard + Regime-Sperrfilter)
Vom User freigegeben, umgesetzt und verifiziert (testing_agent iteration_40: alles grün,
Backend + Frontend, 23 Testblöcke):
1. **Stale-Price-Guard (aktiv, Default 10 min)**: `ai_engine._emit_signal` lehnt
   KI-Einstiege ab, wenn die letzte Kerze des Symbols älter als
   `stale_price_max_min` ist (Klemme 0–120, 0=aus). Gilt für Live- UND
   Sammel-Trades (veraltete Preise sind auch als ML-Label wertlos). Leerer
   Buffer (Boot) blockt nicht. RCA: EURUSD-Doppel-Trade auf ~45 min altem Preis.
   UI: Dropdown "Preis-Frische (min)" (`ai-stale-price-select`).
2. **Regime-Sperrfilter (eingebaut, Default AUS — bewusst, User-Entscheidung)**:
   `regime_block_enabled=false`, `regime_block_list=["range_ruhig"]`. Wenn aktiv:
   blockt NUR Live-Neueinstiege (Governance-Feed-Eintrag); Sammel-Trades laufen
   bewusst weiter, damit die Statistik den Filter beweisen/widerlegen kann.
   Regime-Quelle: `market_observer.snapshots` (aktuellstes Regime pro Symbol).
   UI: Dropdown "Regime-Sperre" (`ai-regime-block-select`).
   AKTIVIEREN erst nach 1–2 Wochen sauberer Nach-Wochenend-Guard-Daten —
   dann Winrate je Regime neu prüfen (big_check_prod.py Abschnitt J/G).
3. Guard-Reihenfolge in `_emit_signal`: Wochenend-Guard → Stale-Price-Guard →
   Regime-Sperrfilter → MasterPrompt → Tages-Limits → Diversifikation → Cooldowns.
4. Neue Config-Keys sind NICHT in der KI-Selbst-Tuning-Whitelist (KI kann sie
   nicht selbst verstellen).
- Tests: NEU `tests/test_stale_price_and_regime_filter.py` (5 Blöcke; nutzt
  gemockten `master_prompt.check_trade` als kontrollierten Endpunkt, um
  Guard-Passieren nachzuweisen). Regression grün: test_weekend_guard_and_fee_v2,
  test_fee_guard, test_phase4_collection_mode.
- Hebel-Frage des Users beantwortet (siehe Chat 17.08.): Hoher Hebel erschafft
  keinen Edge — bei fixer Marge skaliert er Gewinn/Verlust/Fees proportional und
  erhöht nur Varianz + $-Risiko pro Trade (50x + 0,5% SL = 25% der Marge pro
  Stop; 83x ≈ 42%; live zusätzlich Liquidationsnähe ~1,2%). Der Fee-Anteil am
  Verlust hängt an der SL-Distanz (0,12%/sl_pct), NICHT am Hebel. Empfehlung:
  KI-Proposals (8–14x) in Prod bestätigen; "Lotterie-Charakter" besser über CRV
  (kleines Risiko, weites TP/Runner) abbilden als über Hebel.

## Aktueller Stand (Datum: 2026-08-17, 9. Handover — Bugfix Git-Push)
- BUG: Push über die Emergent-Plattform ("Save to GitHub") schlug fehl mit
  "did not receive expected object / index-pack failed". ROOT CAUSE: /app war beim
  Setup als Shallow-Clone (--depth 1) geklont — dem Repo fehlten die historischen
  Git-Objekte; zusätzlich hatte der Plattform-Push das origin-Remote entfernt.
- FIX: origin neu gesetzt (dean06greif-ai/Krypto_Trader.git) + `git fetch
  --unshallow` — Historie vollständig. Verifiziert (testing_agent iteration_39,
  alles grün): kein Shallow-Repo mehr, git fsck sauber, Push-Simulation in lokales
  Bare-Repo erfolgreich, KEINE .env-Dateien getrackt (Keys bleiben lokal),
  /api/health ok. Lehre für künftige Setups: Repo NIE mit --depth 1 klonen,
  wenn später über die Plattform gepusht werden soll.
- Hinweis: Der Plattform-Push geht zum verbundenen GitHub-Account des Users
  (Ziel-Repo wählt die Plattform-UI); für das Render-Deploy muss der Stand danach
  in dean06greif-ai/Krypto_Trader (Branch) gemerged/gepusht werden.

## Aktueller Stand (Datum: 2026-08-17, 9. Handover — Paket "Guard-Fixes" UMGESETZT)
Vom User freigegeben und implementiert (testing_agent iteration_38: Backend+Frontend 100%,
22/22 Tests grün, keine offenen Issues):

1. **Wochenend-Guard (P0, NEU `backend/core/market_hours.py`)**
   - `is_weekend_closed(symbol, now)` (pur, testbar): Krypto immer offen; Forex zu
     Fr 21:00 UTC → So 21:15 UTC; Rohstoffe/Indizes (CME) zu Fr 21:00 → So 22:05 UTC.
   - Durchgesetzt an 3 Stellen: (a) `ai_engine._emit_signal` blockt Live- UND
     Sammel-Einstiege hart (blocked_by am Decision, bewusst KEIN Governance-Feed-
     Eintrag gegen 15-min-Spam); (b) `run_analysis` überspringt Symbole mit
     geschlossenem Markt komplett (spart Analyst-Tokens, keine Decisions auf
     eingefrorenen Kursen); (c) `ai_market_observer.collect()` markiert Snapshots
     mit `features.market_closed=true` (Regime-/ML-Statistik filterbar).
2. **Fee-Wächter V2 (ATR-Floor)**: `fee_guard_min_sl_pct/fee_guard_check` in
   bitunix_trade.py um `atr_pct`/`atr_mult` erweitert — Mindest-SL =
   max(fee_guard_mult × Roundtrip-Fees, fee_guard_atr_mult × 1m-ATR%).
   Neuer Engine-Config-Key `fee_guard_atr_mult` (Default 4.0 ≈ 15-min-Rauschband,
   Klemme 0–30, 0=aus; NICHT in der KI-Selbst-Tuning-Whitelist). UI: neues Dropdown
   "ATR-Faktor (× ATR)" (`ai-fee-guard-atr-select`) im KI-Setup. Block-Grund nennt
   die bindende Komponente ("ATR-Minimum" vs "Roundtrip-Fees"). Rückwärtskompatibel:
   Aufrufe ohne atr verhalten sich exakt wie V1.
3. **Prompt-Regel gegen SL-Minimum-Kleben**: FEE-WÄCHTER-Block im Prompt sagt jetzt
   explizit "Minimum ist UNTERGRENZE, kein Zielwert — SL an die Struktur; liegt der
   Struktur-SL unter dem Minimum → HOLD statt SL schrumpfen/aufweiten".
4. **be_mode=crv Default für KI-Trader** (bitunix_trade.on_signal): Breakeven ab
   +1R (be_trigger_crv=1.0) statt erst bei TP1-Touch (CRV 2–3 → griff nie; RCA:
   DOGE +2.54R / ADA +2.15R liefen in den vollen SL). Explizite Nutzer-Settings
   (Coin-/Strategie-/Strategie-Coin-Config) haben Vorrang; andere Strategien
   behalten tp1-Default. Runtime-Logik für crv existierte bereits (Z. ~1732).
- Tests: NEU `tests/test_weekend_guard_and_fee_v2.py` (5 Blöcke: Kalender,
  ATR-Floor-Mathe, Config-Klemme, _emit_signal-Block live+collection, be_mode-
  Default/Vorrang/Scope). Regression grün: test_fee_guard (Fixture setzt jetzt
  fee_guard_atr_mult=0), test_fee_feedback, test_phase4_collection_mode,
  test_ram_and_manager_guards, test_regime_v2_and_gate_domain, test_qqq_range_
  chart_fixes, test_manual_web_trades, test_trades_visibility, test_watchdog_sync_only.
- NICHT umgesetzt (bewusst): Auto-Schließen der bestehenden Wochenend-Zombie-Trades
  in Prod — User kann sie manuell schließen oder auslaufen lassen; der Guard
  verhindert nur NEUE Einstiege.
- NEUER BEFUND (P2, offen): Die beiden EURUSD-Trades vom Mo 05:09 + 05:43 UTC
  nutzten denselben ~45 min alten Snapshot-Preis (Entry identisch 1.158883,
  Yahoo-Forex-Feed hing). → Vorschlag: Stale-Price-Guard vor Entry (Entry ablehnen,
  wenn letzte Kerze älter als X min). WARTET AUF USER-ENTSCHEIDUNG.

## Aktueller Stand (Datum: 2026-08-17, 9. Handover — GROSSER CHECK, nur Analyse)
- 9. Handover: Repo Branch 16.8-New nach /app geklont, .env selektiv übernommen
  (LLM-Keys, ADMIN, PROD_MONGO_URL/PROD_DB_NAME nur lesend; Bitunix/Telegram/Supabase
  bewusst NICHT). App lauffähig verifiziert: Backend healthy, Admin-Login ok, Frontend
  inkl. Chart + Regime-Badge ok, Boot-Backfill 22/22 Symbole, Prod-Lesezugriff ok
  (auto_trades=171, ai_decisions=42.478, ai_rewards=61, ml_gate_models=6).
- LÜCKE GESCHLOSSEN: Diese Status-Datei war für die Iterationen 28–40 (siehe PRD.md)
  nicht nachgezogen worden. Kurzfassung: Bugfix-Sessions (manuelle Trades sichtbar,
  Fee-Doppelzählung, %-Anzeige geschlossener Trades), RAM-Guard (malloc_trim),
  neue Modell-Kataloge, Iteration 39/40 = Prod-Settings live geflippt
  (use_liquidation_data=true, Learner-Rolle → groq gpt-oss-120b). Details in PRD.md.
- DIESE SESSION (nur lesend, KEINE Code-Änderungen): Großer Check auf User-Wunsch.
  Skripte: backend/scripts/big_check_prod.py + backend/scripts/trade_review_chart.py
  (beide read-only gegen PROD). Roh-Trade-Review: /app/memory/trade_review_raw.txt.

### Befunde des großen Checks (2026-08-17, Prod nur lesend)
1. **P0 WOCHENEND-/STALE-TRADING (neu, kritisch)**: Die KI eröffnet am Wochenende
   Trades auf Nicht-Krypto (EURUSD, GBPUSD, USDJPY, GOLD, SILVER, OIL, SPY, QQQ),
   obwohl die realen Märkte zu sind — Bitunix liefert quasi eingefrorene Preise
   (Bewegung ~0,01–0,1%/h). Beweise: EURUSD LONG und SHORT GLEICHZEITIG offen zum
   IDENTISCHEN Entry 1.158883 (17.08. 05:09 + 05:43, Sonntag); GOLD-Trades mit
   identischem Entry 4432.100098 in zwei Trades; 5 von 8 offenen Trades sind
   Wochenend-Nicht-Krypto. Folgen: (a) Zombie-Trades verstopfen die Richtungs-Slots
   → Richtungs-Guard blockt alles Weitere (= das "Limit guard"-Problem des Kumpels),
   (b) ML-Labels/Regime-Statistik verschmutzt, (c) Paper-PnL auf nicht handelbaren
   Preisen ist wertlos.
2. **"LIMIT GUARD" AUFGEKLÄRT**: fee_guard_blocks=0 (Fee-Wächter blockt NICHTS mehr).
   Was wirklich blockt (7d, decisions.blocked_by): ~220x Richtungs-Guard ("bereits
   5 offene LONG/SHORT-Risiken" = collection_max_same_direction), ~35x Korrelations-
   Guard, Governance-Feed 16x "Limit 3, kein weiterer gleichgerichteter Trade"
   (= live max_same_direction=3). Kette: Wochenend-Zombie-Trades halten die Slots
   dauerhaft besetzt. Von 1.343 LONG/SHORT-Decisions in 7d wurden nur 111 signaled.
   959 fielen still durch (unter min_confidence 68 bzw. Collection-Cooldown).
3. **FEE-WÄCHTER-NEBENWIRKUNG**: Die KI klebt SLs fast immer exakt ans
   0,5%-Minimum (4×Fees), teils wörtlich "SL 0,5% aufgrund Fee-Blockade" — Struktur-
   SLs gehen verloren, der Wächter blockt nie (0 Blocks), weil die KI ihn durch
   SL-Schrumpfen umgeht. Chart-Review zeigt: MAE > 1R in vielen Verlierern.
4. **MFE-VERSCHENKEN**: Verlierer mit MFE ≥ 2R vor dem Stop-out (DOGE 2.54R,
   ADA 2.15R, offener BNB 2.06R). Ursache: be_mode="tp1" (Breakeven erst bei
   TP1-Touch), TP1 aber bei CRV 2–3 → BE greift fast nie. be_mode="crv"
   (BE ab 1R, be_trigger_crv existiert schon) wäre per Setting umstellbar.
5. **REGIME v2 LÄUFT** (Deploy 15.08. mittags): Verteilung 7d: ruhig 71% / normal
   20% / volatil 9%; breakout 13% (vorher 28%); range_ruhig dominiert mit 47,5% —
   ABER durch Wochenend-Stale-Nicht-Krypto aufgebläht. vol_basis 50/50
   percentile/fixed_fallback (48h-Historie nach Render-Neustarts noch dünn).
   Rewards nach Regime: range_ruhig nur 10% Winrate (n=20) → stützt einen
   Regime-Entry-Filter, aber erst nach sauberen (Weekend-bereinigten) Daten urteilen.
6. **TOKEN-USAGE**: 1–5M Tokens/Tag, Analyst = 80–90% davon. Kein Kostenproblem
   (free Modelle; deepseek-v4-pro für trade_manager/learner ≈ 0,2$/Tag). ABER
   Qualitätsproblem: Analyst (konfiguriert cerebras/gpt-oss-120b) lief am 16.+17.08.
   per Doppel-Fallback auf openai/gpt-oss-20b:free (schwächstes Modell der
   OpenRouter-Kette) — Cerebras-Free-Limits (1M/Tag/Key, 30k TPM) + OpenRouter-
   Free-Limits reichen am Wochenende nicht. Trade-Begründungen der 20b-Trades sind
   sichtbar generischer ("SL 0,5%, TP1 1,5%, 3:1").
7. **GATE-SHADOW**: would_block=94% bei threshold 0.45 (271 Decisions mit
   gate_shadow) — Gate hält fast alles für Verlierer; bei realer Winrate ~30%
   halbwegs ehrlich, als aktives Gate so unbrauchbar. v6 (auto-Retrain läuft):
   OOS-AUC 0.597, Brier kalibriert 0.2366 < Baseline 0.2589. Kalibrierungs-Bins
   an den Rändern verzerrt (0.2–0.3: predicted 0.29 / actual 0.62, n=34).
8. **KONFIDENZ-KALIBRIERUNG**: Das alte "≥80% = 0% Winrate"-Paradox ist in den
   Daten seit dem Reset NICHT mehr sichtbar (die KI vergibt schlicht kaum noch ≥80;
   Buckets: 65-69→25%, 70-74→29%, 75-79→33%, n klein). 21/40 Verluste fee-dominiert.
9. **WERTE-KORREKTHEIT Frontend/Backend**: risk_usd (= |entry−initial_sl|×qty),
   Fees-vs-Risiko, pnl_pct_margin (Fix #34), Bitunix-uPnL-Quelle — Logik geprüft,
   rechnet korrekt. Das echte Anzeige-Problem sind stale Wochenend-Preise bei
   Nicht-Krypto (werden als live angezeigt).
10. **OFFENE KI-PROPOSALS** hängen in needs_confirmation: Hebel senken (8–14x),
    auto_lev_max=10, sl_mode=atr, ENGINE min_confidence 63. Die KI will selbst
    weniger Hebel — wartet auf User-Klick im UI.

### Bewertung der 4 KI-Wünsche (User-Frage "was hältst du davon")
1. Tick-Orderflow/CVD/Footprint: ABGELEHNT als Plan. LLM-Zyklus 10–15min kann
   Millisekunden-Orderflow weder empfangen noch verwerten; Infra (WS-Streams,
   Footprint-Aggregation) sprengt Render 512MB. Kein Edge auf diesem Horizont.
2. Dynamischer Fee-Filter: BERECHTIGT — existiert (Fee-Wächter), braucht aber v2
   wegen Befund 3: Mindest-SL = max(fee_guard_mult×Fees, k×ATR) + Prompt-Regel
   "Struktur-SL unter Minimum → KEIN Trade statt SL aufweiten/schrumpfen".
3. Anti-Overfitting-Feedback: EXISTIERT im Kern (Gate-Shadow + Platt-Kalibrierung
   + Konfidenz-Stats im Lern-Prompt). Kein neuer Baustein — Shadow-Beweis abwarten.
4. Regime-Sperrfilter: BERECHTIGT (range_ruhig 10% Winrate), ABER erst nach
   Weekend-Guard + 1–2 Wochen sauberer Daten entscheiden (sonst auf verschmutzter
   Statistik gebaut).

### Vorgeschlagener Maßnahmen-Plan (WARTET AUF USER-FREIGABE, nichts umgesetzt)
- [P0] Markt-Kalender-Guard: Nicht-Krypto-Symbole am Wochenende (Fr 22:00 UTC –
  So 22:00 UTC, konfigurierbar) für KI-Entries sperren + Snapshots als
  market_closed markieren (Regime-/ML-Statistik filterbar). Kleiner, isolierter Fix.
- [P1] Fee-Wächter v2: ATR-Kopplung + Prompt-Regel (siehe oben).
- [P1] be_mode="crv" für ai_trader als Empfehlung/Default prüfen (Setting, evtl.
  0-Code; User kann es sofort in Prod im UI umstellen).
- [P1] Analyst-Modell: Fallback-Kette prüfen; Empfehlung an User: Rolle analyst
  auf groq/gpt-oss-120b (wie learner) oder bezahltes deepseek-v4-flash (~0,06$/M).
- [P2] Richtungs-Guard nach Weekend-Fix neu bewerten (Slots werden dann frei;
  erst dann entscheiden, ob 3/5 zu streng ist).
- [P2] Regime-Entry-Filter (Sperre range_ruhig/drift_ruhig) nach sauberen Daten.
- User-Aktion ohne Deploy: offene KI-Hebel-Proposals im UI bestätigen.

### Emergent-Key-Frage (beantwortet 17.08.)
Universal Key funktioniert NUR in Emergent-gehosteten Apps — für die extern auf
Render deployte App NICHT nutzbar. Bestehende Provider bleiben.
