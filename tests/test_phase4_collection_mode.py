...
async def main():
    ...
    try:
        ...
        # Seit dem Proposal-Aufräumer (supersede bei Insert) kann der 85er durch
        # den neueren 70er als 'superseded' markiert sein – beides ok, nur
        # 'auto_applied' wäre ein Bug.
        await ai_engine.review_parked_proposals()
        parked = await db.ai_proposals.find_one({"changes.min_confidence": 85})
        assert parked["status"] in ("needs_confirmation", "superseded"), parked
        assert ai_engine.config["min_confidence"] == 70
    finally:
