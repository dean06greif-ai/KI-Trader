...
def test_compute_reproduces_prod_case_and_fixes_it():
    ...
    # Hebel-Deckel explizit 50x (Stand 01.09.; Standard seit 02.09. = 15x)
    params = ps.build_params({**ps.DEFAULTS, "risk_max_leverage": 50}, {"capital_pct": 100},
                             is_swing=False, ml_scale=1.0)
...
def test_ai_engine_defaults_include_sizing_and_prompt_enum():
    ...
    # Seit den Event-Setups (fomc/cpi/nfp/ppi/pce) endet die Enum nicht mehr
    # auf "divergence" – Kern-Setups müssen enthalten sein:
    for _s in ("funding_fade", "session_open", "divergence",
               "fomc_event", "cpi_event"):
        assert _s in ai_playbook.SETUP_ENUM.split("|"), _s
...
async def _db_tests():
    ...
    # funding_fade (nur Krypto) darf in der Forex-SETUP-Liste nicht auftauchen;
    # in der klassenunabhängigen ALIAS-CHECK-Zeile ist der Name erlaubt.
...
def test_execute_signal_uses_risk_sizing():
    async def run():
        ...
        # Seit Fix 06/2026 hat der Hebel aus den Trade-Einstellungen (7.2x)
        # Vorrang vor risk_max_leverage -> Marge 10/0.005/7.2 = 277.8 USDT,
        # gedeckelt auf 15 % Equity = 150 -> Risiko 150×7.2×0.5 % = 5.4 USDT.
        sig2 = dict(base)
        sig2["ai_sizing"] = ps.build_params({**ps.DEFAULTS, "risk_max_leverage": 50},
                                            {"capital_pct": 30}, is_swing=False, ml_scale=0.5)
        ...
        s = t.get("sizing") or {}
        assert s.get("risk_usdt") and approx(s["risk_usdt"], 5.4, 0.05), s
        assert float(t["leverage"]) == 7.2, t["leverage"]  # Preset-Hebel hat Vorrang
        assert "Trade-Einstellungen" in s.get("leverage_source", ""), s
        assert s.get("capped") is True, s
        assert approx(float(t["max_capital"]), 150.0, 0.05), t["max_capital"]  # 15 % × 1000
        assert float(t["max_capital"]) > 4.5 * 5
        await client.drop_database(db.name)
        print("  ✓ execute_signal: Legacy unverändert, Risiko-Modus Marge 150 USDT @ 7.2x (Preset-Hebel + Margen-Deckel)")
    asyncio.run(run())
