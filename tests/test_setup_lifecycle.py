...
async def _db_tests():
    ...
    # Seit 06/2026 laufen Reife/Rückstufung je Anlageklasse
    ...
    assert "breakout" in crypto["live_blocked"], crypto["live_blocked"]
    assert data["live_ready"]["breakout"] is False
    assert crypto["live_blocked"]["breakout"]["paper_since"]["trades"] == 0
    st = await pb.status(db)
    # Reife/Rückstufung ist je Anlageklasse ausgewiesen (globale Zeile = Aggregat)
    row = {r["setup"]: r for r in st["classes"]["crypto"]["maturity"]}["breakout"]
    ...
    data = await pb.refresh(db)
    crypto = data["classes"]["crypto"]
    assert "breakout" not in crypto["live_blocked"]
    assert data["live_ready"]["breakout"] is True
    assert crypto["live_since"].get("breakout")
    # alte Live-Verluste vor der Freischaltung zählen nicht mehr
    assert crypto["live_stats"].get("breakout", {}).get("trades", 0) == 0
