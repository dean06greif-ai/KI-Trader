...
def _run(coro):
    # T1: eigener frischer Loop je Aufruf – get_event_loop() konnte unter
    # pytest einen bereits geschlossenen Loop eines Vorgänger-Moduls liefern.
...
class _FakeAutotrader:
    ...
    def ai_manage_allowed(self, strategy_id, symbol=None):
        # wie Produktions-Default: KI managt nur eigene (ai_trader-)Trades
        return not strategy_id or strategy_id == "ai_trader"
...
def test_ml_lab_projection_dataset_identical(db):
    async def run():
        ...
        try:
            # seit Audit 2.2: (X, y, timestamps, meta)
            X, y, _ts, meta = await lab.load_training_data()
