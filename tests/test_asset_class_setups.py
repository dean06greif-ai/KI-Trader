...
def test_asset_factor_escalation():
    ...
    # schlecht, aber noch nicht genug Trades zum Aussetzen -> halbieren
    f, note = cap.asset_factor({"trades": 4, "wins": 1, "pnl": -2, "margin": 100})
    assert f == 0.5 and "reduziert" in note
    ...
    # PnL -4 % der Margin bei WR 50 % -> nur reduziert? WR 50 >= 45 -> ok (nicht < 45)
    assert cap.asset_factor({"trades": 6, "wins": 3, "pnl": -4, "margin": 100})[0] == 1.0
...
def test_setup_descriptions_stay_compact_for_token_budget():
    ...
    # (TF2-Signal-Zeile) und darf etwas länger sein.
    limits = {"trend_follow2": 200}
