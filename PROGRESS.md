...
## Paket AP13 – Gezielte Bereinigung & Forschungs-Erweiterung ✅ (26.06.2026)
- Bezug: Plan AP13 (P2) · Basis: Branch `conflict_160926_0048`.
  Baseline nach Repo-Import EXAKT wie dokumentiert verifiziert: Offline-Solltests
  **163 passed**, Unit-Suite **1629 passed**, 2 skipped.
- **AP13a Archiv-Katalog** (dokumentierte Archivierung, kein blindes Löschen):
  NEU `ARCHIV_KATALOG.md` (Root). Verschoben nach `archive/backend_fixes/`:
  `backend/README_FIX*.md` (3), `backend/*.patch` (2 – Fixes längst im Code);
  nach `archive/reviews/`: `KI_TRADER_ANALYSE_0609.md`, `KI_TRADER_REVIEW_0709.md`
  (durch analysis_paket überholt). Vor dem Verschieben Referenz-Suche über
  backend/frontend/scripts/.github: keine Code-Referenzen. `KI_TRADER_AUDIT.md`,
  Nutzer-Guides und `backend/test_bitunix_precision_fix.py` bewusst AM ORT.
- **AP13b Indikator-Ablation** (Zielbild §6B: „derselbe Plan ohne den
  Indikator, mit einfacher Alternative“):
  - `backend/services/research_ablation.py` (NEU, rein): `ablation_variants`
    je Detektor (reactive: −MTF/−Volumen/−EMA-Bestätigung; kombi: −Umkehrpunkt-
    Beschleuniger/−Trend-Dominanz; jeweils + einfache Alternative 'ema' bzw.
    'regression'; abgeschaltete Komponenten werden übersprungen, Basis-Config
    wird nicht mutiert) und `component_verdicts` (Beitrag = volle Konfiguration
    minus Variante auf der INNEREN Validierung: trägt_bei/redundant/schadet).
  - `backend/services/regime_lab.py::run_ablation` (NEU, Muster run_ema_compare):
    Auswahl strikt via `research_validation.select_best_row` (AP07 – Holdout
    bleibt finaler Test), Versuchszähler, Experiment-Manifest, Persistenz in
    `regime_lab_runs`. Router NEU `POST /api/regime-lab/ablation` (admin,
    Cloud-Ausführung wie ema-compare).
  - UI: `RegimeLab.js` NEU Sektion „Indikator-Ablation“ (testids `ablation-*`:
    run/status/table/row-{key}/pooling/selection-note) mit Beitrags-Spalte
    und Pooling-Zeile.
- **AP13c Assetgruppen-Pooling** (Zielbild §4: assetspezifisch nur bei
  Datenstärke, sonst Klassen-Pooling mit sichtbarer Abweichung):
  `backend/services/research_pooling.py` (NEU, rein): `pool_rows` – Kennzahlen
  je Symbol kerzen-gewichtet zu Anlageklassen (setup_asset_class) gepoolt,
  assetspezifische Abweichung je Symbol bleibt ausgewiesen. Verdrahtet in
  jede Ablation-Variante (`row.pooling`).
- **AP13d Unsicherheitskalibrierung** (Zielbild §4: heuristische Scores sind
  KEINE kalibrierten Wahrscheinlichkeiten):
  - `backend/services/research_calibration.py` (NEU, rein): `hit_points`
    (Score vs. Live=Final-RICHTUNGS-Treffer via split_id, keine Label-
    Substrings), Score-Bins mit Rohsummen (mergebar), `calibration_report`
    (Basis `symbol` nur ab min. 300 Punkten, sonst `pooled:{klasse}`,
    zu dünner Pool = `insufficient`; überall
    `score_is_calibrated_probability=false`).
  - `regime_reactive.full_payload` additiv `live_conf` (kausaler Score je
    Kerze); `regime_lab._symbol_payload` speichert je Symbol
    `uncertainty` (Bins); `run_analysis` speichert am combined-Scope den
    Pooling-Report (`combined.uncertainty`) – alles additiv, Altdokumente
    unverändert lesbar.
  - UI: Analyse-Detail-Kopf NEU „Score-Kalibrierung: Ø-Abweichung X pp /
    zu wenig Daten (Klassen-Pool)“ (testid `regime-detail-uncertainty`).
- Solltests: `tests/analysis_regression/test_ap13_research_ext.py` (18 neu:
  Varianten je Detektor inkl. Skip/No-Mutation, Verdikt-Klassen+Fallback,
  gewichteter Pool+Klassentrennung+Abweichung, Richtungs-Punkte/Bin-Merge/
  Report-Datenstärke/insufficient, live_conf funktional, _symbol_payload-
  uncertainty funktional, **run_ablation offline end-to-end** (gefakte
  Historie, echtes v2-Modell), keine Eigen-Bestenwahl neben AP07,
  Router-/run_analysis-Verdrahtung).
- Bekannte Grenze (ehrlich): Detektor 'regression' liefert keine kausalen
  live/final-Paare – Ablations-Zeile 'alt_regression' zeigt dann keine
  Trefferquoten (None), Kalibrierung nur für reaktive Detektoren
  (reactive/ema/kombi = Standardfälle).
- NICHT umgesetzt (AP13-Restpunkte, bewusst): Herauslösen großer Fachmodule
  (nur nach Pilotnachweis; kein Refactoring mit Fachänderungen mischen),
  HMM/ML-Regimemodelle und Performanceoptimierung (kein gemessener Engpass).
...
## Prüfung & Fix – Setups nicht getradet / Token-Verbrauch / Rest-Kapital (19.09.2026)
Befund aus der Prod-DB (nur lesend):
- **Rest-Kapital-Trading** (`services/capital_fit.py`) ist vollständig: Market-
  Entries, Live-Limit-Orders und Retry nach 'insufficient balance' (Bitunix).
  Ergänzt: gleiche Regel jetzt auch vor **IBKR**-Orders (freies IBKR-Guthaben
  statt Bitunix-Guthaben; `bitunix_trade.on_signal`).
- **Token-Verbrauch** (ai_token_usage, 10.–19.09.): analyst 57 Mio (free-Modell),
  trade_manager 4,8 Mio auf deepseek (bis 233 Aufrufe/Tag bei 1 Aktion/Tag),
  Bewegungs-Scanner (research_analyst) <35k/Tag – der Scanner ist LLM-frei bis
  auf max. 12 Setup-Lücken-Analysen/Tag. Fresser war der 5-min-Trade-Review
  ohne Smart-Skip (`services/trade_review_gate.py` ist im Branch, noch nicht
  deployt). Fix: LLM-Antwortsprache Deutsch im Move-Scanner (Prod lieferte
  chinesische Ursachen-Texte).
- **Setups (session_open, momentum_news, htf_range, divergence …) wurden nie
  gehandelt**: in ALLEN Klassen `live_blocked` mit 'Revision v1: Validierung
  neu gestartet' und 0 Paper-Trades seit Revision. Zwei Schutzmechanismen
  liefen gegeneinander (Revision verlangt Paper-Trades, Backtest-Seeding war
  für rückgestufte Setups ausgeschlossen, kein Auslöser erzeugte Paper-Trades).
  Fix (rückwärtskompatibel):
  * `setup_lifecycle.revision_demotion()` + `kind: revision` an der Rückstufung;
  * `ai_playbook._refresh_scope`: Revisions-Rückstufung wird per gewichtetem
    Backtest-Edge + ≥2 profitablen Paper-Trades seit Revision aufgehoben
    (`bt_validated`), Seeding zählt danach weiter; Rückstufungen wegen
    Schwäche unverändert (echte Paper-Trades nötig), erneute Schwäche löscht
    `bt_validated`;
  * `setup_trigger.paper_wanted()`: rückgestufte Setups bekommen LLM-freie
...
2. **AP13-Rest (P2, erst nach Pilotnachweis):** Herauslösen großer Fachmodule
   entlang stabilisierter Verantwortlichkeiten; HMM/ML-Regimemodelle nur bei
   nachgewiesenem Bedarf; Performanceoptimierung nur bei messbarem Engpass.
